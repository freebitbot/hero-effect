...
import type IProfile from "@double-agent/collect-http2-session/interfaces/IProfile";
...
export default class CheckGenerator {
	...
	constructor(profile: IProfile) {
		...
		if (data.sessions.length) {
			...
			const remoteSettings = session.activity.find(
				(x) => x.type === "remoteSettings",
			);
			if (remoteSettings) {
				for (const [key, value] of Object.entries(remoteSettings.data)) {
					...
					if (typeof value === "number") {
						...
						this.checks.push(
							new BooleanCheck({ userAgentId }, { path }, value),
						);
					}
				}
			}
			...
			if (firstStream) {
				...
				this.checks.push(
					new NumberCheck({ userAgentId }, { path: `frame.weight` }, weight),
				);
				const flagOrder = [
					"END_STREAM",
					"RESERVED2",
