...
import type IHttpBasicCookiesProfile from "@double-agent/collect-http-basic-cookies/interfaces/IProfile";
import type {
	ICollectedCookieData,
	ICreatedCookieData,
} from "@double-agent/collect-http-basic-cookies/interfaces/IProfile";
import type ICookieSetDetails from "../interfaces/ICookieSetDetails";
...
export default class CheckGenerator {
	...
	private extractChecks(): void {
		...
		for (const cookieData of this.profile.data) {
			const { group, getter, httpProtocol, cookies } =
				cookieData as ICollectedCookieData;
			if (!getter) continue;
	...
	private extractSetCookieDetails(): {
		[key: string]: { [name: string]: ICookieSetDetails };
	} {
		const cookiesMap: { [key: string]: { [name: string]: ICookieSetDetails } } =
			{};

		for (const cookieData of this.profile.data) {
			const { group, setter, httpProtocol, cookies } =
				cookieData as ICreatedCookieData;
			const key = `${httpProtocol}-${group}`;
			if (!setter) continue;
