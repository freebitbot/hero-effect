...
import type ITlsClienthelloProfile from "@double-agent/collect-tls-clienthello/interfaces/IProfile";
...
export default class CheckGenerator {
	...
	private createCipherChecks() {
		...
		const ciphers = clientHello.ciphers.map((cipher) => {
			...
			const valueInt = parseInt(
				matches[1].replace(/0x/g, "").replace(", ", ""),
				16,
			);
			const valueStr = matches[2];
	...
	private createCurveChecks() {
		...
		for (let i = 0; i < values.length; i += 1) {
			...
			if (this.isGreased(valueInt)) continue;
			this.checks.push(
				new NumberCheck({ userAgentId }, { path }, valueInt, valueStr),
			);
	...
	private createAlpnChecks() {
		...
		const path =
			"clientHello.extensions.application_layer_protocol_negotiation";
		const extension = clientHello.extensions.find(
