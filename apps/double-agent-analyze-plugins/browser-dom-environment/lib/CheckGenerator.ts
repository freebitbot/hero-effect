import type BaseCheck from "@double-agent/analyze/lib/checks/BaseCheck";
...
import EndpointType, { type IEndpointType } from "../interfaces/EndpointType";
import type IDomDescriptor from "../interfaces/IDomDescriptor";
...
import extractDomEndpoints, { type IEndpoint } from "./extractDomEndpoints";
...
export default class CheckGenerator {
	...
	private addFlagChecks(path: string, object: IDomDescriptor) {
		...
		if (object._$flags) {
			this.add(
				new FlagsCheck({ userAgentId }, { path }, object._$flags.split("")),
			);
		}
	...
	private addPrototypeChecks(path: string, object: IDomDescriptor) {
		if (
			!["prototype", "object", "constructor", "array"].includes(object._$type)
		)
			return;
	...
	private addNumberChecks(path: string, object: IDomDescriptor): IChecks {
		...
		if (Config.shouldIgnorePathValue(path)) {
			...
			this.add(
				new NumberCheck({ userAgentId }, { path }, object._$value as number),
			);
		} else if (String(object._$value).includes(".")) {
			const decimalStr = String(object._$value).split(".")[1];
			this.add(
				new DecimalLengthCheck({ userAgentId }, { path }, decimalStr.length),
			);
		} else {
			this.add(
				new NumberLengthCheck(
					{ userAgentId },
					{ path },
					String(object._$value).length,
				),
			);
		}
	}
		...
		if (Config.shouldIgnorePathValue(path)) {
			...
			this.add(
				new StacktraceCheck(
					{ userAgentId },
					{ path },
					object._$value as string,
				),
			);
		} else {
			this.add(
				new StringCheck({ userAgentId }, { path }, object._$value as string),
			);
		}
	}
	...
	private addGetterChecks(path: string, object: IDomDescriptor) {
		...
		const { userAgentId } = this;
		this.add(
			new GetterCheck({ userAgentId }, { path }, { codeString: object._$get }),
		);
		this.add(
			new GetterCheck(
				{ userAgentId },
		...
		if (object._$accessException) {
			this.add(
				new GetterCheck(
					{ userAgentId },
					{ path },
					{ accessException: object._$accessException },
				),
			);
		}
	}
	...
	private addSetterChecks(path: string, object: IDomDescriptor) {
		...
		const { userAgentId } = this;
		this.add(
			new SetterCheck({ userAgentId }, { path }, { codeString: object._$set }),
		);
		this.add(
			new SetterCheck(
				{ userAgentId },
	...
	private addClassChecks(path: string, object: IDomDescriptor) {
		...
		if (constructorException) {
			this.add(
				new ClassCheck({ userAgentId }, { path }, { constructorException }),
			);
		}
	}
	...
	private addBooleanChecks(path: string, object: IDomDescriptor) {
		...
		if (Config.shouldIgnorePathValue(path)) {
			...
			this.add(
				new BooleanCheck({ userAgentId }, { path }, object._$value as boolean),
			);
		}
	}
	...
	private addSymbolChecks(path: string, object: IDomDescriptor) {
		...
		const { userAgentId } = this;
		this.add(
			new SymbolCheck({ userAgentId }, { path }, object._$value as string),
		);
	}
