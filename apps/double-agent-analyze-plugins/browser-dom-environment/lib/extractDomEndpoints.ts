import type { IProfileData } from "@double-agent/collect-browser-dom-environment/interfaces/IProfile";
...
export default function extractDomEndpoints(dom: IProfileData): {
	...
	while (objectsToExtract.length) {
		...
		for (const key of Object.keys(objectToExtract.object)) {
			...
			if (
				["object", "class", "function", "prototype", "constructor"].includes(
					objectMeta._$type,
				)
			) {
...
function extractObjectMeta(
	...
	if (
		...
		path.join(".") === "window.Function.prototype"
	) {
		object._$type = "prototype";
		...
		for (const key of Object.keys(object).filter((x) =>
			["name", "length"].includes(x),
		)) {
