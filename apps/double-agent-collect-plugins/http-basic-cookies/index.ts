import type IPlugin from "@double-agent/collect/interfaces/IPlugin";
...
import {
	cleanDomains,
	DomainType,
} from "@double-agent/collect/lib/DomainUtils";
...
import {
	CookieGetter,
	CookieSetter,
	type ICollectedCookies,
	type ICookieGetter,
	type ICookieSetter,
	type ICreatedCookies,
	type IProfileData,
...
export default class HttpCookiesPlugin extends Plugin {
	public initialize() {
		...
		this.registerRoute(
			"allHttp1",
			"/saveAndRedirectToNextPage",
			this.saveAndRedirectToNextPage,
		);
		this.registerRoute(
			"allHttp1",
			"/setAndRedirectToNextPage",
			this.setAndRedirectToNextPage,
		);
		...
		["http", "https"].forEach((protocol) => {
			...
			pages.push(
				...
				{
					route: this.routes[protocol]["/save"],
					domain: MainDomain,
					clickNext: true,
					data,
				},
			...
			pages.push(
				{
					route: this.routes[protocol]["/set"],
					domain: SubDomain,
					clickNext: true,
					data,
				},
	...
	private start(ctx: IRequestContext) {
      ...
      (function() {
		...
		this.saveCreatedCookiesToProfile([jsCookieToSet], ctx, {
			setter: "JsScript",
		});
	}
