...
export default class BrowserDomPlugin extends Plugin {
	...
	public initialize() {
		...
		this.registerRoute(
			"allHttp1",
			"/load-iframe-sandbox",
			this.loadIFrameSandbox,
		);
		this.registerRoute(
			"allHttp1",
			"/load-iframe-cross-domain",
			this.loadIFrameCrossDomain,
		);
		...
		this.registerRoute(
			"allHttp1",
			"/wait-until-finished.js",
			this.waitUntilFinishedJs,
		);
