function DomExtractor(selfName, pageMeta = {}) {
	...
	async function extractPropsFromObject(obj, parentPath) {
		...
		if (obj.prototype) {
			...
			if (constructorException) {
				newObj["new()"] = {
					_$type: "constructor",
					_$constructorException: constructorException,
				};
			} else {
				try {
					newObj["new()"] = await extractPropsFromObject(
						instance,
						`${parentPath}.new()`,
					);
	...
	async function getDescriptor(obj, key, accessException, path) {
		...
		if (objDesc) {
			...
			return {
				...
				_$flags: flags.join(""),
				_$accessException: accessException
					? accessException.toString()
					: undefined,
				_$value: value,
				_$get: objDesc.get ? objDesc.get.toString() : undefined,
				_$set: objDesc.set ? objDesc.set.toString() : undefined,
				_$getToStringToString: objDesc.get
					? objDesc.get.toString.toString()
					: undefined,
				_$setToStringToString: objDesc.set
					? objDesc.set.toString.toString()
					: undefined,
			};
		}
		const plainObject = {};
		if (
			accessException &&
			String(accessException).includes("Likely a Promise")
		) {
	...
	async function getFunctionDetails(value, obj, key, type, path) {
		...
		return {
			...
			func,
			invocation:
				func || invocation !== undefined
					? getJsonUsableValue(invocation)
					: undefined,
			isAsync,
		};
	}
