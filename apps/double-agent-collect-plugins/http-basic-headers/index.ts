...
import type ResourceType from "@double-agent/collect/interfaces/ResourceType";
...
export default class HttpHeadersPlugin extends Plugin {
	public initialize() {
		...
		this.registerRoute(
			"all",
			"/useJsToLoadNextPage",
			this.saveAndUseJsToLoadNextPage,
		);
		this.registerRoute(
			"all",
			"/redirectToNextPage",
			this.saveAndRedirectToNextPage,
		);
		this.registerRoute(
			"all",
			"/setCookies1",
			this.saveSetCookiesAndLinkToNextPage,
		);
		this.registerRoute(
			"all",
			"/setCookies2",
		...
		this.registerRoute(
			"all",
			"/page1CookieSameOrigin",
			this.saveAndLinkToNextPage,
		);
		...
		this.registerRoute(
			"all",
			"/cleanCookies",
			this.cleanCookiesAndLinkToNextPage,
		);
		...
		["http", "https", "http2"].forEach((protocol) => {
			pages.push(
				...
				{
					route: this.routes[protocol]["/linkToNext"],
					domain: CrossDomain,
					clickNext: true,
				},
				{
...
`);
	...
	public cleanCookiesAndLinkToNextPage(ctx: IRequestContext) {
		...
		ctx.res.writeHead(200, {
			"set-cookie": "cookie1=true; expires=Thu, 01 Jan 1970 00:00:00 GMT",
		});
		ctx.res.end(document.html);
	}
...
function saveHeadersToProfile(
	...
	const { domainType, originType, method, referer, resourceType } =
		ctx.requestDetails;
	...
	const profileData = ctx.session.getPluginProfileData<IProfileData>(
		plugin,
		[],
	);
	profileData.push({
