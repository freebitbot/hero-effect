...
import type { IRectLookup } from "@ulixee/unblocked-specification/agent/interact/IInteractionsHelper";
...
export default class DefaultHumanEmulator implements IUnblockedPlugin {
	...
	public async playInteractions(
		...
		for (let i = 0; i < interactionGroups.length; i += 1) {
			if (i > 0) {
				const millis =
					Math.random() * DefaultHumanEmulator.maxDelayBetweenInteractions;
				await delay(millis);
			}
					...
					for (const keyboardCommand of step.keyboardCommands) {
						...
						if ("string" in keyboardCommand) {
							for (const char of keyboardCommand.string) {
								await runFn(
									this.getKeyboardCommandWithDelay(
										{ string: char },
										millisPerCharacter,
									),
								);
	...
	protected async scroll(
		...
		for (const { x, y } of scrollVector) {
			...
			const shouldAddMouseJitter =
				counter % Math.round(Math.random() * 6) === 0;
		...
		for (let i = 0; i < retries; i += 1) {
			...
			if (targetRect.nodeVisibility?.isClickable === false) {
				interactionStep.mousePosition =
					await this.resolveMoveAndClickForInvisibleNode(
						interactionStep,
						runFn,
						helper,
						targetRect,
	...
	protected async moveMouse(
		...
		const rect = await helper.lookupBoundingRect(
			interactionStep.mousePosition,
			{
	...
	protected async jitterMouse(
		...
		const jitterX = Math.max(
			mousePosition.x + Math.round(getRandomPositiveOrNegativeNumber()),
			0,
		);
		const jitterY = Math.max(
			mousePosition.y + Math.round(getRandomPositiveOrNegativeNumber()),
			0,
		);
	...
	protected getKeyboardCommandWithDelay(
		...
		const randomFactor =
			getRandomPositiveOrNegativeNumber() * (millisPerChar / 2);
	...
	protected calculateMillisPerChar(): number {
		if (!this.millisPerCharacter) {
			const wpmRange =
				DefaultHumanEmulator.wordsPerMinuteRange[1] -
				DefaultHumanEmulator.wordsPerMinuteRange[0];
			const wpm =
				Math.floor(Math.random() * wpmRange) +
				DefaultHumanEmulator.wordsPerMinuteRange[0];
	...
	private async getScrollVector(
		...
		const { mousePosition, relativeToScrollOffset, verification } =
			interactionStep;
		if (isMousePositionXY(mousePosition)) {
			...
			if (relativeToScrollOffset) {
				scrollToPoint.y =
					scrollToPoint.y + relativeToScrollOffset.y - currentScrollOffset.y;
				scrollToPoint.x =
					scrollToPoint.x + relativeToScrollOffset.x - currentScrollOffset.x;
			}
			...
			const isRectVisible = helper.isRectInViewport(
				targetRect,
				viewportSize,
				50,
			);
		...
		for (let point of scrollVector) {
			...
			const scrollX = shouldScrollX
				? Math.round(point.x)
				: currentScrollOffset.x;
			const scrollY = shouldScrollY
				? Math.round(point.y)
	...
	private async resolveMoveAndClickForInvisibleNode(
		...
		const isOffscreen =
			!nodeVisibility.isOnscreenVertical ||
			!nodeVisibility.isOnscreenHorizontal;
		...
		if (hasDimensions && !!nodeVisibility.obstructedByElementRect) {
			...
			if (
				obstructedByElementRect.tag === "html" &&
				targetRect.elementTag === "option"
			) {
				return interactionStep.mousePosition;
			}
			...
			if (obstructedByElementRect.height >= viewport.height * 0.9)
				throw new Error(
					`${interactionName} element is obstructed by a full screen element`,
				);
		...
		throw new Error(
			`${interactionName} element isn't a ${interactionStep.command}-able target.`,
		);
	}
}
...
async function delay(millis: number): Promise<void> {
	...
	await new Promise<void>((resolve) =>
		setTimeout(resolve, Math.floor(millis)).unref(),
	);
}
