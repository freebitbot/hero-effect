...
import type IUserAgentData from "./interfaces/IUserAgentData";
import BrowserEngine from "./lib/BrowserEngine";
import BrowserEngineOptions from "./lib/BrowserEngineOptions";
import DataLoader from "./lib/DataLoader";
import type DomOverridesBuilder from "./lib/DomOverridesBuilder";
...
export default class DefaultBrowserEmulator<T = IEmulatorOptions>
	implements IUnblockedPlugin<T>
{
	...
	constructor(
		emulationProfile: IEmulationProfile<T>,
	...
	configure(emulationProfile: IEmulationProfile<T>): void {
		...
		emulationProfile.timezoneId ??=
			Intl.DateTimeFormat().resolvedOptions().timeZone;

		if (
			emulationProfile.upstreamProxyUrl &&
		...
		emulationProfile.browserEngine.isHeaded =
			emulationProfile.options.showChrome;
	}
	...
	public async onHttpAgentInitialized(agent: IHttpSocketAgent): Promise<void> {
		...
		if (upstreamProxyIpMask && profile.upstreamProxyUrl) {
			upstreamProxyIpMask.publicIp ??= await lookupPublicIp(
	...
	public addDomOverride(
		...
		if (runOn === "page") {
			...
			if (callback) {
				throw new Error(
					"Sorry, we can't add a callback function to a Worker environment.",
				);
			}
	...
	protected getUserAgentData(): IUserAgentData {
		...
		const { browserVersion, uaClientHintsPlatformVersion } =
		...
		const fullVersionList = brandData.map((x) => ({
			brand: x.brand,
			version:
				x.brand === "Chromium" || x.brand === "Chrome"
					? uaFullVersion
					: `${x.version}.0.0.0`,
		}));
		return {
			...
			fullVersionList,
			platform:
				this.data.windowNavigator.navigator.userAgentData.platform._$value,
	...
	public static shouldActivate(
		emulationProfile: IEmulationProfile<IEmulatorOptions>,
	): boolean {
		if (
		...
		if (!emulationProfile.userAgentOption) {
			try {
				const { browserEngine, userAgentOption } =
					DefaultBrowserEmulator.selectBrowserMeta(
		...
		if (emulationProfile.userAgentOption && !emulationProfile.browserEngine) {
			try {
				emulationProfile.browserEngine =
					DefaultBrowserEmulator.getBrowserEngine(
						emulationProfile.userAgentOption,
					);
			} catch (e) {
	...
	public static selectBrowserMeta(userAgentSelector?: string): {
		...
		const browserEngine =
			DefaultBrowserEmulator.getBrowserEngine(userAgentOption);

		return { browserEngine, userAgentOption };
	}

	public static getBrowserEngine(
		userAgentOption: IUserAgentOption,
	): IBrowserEngine {
