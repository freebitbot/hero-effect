...
export default class BrowserData implements IBrowserData {
	...
	constructor(dataLoader: DataLoader, userAgentOption: IUserAgentOption) {
		...
		if (!this.dataLoader.isSupportedEmulatorOs(this.osDataDir)) {
			...
			const closestVersionMatch = findClosestVersionMatch(
				os.version,
				otherVersions,
			);
...
function extractPolyfillFilename(dataDir: string): string {
	...
	if (!filenameMap) {
		...
		for (const filename of Fs.readdirSync(dataDir)) {
			const matches = filename.match(
				/^dom-polyfill-when-runtime-([a-z-]+)(-([0-9-]+))?.json$/,
			);
			if (!matches) continue;
	...
	if (!filenameMap[localOsMeta.name]) {
		throw new Error(
			`Your OS (${localOsMeta.name}) is not supported by this emulator.`,
		);
	}
...
export function createBrowserId(userAgentOption: IUserAgentOption): string {
	...
	return [browserName, browserVersion.major, browserVersion.minor]
		.filter((x) => x)
		.join("-");
}
...
function getOperatingSystemParts(userAgentOption: IUserAgentOption): {
	...
	const { operatingSystemCleanName: name, operatingSystemVersion: version } =
		userAgentOption;
	let { major, minor } = version;

	if (name.startsWith("mac")) {
		[major, minor] = convertMacOsVersionString(
			[major, minor].filter((x) => x).join("."),
		).split(".");
	}
