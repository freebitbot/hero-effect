...
export default function modifyHeaders(
	...
	const defaultOrder = getResourceHeaderDefaults(
		emulationProfile,
		data.headers,
		resource,
	);
	...
	const isXhr =
		resource.resourceType === "Fetch" || resource.resourceType === "XHR";
	...
	for (const headerName of defaultOrder.order) {
		...
		if (lowerName === "accept-language") {
			...
			lowerName === "sec-ch-viewport-width" ||
			lowerName === "viewport-width"
		) {
			value = emulationProfile.viewport.width;
		} else if (
			lowerName === "sec-ch-viewport-height" ||
			lowerName === "viewport-height"
		) {
			...
			lowerName === "sec-ch-device-memory" ||
			lowerName === "device-memory"
		) {
...
function getResourceHeaderDefaults(
	...
	if (defaultOrders.length > 1) {
		const filtered = defaultOrders.filter((x) =>
			x.originTypes.includes(originType),
		);
	...
	if (defaultOrders.length > 1) {
		...
		const filtered = defaultOrders.filter(
			(x) => (x.isRedirect ?? false) === isRedirect,
		);
		if (filtered.length) defaultOrders = filtered;
	}

	if (
		defaultOrders.length > 1 &&
		(headers["sec-fetch-user"] || headers["Sec-Fetch-User"])
	) {
		const filtered = defaultOrders.filter((x) =>
			x.orderKeys.has("sec-fetch-user"),
		);
		if (filtered.length) defaultOrders = filtered;
	}
