...
import type BrowserEngineOptions from "./BrowserEngineOptions";
import type DataLoader from "./DataLoader";
import type UserAgentSelector from "./UserAgentSelector";
...
export default class UserAgentOptions {
	...
	public findClosestInstalledToUserAgentString(
		...
		if (
			...
			!this.browserEngineOptions.installedOptions.some(
				(x) => x.id === browserId,
			)
		) {
	...
	private static parse(userAgentString: string): IUserAgentOption {
		if (UserAgentOptions.parsedCached[userAgentString])
			return UserAgentOptions.parsedCached[userAgentString];
		...
		let [osVersionMajor, osVersionMinor, osVersionPatch] =
			uaOs.version.split(".");
		...
		UserAgentOptions.parsedCached[userAgentString] = ua;
		return UserAgentOptions.parsedCached[userAgentString];
	}
	...
	private static random(dataUserAgentOptions: UserAgent[]): IUserAgentOption {
		...
		return UserAgentOptions.convertToUserAgentOption(dataUserAgentOption);
	}
	...
	private static convertToUserAgentOption(agent: UserAgent): IUserAgentOption {
		...
		const uaClientHintsPlatformVersion = agent.uaClientHintsPlatformVersions
			.length
		...
		const parsed = UserAgentOptions.parse(userAgent.string);

		if (UserAgentOptions.canTrustOsVersionForAgentString(parsed)) {
			userAgent.operatingSystemVersion = parsed.operatingSystemVersion;
		} else {
			UserAgentOptions.chooseOsPatchValue(userAgent);
		}
		return userAgent;
	}

	private static canTrustOsVersionForAgentString(
		agentOption: IUserAgentOption,
	): boolean {
	...
	private static replaceOperatingSystem(
		...
		const realOperatingSystem = dataUserAgentOptions.find(
			(x) =>
				...
				cleanName(x.operatingSystemName) ===
					userAgent.operatingSystemCleanName &&
				isLeftVersionGreater(
					x.operatingSystemVersion,
					userAgent.operatingSystemVersion,
				),
		...
		if (realOperatingSystem) {
			userAgent.operatingSystemVersion = {
				...realOperatingSystem.operatingSystemVersion,
			};
			UserAgentOptions.chooseOsPatchValue(userAgent);
		}
	}
