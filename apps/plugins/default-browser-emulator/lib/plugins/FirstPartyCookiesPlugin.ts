...
export default class FirstPartyCookiesPlugin implements IHooksProvider {
	...
	public async beforeHttpRequest(
		...
		if (request.method !== "OPTIONS") {
			...
			const existingCookies = Object.entries(request.requestHeaders).find(
				([key]) => key.match(/^cookie/i),
			);
	...
	public async beforeHttpResponse(
		...
		let cookies =
			resource.responseHeaders["set-cookie"] ??
			resource.responseHeaders["Set-Cookie"] ??
			[];
	...
	private loadProfileCookies(cookies, storage): void {
		const originUrls = (Object.keys(storage ?? {}) ?? []).map(
			(x) => new URL(x),
		);
		for (const cookie of cookies) {
			...
			const cookieProps = {
				...
				expires:
					cookie.expires !== "-1"
						? new Date(parseInt(cookie.expires, 10))
						: undefined,
			};
	...
	private documentHasUserActivity(url: URL): void {
		...
		if (!this.sitesWithUserInteraction.includes(hostname)) {
			...
			(async () => {
				for (const { cookie, sourceUrl, sameSiteContext } of this
					.cookiesPendingSiteInteraction[hostname] ?? []) {
	...
	private async getCookieHeader(
		...
		if (resource.isFromRedirect) {
			...
			if (
				getPublicSuffix(previousDomain.hostname) ===
				getPublicSuffix(currentUrl.hostname)
			) {
		...
		if (sameSiteContext === "none") {
			cookies = cookies.filter((x) =>
				this.hasFirstPartyInteractionForDomain(x.domain),
			);
		}

		cookies = this.handleNov2019ITPUpdates(
			sourceDocumentUrl,
			sameSiteContext,
			cookies,
		);
		 ...
		 */
		if (
			this.enableFeb2020ITPSupport &&
			this.isMinimumVersion(1) &&
	...
	private isMinimumVersion(minor: number, patch = 0): boolean {
		...
		return (
			Number(browserVersion.minor) >= minor &&
			Number(browserVersion.patch) >= patch
		);
	}
}

function getSameSiteContext(
	resource: IHttpResourceLoadDetails,
): SameSiteContext {
