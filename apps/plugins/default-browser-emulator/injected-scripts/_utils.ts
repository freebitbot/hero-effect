...
function internalModifyDescriptor<T, K extends keyof T>(
	...
	const originalValueDescriptors = ObjectCached.getOwnPropertyDescriptors(
		ObjectCached.getOwnPropertyDescriptor(descriptor, opts.descriptorKey)!
			.value,
	);
	// By defining these again we make sure we have all the correct properties set (eg name)
	ObjectCached.defineProperties(
		newDescriptor[opts.descriptorKey],
...
export function replaceFunction<T, K extends keyof T>(
	...
	return internalModifyDescriptor(obj, key, newFunction, {
		...opts,
		descriptorKey: "value",
	});
}
...
const nativeToStringFunctionString = `${Function.toString}`;
...
const proxyThisTracker = new Map<string, WeakRef<Object | symbol> | any>([
	["External", undefined],
]);
...
function runAndInjectProxyInStack(
	target: any,
	thisArg: any,
	argArray: any,
	proxy: any,
...
(function trackProxyInstances() {
	...
	ObjectCached.defineProperty(self, "Proxy", {
		...
		value: function Proxy(this, target, handler) {
			...
			const result = ReflectCached.construct(
				OriginalProxy,
				[target, handler],
				new.target,
			);
...
function internalCreateFnProxy<T extends Function>(opts: {
				...
				inner: {
					...
					apply: (fnTarget, fnThisArg, fnArgArray) => {
						...
						if (opts.inner?.fixThisArg && fnThisArg === receiver) {
							return runAndInjectProxyInStack(
								fnTarget,
								target,
								fnArgArray,
								proxy,
	...
	if (proxy instanceof Function) {
		const toString =
			overriddenFns.get(opts.target as Function) ?? opts.target.toString();
...
function proxyFunction<T, K extends keyof T>(
	...
	if (!descriptorInHierarchy) {
		throw new Error(
			`Could not find descriptor for function: ${String(functionName)}`,
		);
	}
		...
		inner: {
			apply: (target, thisArg, argArray) => {
				const shouldOverride =
					!opts?.overrideOnlyForInstance || thisArg === thisObject;
				if (shouldOverride) {
...
function proxyGetter<T, K extends keyof T>(
	...
	if (!descriptorInHierarchy) {
		throw new Error(
			`Could not find descriptor for getter: ${String(propertyName)}`,
		);
	}
	...
	if (!descriptor.get) {
		throw new Error(
			"Trying to apply a proxy getter on something that doesnt have a getter",
		);
	}
...
function proxySetter<T, K extends keyof T>(
	...
	if (!descriptorInHierarchy) {
		throw new Error(
			`Could not find descriptor for setter: ${String(propertyName)}`,
		);
	}
	const { descriptorOwner, descriptor } = descriptorInHierarchy;
	if (!descriptor.set) {
		throw new Error(
			"Trying to apply a proxy setter on something that doesnt have a setter",
		);
	}
...
function addDescriptorAfterProperty(
	...
	if (!owner) {
		console.log(
			`ERROR: Parent for property descriptor not found: ${path} -> ${propertyName}`,
		);
		return;
	}
	...
	if (inHierarchy && descriptor.value) {
		if (inHierarchy.descriptor.get) {
			replaceGetter(owner, propertyName, () => descriptor.value, {
	...
	if (owner === self) {
		...
		return reorderNonConfigurableDescriptors(
			path,
			propertyName,
			prevProperty,
			propertyName,
...
replaceFunction(
	...
	"getOwnPropertyDescriptors",
	(target, thisArg, argArray) => {
		const descriptors = ReflectCached.apply(
			target,
			thisArg,
			argArray,
		) as ReturnType<ObjectConstructor["getOwnPropertyDescriptors"]>;
		const reorders =
			reordersByObject.get(thisArg) ?? reordersByObject.get(argArray.at(0));
		if (reorders) {
...
function reorderDescriptor(path, propertyName, prevProperty, throughProperty) {
	...
	const prevDescriptor = ObjectCached.getOwnPropertyDescriptor(
		owner,
		prevProperty,
	);
	if (!prevDescriptor) {
...
function newObjectConstructor(
	...
	return function () {
		...
		ObjectCached.setPrototypeOf(
			obj,
			prototypesByPath[newProps._$protos[0]] ??
				getObjectAtPath(newProps._$protos[0]),
		);
		for (const [prop, value] of props) {
			...
			ObjectCached.defineProperty(
				obj,
				propName,
				buildDescriptor(value, `${path}.${prop}`),
			);
		}
		return obj;
	};
...
function buildDescriptor(entry: IDescriptor, path: string): PropertyDescriptor {
	...
	if (entry.prototype) {
		...
		if (entry._$function) {
			overriddenFns.set(
				prototypeDescriptor.value.constructor,
				entry._$function,
			);
			toOriginalFn.set(attrs.set!, entry._$set);
		}
	...
	if (entry._$function) {
		...
		if (newProps) {
			attrs.value = newObjectConstructor(
				newProps,
				path,
				entry._$invocation,
				entry._$isAsync,
			...
			ObjectCached.keys(entry)
				.filter((key): key is OtherInvocationKey =>
					key.startsWith("_$otherInvocation"),
				)
				// Not supported currently
				.filter((key) => !key.includes("new()"))
				.forEach((key) =>
					OtherInvocationsTracker.addOtherInvocation(path, key, entry[key]),
				);
			...
			attrs.value = new Proxy(Function.prototype.call.bind({}), {
				apply(_target, thisArg) {
					const invocation =
						OtherInvocationsTracker.getOtherInvocation(path, thisArg)
							?.invocation ?? entry._$invocation;
	...
	if (typeof entry === "object") {
		const props = ObjectCached.entries(entry).filter(
		...
		if (entry._$protos) {
			const proto =
				prototypesByPath[entry._$protos[0]] ??
				getObjectAtPath(entry._$protos[0]);
...
function invocationReturnOrThrow(
	invocation: any,
	isAsync?: boolean,
): any | Promise<any> {
...
class OtherInvocationsTracker {
	...
	static addOtherInvocation(
		...
		OtherInvocationsTracker.basePaths.add(basePath);
		OtherInvocationsTracker.otherInvocations.set(
			OtherInvocationsTracker.key(basePath, otherPath),
			{
	...
	static getOtherInvocation(
		...
		const info = OtherInvocationsTracker.otherInvocations.get(
			OtherInvocationsTracker.key(basePath, otherPath),
		);
		return {
			path: otherPath,
...
class PathToInstanceTracker {
	...
	static addPath(path: string) {
		PathToInstanceTracker.pathsToTrack.add(path);
	}

	static getPath(instance: any) {
		return PathToInstanceTracker.instanceToPath.get(instance);
	}

	static updateAllReferences() {
		PathToInstanceTracker.instanceToPath.clear();
		for (const path of PathToInstanceTracker.pathsToTrack) {
			PathToInstanceTracker.instanceToPath.set(
				PathToInstanceTracker.getInstanceForPath(path),
				path,
			);
		}
	}
