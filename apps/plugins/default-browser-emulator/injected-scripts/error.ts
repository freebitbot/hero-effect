...
export function main({
	args,
	sourceUrl,
	utils: { ObjectCached, ReflectCached, toOriginalFn, replaceFunction },
}: ScriptInput<Args>) {
	...
	ObjectCached.getOwnPropertyNames(self).forEach((key) => {
		...
		const item = self[key];
		if (
			OriginalError.isPrototypeOf(item) &&
			ObjectCached.getPrototypeOf(item) === OriginalError
		) {
	...
	function prepareStackAndStackTraces(
		...
		for (let i = 1; i < lines.length; i++) {
			...
			if (line.includes(sourceUrl) && args.removeInjectedLines) continue;
	...
	function prepareStackTrace(error, stackTraces) {
		const { stack, stackTraces: safeStackTraces } = prepareStackAndStackTraces(
			error,
			stackTraces,
		);
