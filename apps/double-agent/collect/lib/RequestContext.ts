...
import type { IPluginPage } from "./Plugin";
...
export default class RequestContext implements IRequestContext {
	...
	constructor(
		...
		if (pageIndexStr) {
			const pages =
				this.plugin.pagesByAssignmentType[this.session.assignmentType];
	...
	public get page(): IPluginPage {
		return this.plugin.pagesByAssignmentType[this.session.assignmentType][
	...
	public get nextPageLink(): string {
		...
		const page =
			this.plugin.pagesByAssignmentType[this.session.assignmentType][pageIndex];
		return this.plugin.convertToSessionPage(page, this.session.id, pageIndex)
			.url;
	}
	...
	public buildUrl(
		...
		const { port, plugin } = this.plugin.getServer(
			protocol,
			this.session.id,
