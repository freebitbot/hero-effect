...
import type ISessionPage from "../interfaces/ISessionPage";
...
export default abstract class Plugin extends EventEmitter implements IPlugin {
	...
	public convertToSessionPage(
		...
		const domain =
			page.domain || (protocol === Protocol.tls ? TlsDomain : MainDomain);
	...
	private async createServer(
		...
		if (protocol === Protocol.tls) {
			this.tlsServerBySessionId[sessionId] = await new TlsServer(
				port,
				routesByPath,
			).start(serverContext);
			...
			this.httpServer = await new HttpServer(port, routesByPath).start(
				serverContext,
			);
			console.log(`${this.id} listening on ${port} (HTTP)`);
		} else if (protocol === Protocol.https) {
			if (this.httpsServer) return;
			this.httpsServer = await new HttpsServer(port, routesByPath).start(
				serverContext,
			);
			console.log(`${this.id} listening on ${port} (HTTPS)`);
		} else if (protocol === Protocol.http2) {
			if (this.http2Server) return;
			this.http2Server = await new Http2Server(port, routesByPath).start(
				serverContext,
			);
