...
export default async function extractRequestDetails(
	...
	const requestDetails: IRequestDetails = {
		...
		resourceType:
			overrideResourceType ?? getResourceType(req.method, requestUrl.pathname),
	};
...
export function getOriginType(
	...
	if (
		hostDomainType === DomainType.SubDomain &&
		refererDomainType === DomainType.MainDomain
	) {
		return OriginType.SameSite;
	}

	if (
		hostDomainType === DomainType.MainDomain &&
		refererDomainType === DomainType.SubDomain
	) {
		return OriginType.SameSite;
	}
