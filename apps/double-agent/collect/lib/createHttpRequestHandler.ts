...
export default function createHttpRequestHandler(
	...
	return async function requestHandler(
		...
		if (req.method === "HEAD") {
			...
			console.log(
				"HEAD request inbound. Should not be getting this.",
				String(req.url).replace(/\n|\r/g, ""),
				String(req.headers).replace(/\n|\r/g, ""),
			);
			res.end();
			return;
		}
		...
		if (
			!isRecognizedDomain(requestUrl.host, [MainDomain, SubDomain, CrossDomain])
		) {
			req.socket.destroy();
			console.warn(
				"Invalid domain used to access site",
				String(req.url).replace(/\n|\r/g, ""),
				String(req.headers.host).replace(/\n|\r/g, ""),
			);
			return;
		}
		...
		try {
			const sessionId = sessionTracker.getSessionIdFromServerRequest(
				server,
				req,
			);
			...
			const { requestDetails } = await extractRequestDetails(
				server,
				req,
				session,
			);
