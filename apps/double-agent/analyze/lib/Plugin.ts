...
import {
	...
	type ICheckMeta,
	type ICheckType,
} from "./checks/BaseCheck";
import type Layer from "./Layer";
import type Probe from "./Probe";
import type ProbeBucket from "./ProbeBucket";
import ProbeBucketGenerator, {
	type IProbeBucketMeta,
} from "./ProbeBucketGenerator";
...
export default abstract class Plugin {
	...
	protected runProbes(
		...
		const checkType = profileCountOverTime
			? CheckType.OverTime
		...
		const probeBuckets = this.probeBuckets.filter((x) => {
			return (
				x.layerId === layer.id &&
				(!x.userAgentIds.length || x.userAgentIds.includes(userAgentId))
			);
		});
		...
		for (const probeBucket of probeBuckets) {
			for (const probe of probeBucket.probes) {
				...
				const humanScore = probe.check.generateHumanScore(
					toCheck,
					profileCountOverTime,
				);

				if (humanScore < 100) {
					const invalidCheckSignature =
						checksById[probe.check.id]?.signature ?? "[None Provided]";
