...
import type { ICheckType } from "./checks/BaseCheck";
import extractBrowserGroupings from "./extractBrowserGroupings";
import Layer from "./Layer";
import type { BaseMatcher } from "./matchers";
import Probe from "./Probe";
import ProbeBucket from "./ProbeBucket";
import type { BaseScorer } from "./scorers";
	...
	constructor(pluginId: string, meta: IProbeBucketMeta) {
		...
		for (const check of meta.checks) {
			...
			if (check.identity.userAgentId) {
				...
				throw new Error(
					`Check is missing userAgentId and is not universal: ${check.signature}`,
				);
			}
		}
		...
		for (const checkSignature of Object.keys(userAgentIdsByCheckSignature)) {
			...
			const userAgentIds = Array.from(
				userAgentIdsByCheckSignature[checkSignature],
			).sort();
			const groupId = `${checkType}:${userAgentIds.join(":")}`;
			groupsById[groupId] = groupsById[groupId] || {
				userAgentIds,
				checkType,
				checkSignatures: [],
			};
		...
		if (universalCheckSignatures.size) {
			const universalCheckSignaturesByCheckType = Array.from(
				universalCheckSignatures,
			).reduce((byCheckType, checkSignature) => {
				const probe = probesByCheckSignature[checkSignature];
				const checkType = probe.check.type;
			...
			for (const checkType of Object.keys(
				universalCheckSignaturesByCheckType,
			)) {
		...
		for (const groupId of Object.keys(groupsById)) {
			...
			const isBucketed = groupingDetails.every((x) =>
				x[0].includes("AllProfiled"),
			);
		...
		for (const group of sortedGroups) {
			const groupingDetails = extractBrowserGroupings(group.userAgentIds);
			const probes = group.checkSignatures.map(
				(id) => probesByCheckSignature[id],
			);
