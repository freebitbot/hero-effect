...
export default function extractBrowserGroupings(
	userAgentIds: string[],
): [string, string[]][] {
	const { idsByGroup, hasAllOf, hasAllExcept, hasNone } =
		extractGroupedIds(userAgentIds);
	...
	for (const userAgentId of userAgentIds) {
		...
		if (hasNone.includes(userAgentId)) {
			details[`${osName}-${osVersion}`] =
				details[`${osName}-${osVersion}`] || [];
			details[`${osName}-${osVersion}`].push(
				`${browserName}-${browserVersion}`,
			);
		}
	}

	for (const groupName of Object.keys(hasAllOf).concat(
		Object.keys(hasAllExcept),
	)) {
		const titleizedGroupName = (
			groupName.charAt(0).toUpperCase() + groupName.slice(1)
		).replace("-os-x", "");
		if (hasAllOf[groupName]) {
			details[`AllProfiled${titleizedGroupName}`] = [
				idsByGroup[groupName].length.toString(),
			];
		} else if (hasAllExcept[groupName]) {
			details[`AllProfiled${titleizedGroupName}Except`] =
				hasAllExcept[groupName];
		}
	}
...
function extractGroupedIds(userAgentIds: string[]): IBrowserGrouping {
	...
	const browserGrouping = extractGroupedIdsBy(
		Config.browserNames,
		userAgentIds,
	);
