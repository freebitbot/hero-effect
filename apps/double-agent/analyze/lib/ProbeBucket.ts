...
export default class ProbeBucket {
	...
	constructor(
		...
		if (probes.some((x) => x.checkType !== checkType)) {
			throw new Error(
				"Probes within a ProbeBucket must share the same CheckType",
			);
		}
	}
	...
	public static load(
		probeBucketObj: any,
		probesById: { [id: string]: Probe },
	): ProbeBucket {
		const probes = probeBucketObj.probeIds.map((id) => probesById[id]);
		const { id, layerId, checkType, userAgentIds, matcher, scorer } =
			probeBucketObj;
		return new ProbeBucket(
			id,
			layerId,
...
function generateId(layer: Layer): string {
	...
	if (layerMetaMap[layer.key].pluginId !== layer.pluginId) {
		throw new Error(
			`Layer key (${layer.key}) already assigned to ${layer.pluginId} plugin.`,
		);
	}
