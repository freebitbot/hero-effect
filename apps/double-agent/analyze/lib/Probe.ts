...
export default class Probe {
	...
	public get check(): BaseCheck {
		if (!this._check) {
			...
			const pluginChecksDir = Path.resolve(
				__dirname,
				`../plugins/${this.pluginId}/lib/checks`,
			);
			let Check: any;
			try {
	...
	public toJSON(): Pick<
		Probe,
		"id" | "checkName" | "checkMeta" | "checkType" | "args"
	> {
		return {
			id: this.id,
	...
	public static load(probeObj: any, pluginId: string): Probe {
		...
		return new Probe(id, checkName, checkType, checkMeta, args, pluginId);
	}
}
...
function generateId(check: BaseCheck, pluginId: string): string {
	...
	if (!id) {
		...
		id =
			`${check.prefix}-${convertToAlpha(counterByPrefix[check.prefix])}`.toLowerCase();
	}
