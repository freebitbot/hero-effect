...
export default async function buildAllAssignments(
	...
	for (const userAgentToTest of userAgentsToTest) {
		...
		assignments.push(
			buildAssignment(id, assignments.length, id, type, userAgentString, null),
		);
	}
...
function buildAssignmentsOverTime(
	...
	const assignments: IAssignment[] = [];
	const selUserAgents = userAgentsToTest.filter((x) =>
		x.pickTypes.includes(pickType),
	);
	...
	while (assignments.length < 100) {
		...
		for (userAgentToTest of sortedUserAgents) {
			...
			const pctIncluded =
				(countByUserAgentId[userAgentId] / assignments.length) * 100;
...
export function extractMetaFromOverTimeSessionKey(sessionKey: string): {
	...
	const [pickType, indexPos, userAgentId] = sessionKey
		.match(/^([a-z]+)-([0-9]+):(.+)$/)
		.slice(1);
	return {
