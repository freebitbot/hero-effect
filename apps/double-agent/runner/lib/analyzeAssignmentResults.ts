...
export default async function analyzeAssignmentResults(
	...
	for (const userAgentId of userAgentIds) {
		const flags = analyze.addIndividual(
			`${assignmentsDataDir}/individual`,
			userAgentId,
		);
...
async function saveFlagsToPluginFiles(
	saveToDir: string,
	flags: IResultFlag[],
): Promise<void> {
	...
	for (const pluginId of Object.keys(flagsByPluginId)) {
		...
		await Fs.writeFile(
			filePath,
			JSON.stringify(flagsByPluginId[pluginId], null, 2),
		);

		const signaturesFilePath = Path.resolve(
			saveToDir,
			`${pluginId}-signatures.json`,
		);
		await Fs.writeFile(
			signaturesFilePath,
			JSON.stringify(
