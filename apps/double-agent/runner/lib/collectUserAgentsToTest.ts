...
async function writeUserAgentsToTest(
	...
	await FsPromises.writeFile(
		`${outFilePath}.json`,
		JSON.stringify(userAgentsToTest, null, 2),
	);
}
...
async function collectUserAgentsToTest(
	...
	const probeTcpFilePath = Path.join(
		Config.probesDataDir,
		"/probe-buckets/tcp.json",
	);
	if (!(await exists(probeTcpFilePath))) {
		return userAgentsToTest;
	}

	const tcpProbeBuckets = JSON.parse(
		await FsPromises.readFile(probeTcpFilePath, "utf8"),
	);
	const userAgentIds: Set<string> = new Set();
	tcpProbeBuckets.forEach((probeBucket) => {
		probeBucket.userAgentIds.forEach((userAgentId) =>
...
export { collectUserAgentsToTest, writeUserAgentsToTest };
