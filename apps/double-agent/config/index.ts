...
export default class Config {
	...
	static profilesDataDir = Path.resolve(
		Paths.rootDir,
		"../../..",
		...
		if (!probeIdsMap) {
			...
			if (Fs.existsSync(probeIdsDir)) {
				for (const fileName of Fs.readdirSync(probeIdsDir)) {
					...
					probeIdsMap[pluginId] = JSON.parse(
						Fs.readFileSync(`${probeIdsDir}/${fileName}`, "utf8"),
					);
				}
			}
		}
	...
	static get browserNames(): string[] {
		const names = Config.userAgentIds.map(
			(userAgentId) =>
				RealUserAgents.extractMetaFromUserAgentId(userAgentId).browserName,
		);
	...
	static findUserAgentIdsByName(name: string): string[] {
		return Config.userAgentIds.filter((userAgentId) => {
	...
	static shouldIgnorePathValue(path: string): boolean {
		...
		if (
			...
			return true;
		if (
			browserstackIndicators.changedOrder.some((pattern) =>
				pathIsPatternMatch(path, pattern),
			)
		)
			return true;

		if (Config.isVariationPath(path)) return true;
		return false;
	}
}
