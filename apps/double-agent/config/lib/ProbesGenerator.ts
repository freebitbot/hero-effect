...
export default class ProbesGenerator {
	...
	public async clearBuckets(): Promise<void> {
		...
		await Fs.promises
			.rm(probeBucketsDir, { recursive: true })
			.catch(() => null);
	...
	public save(): void {
		...
		for (const plugin of this.plugins) {
			...
			Fs.writeFileSync(
				`${probeBucketsDir}/${plugin.id}.json`,
				probeBucketsData,
			);
			...
			const probeIdsData = JSON.stringify(
				Config.probeIdsMap[plugin.id],
				null,
				2,
			);
			...
			for (const layer of plugin.layers) {
				const probeBuckets = plugin.probeBuckets.filter(
					(x) => x.layerId === layer.id,
				);
				const checkCount = probeBuckets
					.map((p) => p.probes.length)
					.reduce((a, b) => a + b, 0);
