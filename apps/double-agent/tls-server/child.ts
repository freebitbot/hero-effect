...
process.on("message", (message: any) => {
	if (message.start) {
		...
		if (!activeConnection || activeConnection.id !== response.connectionId) {
			process.send({
				error: `ConnectionId could not be found: ${response.connectionId}`,
			});
			return;
		}
...
function millisUntilNextConnect(): number {
	...
	return elapsedMillis >= minMillisBetweenConnects
		? 0
		: minMillisBetweenConnects - elapsedMillis;
}
...
async function onConnection(req, res): Promise<void> {
	...
	if (activeConnection) {
		res.end(
			`Overloaded. Try again in ${minMillisBetweenConnects / 1000} seconds`,
		);
