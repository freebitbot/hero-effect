...
import type IServerHello from "../interfaces/IServerHello";
import * as extensions from "../spec/extensions.json";

export default function parseHelloMessage(
	isClientHello: boolean,
	lines: string[],
): IClientHello | IServerHello {
	...
	for (const line of lines) {
		...
		if (helloLine.startsWith("client_version")) {
			...
			if (helloLine.includes("extension_type=")) {
				...
				message.extensions[message.extensions.length - 1].values.push(
					helloLine.trim(),
				);
			}
		} else {
				...
				for (const x in ext.values) {
					...
					for (const grease of greaseCodes) {
						if (
							...
							value.includes(`(${grease.toString(10)})`)
						) {
