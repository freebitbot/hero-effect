...
import type { IReplayRegistryApis } from "@ulixee/platform-specification/services/ReplayRegistryApis";
	...
	implements ISessionRegistry
{
	...
	constructor(
		...
		if (!this.serviceClient) {
			this.replayStorageRegistry = new ReplayRegistryDiskStore(
				config.queryHeroStorageDir,
			);
		}
		this.defaultSessionRegistry = new DefaultSessionRegistry(
			config.defaultHeroStorageDir,
		);
	}
	...
	public async store(sessionId: string): Promise<void> {
		...
		this.storePromises
			.get(sessionId)
			.push(
				this.storeInternal(sessionId).catch((e) =>
					console.warn(`Error storing cached session`, e),
				),
			);
	}
