...
import type { IHeroExtractorRunOptions } from "@ulixee/datastore-plugins-hero";
...
export default class DatastoreForHeroPluginCore
	implements IExtractorPluginCore
{
	...
	public async onCoreStart(
		...
		options: {
			createConnectionToServiceHost: (
				host: string,
			) => ConnectionToCore<any, any>;
			getSystemCore: (
				name: "heroCore" | "datastoreCore" | "desktopCore",
			) => any;
		},
	): Promise<void> {
		...
		this.replayRegistry = new ReplayRegistry({
			...
			serviceClient: options.createConnectionToServiceHost(
				coreConfigureOptions.replayRegistryHost,
			),
		});
		...
		if (process.platform === "win32") {
			this.nodeVmSandboxList = this.nodeVmSandboxList.map((x) =>
				x.replace(/\//g, "\\"),
			);
		}
	...
	public async afterRunExtractor(
		...
		const actions = [];
		for (const [sessionId, { action, crawler }] of Object.entries(
			cacheUpdates,
		)) {
	...
	public registerHostedServices(
		connectionToClient: ConnectionToClient<any, any>,
	): void {
		this.endpoints?.attachToConnection(connectionToClient, {
