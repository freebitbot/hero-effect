...
export default class SqliteStorageEngine extends AbstractStorageEngine {
	...
	public override query<TResult>(
		...
		if (virtualEntitiesByName) {
			for (const [name, virtualEntity] of Object.entries(
				virtualEntitiesByName,
			)) {
				...
				const columns = Array.from(
					new Set([
						...Object.keys(schema ?? {}),
						...Object.keys(virtualEntity.records[0] ?? []),
					]),
				);
				schemas.push(schema);
		...
		return Promise.resolve(
			this.recordsFromEngine(records, schemas, tmpSchemaFieldTypes),
		);
	}

	protected override async createTable(
		name: string,
		schema: ISchema,
	): Promise<void> {
