...
import type { SqlParser } from "@ulixee/sql-engine";
import type {
	IDbJsTypes,
	IDbTypeNames,
} from "@ulixee/sql-engine/interfaces/IDbTypes";
import type ISqlAdapter from "@ulixee/sql-engine/interfaces/ISqlAdapter";
...
export default abstract class AbstractStorageEngine implements IStorageEngine {
	...
	public bind(
		datastore: IDatastoreComponents<TTables, TExtractors, TCrawlers>,
	): void {
	...
	protected recordToEngineRow(
		...
		for (const key of Object.keys(record)) {
			...
			const [convertedValue, tmpType] = this.adapter.toEngineValue(
				typeName,
				record[key],
			);
	...
	protected recordsFromEngine<TResult = any[]>(
		...
		for (const record of records) {
			for (const key of Object.keys(record)) {
				...
				record[key] = this.adapter.fromEngineValue(
					field || tmpSchemaFieldTypes[key],
					record[key],
				);
			}
		}
