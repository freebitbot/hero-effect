...
export default function cliCommands(): Command {
	...
	cli
		...
		.option(
			"-p, --passphrase <phrase>",
			"Save the private key with a passphrase (pkcs8 format).",
		)
		.option(
	...
	cli
		...
		.option(
			"-p, --passphrase <phrase>",
			"Save identity to a file with a passphrase.",
		)
		.option(
			"-c, --passphrase-cipher <cipher>",
			"Encrypt the internal key with a cipher (pkcs8 format).",
		...
		.action(async ({ privateKey, filename, passphraseCipher, passphrase }) => {
			const ed25519 = Ed25519.createPrivateKeyFromBytes(
	...
	cli
		...
		.action(({ pem, filename, passphrase }) => {
			if (filename) {
				...
				pem = pem?.replaceAll("\\n", "\n");
				const identity = Identity.loadFromPem(pem, {
					keyPassphrase: passphrase,
				});
