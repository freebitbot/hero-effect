...
export default async function cloneDatastore(
	...
	options: {
		embedCredits?: { id: string; secret: string };
	...
	const {
		...
		host,
	} = await DatastoreApiClient.lookupDatastoreHost(
		url,
		options.argonMainchainUrl,
	);
	...
	const meta = await datastoreApiClient.getMetaAndSchema(
		datastoreId,
		datastoreVersion,
	);
	...
	const passthroughExtractors = Object.entries(meta.extractorsByName).map(
		([x, extractor]) => {
			let schemaLine = "";
			...
			if (extractor.schemaAsJson) {
				schemasByName[`${x}`] = {
					isTable: false,
					schemaJson: extractor.schemaAsJson,
				};
				schemaLine = `\n  schema: ${x}(),\n`;
			}
			let descriptionLine = "";
  ...
  remoteExtractor: 'source.${x}',${schemaLine}${descriptionLine}
	...
	const passthroughTables = Object.entries(meta.tablesByName).map(
		([x, table]) => {
			let schemaLine = "";
  ...
  export default datastore;`;
	...
	if (!existsSync(Path.join(folder, "tsconfig.json"))) {
		copyFileSync(
			`${__dirname}/cloned-tsconfig.json`,
