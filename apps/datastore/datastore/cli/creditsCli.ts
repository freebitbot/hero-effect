...
export default function creditsCli(): Command {
	...
	cli
		...
		.action(
			async (
				url,
				{ identityPath, identityPassphrase, argons, argonMainchainUrl },
			) => {
				const microgons =
					BigInt(parseFloat(argons)) * ArgonUtils.MicrogonsPerArgon;
				const identity = Identity.loadFromFile(identityPath, {
					keyPassphrase: identityPassphrase,
				});
				const { datastoreId, version, host } =
					await DatastoreApiClient.lookupDatastoreHost(url, argonMainchainUrl);
...
function requiredOptionWithEnv(
	flags: string,
	description: string,
	envVar: string,
): Option {
