...
import type ILocalUserProfile from "../interfaces/ILocalUserProfile";

export default class LocalUserProfile implements ILocalUserProfile {
	public static path = Path.join(
		getDataDirectory(),
		"ulixee",
		"user-profile.json",
	);
	public clouds: (ILocalUserProfile["clouds"][0] & {
		adminIdentity?: string;
	})[] = [];
	public databrokers: ILocalUserProfile["databrokers"] = [];
	public localchains: ILocalUserProfile["localchains"] = [];
	public localchainForQueryName = "primary";
	public localchainForCloudNodeName: string;
	...
	public get defaultAdminIdentity(): Identity {
		if (this.defaultAdminIdentityPath) {
			this.#defaultAdminIdentity ??= Identity.loadFromFile(
				this.defaultAdminIdentityPath,
			);
	...
	public getAdminIdentity(datastoreId: string, cloudName: string): Identity {
		...
		if (cloud?.adminIdentityPath)
			return Identity.loadFromFile(cloud.adminIdentityPath);
	}
	...
	public async save(): Promise<void> {
		await safeOverwriteFile(
			LocalUserProfile.path,
			TypeSerializer.stringify(this.toJSON()),
		);
	}
	...
	private loadProfile(): void {
		...
		try {
			...
			for (const cloud of this.clouds) {
				if (cloud.adminIdentityPath) {
					cloud.adminIdentity = Identity.loadFromFile(
						cloud.adminIdentityPath,
					).bech32;
				}
			}
