...
import type { ExtractSchemaType } from "@ulixee/schema";
...
import type { IExtractorPluginConstructor } from "../interfaces/IExtractorPluginStatics";
import type IExtractorSchema from "../interfaces/IExtractorSchema";
import type DatastoreApiClient from "./DatastoreApiClient";
import type { IDatastoreExecRelayArgs } from "./DatastoreApiClient";
...
export default class PassthroughExtractor<
	...
	TSchema extends IExtractorSchema = IExtractorSchema<any, any>,
	TPlugin1 extends
		IExtractorPluginConstructor<TSchema> = IExtractorPluginConstructor<TSchema>,
	TPlugin2 extends
		IExtractorPluginConstructor<TSchema> = IExtractorPluginConstructor<TSchema>,
	TPlugin3 extends
		IExtractorPluginConstructor<TSchema> = IExtractorPluginConstructor<TSchema>,
...
> extends Extractor<TSchema, TPlugin1, TPlugin2, TPlugin3, TContext> {
	...
	constructor(
		...
		assert(
			components.remoteExtractor.includes("."),
			"A remote function source is required",
		);
	...
	protected async run(context: TContext): Promise<void> {
		...
		const queryResult = this.upstreamClient.stream<{
			output: TOutput;
			input: any;
		}>(
		...
		if (this.passThroughComponents.onResponse) {
			...
			const responseContext: TContext & { stream: AsyncIterable<TOutput> } =
				context as any;
	...
	protected async injectRemoteClient(): Promise<void> {
		...
		const { datastoreHost, client } =
			await this.datastoreInternal.getRemoteApiClient(this.remoteSource);
