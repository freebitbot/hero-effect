...
> extends Table<TSchema> {
	...
	constructor(components: TComponents) {
		...
		assert(
			components.remoteTable.includes("."),
			"A remote table source is required",
		);
	...
	public override async queryInternal<T = TSchemaType[]>(
		...
		return callbacks.onPassthroughTable(
			this.name,
			options,
			async (modifiedOptions) => {
