...
import type IQueryLogEntry from "../interfaces/IQueryLogEntry";

export default class QueryLog {
	public queriesById: { [id: string]: IQueryLogEntry } = {};
	public queryLogPath = Path.join(
		getDataDirectory(),
		"ulixee",
		"user-querylog.jsonl",
	);
	...
	public monitor(onNewQuery: (query: IQueryLogEntry) => any): {
		stop: () => void;
	} {
	...
	public log(
		...
		const input =
			"boundValues" in query ? query.boundValues : streamQuery.input;
	...
	private publishQueries(): void {
		void this.readQueue.run(async () => {
			try {
				const readable = Fs.createReadStream(this.queryLogPath, {
					start: this.queryLogBytesRead,
				});
