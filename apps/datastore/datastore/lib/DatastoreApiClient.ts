...
import DatastoreApiSchemas, {
	type IChannelHoldApis,
	type IChannelHoldEvents,
	type IDatastoreApis,
	type IDatastoreApiTypes,
} from "@ulixee/platform-specification/datastore";
import type IChannelHoldApiTypes from "@ulixee/platform-specification/datastore/ChannelHoldApis";
import { ChannelHoldApisSchema } from "@ulixee/platform-specification/datastore/ChannelHoldApis";
import type { IDatastoreQueryResult } from "@ulixee/platform-specification/datastore/DatastoreApis";
import type IBalanceChange from "@ulixee/platform-specification/types/IBalanceChange";
...
import type { IQueryOptionsWithoutId } from "../interfaces/IQueryOptions";
import type ITypes from "../types";
import installDatastoreSchema, {
	addDatastoreAlias,
} from "../types/installDatastoreSchema";
...
export default class DatastoreApiClient {
	...
	private manifestByDatastoreUrl = new Map<
		string,
		TimedCache<IDatastoreMeta>
	>();
	...
	constructor(
		host: string,
		options?: { consoleLogErrors?: boolean; storeQueryLog?: boolean },
	) {
	...
	public async disconnect(): Promise<void> {
		await Promise.all([
			this.queryLog?.close(),
			this.connectionToCore.disconnect(),
		]);
	}

	public async getMetaAndSchema(
		id: string,
		version: string,
	): Promise<IDatastoreMeta> {
	...
	> {
		...
		(async () => {
			...
			try {
				...
				const result = await this.runApi("Datastore.stream", query).catch(
					async (err) => {
	...
	>(
		...
		IDatastoreQueryResult & { outputs?: ISchemaOutput[]; queryId: string }
	> {
		const startDate = new Date();
		options ??= {} as any;
		const queryId = options?.queryId ?? nanoid(12);
	...
	public async upload(
		...
		if (options.identity) {
			...
			const message =
				DatastoreApiClient.createUploadSignatureMessage(compressedDbx);
	...
	protected async runApi(
		...
		try {
			if (this.validateApiParameters) {
				const schema =
					DatastoreApiSchemas[command] ?? ChannelHoldApisSchema[command];
	...
	public static async lookupDatastoreHost(
		...
		const mainchainClient = argonMainchainUrl
			...
			: null;
		try {
			return await new DatastoreLookup(mainchainClient).getHostInfo(
				datastoreUrl,
			);
		} finally {
			if (mainchainClient) {
	...
	public static createExecSignatureMessage(
		payment: IPayment,
		nonce: string,
	): Buffer {
		return sha256(
	...
	public static createExecAuthentication(
		...
		nonce ??= nanoid(10);
		const message = DatastoreApiClient.createExecSignatureMessage(
			payment,
			nonce,
		);
		return {
	...
	public static createDownloadSignatureMessage(
		...
		return sha256(
			concatAsBuffer("Datastore.download", id, version, requestDate),
		);
	}
}
