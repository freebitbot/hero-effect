...
import { date, type ISchemaAny, number, string } from "@ulixee/schema";
import type DateSchema from "@ulixee/schema/lib/DateSchema";
import type StringSchema from "@ulixee/schema/lib/StringSchema";
import * as moment from "moment";
import type ICrawlerComponents from "../interfaces/ICrawlerComponents";
import type ICrawlerOutputSchema from "../interfaces/ICrawlerOutputSchema";
import type { CrawlerOutputSchema } from "../interfaces/ICrawlerOutputSchema";
...
import type { ISchemaRecordType } from "../interfaces/IExtractorSchema";
...
export default class Crawler<
	...
	TProvidedSchema extends IExtractorSchema<unknown, never> = IExtractorSchema<
		unknown,
		never
	>,
	...
	TSchema extends IExtractorSchema<
		TFinalInput,
		typeof CrawlerOutputSchema
	> = IExtractorSchema<TFinalInput, typeof CrawlerOutputSchema>,
	TPlugin1 extends
	...
	TContext extends Omit<IExtractorContext<TSchema>, "Output" | "outputs"> &
		...
		TPlugin3["contextAddons"] = Omit<
		IExtractorContext<TSchema>,
		"Output" | "outputs"
	> &
		TPlugin1["contextAddons"] &
		TPlugin2["contextAddons"] &
...
> extends Extractor<TSchema, TPlugin1, TPlugin2, TPlugin3, TContext> {
	...
	private crawlerComponents: ICrawlerComponents<
		TSchema,
		TContext,
		TDisableCache
	> &
	...
	constructor(
		...
		this.components.run = this.runWrapper.bind(
			this,
			this.components.run as any,
		);
	...
	public override attachToDatastore(
		...
		if (!this.crawlerComponents.disableCache) {
			const isBackwardsCompatible =
				this.crawlerComponents.backwardsCompatible ?? false;
	...
	protected async findCached(
		...
		(input as ICrawlerInputSchema).maxTimeInCache ??=
			Crawler.defaultMaxTimeInCache;
