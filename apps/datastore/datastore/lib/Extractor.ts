...
import DatastoreInternal, {
	type IDatastoreBinding,
	type IQueryInternalCallbacks,
...
> {
	...
	protected get datastoreInternal(): DatastoreInternal {
		this.#datastoreInternal ??= new DatastoreInternal({
			extractors: { [this.name]: this },
		});
	...
	public runInternal(
		...
		callbacks
			.onFunction(this.name, options, async (finalOptions) => {
				const extractorInternal = new ExtractorInternal<TSchema>(
					finalOptions,
					this.components,
				);
				...
				try {
					...
					context = await plugins.initialize(
						extractorInternal,
						this.datastoreInternal,
						callbacks,
					);
					...
					error.stack = error.stack
						.split("at Extractor.runInternal")
						.shift()
						.trim();
	...
	public attachToDatastore(
		...
		if (this.#datastoreInternal) {
			throw new Error(
				`${extractorName} Extractor is already attached to a Datastore`,
			);
		}
