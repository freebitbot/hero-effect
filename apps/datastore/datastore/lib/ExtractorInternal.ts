...
import type { IValidationError } from "@ulixee/schema/interfaces/IValidationResult";
...
import type { ExtractSchemaType } from "../interfaces/IExtractorSchema";
import type IObservableChange from "../interfaces/IObservableChange";
import { ObservableChangeType } from "../interfaces/IObservableChange";
import DatastoreSchemaError from "./DatastoreSchemaError";
import createOutputGenerator, { type IOutputClass } from "./Output";
import ResultIterable from "./ResultIterable";

export default class ExtractorInternal<
	TSchema extends IExtractorSchema<any, any>,
	TOptions extends
		IExtractorRunOptions<TSchema> = IExtractorRunOptions<TSchema>,
	TInput extends ExtractSchemaType<TSchema["input"]> = ExtractSchemaType<
		TSchema["input"]
	>,
	TOutput extends ExtractSchemaType<TSchema["output"]> = ExtractSchemaType<
		TSchema["output"]
	>,
> extends TypedEventEmitter<{
	...
	public run(context: IExtractorContext<TSchema>): ResultIterable<TOutput> {
		...
		this.onOutputRecord = (index, output) =>
			extractorResults.push(output, index);
	...
	protected onNewOutput(index: number): void {
		if (this.onOutputChanges)
			this.onOutputChanges(index, [
				{ path: [], value: {}, type: ObservableChangeType.insert },
			]);
	}
			...
			if (err instanceof DatastoreSchemaError) {
				...
				for (const change of changes) {
					...
					for (const error of err.errors) {
						if (
							error.path === path ||
							keyPaths.some((x) => error.path.startsWith(x))
						)
							validErrors.push(error);
					}
				}
				if (validErrors.length)
					throw new DatastoreSchemaError(err.message, validErrors, output);
			}
		}
	}

	public static fillInputWithExamples(
	...
	public static createExampleCall(
		...
		if (schema) {
			...
			if (schema.inputExamples?.length) {
				ExtractorInternal.fillInputWithExamples(schema, args);
			} else {
				for (const [name, field] of Object.entries(
					...
					if (field.format === "time")
						args[name] = moment().format(StringSchema.TimeFormat);
					else if (field.format === "date")
						args[name] = moment().format(StringSchema.DateFormat);
