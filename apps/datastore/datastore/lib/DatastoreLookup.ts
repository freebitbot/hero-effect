...
export default class DatastoreLookup implements IDatastoreHostLookup {
	...
	public async validatePayment(paymentInfo: {
		...
		if (paymentInfo.domain && paymentInfo.recipient) {
			const zoneRecord = await this.lookupDatastoreDomain(
				paymentInfo.domain,
				"any",
			);
			if (zoneRecord) {
	...
	public async lookupDatastoreDomain(
		...
		if (!zoneRecord) {
			...
			const zone = await mainchainClient.getDomainZoneRecord(
				parsed.name,
				parsed.topLevel,
			);
	...
	public static parseTld(tld: string): DomainTopLevel {
		return (
			DomainTopLevel[tld.toLowerCase()] ??
			DomainTopLevel[tld[0].toUpperCase() + tld.slice(1)]
		);
	}
