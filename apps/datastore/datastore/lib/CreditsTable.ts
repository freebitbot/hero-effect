...
import { bigint, buffer, type ExtractSchemaType, string } from "@ulixee/schema";
...
export default class CreditsTable extends Table<typeof CreditsSchema> {
	...
	constructor() {
		super({
			...
			isPublic: false,
			description:
				"Private table tracking Credits issued for the containing Datastore.",
			schema: CreditsSchema,
			basePrice: 0,
			async onVersionMigrated(
				previousVersion: Table<typeof CreditsSchema>,
			): Promise<void> {
	...
	async debit(id: string, secret: string, amount: bigint): Promise<bigint> {
		...
		if (credit.remainingCredits < amount)
			throw new Error(
				"This Credit has insufficient balance remaining to create a payment.",
			);
	...
	private async getUpstreamCreditLimit(): Promise<bigint | undefined> {
		const embeddedCredits =
			this.datastoreInternal.metadata.remoteDatastoreEmbeddedCredits ?? {};
		...
		for (const [source, credit] of Object.entries(embeddedCredits)) {
			try {
				const { client, datastoreHost } =
					await this.datastoreInternal.getRemoteApiClient(source);
