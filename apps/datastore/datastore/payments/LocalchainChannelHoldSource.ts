...
import type Resolvable from "@ulixee/commons/lib/Resolvable";
...
import type DatastoreLookup from "../lib/DatastoreLookup";
...
export default class LocalchainChannelHoldSource implements IChannelHoldSource {
	...
	private readonly openChannelHoldsById: {
		[channelHoldId: string]: OpenChannelHold;
	} = {};
	...
	public async createChannelHold(
		...
		if (Env.allowMinimumAffordableChannelHold) {
			...
			const availableBalance =
				accountOverview.balance - accountOverview.heldBalance;
		...
		const openChannelHold =
			await this.localchain.transactions.createChannelHold(
				microgons,
		...
		const expirationMillis = this.localchain.ticker.timeForTick(
			channelHold.expirationTick,
		);
