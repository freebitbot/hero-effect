	...
	implements IPaymentReserver
{
	...
	constructor(
		...
		this.storePath = Path.join(
			baseDir,
			`${credit.id}-${credit.paymentMethod.credits.id}.json`,
		);
	...
	private async writeToDisk(canDelete: boolean): Promise<void> {
		...
		if (
			canDelete &&
			this.paymentDetails.remaining <= CreditReserver.MIN_BALANCE
		) {
	...
	public static async lookup(
		...
		try {
			...
			throw new Error(
				`Error looking up credit ${credit.id} for datastore ${datastoreId}`,
			);
		} finally {
