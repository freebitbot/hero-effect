...
import ArgonReserver, {
	type IChannelHoldAllocationStrategy,
} from "./ArgonReserver";
...
{
	public readonly creditsByDatastoreId: {
	...
	private readonly paymentUuidToService: {
		[uuid: string]: WeakRef<IPaymentService>;
	} = {};
		...
		if (loadCreditFromPath) {
			this.creditsPath =
				loadCreditFromPath === "default"
					? CreditReserver.defaultBasePath
					: loadCreditFromPath;
	...
	public async attachCredit(
		...
		if (!datastoreLookup && Env.argonMainchainUrl) {
			mainchainClientToClose = MainchainClient.connect(
	...
	public static async fromOpenLocalchain(
		...
		const reserver = new ArgonReserver(
			...
			channelHoldAllocationStrategy ??
				localchain.localchainConfig.channelHoldAllocationStrategy,
			apiClients,
		);
		await reserver.load();
		...
		const identity =
			"pemPath" in identityConfig
				? Identity.loadFromFile(
						...
						identityConfig.passphrase
							? { keyPassphrase: identityConfig.passphrase }
							: undefined,
					)
