...
import type { IDatabrokerApis } from "@ulixee/platform-specification/datastore";
...
import type { IChannelHoldDetails, IChannelHoldSource } from "./ArgonReserver";
...
export default class BrokerChannelHoldSource implements IChannelHoldSource {
	...
	private balanceChangeByChannelHoldId: {
		[channelHoldId: string]: IBalanceChange;
	} = {};
	...
	constructor(
		...
		this.connectionToCore = new ConnectionToCore(
			new HttpTransportToCore(this.host),
		);
	...
	public async createChannelHold(
		...
		this.balanceChangeByChannelHoldId[holdDetails.channelHoldId] =
			holdDetails.balanceChange;
		return holdDetails;
	}
	...
	public async updateChannelHoldSettlement(
		...
		if (!balanceChange)
			throw new Error(
				`No balance change found for channel hold ${channelHold.id}`,
			);
		balanceChange.notes[0].microgons = updatedSettlement;
		balanceChange.balance =
			balanceChange.previousBalanceProof.balance - updatedSettlement;
		const json = serdeJson(balanceChange);
		const bytes = BalanceChangeBuilder.toSigningMessage(json);
		const signature = this.keyring
			.getPairs()[0]
			.sign(bytes, { withType: true });
	...
	public static async getBalance(
		...
		const connectionToCore = new ConnectionToCore<IDatabrokerApis, any>(
			remoteTransport,
		);
