...
import {
	readFileAsJson,
	safeOverwriteFile,
} from "@ulixee/commons/lib/fileUtils";
import Logger from "@ulixee/commons/lib/Logger";
import Queue from "@ulixee/commons/lib/Queue";
import TypeSerializer from "@ulixee/commons/lib/TypeSerializer";
import { toUrl } from "@ulixee/commons/lib/utils";
import type { IPayment } from "@ulixee/platform-specification";
import type IPaymentServiceApiTypes from "@ulixee/platform-specification/datastore/PaymentServiceApis";
import type IBalanceChange from "@ulixee/platform-specification/types/IBalanceChange";
import type { IPaymentMethod } from "@ulixee/platform-specification/types/IPayment";
	...
	implements IPaymentReserver
{
	...
	constructor(
		...
		this.storePath = Path.join(
			ArgonReserver.baseStorePath,
			`${channelHoldSource.sourceKey}.json`,
		);
	...
	public async load(): Promise<void> {
		...
		this.loadPromise = (async () => {
			const paymentsByDatastore =
				await readFileAsJson<IPaymentDetailsByDatastoreId>(
					this.storePath,
				).catch(() => ({}));
	...
	public async reserve(
		...
		return await this.reserveQueueByDatastoreId[datastoreId].run(async () => {
			this.paymentsByDatastoreId[datastoreId] = this.paymentsByDatastoreId[
				datastoreId
			].filter(
				(x) =>
					x.remaining > 0 &&
					(!x.expirationDate || x.expirationDate > new Date()),
			);
			...
			const holdAmount = this.calculateChannelHoldAmount(
				datastoreId,
				microgons,
			);
	...
	public async createChannelHold(
		...
		return await this.channelHoldQueue.run(async () => {
			const channelHold = await this.channelHoldSource.createChannelHold(
				paymentInfo,
				microgons,
			);
	...
	protected calculateChannelHoldAmount(
		_datastoreId: string,
		microgons: bigint,
	): bigint {
	...
	private async charge(
		...
		this.paymentsPendingFinalization[payment.uuid] = {
			...
			paymentId:
				details.paymentMethod.credits?.id ??
				details.paymentMethod.channelHold?.id,
		};
		this.emit("reserved", {
			payment,
			datastoreId: details.id,
	...
	private async updateSettlement(
		...
		if (updatedSettlement > channelHold.settledMicrogons) {
			await this.channelHoldSource.updateChannelHoldSettlement(
				channelHold,
				updatedSettlement,
			);
			this.needsSave = true;
