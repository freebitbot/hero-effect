...
import {
	...
	type DomainStore,
	type Keystore,
	type KeystorePasswordOption,
	Localchain,
	type LocalchainOverview,
	MainchainClient,
	type MainchainTransferStore,
	type OpenChannelHoldsStore,
	type TickerRef,
	type Transactions,
} from "@argonprotocol/localchain";
import { Keyring, type KeyringPair$Json } from "@argonprotocol/mainchain";
...
import Env from "../env";
import type ILocalchainConfig from "../interfaces/ILocalchainConfig";
import type ILocalchainRef from "../interfaces/ILocalchainRef";
import type DatastoreApiClients from "../lib/DatastoreApiClients";
	...
	implements ILocalchainRef
{
	...
	constructor(
		readonly localchainConfig: Omit<
			ILocalchainConfig,
			"localchainCreateIfMissing"
		> = {},
	) {
		super();
		this.localchainConfig.mainchainUrl ||= Env.argonMainchainUrl;
		if (
			this.localchainConfig.localchainName &&
			!this.localchainConfig.localchainPath
		) {
			let dbName = this.localchainConfig.localchainName;
			if (!dbName.endsWith(".db")) {
				dbName += ".db";
			}
			this.localchainConfig.localchainPath = Path.join(
				Localchain.getDefaultDir(),
				dbName,
			);
		}
	}
		...
		if (mainchainUrl) {
			await Promise.race([
				...
				this.connectToMainchain(mainchainUrl)
					.then(async () => {
						this.datastoreLookup = new DatastoreLookup(
							this.#localchain.mainchainClient,
						);
						return null;
					})
					.catch((error) => {
	...
	public async createIfMissing(account?: {
		...
		log.info("Creating localchain", {
			path: Localchain.getDefaultPath(),
		} as any);
		if (account?.suri) {
			const keystorePassword = this.getPassword();
	...
	public async connectToMainchain(
		argonMainchainUrl: string,
		timeoutMs = 10e3,
	): Promise<void> {
		try {
			const mainchain = await MainchainClient.connect(
				argonMainchainUrl,
				timeoutMs,
			);
				...
				if (this.enableLogging) {
					if (
						...
						log.info("Localchain Sync result", {
							...
							balanceChanges: await Promise.all(
								result.balanceChanges.map(gettersToObject),
							),
							channelHoldNotarizations: await Promise.all(
								result.channelHoldNotarizations.map(gettersToObject),
							),
							mainchainTransfers: await Promise.all(
								result.mainchainTransfers.map(gettersToObject),
							),
							channelHoldsUpdated: await Promise.all(
								result.channelHoldsUpdated.map(gettersToObject),
							),
							blockVotes: await Promise.all(
								result.blockVotes.map(gettersToObject),
							),
						} as any);
					}
				}
