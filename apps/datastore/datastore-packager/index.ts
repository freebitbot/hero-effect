...
import type { IFetchMetaResponseData } from "@ulixee/datastore-core/interfaces/ILocalDatastoreProcess";
...
import { type ISchemaAny, object } from "@ulixee/schema";
import schemaFromJson from "@ulixee/schema/lib/schemaFromJson";
import schemaToInterface, {
	printNode,
} from "@ulixee/schema/lib/schemaToInterface";
...
export default class DatastorePackager extends TypedEventEmitter<{
	...
	public async build(options?: {
		...
		const rollup = await rollupDatastore(
			options?.compiledSourcePath ?? this.entrypoint,
			{
		...
		await this.generateDetails(
			rollup.code,
			rollup.sourceMap,
			options?.createTemporaryVersion,
		);
	...
	public async createOrUpdateManifest(
		...
		if (this.meta.extractorsByName) {
			for (const [name, extractorMeta] of Object.entries(
				this.meta.extractorsByName,
			)) {
				...
				if (extractorMeta.remoteExtractor) {
					const extractorDetails =
						await this.lookupRemoteDatastoreExtractorPricing(
							this.meta,
							extractorMeta,
						);
					extractorsByName[name].prices.push(
						...extractorDetails.priceBreakdown,
					);
				}
			}
		}
		...
		if (this.meta.crawlersByName) {
			for (const [name, crawler] of Object.entries(this.meta.crawlersByName)) {
				...
				if (crawler.remoteCrawler) {
					const extractorDetails =
						await this.lookupRemoteDatastoreCrawlerPricing(this.meta, crawler);
		...
		if (this.meta.tablesByName) {
			for (const [name, tableMeta] of Object.entries(this.meta.tablesByName)) {
				...
				if (tableMeta.remoteTable) {
					const paymentDetails = await this.lookupRemoteDatastoreTablePricing(
						this.meta,
						tableMeta,
					);
	...
	protected async lookupRemoteDatastoreExtractorPricing(
		...
		IDatastoreApiTypes["Datastore.meta"]["result"]["extractorsByName"][0]
	> {
		const name = extractor.remoteExtractor.split(".").pop();

		const { remoteHost, datastoreId, datastoreVersion } =
			this.getRemoteSourceAndVersion(meta, extractor.remoteSource);
		...
		try {
			const upstreamMeta = await this.getDatastoreMeta(
				remoteHost,
				datastoreId,
				datastoreVersion,
			);
			...
			console.error(
				"ERROR loading remote datastore pricing",
				remoteMeta,
				error,
			);
			throw error;
	...
	protected async lookupRemoteDatastoreCrawlerPricing(
		...
		IDatastoreApiTypes["Datastore.meta"]["result"]["crawlersByName"][0]
	> {
		const name = crawler.remoteCrawler.split(".").pop();
		const { remoteHost, datastoreId, datastoreVersion } =
			this.getRemoteSourceAndVersion(meta, crawler.remoteSource);
		const remoteMeta = {
			host: remoteHost,
			datastoreId,
			datastoreVersion,
	...
	protected async lookupRemoteDatastoreTablePricing(
		...
		IDatastoreApiTypes["Datastore.meta"]["result"]["tablesByName"][0]
	> {
		const name = table.remoteTable.split(".").pop();

		const { remoteHost, datastoreId, datastoreVersion } =
			this.getRemoteSourceAndVersion(meta, table.remoteSource);
		const remoteMeta = {
			host: remoteHost,
			datastoreId,
			datastoreVersion,
	...
	private getRemoteSourceAndVersion(
		...
		try {
			...
			throw new Error(
				`The remoteDatastore url for "${remoteSource}" is not a valid url (${url})`,
			);
		}
		...
		if (!semverRegex.test(datastoreVersion))
			throw new Error(
				`The remote datastore version is not a semver (${datastoreVersion})`,
			);
