...
import type IDocpageConfig from "../interfaces/IDocpageConfig";
...
export default class Dbx {
	...
	constructor(public readonly path: string) {
		this.manifest = new DatastoreManifest(
			Path.join(this.path, "datastore-manifest.json"),
		);
	}
	...
	public async createOrUpdateDocpage(
		...
		const title =
			meta.name || entrypoint.match(/([^/\\]+)\.(js|ts)$/)[1] || "Untitled";
		...
		if (!defaultExample) {
			...
			const useFunction =
				functions.find((x) => x.schema?.inputExamples?.length) ?? functions[0];
			const type = meta.extractorsByName[useFunction?.name]
				? "extractor"
				: "crawler";
			if (useFunction) {
		...
		const config: IDocpageConfig = {
			...
			extractorsByName: Object.entries(meta.extractorsByName).reduce(
				(obj, [name, entry]) => {
			...
			crawlersByName: Object.entries(meta.crawlersByName).reduce(
				(obj, [name, entry]) => {
					return Object.assign(obj, {
						[name]: {
							name,
			...
			tablesByName: Object.entries(meta.tablesByName).reduce(
				(obj, [name, entry]) => {
		...
		await Fs.writeFile(
			Path.join(this.path, "docpage.json"),
			TypeSerializer.stringify(config),
		);
	}
