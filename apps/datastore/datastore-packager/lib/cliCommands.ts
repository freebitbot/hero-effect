...
import type DatastoreManifest from "@ulixee/datastore-core/lib/DatastoreManifest";
...
async function upload(
	...
	if (cloudHost?.startsWith("localhost")) {
		cloudHost = await UlixeeHostsConfig.global.checkLocalVersionHost(
			version,
			cloudHost,
		);
		if (!identityPath) {
...
export async function startDatastore(
	...
	console.log(
		"%s Datastore",
		options.watch ? "Started + watching" : "Started",
		{
...
function openDocsPage(manifest: DatastoreManifest, cloudHost: string): void {
	...
	execSync(
		`${openCommand} "http://${cloudHost}/${manifest.id}@v${manifest.version}"`,
	);
}
