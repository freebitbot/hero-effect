...
if (env.ULX_DATASTORE_DIR)
	env.ULX_DATASTORE_DIR = parseEnvPath(env.ULX_DATASTORE_DIR);
if (env.ULX_DATASTORE_TMP_DIR)
	env.ULX_DATASTORE_TMP_DIR = parseEnvPath(env.ULX_DATASTORE_TMP_DIR);
if (env.ULX_QUERY_HERO_SESSIONS_DIR)
	env.ULX_QUERY_HERO_SESSIONS_DIR = parseEnvPath(
		env.ULX_QUERY_HERO_SESSIONS_DIR,
	);
...
export default {
	...
	datastoresTmpDir:
		env.ULX_DATASTORE_TMP_DIR ?? Path.join(Os.tmpdir(), ".ulixee", "datastore"),
	...
	cloudAdminIdentities: parseIdentities(
...
function getLocalchainConfig(): ILocalchainConfig | undefined {
	...
	if (env.ARGON_LOCALCHAIN_PASSWORD_FILE)
		env.ARGON_LOCALCHAIN_PASSWORD_FILE = parseEnvPath(
	...
	return <ILocalchainConfig>{
		...
		blockRewardsAddress: parseAddress(
			env.ARGON_BLOCK_REWARDS_ADDRESS,
			"Block Rewards Address",
		),
		notaryId: parseEnvInt(env.ARGON_NOTARY_ID),
		keystorePassword: {
			interactiveCli: parseEnvBool(
