import { Localchain, MainchainClient } from "@argonprotocol/localchain";
...
import type { IServicesSetupApiTypes } from "@ulixee/platform-specification/services/SetupApis";
...
import HostedServicesEndpoints, {
	type TConnectionToServicesClient,
} from "./endpoints/HostedServicesEndpoints";
import Env from "./env";
import type IDatastoreApiContext from "./interfaces/IDatastoreApiContext";
import type IDatastoreConnectionToClient from "./interfaces/IDatastoreConnectionToClient";
import type IDatastoreCoreConfigureOptions from "./interfaces/IDatastoreCoreConfigureOptions";
...
import DatastoreRegistry, {
	type IDatastoreManifestWithRuntime,
} from "./lib/DatastoreRegistry";
import type { IDatastoreSourceDetails } from "./lib/DatastoreRegistryDiskStore";
...
export default class DatastoreCore extends TypedEventEmitter<{
	...
	constructor(
		options: Partial<IDatastoreCoreConfigureOptions>,
		plugins?: IExtractorPluginCore[],
	) {
		super();
		this.options = {
			...
			datastoresMustHaveOwnAdminIdentity:
				Env.datastoresMustHaveOwnAdminIdentity,
			...
			upstreamPaymentsServiceHost: Env.upstreamPaymentsServiceHost,
	...
	public addConnection(transport: ITransport): IDatastoreConnectionToClient {
		...
		const connection: IDatastoreConnectionToClient =
			this.apiRegistry.createConnection(transport, context);
	...
	public addHostedServicesConnection(
		...
		if (!this.hostedServicesEndpoints) {
			throw new Error(
				"This CloudNode has not been configured to provide Services services.",
			);
		}
		const context = this.getApiContext(transport.remoteId);
		const connection = this.hostedServicesEndpoints.addConnection(
			transport,
			context,
		);
	...
	public registerHttpRoutes(
		...
		addHttpRoute(
			/.*\/free-credit\/?\?crd[A-Za-z0-9_]{8}.*/,
			"GET",
			(req, res) => this.docPages.routeCreditsBalanceApi(req, res),
		);
	...
	public async start(options: {
		...
		try {
			...
			if (
				this.options.datastoreRegistryHost &&
			...
			this.connectionToThisCore = new ConnectionToDatastoreCore(
				bridge.transportToCore,
			);
			this.datastoreApiClients = new DatastoreApiClients();

			const lookupConnection = createConnectionToServiceHost(
				this.options.datastoreLookupHost,
			);
			if (lookupConnection)
				this.datastoreHostLookup = new DatastoreHostLookupClient(
					lookupConnection,
				);

			if (this.options.localchainConfig?.localchainCreateIfMissing) {
				this.options.localchainConfig.localchainPath ??=
					Localchain.getDefaultPath();
			}

			if (this.options.localchainConfig?.localchainPath) {
				this.localchain = await LocalchainWithSync.load(
					this.options.localchainConfig,
				);
				if (this.options.localchainConfig?.localchainCreateIfMissing) {
				...
				this.upstreamDatastorePaymentService =
					await this.localchain.createPaymentService(this.datastoreApiClients);
				...
				const paymentServiceConnection = createConnectionToServiceHost(
					this.options.upstreamPaymentsServiceHost,
				);
				...
				if (channelHoldConnection) {
					this.argonPaymentProcessor = new ArgonPaymentProcessorClient(
			...
			if (!this.datastoreHostLookup) {
				const mainchainClient = Env.argonMainchainUrl
					? MainchainClient.connect(Env.argonMainchainUrl, 10e3).catch(
							(err) => {
								log.warn("Unable to connect to mainchain client", {
									error: err,
								} as any);
								return null;
							},
			...
			this.docPages = new DocpageRoutes(
				this.datastoreRegistry,
				this.cloudNodeAddress,
				(args) => DatastoreCreditsBalance.handler(args, this.getApiContext()),
			);
	...
	public async close(): Promise<void> {
		...
		try {
			await this.workTracker?.stop(
				this.options.waitForDatastoreCompletionOnShutdown,
			);
			...
			log.stats("DatastoreCore.Closed", {
				parentLogId: logid,
				sessionId: null,
			});
		}
	...
	private async onDatastoreInstalled(
		...
		if (
			this.storageEngineRegistry.isHostingStorageEngine(
				version.storageEngineHost,
			)
		) {
			...
			const versionDbx =
				await this.datastoreRegistry.diskStore.getCompressedDbx(
					version.id,
					version.version,
				);
			const previousDbx =
			...
			await this.storageEngineRegistry.createRemote(
				version,
				versionDbx,
				previousDbx,
			);
