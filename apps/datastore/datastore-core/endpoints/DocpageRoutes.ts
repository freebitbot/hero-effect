...
export default class DocpageRoutes {
	...
	public async routeCreditsBalanceApi(
		req: IncomingMessage,
		res: ServerResponse,
	): Promise<boolean> {
		...
		if (!version) {
			...
			res.end(
				JSON.stringify({ error: "No valid Datastore version could be found." }),
			);
		}
	...
	public async routeHttp(
		...
		if (req.url.includes("docpage.json")) {
			...
			const { runtimePath } = await this.datastoreRegistry.get(
				datastoreId,
				version,
			);
			const docpagePath = runtimePath.replace("datastore.js", "docpage.json");
			res.writeHead(200, { "content-type": "application/json" });
			createReadStream(docpagePath, { autoClose: true }).pipe(res, {
				end: true,
			});
			return true;
		}
