...
async function install(
	...
	try {
		...
		if (
			!storageEngineRegistry.isHostingStorageEngine(manifest.storageEngineHost)
		) {
			throw new Error(
		...
		const result = await datastoreRegistry.diskStore.install(
			...
			{
				adminIdentity,
				hasServerAdminIdentity: configuration.cloudAdminIdentities.includes(
					adminIdentity ?? "-1",
				),
			},
			sourceDetails,
		...
		if (forceInstall && !result.didInstall) {
			await datastoreRegistry.diskStore.onInstalled(
				result.manifest,
				sourceDetails,
			);
		}
