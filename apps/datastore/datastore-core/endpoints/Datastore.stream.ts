...
import type { IQueryInternalCallbacks } from "@ulixee/datastore/lib/DatastoreInternal";
import type ResultIterable from "@ulixee/datastore/lib/ResultIterable";
import type { IDatastoreMetadataResult } from "@ulixee/platform-specification/datastore/DatastoreApis";
...
export default new DatastoreApiHandler("Datastore.stream", {
	async handler(request, context) {
		...
		const datastoreFunction =
			datastore.extractors[name] ?? datastore.crawlers[name];
