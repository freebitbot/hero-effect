...
export default new DatastoreApiHandler("Datastore.download", {
	async handler(
		...
		if (Date.now() - requestDate.getTime() > 10e3) {
			throw new Error(
				"This download request date is too old. Please try again.",
			);
		}

		// verify admin permissions to download
		let isValidAdmin =
			configuration.cloudAdminIdentities.includes(adminIdentity);
		if (!isValidAdmin) {
			if (!adminIdentity && configuration.serverEnvironment === "production") {
				throw new InvalidPermissionsError(
					`Download from this cloud require Admin Signatures.`,
				);
			}
		...
		const result = await datastoreRegistry.diskStore.getCompressedDbx(
			id,
			version,
		);
		if (!result)
			throw new DatastoreNotFoundError(
				"Could not find this Datastore runtime.",
				{ version },
			);
		return result;
	},
