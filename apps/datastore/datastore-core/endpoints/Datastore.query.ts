...
export default new DatastoreApiHandler("Datastore.query", {
	async handler(request, context) {
		request.boundValues ??= [];
		const { sql, boundValues, queryId, payment, authentication, version, id } =
			request;
			...
			.queryInternal<any[]>(sql, boundValues, innerOptions, {
				async beforeQuery({ sqlParser, entityCalls }) {
					...
					for (const name of sqlParser.tableNames) {
						...
						if (!table)
							throw new Error(
								`There is no table named "${name}" in this datastore.`,
							);
						if (!table.isPublic)
							throw new Error(`Table ${name} is not publicly accessible.`);
					}
					const didFail = await queryRunner.beforeAll(
						sql,
						boundValues,
						entityCalls,
					);
