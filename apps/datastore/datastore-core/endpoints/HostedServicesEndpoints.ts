import type { IAsyncFunc } from "@ulixee/net/interfaces/IApiHandlers";
	...
	constructor() {
		this.handlersByCommand = {
			"DatastoreRegistry.downloadDbx": async ({ id, version }, ctx) => {
				...
				const result = await ctx.datastoreRegistry.diskStore.getCompressedDbx(
					id,
					version,
				);
				if (!result) {
					throw new DatastoreNotFoundError(
						"Datastore could not be download. Not found locally.",
						{
							version,
						},
					);
			...
			"DatastoreRegistry.upload": async (request, ctx) => {
				...
				const result = await workTracker.trackUpload(
					datastoreRegistry.saveDbx(
						request,
						ctx.connectionToClient?.transport.remoteId,
					),
				);
			...
			"ArgonPaymentProcessor.importChannelHold": async (
