...
export default new DatastoreApiHandler("Datastore.admin", {
	async handler(
		request,
		context,
	): Promise<IDatastoreApiTypes["Datastore.admin"]["result"]> {
		const { adminSignature, adminIdentity, adminFunction, functionArgs } =
			request;
		...
		if (!Identity.verify(adminIdentity, message, adminSignature)) {
			throw new InvalidSignatureError(
				"Your Admin signature is invalid for this function call.",
			);
		}
		...
		if (adminFunction.ownerType === "datastore") {
			if (typeof datastore[functionName] !== "function")
				throw new Error(
					`An admin function called ${functionName} could not be found`,
				);
