...
export default new DatastoreApiHandler("Datastore.creditsIssued", {
	async handler(
		...
		const storage = context.storageEngineRegistry.get(datastoreVersion, {
			...
			queryId:
				context.connectionToClient?.transport.remoteId ?? "creditsIssued",
		});
		...
		const { count, microgons } =
			await datastore.tables[CreditsTable.tableName].summary();
		return { count, issuedCredits: microgons ?? 0n };
	},
});
