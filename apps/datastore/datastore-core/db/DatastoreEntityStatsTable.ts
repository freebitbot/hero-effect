...
export default class DatastoreEntityStatsTable extends SqliteTable<IDatastoreEntityStatsRecord> {
	private static byIdVersionAndName: {
		[id_version_name: string]: IDatastoreEntityStatsRecord;
	} = {};

	private static byIdAndName: {
		[id_name: string]: IDatastoreEntityStatsRecord;
	} = {};
	...
	public getByVersion(
		...
		DatastoreEntityStatsTable.byIdVersionAndName[
			`${datastoreId}_${version}_${name}`
		] ??= this.process(
			this.getByVersionQuery.get(datastoreId, version, name) ??
				this.emptyStats(name, datastoreId, version),
		);
		return DatastoreEntityStatsTable.byIdVersionAndName[
			`${datastoreId}_${version}_${name}`
		];
	}

	public getByDatastore(
		datastoreId: string,
		name: string,
	): IDatastoreEntityStatsRecord {
	...
	private process(
		record: IDatastoreEntityStatsRecord,
	): IDatastoreEntityStatsRecord {
	...
	private addToStats(
		...
		stats.averagePrice = calculateNewAverageBigInt(
			stats.averagePrice,
			price,
			stats.runs,
		);
		...
		stats.averageBytes = calculateNewAverage(
			stats.averageBytes,
			bytes,
			stats.runs,
		);
