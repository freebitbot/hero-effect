...
export default class DatastoreChannelHoldsDb {
	...
	private readonly finalizeStatement: Statement<{
		id: string;
		microgons: bigint;
	}>;
    ...
    `);
		...
		this.getStatement = this.db.prepare(
			"SELECT * FROM channelHolds WHERE id = :id LIMIT 1",
		);
	...
	debit(queryId: string, payment: IPayment): { shouldFinalize: boolean } {
		...
		const existing = this.paymentIdByChannelHoldId
			.get(channelHoldId)
			.get(payment.uuid);
		if (existing) {
		...
		if (!result.changes || result.changes < 1) {
			...
			if (channelHold.remaining < payment.microgons) {
				throw new Error(
					"This channelHold does not have enough remaining funds.",
				);
			}
			const neededSettlement =
				(channelHold.allocated -
					channelHold.remaining +
					payment.microgons +
					999n) /
				ArgonReserver.settlementThreshold;
			const neededSettlementMicrogons =
				neededSettlement * ArgonReserver.settlementThreshold;
	...
	finalize(channelHoldId: string, uuid: string, finalMicrogons: bigint): void {
		...
		if (!entry)
			throw new Error(
				"Could not find the initial payment for the given ChannelHold.",
			);
		if (finalMicrogons < 0)
	...
	cleanup(cleanWithExpiredMillis = 60 * 60e3): void {
		...
		this.db
			.prepare(
				`DELETE FROM channelHolds WHERE id IN (${toDelete.map(() => "?").join(",")})`,
			)
			.run(...toDelete);
