...
import type { IStatsTrackerApiTypes } from "@ulixee/platform-specification/services/StatsTrackerApis";
...
import type { IDatastoreStats } from "./StatsTracker";
	...
	public getForDatastore(manifest: IDatastoreManifest): IDatastoreStats {
		...
		for (const [type, name] of [
			...
			statsByEntityName[name] = {
				...
				type: type as any,
				stats: translateStats(
					this.statsDb.datastoreEntities.getByDatastore(id, name),
				),
			};
		}
