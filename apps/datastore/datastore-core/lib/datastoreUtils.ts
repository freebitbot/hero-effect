...
export function validateFunctionCoreVersions(
	...
	if (
		!registryEntry.extractorsByName[extractorName] &&
		!registryEntry.crawlersByName[extractorName]
	)
		throw new Error(
			`${extractorName} is not a valid function name for this Datastore`,
		);
	...
	for (const [pluginName, pluginVersion] of Object.entries(corePlugins ?? {})) {
		...
		if (!pluginCore) {
			throw new Error(
				`This Cloud does not support required runtime dependency: ${pluginName}`,
			);
		}
...
export async function validateAuthentication(
	...
	if (!Identity.verify(identity, message, signature)) {
		throw new Error(
			"The provided Datastore.query authentication signature is invalid.",
		);
	}
}
