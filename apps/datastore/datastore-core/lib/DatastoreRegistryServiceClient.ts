...
import type { TDatastoreUpload } from "./DatastoreRegistry";

export default class DatastoreRegistryServiceClient
	implements IDatastoreRegistryStore
{
	...
	async downloadDbx(
		...
		const { adminIdentity, adminSignature, compressedDbx } =
			await this.client.sendRequest({
