import {
	type ChannelHold,
	DomainStore,
	type OpenChannelHold,
} from "@argonprotocol/localchain";
import Logger from "@ulixee/commons/lib/Logger";
import type { LocalchainWithSync } from "@ulixee/datastore";
import type ILocalchainRef from "@ulixee/datastore/interfaces/ILocalchainRef";
import type IArgonPaymentProcessor from "@ulixee/datastore-core/interfaces/IArgonPaymentProcessor";
import type { IArgonPaymentProcessorApiTypes } from "@ulixee/platform-specification/services/ArgonPaymentProcessorApis";
...
export default class ArgonPaymentProcessor implements IArgonPaymentProcessor {
	private readonly channelHoldDbsByDatastore = new Map<
		string,
		DatastoreChannelHoldsDb
	>();
	...
	public async getPaymentInfo(): Promise<
		IDatastorePaymentRecipient | undefined
	> {
		if (!this.cachedPaymentInfo) {
			this.cachedPaymentInfo = await (this.localchain as LocalchainWithSync)
				.paymentInfo?.promise;
		}
		if (!this.cachedPaymentInfo) {
			const account = await this.localchain.accountOverview();
			const notaryId = (await this.localchain.accounts.getDepositAccount())
				.notaryId;
	...
	public async debit(
		...
		IArgonPaymentProcessorApiTypes["ArgonPaymentProcessor.debit"]["result"]
	> {
	...
	public finalize(
		...
		IArgonPaymentProcessorApiTypes["ArgonPaymentProcessor.finalize"]["result"]
	> {
	...
	public async importChannelHold(
		...
		IArgonPaymentProcessorApiTypes["ArgonPaymentProcessor.importChannelHold"]["result"]
	> {
		...
		const channelHold = await this.importToLocalchain(
			data.datastoreId,
			data.channelHold,
		);
	...
	private async updateSettlement(
		...
		if (settledMicrogons > internal.settledAmount) {
			await channelHold.recordUpdatedSettlement(
				settledMicrogons,
				settledSignature,
			);
		}
	...
	private async importToLocalchain(
		...
		log.stats("Importing channelHold to localchain", {
			datastoreId,
			balanceChange,
		} as any);
	...
	private getDb(datastoreId: string): DatastoreChannelHoldsDb {
		if (!datastoreId)
			throw new Error(
				"No datastoreId provided to get channelHold spend tracking db.",
			);
