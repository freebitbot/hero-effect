...
import Datastore, {
	type ConnectionToDatastoreCore,
	Crawler,
...
export default class DatastoreVm {
	...
	constructor(
		...
		this.whitelist = new Set([
			...this.plugins.flatMap((x) => x.nodeVmRequireWhitelist || []),
			"@ulixee/datastore",
			"@ulixee/*-plugin",
			"@ulixee/net",
			"@ulixee/commons",
	...
	public async open(
		...
		await this.remotePaymentService?.whitelistRemotes?.(
			datastore.metadata,
	...
	private getVMScript(path: string): Promise<Script> {
		...
		if (
			this.compiledScriptsByPath.has(path) &&
			!DatastoreVm.doNotCacheList.has(path)
		) {
