...
import type Datastore from "@ulixee/datastore";
import type { IExtractorRunOptions } from "@ulixee/datastore";
import type { ICacheUpdates } from "@ulixee/datastore/interfaces/IExtractorPluginCore";
import type IQueryOptions from "@ulixee/datastore/interfaces/IQueryOptions";
import type IStorageEngine from "@ulixee/datastore/interfaces/IStorageEngine";
...
import type IDatastorePricing from "@ulixee/platform-specification/types/IDatastorePricing";
...
export default class QueryRunner {
	...
	public async openDatastore(): Promise<Datastore> {
		...
		const manifestWithRuntime = await this.context.datastoreRegistry.get(
			id,
			version,
		);
		this.datastoreManifest = manifestWithRuntime;

		const storage = this.context.storageEngineRegistry.get(
			manifestWithRuntime,
			this.queryDetails,
		);
		...
		this.paymentsProcessor = new PaymentsProcessor(
			payment,
			id,
			datastore,
			this.context,
	...
	public async beforeAll(
		...
		try {
			...
			this.localMachineTableCalls =
				this.storageEngine.filterLocalTableCalls(entityCalls);
		} catch (error) {
	...
	public async onPassthroughTable<TOutput>(
		...
		this.paymentsProcessor.trackCallResult(
			name,
			BigInt(pricing[0]?.basePrice ?? 0),
			upstreamMeta,
		);
		return result;
	}
	...
	public async finalize(
		...
		try {
			...
			if (!runError) runError = error;
				...
				log.error("Error finalizing payment", {
					error,
					sessionId: this.queryDetails.queryId,
				});
			}
		}
