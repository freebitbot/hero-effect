...
import DatastoreRegistryDiskStore, {
	type IDatastoreSourceDetails,
} from "./DatastoreRegistryDiskStore";
...
export default class DatastoreRegistry extends TypedEventEmitter<{
	...
	constructor(
		datastoreDir: string,
		connectionToHostedServiceCore?: ConnectionToCore<
			IDatastoreRegistryApis,
			{}
		>,
		...
		if (connectionToHostedServiceCore) {
			this.clusterStore = new DatastoreRegistryServiceClient(
				connectionToHostedServiceCore,
			);
		}
	...
	public async get(
		...
		for (const service of this.stores) {
			manifestWithLatest = (await service.get(
				id,
				version,
			)) as IDatastoreManifestWithRuntime;
		...
		if (throwIfNotExists) {
			throw new DatastoreNotFoundError(
				`Datastore version (${id}/v${version}) not found on Cloud.`,
				{
					latestVersion,
					version,
				},
	...
	public async saveDbx(
		...
		try {
			...
			return await this.save(
				...
				{
					adminIdentity,
					hasServerAdminIdentity: cloudAdminIdentities.includes(
						adminIdentity ?? "-1",
					),
					datastoresMustHaveOwnAdminIdentity,
				},
	...
	public uploadToSourceOfTruth(
		datastore: TDatastoreUpload,
	...
	public async save(
		...
		adminDetails ??= {};
		return await this.diskStore.install(
			datastoreTmpPath,
			adminDetails,
			uploaderSource,
		);
