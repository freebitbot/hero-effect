...
import type IDatastoreApiTypes from "@ulixee/platform-specification/datastore/DatastoreApis";
import type { IDatastoreQueryMetadata } from "@ulixee/platform-specification/datastore/DatastoreApis";
...
import type DatastoreVm from "./DatastoreVm";
...
export default class StorageEngineRegistry {
	...
	public async close(): Promise<void> {
		await Promise.allSettled(
			[...this.#localStorageByIdAndVersion.values()].map((x) => x.close()),
		);
		await Promise.allSettled(
			[...this.#remoteConnectionsByHost.values()].map((x) => x.disconnect()),
		);
	...
	public async deleteExisting(
		...
		if (!entry) {
			...
			if (await existsAsync(localDbPath))
	...
	public async create(
		...
		if (options?.isWatching) {
			...
			if (process.platform === "win32" || process.platform === "darwin") {
				watcher = Fs.watch(runtimePath, { persistent: false }, () =>
					callback(),
				);
			} else {
	...
	private onEngineDisconnected(
		storageHost: string,
		connection: ConnectionToDatastoreCore,
	): void {
	...
	private getRemoteConnection(storageHost: string): ConnectionToDatastoreCore {
		...
		if (!this.#remoteConnectionsByHost.has(cleanStorageHost)) {
			...
			connection.once("disconnected", () =>
				this.onEngineDisconnected(storageHost, connection),
			);
