...
import type { IDatastoreStatsRecord } from "../db/DatastoreStatsTable";
...
export type IDatastoreStats =
	IStatsTrackerApiTypes["StatsTracker.get"]["result"];
...
export default class StatsTracker extends TypedEventEmitter<{
	...
	constructor(
		...
		if (connectionToServiceCore) {
			this.serviceClient = new StatsTrackerServiceClient(
				connectionToServiceCore,
			);
		} else {
	...
	public async getForDatastoreVersion(
		...
		if (this.serviceClient)
			return this.serviceClient.getForDatastoreVersion(
				manifest.id,
				manifest.version,
			);
	...
	public async getSummary(
		datastoreId: string,
	): Promise<Pick<IDatastoreStats, "stats">> {
		if (this.serviceClient)
