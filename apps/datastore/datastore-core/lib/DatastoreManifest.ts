...
import { DatastoreManifestSchema } from "@ulixee/platform-specification/types/IDatastoreManifest";
...
export default class DatastoreManifest implements IDatastoreManifest {
	...
	public async update(
		...
		const projectPath = Path.resolve(
			await findProjectPathAsync(absoluteScriptEntrypoint),
		);
		const scriptEntrypoint = Path.relative(
			`${projectPath}/..`,
			absoluteScriptEntrypoint,
		);

		const manifestSources = DatastoreManifest.getCustomSources(
			absoluteScriptEntrypoint,
		);
	...
	public async load(): Promise<boolean> {
		if (await this.exists()) {
			let data: IDatastoreManifestJson =
				(await readFileAsJson(this.path)) ?? ({} as any);
			...
			if (data) {
				const { __GENERATED_LAST_VERSION__: generated, ...explicitSettings } =
					data;
	...
	private async syncGeneratedManifests(
	...
	private async loadGeneratedManifests(
		sources: IDatastoreSources,
	): Promise<void> {
		for (const source of sources) {
			if (!source) continue;
	...
	public static validateId(id: string): void {
		try {
			...
			throw ValidationError.fromZodValidation(
				"This is not a valid datastore id",
				error,
			);
		}
	}
	...
	private static getCustomSources(
		...
		return [
			DatastoreManifest.loadGlobalManifest(manifestPath),
			DatastoreManifest.loadProjectManifest(manifestPath),
			DatastoreManifest.loadEntrypointManifest(manifestPath),
		];
	}
	...
	private static async writeToDisk(path: string, json: any): Promise<void> {
		...
		await safeOverwriteFile(
			path,
			TypeSerializer.stringify(json, { format: true }),
		);
	}
}
