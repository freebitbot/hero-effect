...
import type { IDatastoreListEntry } from "@ulixee/platform-specification/services/DatastoreRegistryApis";
...
import type IDatastoreRegistryStore from "../interfaces/IDatastoreRegistryStore";
import type { IDatastoreManifestWithLatest } from "../interfaces/IDatastoreRegistryStore";
import DatastoreManifest from "./DatastoreManifest";
import type { IDatastoreManifestWithRuntime } from "./DatastoreRegistry";
...
export default class DatastoreRegistryDiskStore
	implements IDatastoreRegistryStore
{
	...
	async list(
		...
		const { datastores, total } = this.datastoresDb.versions.list(
			count,
			offset,
		);
	...
	async get(
		...
		if (!manifest) {
			...
			manifest = await readFileAsJson<IDatastoreManifest>(manifestPath).catch(
				() => null,
			);
			if (
				manifest &&
	...
	async installFromService(
		...
		const { compressedDbx, adminIdentity, adminSignature } =
			await service.downloadDbx(id, version);
		const tmpDir = Path.join(
			this.datastoresDir,
			`${version}.dbx.tmp.${nanoid(3)}`,
		);
		try {
	...
	async getPreviousInstalledVersion(
		id: string,
		version: string,
	): Promise<string> {
		const previousVersions = this.datastoresDb.versions
			...
			.filter(
				(x) =>
					x.localeCompare(version, undefined, {
						numeric: true,
						sensitivity: "base",
					}) < 0,
			);
	...
	async install(
		...
		const storedPath = this.datastoresDb.versions.get(
			manifest.id,
			manifest.version,
		)?.dbxPath;
		if (storedPath) {
		...
		if (
			...
			throw new Error(
				"This Cloud requires Datastores to include an AdminIdentity",
			);
		}
	...
	async installOnDiskUploads(
		...
		for (const filepath of await Fs.readdir(this.datastoresDir, {
			withFileTypes: true,
		})) {
			...
			log.info(
				"Found Datastore folder in datastores directory. Checking for import.",
				{
					file,
					sessionId: null,
				},
	...
	async startAtPath(
		...
		const manifest = await readFileAsJson<IDatastoreManifest>(
			`${dbxPath}/datastore-manifest.json`,
		);
		this.checkDatastoreCoreInstalled(manifest.coreVersion);
	...
	recordPublished(id: string, version: string, timestamp: number): void {
		this.datastoresDb.versions.recordPublishedToNetworkDate(
			id,
			version,
			timestamp,
		);
	...
	private async verifyAdminIdentity(
		...
		if (
			manifest.adminIdentities.length &&
			!manifest.adminIdentities.includes(adminIdentity)
		) {
			if (adminIdentity)
		...
		if (previousVersion) {
			const previousVersionManifest = await this.get(
				manifest.id,
				previousVersion,
			);
			...
			if (
				...
				(!previousVersionManifest.adminIdentities.includes(adminIdentity) ||
					!adminIdentity)
			) {
				throw new InvalidPermissionsError(
...
Please try to re-upload after testing with the version available on this Cloud.`,
	...
	private createDbxPath(manifest: IDatastoreManifest): string {
		return Path.resolve(
			this.datastoresDir,
			`${manifest.id}@${manifest.version}.dbx`,
		);
	}
	...
	private saveManifestMetadata(
		...
		const existing = this.datastoresDb.versions.get(
			manifest.id,
			manifest.version,
		);
		if (!existing) {
