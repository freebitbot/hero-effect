...
export default new DatabrokerApiHandler("Databroker.createChannelHold", {
	async handler(request, context) {
		...
		assert(
			Identity.verify(identity, signatureMessage, signature),
			"Invalid signature",
		);
		...
		return await db.transaction(async () => {
			...
			const openChannelHold =
				await context.localchain.transactions.createChannelHold(
					microgons,
					recipient.address,
					domain,
					recipient.notaryId,
			...
			const expirationDate = context.localchain.timeForTick(
