...
import type { BalanceSyncResult } from "@argonprotocol/localchain";
...
import AdminApiEndpoints, {
	type TAdminApis,
} from "./endpoints/AdminApiEndpoints";
...
import type IDatabrokerCoreConfigureOptions from "./interfaces/IDatabrokerCoreConfigureOptions";
...
export default class DataBroker {
	...
	constructor(readonly configuration: IDatabrokerCoreConfigureOptions) {
		...
		void this.#localchain
			.load()
			.then(() => this.#isStarted.resolve(), this.#isStarted.reject);
		...
		const fileServer = staticServe(
			adminDist,
			adminDist.includes("build/") ? 0 : 3600 * 24,
		);
		...
		this.#adminServer.addWsRoute("/", (ws, req) => {
			...
			const connection: IConnectionToClient<TAdminApis, any> =
				this.adminApis.addConnection(transport, context);
		...
		for (const notarization of sync.channelHoldNotarizations) {
			for (const channelHold of await notarization.channelHolds) {
				try {
					...
					log.error(
						"Error updating settlement in db after finalized in localchain",
						{
							error,
