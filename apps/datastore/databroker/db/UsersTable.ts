...
export default class UsersTable {
	...
	constructor(private db: SqliteDatabase) {
      ...
      FOREIGN KEY (organizationId) REFERENCES Organizations(id)
		...
		db.exec(
			`CREATE INDEX IF NOT EXISTS Users_organizationId ON Users(organizationId)`,
		);
	...
	public editName(identity: string, name: string): void {
		this.db
			.prepare(
				`UPDATE Users SET name = :name, modified = :modified WHERE identity = :identity`,
			)
	...
	public count(): number {
		const count =
			(this.db.prepare(`SELECT COUNT(*) FROM Users`).pluck().get() as bigint) ??
			0n;
		return Number(count);
	}
