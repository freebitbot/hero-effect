...
`);
		...
		this.whitelistQuery = this.db.prepare(
			`SELECT 1 FROM DomainWhitelist WHERE domain = :domain`,
		);
	}
	...
	public add(domain: string): void {
		this.db
			.prepare(
				`INSERT OR IGNORE INTO DomainWhitelist (domain) VALUES (:domain);`,
			)
			.run({ domain });
	}
	...
	public delete(domain: string): void {
		this.db
			.prepare(`DELETE FROM DomainWhitelist WHERE domain = :domain;`)
			.run({ domain });
	}

	public list(): string[] {
		return this.db
			.prepare(`SELECT domain FROM DomainWhitelist;`)
			.pluck()
			.all() as string[];
	}
