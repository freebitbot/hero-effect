...
export default class OrganizationsTable {
	...
	private readonly updateNameQuery: Statement<{
		name: string;
		id: string;
		modified: number;
	}>;
	private readonly debitQuery: Statement<{
		amount: bigint;
		id: string;
		modified: number;
	}>;
	private readonly grantQuery: Statement<{
		amount: bigint;
		id: string;
		modified: number;
	}>;
...
`);
	...
	public get(organizationId: string): IOrganizationRecord {
		const org = this.getQuery.get({
			id: organizationId,
		}) as IOrganizationRecord;
