...
import {
	...
	type IExtractorComponents,
	type IExtractorRunOptions,
} from "@ulixee/datastore";
import type ICrawlerOutputSchema from "@ulixee/datastore/interfaces/ICrawlerOutputSchema";
import type IExtractorContextBase from "@ulixee/datastore/interfaces/IExtractorContext";
import type IExtractorSchema from "@ulixee/datastore/interfaces/IExtractorSchema";
import type IObservableChange from "@ulixee/datastore/interfaces/IObservableChange";
import type ExtractorInternal from "@ulixee/datastore/lib/ExtractorInternal";
import Hero, {
	HeroReplay,
	type IHeroCreateOptions,
	type IHeroReplayCreateOptions,
} from "@ulixee/hero";
import type ICoreSession from "@ulixee/hero/interfaces/ICoreSession";
import type { IOutputChangeToRecord } from "@ulixee/hero/interfaces/ICoreSession";
...
export type IHeroExtractorRunOptions<ISchema> = IExtractorRunOptions<ISchema> &
	IHeroCreateOptions;
...
export class HeroExtractorPlugin<ISchema extends IExtractorSchema> {
	...
	public extractorInternal: ExtractorInternal<
		ISchema,
		IHeroExtractorRunOptions<ISchema>
	>;
		...
		try {
			...
			context.Hero = class Hero extends HeroBase {
				constructor(options: IHeroCreateOptions = {}) {
					if (container.hero) {
						throw new Error(
							"Multiple Hero instances are not supported in a Datastore Extractor.",
						);
					}
					...
					void this.once(
						"connected",
						container.onConnected.bind(container, this),
					);
			...
			context.HeroReplay = class HeroReplay extends HeroReplayBase {
				constructor(options: IHeroReplayCreateOptions) {
					const replaySessionId =
						(options as any).replaySessionId ||
						process.env.ULX_REPLAY_SESSION_ID;
	...
	protected uploadOutputs(): void {
		...
		const promise = this.coreSessionPromise
			.then((x) => x.recordOutput(records))
			.catch(() => null);
