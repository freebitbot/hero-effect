...
export default class CertificateGenerator extends BaseIpcHandler {
	...
	public override close(): void {
		...
		for (const pending of this.pendingCertsById.values())
			pending.reject(
				new CanceledPromiseError("Closing Certificate Generator"),
				true,
			);
	}
	...
	protected async generateCerts(
		...
		const resolvable = new Resolvable<{ cert: string; expireDate: number }>(
			10e3,
		);
	...
	protected onMessage(rawMessage: string): void {
		...
		if (message.status === "error") {
			...
			pending.resolve({
				cert: message.cert,
				expireDate: message.expireDate * 1e3,
			});
		}
	}
	...
	protected beforeExit(): void {
		for (const cert of this.pendingCertsById.values()) {
			cert.reject(
				new CanceledPromiseError("Canceling certificate generation"),
				true,
			);
		}
		if (
			this.hasWaitForInitListeners &&
			this.waitForInit &&
			!this.waitForInit.isResolved
		) {
			this.waitForInit.reject(
				new CanceledPromiseError("Canceling ipc initialization"),
				true,
			);
		}
	}
