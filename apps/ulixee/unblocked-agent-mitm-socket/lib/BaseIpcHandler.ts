...
export default abstract class BaseIpcHandler {
	...
	protected constructor(options: Partial<IGoIpcOpts>) {
		...
		if (!distExists) {
			throw new Error(
				`Required files missing! The MitmSocket library was not found at ${libPath}`,
			);
		}
		...
		if (
			!hasInitializedStore &&
			options.storageDir &&
			!Fs.existsSync(options.storageDir)
		) {
	...
	public close(): void {
		...
		if (!this.waitForConnect.isResolved && this.hasWaitListeners) {
			this.waitForConnect.reject(
				new CanceledPromiseError("Canceling ipc connect"),
				true,
			);
		}
