	...
	implements IHttpSocketWrapper
{
	...
	public async connect(
		session: MitmSocketSession,
		connectTimeoutMillis = 30e3,
	): Promise<void> {
		if (!this.server.listening) {
			await new Promise((resolve) =>
				this.events.once(this.server, "listening", resolve),
			);
		}
		...
		try {
			...
			if (
				...
				(session.isClosing || this.isClosing)
			)
				return;
			throw error;
		}
	...
	public onMessage(message: any): void {
		...
		if (status === "connected") {
			...
			if (message.rawApplicationSettings) {
				...
				this.rawApplicationSettings = Buffer.from(
					message.rawApplicationSettings,
					"base64",
				);
				...
				if (acceptChFrame) {
					...
					const origin = acceptChFrame
						.subarray(2, originLength + 2)
						.toString("utf8");
					...
					acceptCh = {
						domain: origin,
						headers: headers.split(",").map((x) => x.trim()),
					};
				}
...
function buildConnectError(connectError: string, callStack: string): Error {
	...
	if (connectError.includes("SOCKS5_PROXY_ERR")) {
		error = new Socks5ProxyConnectError(
			connectError
				.replace("SOCKS5_PROXY_ERR", "Socks5 Proxy Connect Error")
				.trim(),
		);
