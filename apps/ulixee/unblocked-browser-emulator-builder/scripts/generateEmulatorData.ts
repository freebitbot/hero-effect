...
async function generate(): Promise<void> {
	const chromeEngines = stableChromeVersions.filter(
		(x) => x.majorVersion >= 115,
	);

	for (const browserId of Object.keys(userAgentIdsByBrowserId).sort(
		(a, b) =>
			Number(a.split("-").slice(1).join(".")) -
			Number(b.split("-").slice(1).join(".")),
	)) {
		...
		const browserEngineOption = chromeEngines.find(
