...
export default function generatePolyfill(dom1: any, dom2: any): IDomPolyfill {
	...
	add.sort((a, b) => {
		...
		if (
			aProtos &&
			b.path === "window" &&
			(aProtos.includes(b.propertyName) ||
				aProtos.includes(`${b.propertyName}.prototype`))
		) {
			return 1;
		}
		...
		if (
			bProtos &&
			a.path === "window" &&
			(bProtos.includes(a.propertyName) ||
				bProtos.includes(`${a.propertyName}.prototype`))
		) {
			return -1;
		}
	...
	for (const order of diff.changedOrder) {
		...
		for (let i = 0; i < provided.length; i += 1) {
			...
			if (
				...
				const expectedPrev =
					expectedIndex > 0 ? expected[expectedIndex - 1] : null;
	...
	reorder.sort((a, b) => {
		if (a.path === b.path) {
			...
			const aHasDependency = reorder.some(
				(x) => x.propertyName === a.prevProperty,
			);
			const aHasThroughDependency = reorder.some(
				(x) => x.throughProperty === a.prevProperty,
			);
			const bHasDependency = reorder.some(
				(x) => x.propertyName === b.prevProperty,
			);
			const bHasThroughDependency = reorder.some(
