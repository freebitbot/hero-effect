import type IProfile from "@double-agent/collect-tls-clienthello/interfaces/IProfile";
...
export default class ClienthelloJson {
	...
	constructor(config: Config, userAgentIds: string[]) {
		for (const userAgentId of userAgentIds) {
			const profile = BrowserProfiler.getProfile<IProfile>(
				"tls-clienthello",
				userAgentId,
			);
			const { browserId, operatingSystemId } =
				BrowserProfiler.extractMetaFromUserAgentId(userAgentId);
			const { version, ciphers, compressionMethods, extensions } =
				profile.data.clientHello;
