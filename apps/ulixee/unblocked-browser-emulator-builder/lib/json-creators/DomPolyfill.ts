...
import type IDomProfile from "@double-agent/collect-browser-dom-environment/interfaces/IProfile";
...
const browserstackProfilesDir = Path.join(
	BrowserProfiler.dataDir,
	"profiled-doms/browserstack",
);
const localProfilesDir = Path.join(
	BrowserProfiler.dataDir,
	"profiled-doms/local",
);
...
export default class DomPolyfillJson {
	private readonly dataMap: {
		[osId: string]: { [osId: string]: IDomPolyfill };
	} = {};
	...
	constructor(config: Config, userAgentIds: string[]) {
		...
		for (const userAgentId of userAgentIds) {
			const profile = BrowserProfiler.getProfile<
				IBaseProfile<IProfileDataByProtocol>
			>("browser-dom-environment", userAgentId);
			...
			for (const runtimeOsId of Object.keys(foundationDomsByOsId)) {
				...
				DomBridger.removeCustomCallbackFromPolyfill(
					polyfill,
					filterNotSupportedByPolyfillPlugin,
				);
	...
	public save(dataDir: string): void {
		for (const [emulateOsId, dataByRuntimeOsId] of Object.entries(
			this.dataMap,
		)) {
			...
			for (const runtimeOsId of Object.keys(dataByRuntimeOsId)) {
				const dataString = JSON.stringify(
					dataByRuntimeOsId[runtimeOsId],
					null,
					2,
				);
				const domDiffsPath = Path.join(
					dataOsDir,
					`dom-polyfill-when-runtime-${runtimeOsId}.json`,
				);
	...
	public static hasAllDomPolyfills(
		...
		for (const userAgentId of userAgentIds) {
			...
			const files = Fs.readdirSync(dataOsDir).filter((x) =>
				x.startsWith("dom-polyfill"),
			);
...
function filterNotSupportedByPolyfillPlugin(
	...
	if (
		propertyName.includes("_$otherInvocation") &&
		propertyName.includes("new()")
	) {
		return true;
	}
...
function getFoundationDoms(forBrowserId: string): {
	[operatingSystemId: string]: IDomProfile;
} {
	...
	for (const dirName of Fs.readdirSync(browserstackProfilesDir)) {
		...
		const { data: dom } = JSON.parse(
			gunzipSync(Fs.readFileSync(startingDomPath)).toString(),
		);
		domsByOsId[osId] = dom;
