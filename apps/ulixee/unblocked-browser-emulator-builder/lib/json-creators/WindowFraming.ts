...
export default class WindowFraming {
	...
	constructor(config: Config, userAgentIds: string[]) {
		...
		for (const userAgentId of userAgentIds) {
			...
			const screenGapBottom =
				window.screen.height._$value -
				window.screen.availHeight._$value -
				screenGapTop;
			const screenGapRight =
				window.screen.width._$value -
				window.screen.availWidth._$value -
				screenGapLeft;
			const frameBorderWidth =
				window.outerWidth._$value - window.innerWidth._$value;
			const frameBorderHeight =
				window.outerHeight._$value - window.innerHeight._$value;
	...
	public save(dataDir: string): void {
		...
		}
		{
			...
			Fs.writeFileSync(
				`${dataDir}/window-base-framing.json`,
				`${dataString}\n`,
			);
		}
	}
