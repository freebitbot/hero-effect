...
import type IDomPolyfill from "@ulixee/unblocked-specification/plugin/IDomPolyfill";
import type { IDomPolyfillRemove } from "@ulixee/unblocked-specification/plugin/IDomPolyfill";
import type IBridgeDefinitions from "./interfaces/IBridgeDefinitions";
...
export default class DomBridger {
	public static removeDevtoolsFromPolyfill(polyfill: IDomPolyfill): void {
		polyfill.add = polyfill.add.filter(
			(x) => !isDevtoolsIndicator(x.path, x.propertyName),
		);
	...
	public static removeBrowserstackFromPolyfill(polyfill: IDomPolyfill): void {
		polyfill.add = polyfill.add.filter((x) => {
			return (
				!browserstackIndicators.added.some((pattern) =>
					pathIsPatternMatch(x.path, pattern),
				) &&
				!browserstackIndicators.extraAdded.some((pattern) =>
					pathIsPatternMatch(x.path, pattern),
				)
			);
		});
		polyfill.modify = polyfill.modify.filter((x) => {
			return (
				!browserstackIndicators.changed.some((pattern) =>
					pathIsPatternMatch(x.path, pattern),
				) &&
				!browserstackIndicators.extraChanged.some((pattern) =>
					pathIsPatternMatch(x.path, pattern),
				)
			);
		});
	...
	public static removeHeadlessFromPolyfill(polyfill: IDomPolyfill): void {
		polyfill.add = polyfill.add.filter(
			(x) => !isHeadlessAdded(x.path, x.propertyName),
		);
	...
	public static removeVariationsFromPolyfill(polyfill: IDomPolyfill): void {
		polyfill.modify = polyfill.modify.filter(
			(x) => !isVariationChange(x.path, x.propertyName),
		);
		polyfill.remove = polyfill.remove.filter(
			(x) => !isVariationChange(x.path, x.propertyName),
		);
		...
		polyfill.reorder = polyfill.reorder.filter(
			(x) =>
				!isVariationChange(x.path, x.propertyName) &&
				!isVariationChange(x.path, x.prevProperty),
		);
	}
...
function isVariationChange(path: string, propertyName: string): boolean {
	...
	if (
		instanceVariations.changed.some((pattern) =>
			pathIsPatternMatch(path, pattern),
		)
	)
		return true;
	if (
		instanceVariations.extraChanged.some((pattern) =>
			pathIsPatternMatch(path, pattern),
		)
	)
		return true;
	if (
		locationVariations.changed.some((pattern) =>
			pathIsPatternMatch(path, pattern),
		)
	)
		return true;
	if (
		locationVariations.extraChanged.some((pattern) =>
			pathIsPatternMatch(path, pattern),
		)
	)
		return true;
	if (
		windowVariations.changed.some((pattern) =>
			pathIsPatternMatch(path, pattern),
		)
	)
		return true;
	if (
		windowVariations.extraChanged.some((pattern) =>
			pathIsPatternMatch(path, pattern),
		)
	)
		return true;
	if (
		devtoolsIndicators.changed.some((pattern) =>
			pathIsPatternMatch(path, pattern),
		)
	)
		return true;
	if (
		devtoolsIndicators.extraChanged.some((pattern) =>
			pathIsPatternMatch(path, pattern),
		)
	)
		return true;
...
function isDevtoolsIndicator(path: string, propertyName: string): boolean {
	...
	if (
		devtoolsIndicators.added.some((pattern) =>
			pathIsPatternMatch(path, pattern),
		)
	)
		return true;
	if (
		devtoolsIndicators.extraAdded.some((pattern) =>
			pathIsPatternMatch(path, pattern),
		)
	)
		return true;
...
function isHeadlessAdded(path: string, _propertyName: string): boolean {
	if (
		headlessIndicators.added.some((pattern) =>
			pathIsPatternMatch(path, pattern),
		)
	)
		return true;
	if (
		headlessIndicators.extraAdded.some((pattern) =>
			pathIsPatternMatch(path, pattern),
		)
	)
		return true;
...
function injectDevtoolsIndicatorPolyfills(polyfill: IDomPolyfill): void {
	const toRemove = [
		...devtoolsIndicators.added,
		...devtoolsIndicators.extraAdded,
	];
...
function injectHeadlessIndicatorsToRemove(
	remove: IDomPolyfillRemove,
): IDomPolyfillRemove {
	const toRemove = [
		...headlessIndicators.added,
		...headlessIndicators.extraAdded,
	];
	for (const fullPath of toRemove) {
		const pathParts = fullPath.split(".");
		const propertyName = pathParts.pop();
