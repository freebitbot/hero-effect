...
export default abstract class BaseExtractor {
	...
	public get definitePathsMatched(): string[] {
		return [].concat(
			...Object.values(this.definitePathsMap).map((x) => Array.from(x)),
		);
	}
	...
	public get definitePathsNotMatched(): string[] {
		...
		return Array.from(this.definitePathsFound).filter((path) => {
			if (
				this.patternsHandledElsewhere.some((x) => pathIsPatternMatch(path, x))
			)
				return false;
			return !Extractor.definitePatterns.some((x) =>
				pathIsPatternMatch(path, x),
			);
		});
	}
	...
	public get definitePathsHandledElsewhere(): string[] {
		return Array.from(this.definitePathsFound).filter((path) => {
			return this.patternsHandledElsewhere.some((x) =>
				pathIsPatternMatch(path, x),
			);
		});
	}
	...
	public get extraPathsMatched(): string[] {
		...
		return Array.from(this.extraPathsFound).filter((path) => {
			if (Extractor.definitePatterns.some((x) => pathIsPatternMatch(path, x)))
				return false;
			if (Extractor.extraAddPatterns.some((x) => pathIsPatternMatch(path, x)))
				return true;
			return Extractor.extraChangePatterns.some((x) =>
				pathIsPatternMatch(path, x),
			);
		});
	}
	...
	public toJSON(): IBridgeDefinitions {
		...
		const addedPatterns = Array.from(this.definitePathsMap.added).map(
			(path) => {
				return Extractor.definitePatterns.find((x) =>
					pathIsPatternMatch(path, x),
				);
			},
		);

		const removedPatterns = Array.from(this.definitePathsMap.removed).map(
			(path) => {
				return Extractor.definitePatterns.find((x) =>
					pathIsPatternMatch(path, x),
				);
		...
		const changedPatterns = Array.from(this.definitePathsMap.changed).map(
			(path) => {
				return Extractor.definitePatterns.find((x) =>
					pathIsPatternMatch(path, x),
				);
		...
		const changedOrderPatterns = Array.from(
			this.definitePathsMap.changedOrder,
		).map((path) => {
			return Extractor.definitePatterns.find((x) =>
				pathIsPatternMatch(path, x),
			);
		...
		const extraAddPatterns = Array.from(this.extraPathsMatched)
			.map((path) => {
				return Extractor.extraAddPatterns.find((x) =>
					pathIsPatternMatch(path, x),
				);
			})
		...
		const extraChangePatterns = Array.from(this.extraPathsMatched)
			.map((path) => {
				return Extractor.extraChangePatterns.find((x) =>
					pathIsPatternMatch(path, x),
				);
			})
			.filter((path) => path !== undefined);
	...
	private addDefinitePath(path, pathType: IPathType): void {
		...
		const isDefinitive = (
			this.constructor as typeof BaseExtractor
		).isDefinitePath(path);
		if (isDefinitive) {
	...
	public static isDefinitePath(path: string): boolean {
		return BaseExtractor.definitePatterns.some((x) =>
			pathIsPatternMatch(path, x),
		);
	}
	...
	public static fitsAnyPattern(path: string): boolean {
		const allPatterns = [
			...BaseExtractor.definitePatterns,
			...BaseExtractor.extraAddPatterns,
			...BaseExtractor.extraChangePatterns,
		];
		return allPatterns.some((x) => pathIsPatternMatch(path, x));
	}

	public static isIgnoredExtraPath(path: string): boolean {
		return BaseExtractor.ignoredExtraPatterns.some((x) =>
			pathIsPatternMatch(path, x),
		);
	}

	public static getRegexps(path: string): RegExp[] {
		return BaseExtractor.regexps.filter((x) => x.test(path));
	}
}
