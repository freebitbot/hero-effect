...
Fs.writeFileSync(
	devtoolsToNodevtoolsPath,
	JSON.stringify(devtoolsToNodevtools, null, 2),
);
...
Fs.writeFileSync(
	headlessToHeadedPath,
	JSON.stringify(headlessToHeaded, null, 2),
);
...
Fs.writeFileSync(
	instanceToInstancePath,
	JSON.stringify(instanceToInstance, null, 2),
);
...
Fs.writeFileSync(
	localToBrowserstackPath,
	JSON.stringify(localToBrowserstack, null, 2),
);

// HELPERS /////////////////////////////////////////////////////////////

function generateRawMappings(
	...
	for (const path of Object.keys(startingMap.changed)) {
		const lhs = extractDiffValue(
			startingMap.changed[path].lhsKeysByValue,
			startingMap.allFileKeys,
		);
		const rhs = extractDiffValue(
			startingMap.changed[path].rhsKeysByValue,
			startingMap.allFileKeys,
		);
