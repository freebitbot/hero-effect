...
async function generatePathPatterns(): Promise<void> {
	...
	const instanceExtractor = new InstanceChangeExtractor(
		instanceToInstanceMappings,
	);
	const locationExtractor = new LocationChangeExtractor(
		instanceToInstanceMappings,
	);
	const windowExtractor = new WindowChangeExtractor(instanceToInstanceMappings);
	const devtoolsExtractor = new DevtoolsIndicatorExtractor(
		nodevtoolsToDevtoolsMappings,
	);
	const headlessExtractor = new HeadlessIndicatorExtractor(
		headedToHeadlessMappings,
	);
	const browserstackExtractor = new BrowserstackIndicatorExtractor(
		localToBrowserstack,
	);
	...
	for (const profileType of ["local", "browserstack"]) {
		...
		for (const profileName of dirNames) {
			...
			const contents = await readFile(
				`${domDir}/browser-dom-environment--https--1.json.gz`,
			);
			const { data: dom } = JSON.parse(
				await new Promise<string>((resolve) =>
					gunzip(contents, (err, x) => resolve(x.toString())),
				),
			);
	...
	for (const [title, extractor] of Object.entries(dataGroups)) {
		...
		await Fs.promises.writeFile(
			filePath,
			JSON.stringify(extractor.toJSON(), null, 2),
		);
		...
		extractor.definitePathsNotMatched.forEach((x) =>
			console.log(x, extractor.getRegexps(x)),
		);
