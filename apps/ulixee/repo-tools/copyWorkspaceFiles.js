...
module.exports = function copyWorkspace(pkg, baseDir, copyToDir) {
	...
	for (const packageGlob of packages || []) {
		if (packageGlob.startsWith("../") || packageGlob.includes("/build"))
			continue;

		let workspacePath = packageGlob;
		if (workspacePath.endsWith("/*")) {
			workspacePath = workspacePath.replace("/*", "");
			for (const subdir of Fs.readdirSync(
				Path.resolve(baseDir, workspacePath),
			)) {
				if (subdir === "node_modules") continue;
				if (
					!Fs.statSync(
						Path.resolve(baseDir, workspacePath, subdir),
					).isDirectory()
				)
					continue;
				if (
					!Fs.existsSync(
						Path.resolve(baseDir, workspacePath, subdir, "package.json"),
					)
				)
					continue;
