...
module.exports = function transformPackageJsons(
	rootPackageJson,
	startDir,
	buildDir,
	isDist,
	...
	console.log(
		"Preparing %s package.json files",
		isDist ? "/build-dist" : "/build",
		buildDir,
	);
	...
	lookForPackageJsons(buildDir, (packagePath) => {
		...
		if (fs.existsSync(`${packagePath}/${overridesFile}`)) {
			overridesJson =
				JSON.parse(
					fs.readFileSync(`${packagePath}/${overridesFile}`, "utf8"),
				) ?? {};
		...
		if (isDist && !finalPackageJson.private) {
			...
			if (
				!overridesJson.devDependencies ||
				(!finalPackageJson.devDependencies &&
					!finalPackageJson.devDependencies.length)
			) {
				delete finalPackageJson.devDependencies;
			}

			if (
				!fs.existsSync(`${packagePath}/LICENSE`) &&
				!fs.existsSync(`${packagePath}/LICENSE.md`)
			) {
...
`,
		...
		if (!finalPackageJson.types && fs.existsSync(`${packagePath}/index.d.ts`)) {
			...
			else finalPackageJson.types = "index.d.ts";
		}
		...
		if (finalPackageJson.scripts && !overridesJson.scripts) {
			for (const [key, value] of Object.entries(finalPackageJson.scripts)) {
				if (
					...
					(!finalPackageJson.private &&
						(key.startsWith("tsc") || key.startsWith("lint")))
				) {
					delete finalPackageJson.scripts[key];
				}
				if (key === "prepare" && value.startsWith("husky"))
					delete finalPackageJson.scripts[key];
			}
		}
		...
		const transformPackages = (x) => {
			if (
				x.startsWith("../") &&
				!fs.existsSync(`${buildDir}/${x.replace("/*", "")}`)
			) {
				return `../${x}`;
			}
			if (
		...
		if (finalPackageJson.workspaces) {
			if (Array.isArray(finalPackageJson.workspaces.packages)) {
				finalPackageJson.workspaces.packages =
					finalPackageJson.workspaces.packages.map(transformPackages);
			} else if (Array.isArray(finalPackageJson.workspaces)) {
				finalPackageJson.workspaces =
					finalPackageJson.workspaces.map(transformPackages);
			}
		}
		fs.writeFileSync(
