...
if (modified.workspaces) {
	...
	if (Array.isArray(modified.workspaces)) {
		...
		modified.workspaces.packages =
			modified.workspaces.packages?.filter(filterNonBuild);
	}
}
...
(async () => {
	...
	if (
		!(
			Array.isArray(modified.workspaces?.packages) &&
			modified.workspaces.packages.length
		) &&
		...
		if (!versionArgs[0].startsWith("--"))
			versionArgs[0] = "--" + versionArgs[0];
