...
import type { PluginConfigs } from "@ulixee/unblocked-specification/plugin/IUnblockedPlugin";
...
export default class CorePlugins implements ICorePlugins {
	...
	constructor(
		...
		for (const plugin of Object.values(corePluginsById)) {
			...
			const shouldActivate = (): boolean | undefined =>
				plugin.shouldActivate?.(
					agent.emulationProfile,
					getSessionSummary(),
					config,
				);
	...
	private loadDependencies(dependencyMap: IDependencyMap): void {
		for (const [clientPluginId, corePluginIds] of Object.entries(
			dependencyMap,
		)) {
	...
	private loadCorePluginPaths(corePluginPaths: string[]): void {
		for (const corePluginPath of corePluginPaths) {
			...
			const Plugins = requirePlugins<ICorePluginClass>(
				corePluginPath,
				PluginTypes.CorePlugin,
			);
