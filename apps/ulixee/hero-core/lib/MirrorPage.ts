...
export default class MirrorPage extends TypedEventEmitter<{
			...
			if (page[installedScriptsSymbol]) {
				...
				promises.push(
					this.showChromeInteractions
						? InjectedScripts.installInteractionScript(
								page,
	...
	public async openInContext(
		...
		try {
			...
			if (viewport) {
				await this.page.devtoolsSession.send(
	...
	public subscribe(tab: Tab, cleanupOnTabClose = true): void {
		...
		this.domRecording.mainFrameIds = tab.session.db.frames.mainFrameIds(
			tab.tabId,
		);
		...
		this.events.once(tab, "close", () => {
			...
			if (cleanupOnTabClose) {
				...
				this.domRecording.mainFrameIds = new Set(
					tab.session.db.frames.mainFrameIds(tab.tabId),
				);
			}
		});
		...
		return await this.loadQueue.run<T>(async () => {
			...
			if (
				...
				(loadingDocument.url !== this.page.mainFrame.url ||
					newPaintIndex === -1 ||
					loadingDocument.paintStartTimestamp !==
						this.loadedDocument?.paintStartTimestamp)
			) {
				...
				await this.page.mainFrame.waitForLifecycleEvent(
					"DOMContentLoaded",
					loader.loaderId,
				);
			...
			if (newPaintIndex >= 0) {
				...
				const showOverlay =
					(overlayLabel || isLoadingDocument) && this.showChromeInteractions;
	...
	private getActiveDocument(newPaintIndex: number): IDocument {
		...
		if (newPaintIndex < 0) {
			if (this.domRecording.documents.length) {
				activeDocumentTimestamp =
					this.domRecording.documents[0]?.paintStartTimestamp;
			}
		} else {
			activeDocumentTimestamp =
				this.domRecording.paintEvents[newPaintIndex].timestamp;
		}
	...
	private processPendingDomChanges(): void {
		...
		const nextDocument = this.loadedDocument
			? this.domRecording.documents.find(
					(x) =>
						x.isMainframe &&
						x.paintStartTimestamp > this.loadedDocument.paintStartTimestamp,
				)
			: null;

		let needsPaintSort = false;
		const lastPaint =
			this.domRecording.paintEvents[this.domRecording.paintEvents.length - 1];
		for (const paint of domRecording.paintEvents) {
			const existing = this.paintEventByTimestamp[paint.timestamp];

			if (
				this.loadedDocument &&
				paint.timestamp > this.loadedDocument.paintStartTimestamp
			) {
				if (
					!nextDocument ||
					paint.timestamp < nextDocument.paintStartTimestamp
				) {
					this.isLoadedDocumentDirty = true;
				}
			}
	...
	private applyFrameNodePath<T extends { frameId: number }>(
		item: T,
	): T & { frameIdPath: string } {
	...
	private isLoadedDocument(document: IDocument): boolean {
		...
		return (
			this.loadedDocument.paintStartTimestamp ===
				document.paintStartTimestamp && this.loadedDocument.url === document.url
		);
	}
	...
	private async injectPaintEvents(document: IDocument): Promise<void> {
		...
		const nextDocument = this.domRecording.documents.find(
			(x) =>
				x.isMainframe && x.paintStartTimestamp > document.paintStartTimestamp,
		);
		...
		for (let idx = 0; idx < paintEvents.length; idx += 1) {
			...
			if (
				nextDocument &&
				paintEvent.timestamp >= nextDocument.paintStartTimestamp
			)
				continue;
