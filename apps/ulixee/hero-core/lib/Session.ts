...
import type Agent from "@ulixee/unblocked-agent/lib/Agent";
...
import type Resources from "@ulixee/unblocked-agent/lib/Resources";
import type WebsocketMessages from "@ulixee/unblocked-agent/lib/WebsocketMessages";
...
import type { IOutputChangeRecord } from "../models/OutputTable";
	...
	implements ICommandableTarget, IRemoteEventListener
{
	...
	protected constructor(
		...
		options.sessionPersistence ??=
			core.options.disableSessionPersistence !== true;
	...
	public isAllowedCommand(method: string): boolean {
		return (
			this.commandRecorder.fnNames.has(method) || method === "recordOutput"
		);
	}
	...
	public getTabs(): Promise<Tab[]> {
		return Promise.resolve(
			[...this.tabsById.values()].filter((x) => !x.isClosing),
		);
	}
	...
	public setSnippet(key: string, value: any, timestamp: number): Promise<void> {
		const asset = this.db.snippets.insert(
			key,
			value,
			timestamp,
			this.commands.lastId,
		);
	...
	public async openBrowser(): Promise<void> {
		...
		requestSession.bypassResourceRegistrationForHost =
			this.bypassResourceRegistrationForHost;
		this.events.on(
			requestSession,
			"resource-state",
			this.onResourceStates.bind(this),
		);
		this.events.on(
			requestSession,
			"socket-close",
		...
		this.events.on(
			resources,
			"browser-loaded",
			this.onBrowserLoadedResource.bind(this),
		);
		this.events.on(
			resources,
			"browser-requested",
			this.onBrowserRequestedResource.bind(this),
		);
		...
		agent.mitmRequestSession.respondWithHttpErrorStacks =
			...
			this.options.showChromeInteractions === true;
	...
	public async createTab(): Promise<Tab> {
		...
		this.agent.plugins.addDomOverride(
			...
			(data, frame: IFrame) => {
				...
				if (tab) {
					void tab.frameEnvironmentsById
						.get(frame.frameId)
						?.onShadowDomPushed(data);
				}
			},
		);
	...
	private onResource(
		event: BrowserContext["resources"]["EventTypes"]["change"],
	): void {
		...
		if (
			event.type === "mitm-response" &&
			!event.requestProcessingDetails.wasIntercepted
		) {
	...
	private onCookieChange(
		event: BrowserContext["resources"]["EventTypes"]["cookie-change"],
	): void {
	...
	private onResourceNeedsMerge(
		event: BrowserContext["resources"]["EventTypes"]["merge"],
	): void {
	...
	private onBrowserLoadedResource(
		...
		this.db.resources.updateReceivedTime(
			event.resourceId,
			event.browserLoadedTime,
		);
	}
	...
	private onDevtoolsMessage(
		event: DevtoolsSessionLogger["EventTypes"]["devtools-message"],
	): void {
...
${data}`,
	...
	public static async create(
		...
		if (resumeSessionId) {
			...
			if (!session) {
				await Session.restoreOptionsFromSessionRecord(
					options,
					resumeSessionId,
					core,
				);
	...
	public static hasKeepAliveSessions(): boolean {
		for (const session of Object.values(Session.byId)) {
	...
	public static sessionsWithBrowserId(browserId: string): Session[] {
		return Object.values(Session.byId).filter(
			(x) => x.browserContext?.browserId === browserId,
		);
	}
}
