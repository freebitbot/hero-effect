...
export default class MirrorNetwork {
	...
	public async mirrorNetworkRequests(
			...
			return {
				...
				responseHeaders: [
					{ name: "Content-Type", value: "text/html; charset=utf-8" },
					{
						...
						value: "script-src 'nonce-hero-timetravel'; connect-src 'self'",
					},
		...
		if (
			...
			return {
				...
				responseCode: 200,
				responseHeaders: [
					{ name: "Content-Type", value: matches[0].contentType },
				],
				body: "",
			};
		}
		...
		const { headers, contentEncoding, isJavascript } =
			this.getMockHeaders(resource);
	...
	public addResource(resource: IResourceSummary): void {
		...
		if (!this.resourceLookup[key]?.length) {
			...
			if (pendingResolutionIdx >= 0) {
				this.resourceLookup[key][pendingResolutionIdx].responsePromise.resolve(
					resource,
				);
	...
	public setResources(
		...
		for (const resourceSet of Object.values(this.resourceLookup)) {
			for (const resource of resourceSet) {
				if (resource.responsePromise && !resource.responsePromise.isResolved)
					resource.responsePromise.reject(
						new CanceledPromiseError("Replacing resources"),
						true,
					);
			}
		}
		...
		for (let resource of resources) {
			if (!(resource as IResourceSummary).method) {
				resource = ResourcesTable.toResourceSummary(
					resource as IResourcesRecord,
				);
			}
	...
	public static createFromSessionDb(
		...
		options.loadResourceDetails ??= MirrorNetwork.loadResourceFromDb.bind(
			MirrorNetwork,
			db,
		);
