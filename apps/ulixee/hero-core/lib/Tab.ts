...
import type FrameNavigationsObserver from "@ulixee/unblocked-agent/lib/FrameNavigationsObserver";
import type Page from "@ulixee/unblocked-agent/lib/Page";
import type { IWebsocketMessage } from "@ulixee/unblocked-agent/lib/WebsocketMessages";
import type IDialog from "@ulixee/unblocked-specification/agent/browser/IDialog";
import type IExecJsPathResult from "@ulixee/unblocked-specification/agent/browser/IExecJsPathResult";
import type IFileChooserPrompt from "@ulixee/unblocked-specification/agent/browser/IFileChooserPrompt";
import type INavigation from "@ulixee/unblocked-specification/agent/browser/INavigation";
import type { IPageEvents } from "@ulixee/unblocked-specification/agent/browser/IPage";
import type IScreenshotOptions from "@ulixee/unblocked-specification/agent/browser/IScreenshotOptions";
import {
	type ILoadStatus,
	type ILocationTrigger,
	LoadStatus,
} from "@ulixee/unblocked-specification/agent/browser/Location";
import type { IInteractionGroups } from "@ulixee/unblocked-specification/agent/interact/IInteractions";
import type IResourceMeta from "@ulixee/unblocked-specification/agent/net/IResourceMeta";
import type {
	IRemoteEmitFn,
	IRemoteEventListener,
} from "../interfaces/IRemoteEventListener";
import type { IDomChangeRecord } from "../models/DomChangesTable";
import type { IFocusRecord } from "../models/FocusEventsTable";
...
import type { IStorageChangesEntry } from "../models/StorageChangesTable";
	...
	implements ISessionMeta, ICommandableTarget, IRemoteEventListener
{
	...
	public readonly frameEnvironmentsByDevtoolsId = new Map<
		string,
		FrameEnvironment
	>();
	public page: Page;
	...
	private constructor(session: Session, page: Page, parentTabId?: number) {
		...
		for (const frame of page.frames) {
			...
			this.frameEnvironmentsByDevtoolsId.set(
				frameEnvironment.devtoolsFrameId,
				frameEnvironment,
			);
		...
		this.mirrorNetwork = new MirrorNetwork({
			...
			loadResourceDetails: MirrorNetwork.loadResourceFromDb.bind(
				MirrorNetwork,
				this.session.db,
			),
		});
	...
	public getFrameEnvironment(frameId?: number): FrameEnvironment {
		return frameId
			? this.frameEnvironmentsById.get(frameId)
	...
	public async setBlockedResourceTypes(
		...
		let interceptor = mitmSession.interceptorHandlers.find(
			(x) => x.types && !x.handlerFn,
		);
		if (!interceptor) {
			mitmSession.interceptorHandlers.push({ types: [] });
			interceptor =
				mitmSession.interceptorHandlers[
					mitmSession.interceptorHandlers.length - 1
				];
		}
		let enableJs = true;
	...
	public setInterceptedResources(
	...
	public async close(): Promise<void> {
		...
		try {
			...
			Timer.expireAll(
				this.waitTimeouts,
				new CanceledPromiseError(cancelMessage),
			);
	...
	public async detachResource(
		...
		resource.response.buffer =
			await this.session.db.resources.getResourceBodyById(resourceId, true);

		if (resource.type === "Websocket") {
			detachedResource.websocketMessages =
				this.session.websocketMessages.getMessages(resourceId);
		}
		this.session.emit("collected-asset", {
			type: "resource",
			asset: detachedResource,
		});
	...
	public async getResourceProperty(
		...
		if (propertyPath === "response.buffer") {
			return await this.session.db.resources.getResourceBodyById(
				resourceId,
				true,
			);
		}
	...
	public findResource(
		...
		const sinceCommandId =
			...
			this.navigations.lastHttpNavigationRequest?.startCommandId;
		// find latest resource
		for (const resourceMeta of this.session.resources
			.getForTab(this.id)
			.reverse()) {
	...
	public findResources(
		...
		const sinceCommandId =
			...
			this.navigations.lastHttpNavigationRequest?.startCommandId;
		// find all resources
		const resourceMetas = this.session.resources
			.getForTab(this.id)
			.filter((meta) => {
	...
	public getFrameEnvironments(): Promise<IFrameMeta[]> {
		return Promise.resolve(
			[...this.frameEnvironmentsById.values()]
	...
	public onResource(x: ITabEventParams["resource"]): void {
		...
		const resourceSummary: IResourceSummary = {
			...
			hasResponse:
				x.response.headers && Object.keys(x.response.headers).length > 0,
			contentType: x.response.headers
				? ((x.response.headers["content-type"] ??
						x.response.headers["Content-Type"]) as string)
				: "",
		};
	...
	public async getElementHtml(
		detachedElement: IDetachedElement,
	): Promise<IDetachedElement> {
		await this.flushDomChanges();
		const paintIndex = this.mirrorPage.getPaintIndex(
		...
		try {
			...
			const frameDomNodeId = this.frameEnvironmentsById.get(
	...
	public async waitForNewTab(options: IWaitForOptions = {}): Promise<Tab> {
		...
		if (startCommandId >= 0) {
			for (const tab of this.session.tabsById.values()) {
				if (
					tab.parentTabId === this.id &&
					tab.createdAtCommandId >= startCommandId
				) {
					newTab = tab;
					break;
				}
		...
		if (newTab.url === "about:blank" || !newTab.url) {
			...
			timeoutMs -= millis;
			await newTab.navigations
				.waitOn("navigation-requested", null, timeoutMs)
				.catch(() => null);
		}
	...
	public async waitForResources(
		...
		if (
			options?.sinceCommandId !== undefined &&
			Number.isInteger(options.sinceCommandId)
		) {
		...
		try {
			...
			const shouldIgnoreError =
				err instanceof TimeoutError && options?.throwIfTimeout === false;
	...
	public async waitForFileChooser(
		...
		if (!startCommandId && this.session.commands.length >= 2) {
			startCommandId =
				this.session.commands.history[this.session.commands.length - 2]?.id;
		}
	...
	public async runPluginCommand(toPluginId: string, args: any[]): Promise<any> {
		...
		return await this.session.plugins.onPluginCommand(
			toPluginId,
			commandMeta,
			args,
		);
	...
	public willRunCommand(command: ICommandMeta): void {
		...
		if (lastCommand && prevFrameId !== command.frameId) {
			...
			this.frameEnvironmentsById
				.get(prevFrameId)
				?.setInteractionDisplay(false, true, true);
		}
	}

	public addDomStateListener(
		id: string,
		options: IDomStateListenArgs,
	): DomStateListener {
		...
		this.events.once(
			listener,
			"resolved",
			() => delete this.domStateListenersByJsPathId[id],
		);
	...
	public async getDomChanges(
		...
		return this.session.db.domChanges.getFrameChanges(
	...
	public async assert(
		...
		const domStateListener =
			this.domStateListenersByJsPathId[JSON.stringify(domStateIdJsPath)];
	...
	public addRemoteEventListener(
		...
		const listener = this.session.commands.observeRemoteEvents(
			type,
			emitFn,
			jsPath,
			this.id,
		...
		if (type === "message") {
			...
			if (domain !== "resources") {
				throw new Error(
					`Unknown "message" type requested in JsPath - ${domain}`,
				);
			}
			// need to give client time to register function sending events
			process.nextTick(() =>
				this.session.websocketMessages.listen(
					Number(resourceId),
					listener.listenFn,
				),
			);
	...
	public removeRemoteEventListener(
		listenerId: string,
		options?: any,
	): Promise<any> {
	...
	private async waitForReady(): Promise<void> {
		...
		if (this.session.options?.interceptedResources) {
			await this.setInterceptedResources(
				this.session.options.interceptedResources,
			);
		}
	}
	...
	private listen(): void {
		...
		this.events.on(
			page,
			"dom-storage-updated",
			this.onStorageUpdated.bind(this),
			true,
		);
	}
	...
	private onPageCallback(event: IPageEvents["page-callback-triggered"]): void {
		if (event.name === InjectedScripts.PageEventsCallbackName) {
			...
			this.frameEnvironmentsById
				.get(frameId)
				.onPageRecorderEvents(JSON.parse(payload));
		}
	}
	...
	private isResourceFilterMatch(
		...
		if (!resourceMeta.seenAtCommandId) {
			...
			sinceCommandId &&
			resourceMeta.seenAtCommandId <= sinceCommandId
		) {
			return false;
		}
	...
	private onStorageUpdated(
		event: Page["EventTypes"]["dom-storage-updated"],
	): void {
	...
	private translateDevtoolsFrameId(devtoolsFrameId: string): number {
		return (
			this.frameEnvironmentsByDevtoolsId.get(devtoolsFrameId)?.id ??
			this.mainFrameId
		);
	}
