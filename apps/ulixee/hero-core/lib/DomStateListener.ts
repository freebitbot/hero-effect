...
export default class DomStateListener extends TypedEventEmitter<IDomStateEvents> {
	...
	public readonly rawBatchAssertionsById = new Map<
		string,
		IDomStateAssertionBatch
	>();
	...
	constructor(
		...
		this.startTime = previousCommand
			? previousCommand.runStartDate + 1
			: Date.now();
	...
	public async runBatchAssert(batchId: string): Promise<boolean> {
		...
		for (const assertion of assertions) {
			...
			if (assertType === "url") {
				const frame =
					this.tab.frameEnvironmentsById.get(frameId) ??
					this.tab.mainFrameEnvironment;
				const url = frame.url;
		...
		for (const [frameId, frameAssertions] of domAssertionsByFrameId) {
			...
			const failedDomAssertions = await frame.runDomAssertions(
				batchId,
				frameAssertions,
			);
	...
	private async validateState(): Promise<void> {
		...
		try {
			...
			const promises = [...this.commandFnsById].map(
				async ([id, runCommandFn]) => {
