...
import type { IJsPath, INodePointer } from "@ulixee/js-path";
import type Frame from "@ulixee/unblocked-agent/lib/Frame";
import type FrameNavigations from "@ulixee/unblocked-agent/lib/FrameNavigations";
import type { ISerializable } from "@ulixee/unblocked-agent/lib/JsPath";
import type IExecJsPathResult from "@ulixee/unblocked-specification/agent/browser/IExecJsPathResult";
import type { IFrameEvents } from "@ulixee/unblocked-specification/agent/browser/IFrame";
import type { IFrameNavigationEvents } from "@ulixee/unblocked-specification/agent/browser/IFrameNavigations";
	...
	implements ICommandableTarget, IRemoteEventListener
{
	...
	private flushPageEventsRecorderResolvers = new Map<
		string,
		Resolvable<void>
	>();
	...
	constructor(tab: Tab, frame: Frame) {
		...
		if (this.session.options.showChromeInteractions) {
			frame.interactor.beforeEachInteractionStep =
				this.beforeEachInteractionStep.bind(this);
			frame.interactor.afterInteractionGroups =
				this.afterInteractionGroups.bind(this);
		}
		frame.interactor.afterEachInteractionStep =
			this.afterEachInteractionStep.bind(this);
	...
	public async getJsValue<T>(expression: string): Promise<T> {
		return await this.frame.evaluate<T>(expression, {
	...
	public isPaintingStable(): Promise<boolean> {
		return Promise.resolve(
			this.navigations.hasLoadStatus(LoadStatus.PaintingStable),
		);
	}

	public isDomContentLoaded(): Promise<boolean> {
		return Promise.resolve(
			this.navigations.hasLoadStatus(LoadStatus.DomContentLoaded),
		);
	}

	public isAllContentLoaded(): Promise<boolean> {
		return Promise.resolve(
			this.navigations.hasLoadStatus(LoadStatus.AllContentLoaded),
		);
	}
...
b) Use the UserProfile feature to set cookies for 1 or more domains before they're loaded (https://ulixee.org/docs/advanced/user-profile)
	...
	public async runDomAssertions(
		...
		try {
			const { failedIndices } = await this.runIsolatedFn<{
				failedIndices: Record<number, any>;
			}>("HERO.DomAssertions.run", id);
			return Object.keys(failedIndices).length;
		} catch (error) {
			if (error instanceof CanceledPromiseError) return 0;
			if (
				String(error).includes("This assertion batch has not been installed")
			) {
	...
	public async waitForLocation(
		...
		const resourceId =
			location.resourceId ?? (await location.resourceIdResolvable.promise);
	...
	public onPageRecorderEvents(results: PageRecorderResultSet): boolean {
		const [domChanges, mouseEvents, focusEvents, scrollEvents, loadEvents, id] =
			results;
		if (id) {
		...
		let documentNavigation = this.navigations.get(
			this.lastDomChangeDocumentNavigationId,
		);
		...
		for (const domChange of domChanges) {
			...
			if (
				...
				action === DomActionType.location
			) {
				const url = domChange[1].textContent;
				documentNavigation = this.navigations.findMostRecentHistory(
					(x) => x.finalUrl === url,
				);
				...
				if (documentNavigation) {
					if (
						...
						this.frame.navigations.adjustInPageLocationChangeTime(
							documentNavigation,
							timestamp,
						);
					}
					if (
						...
						documentNavigation.id >
							(this.lastDomChangeDocumentNavigationId ?? 0)
					) {
		...
		for (const mouseEvent of mouseEvents) {
			...
			const record = db.mouseEvents.insert(
				tabId,
				frameId,
				lastCommand.id,
				mouseEvent,
		...
		for (const focusEvent of focusEvents) {
			...
			const record = db.focusEvents.insert(
				tabId,
				frameId,
				lastCommand.id,
				focusEvent,
		...
		for (const scrollEvent of scrollEvents) {
			lastCommand = commands.getCommandForTimestamp(
				lastCommand,
				scrollEvent[2],
			);
			const record = db.scrollEvents.insert(
				tabId,
				frameId,
				lastCommand.id,
				scrollEvent,
	...
	private listen(): void {
		...
		this.events.on(
			frame.navigations,
			"change",
			this.recordNavigationChange.bind(this),
		);
	}
	...
	private recordNavigationChange(
		event: IFrameNavigationEvents["change"],
	): void {
