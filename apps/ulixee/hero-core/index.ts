...
import type { ICorePluginClass } from "@ulixee/hero-interfaces/ICorePlugin";
...
export default class HeroCore extends TypedEventEmitter<
	THeroCoreEvents & { close: void }
> {
	...
	constructor(readonly options: ICoreConfigureOptions = {}) {
		...
		this.pool = new Pool({
			...
			plugins:
				options.defaultUnblockedPlugins ?? HeroCore.defaultUnblockedPlugins,
		});
	...
	public static async start(
		...
		if (HeroCore.instances.length > 0) {
			return HeroCore.instances[0];
		}
	...
	public static addConnection(
		transportToClient?: ITransport,
	): ConnectionToHeroClient {
		if (!HeroCore.instances.length) {
			new HeroCore();
		}
		return HeroCore.instances[0].addConnection(transportToClient);
	}

	public static async shutdown(): Promise<any> {
		if (HeroCore.isShuttingDown) return HeroCore.isShuttingDown;
		ShutdownHandler.unregister(HeroCore.shutdown);
		HeroCore.isShuttingDown = Promise.allSettled(
			HeroCore.instances.map((x) => x.close()),
		);
		await HeroCore.isShuttingDown;
		if (HeroCore.onShutdown) HeroCore.onShutdown();
	}
	...
	public static use(
		...
		for (const Plugin of Plugins) {
			if (Plugin.type === PluginTypes.CorePlugin) {
				HeroCore.defaultCorePluginsById[Plugin.id] = Plugin;
			}
		}
	}

	private static registerSignals(shouldShutdownOnSignals = true): void {
		if (HeroCore.didRegisterSignals) return;
		HeroCore.didRegisterSignals = true;
		if (!shouldShutdownOnSignals) ShutdownHandler.disableSignals = true;
		HeroCore.shutdown = HeroCore.shutdown.bind(HeroCore);
		ShutdownHandler.register(HeroCore.shutdown);
