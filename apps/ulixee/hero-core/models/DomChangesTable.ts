...
export default class DomChangesTable extends SqliteTable<IDomChangeRecord> {
	...
	public insert(
		...
		const record = [
			...
			nodeData.attributeNamespaces
				? JSON.stringify(nodeData.attributeNamespaces)
				: undefined,
	...
	public getFrameChanges(
		...
		return query
			.all(frameId, afterCommandId ?? 0)
			.map(DomChangesTable.inflateRecord);
	}
	...
	public static inflateRecord(record: IDomChangeRecord): IDomChangeRecord {
		...
		record.attributes = record.attributes
			? JSON.parse(record.attributes as any)
			: undefined;
		...
		record.properties = record.properties
			? JSON.parse(record.properties as any)
			: undefined;
		return record;
	}
