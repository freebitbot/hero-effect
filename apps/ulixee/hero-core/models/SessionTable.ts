...
export default class SessionTable extends SqliteTable<ISessionRecord> {
	...
	public insert(
		...
		const record = [
			...
			scriptInvocationMeta?.execArgv
				? JSON.stringify(scriptInvocationMeta.execArgv)
				: null,
	...
	public get(): ISessionRecord {
		const record = this.db
			...
			.get() as ISessionRecord;
		if (!record) return null;
		record.createSessionOptions = TypeSerializer.parse(
			record.createSessionOptions as string,
		);
		record.viewport = JSON.parse((record.viewport as any) ?? "undefined");
		record.deviceProfile = TypeSerializer.parse(
			(record.deviceProfile as any) ?? "undefined",
		);
