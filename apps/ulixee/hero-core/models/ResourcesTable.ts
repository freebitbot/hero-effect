...
export default class ResourcesTable extends SqliteTable<IResourcesRecord> {
	...
	public updateReceivedTime(id: number, timestamp: number): void {
		...
		this.db
			.prepare(
				`update ${this.tableName} set browserLoadedTimestamp=? where id=?`,
			)
			.run(timestamp, id);
	}
	...
	public updateBrowserRequestId(
		...
		this.db
			.prepare(
				`update ${this.tableName} set tabId=?, devtoolsRequestId=? where id=?`,
			)
	...
	public get(id: number): IResourcesRecord {
		...
		return <IResourcesRecord>(
			this.db.prepare(`select * from ${this.tableName} where id=?`).get(id)
		);
	}
		...
		return {
			...
			request: {
				...
				headers: record.requestHeaders
					? JSON.parse(record.requestHeaders)
					: null,
				timestamp: record.requestTimestamp,
				trailers: record.requestTrailers
					? JSON.parse(record.requestTrailers)
					: null,
				postData:
					"requestPostData" in record
						? Buffer.from(record.requestPostData ?? [])
						: null,
			},
			response: {
				...
				headers: record.responseHeaders
					...
					: null,
				trailers: record.responseTrailers
					? JSON.parse(record.responseTrailers)
					: null,
	...
	public mergeWithExisting(
		...
		existingDbRecord.browserBlockedReason =
			resourceFailedEvent.browserBlockedReason;
		...
		existingDbRecord.statusMessage ??=
			newResourceDetails.response.statusMessage;
		existingDbRecord.browserLoadFailure =
			newResourceDetails.response.browserLoadFailure;
		existingDbRecord.browserLoadedTimestamp ??=
			newResourceDetails.response.timestamp;
		...
		if (newResourceDetails.response.headers) {
			const responseHeaders = JSON.stringify(
				newResourceDetails.response.headers,
			);
		...
		if (resourceFailedEvent.responseOriginalHeaders) {
			...
			if (
				...
				existingDbRecord.responseOriginalHeaders?.length
			) {
		...
		existingResource.response.browserLoadFailure =
			newResourceDetails.response?.browserLoadFailure;
	...
	public insert(
		...
		if (meta.response && meta.response.headers) {
			contentEncoding = <string>(
				(meta.response.headers["Content-Encoding"] ??
					meta.response.headers["content-encoding"])
			);
		}
		return this.queuePendingInsert([
...
from ${this.tableName}${whereClause}`,
		...
		return records
			.filter((resource) => {
				if (
					hasResponse &&
					(!resource.responseHeaders || resource.responseHeaders === "{}")
				) {
					return false;
				}
	...
	public getResourcePostDataById(resourceId: number): Buffer | null {
		...
		if (!record) {
			record = <any>(
				this.db
					.prepare(
						`select requestPostData from ${this.tableName} where id=? limit 1`,
					)
					.get(resourceId)
			);
		}
	...
	public async getResourceBodyById(
		resourceId: number,
		decompress = true,
	): Promise<Buffer> {
		const pendingRecords = this.findPendingRecords((x) => x[0] === resourceId);
		...
		if (!record) {
			record = <any>(
				this.db
					.prepare(
						`select responseData, responseEncoding from ${this.tableName} where id=? limit 1`,
					)
					.get(resourceId)
			);
		}
