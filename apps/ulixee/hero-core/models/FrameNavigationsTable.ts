...
export default class FrameNavigationsTable extends SqliteTable<IFrameNavigationRecord> {
	...
	public getAllNavigations(): INavigation[] {
		if (!this.allNavigationsById.size) {
			for (const record of this.all()) {
				this.allNavigationsById.set(
					record.id,
					FrameNavigationsTable.toNavigation(record),
				);
			}
		}
	...
	public get(id: number): INavigation {
		return (
			this.allNavigationsById.get(id) ??
			FrameNavigationsTable.toNavigation(this.getById(id))
		);
	}
	...
	public last(): IFrameNavigationRecord {
		return this.db
			.prepare(
				`select * from ${this.tableName} order by initiatedTime desc limit 1`,
			)
	...
	public getMostRecentTabNavigations(
		...
		const navigations = this.db
			.prepare(
				`select * from ${this.tableName} where tabId=? order by initiatedTime desc`,
			)
