...
import type { IConnectionToClientEvents } from "@ulixee/net/interfaces/IConnectionToClient";
...
import type HeroCore from "../index";
import CommandRunner, { type ICommandableTarget } from "../lib/CommandRunner";
import type { ICommandPresetMeta } from "../lib/Commands";
	...
	implements IConnectionToClient<any, {}>, ICommandableTarget
{
	...
	public async handleRequest(
		...
		const { messageId, command, meta, recordCommands, ...nextCommandMeta } =
			payload;
		...
		try {
			...
			let shouldSkipLogging =
				isClosing && error instanceof CanceledPromiseError;
		...
		try {
			...
			if (
				...
				String(err).includes("Websocket was not open")
			)
				return;
			throw err;
		}
	...
	public isActive(): boolean {
		return (
			this.sessionIdToRemoteEvents.size > 0 ||
			this.activeCommandMessageIds.size > 0
		);
	}
	...
	public async createSession(
		...
		if (!this.sessionIdToRemoteEvents.has(sessionId)) {
			...
			session.once("closing", () =>
				this.sessionIdToRemoteEvents.delete(sessionId),
			);
	...
	private serializeToMetadata(data: any): any {
		...
		for (const [key, value] of Object.entries(data)) {
			if (value instanceof Tab || value instanceof FrameEnvironment)
