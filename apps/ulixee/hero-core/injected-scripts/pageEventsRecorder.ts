...
const eventsCallback = callback.bind(null, callbackName) as (
	data: string,
) => void;
...
class PageEventsRecorder {
	...
	private readonly propertyTrackingElements = new Map<
		Node,
		Map<string, string | boolean>
	>();
	private readonly stylesheets = new Map<
	...
	public trackFocus(eventType: FocusType, focusEvent: FocusEvent) {
		...
		const relatedNodeId = NodeTracker.getNodeId(
	...
	public trackMouse(eventType: MouseEventType, mouseEvent: MouseEvent) {
		...
		const relatedNodeId = NodeTracker.getNodeId(
			mouseEvent.relatedTarget as Node,
		);
		const event = [
			eventType,
			mouseEvent.pageX,
		...
		for (let i = 0; i < path.length; i += 1) {
			...
			if (!element || element.id !== id || element.localName !== localName) {
				element = children.find(
					(x) => (x.id === id || !id) && x.localName === localName,
				);
			}
			if (!element) {
	...
	public listenToInteractionEvents() {
		...
		for (const event of ["input", "keydown", "change"]) {
			document.addEventListener(
				event,
				this.checkForAllPropertyChanges.bind(this),
				{
					capture: true,
					passive: true,
				},
		...
		document.addEventListener(
			"mousemove",
			(e) => this.trackMouse(MouseEventType.MOVE, e),
			{
				capture: true,
				passive: true,
			},
		...
		document.addEventListener(
			"mousedown",
			(e) => this.trackMouse(MouseEventType.DOWN, e),
			{
				capture: true,
				passive: true,
			},
		...
		document.addEventListener(
			"mouseup",
			(e) => this.trackMouse(MouseEventType.UP, e),
			{
				capture: true,
				passive: true,
			},
		...
		document.addEventListener(
			"mouseover",
			(e) => this.trackMouse(MouseEventType.OVER, e),
			{
				capture: true,
				passive: true,
			},
		...
		document.addEventListener(
			"mouseleave",
			(e) => this.trackMouse(MouseEventType.OUT, e),
			{
				capture: true,
				passive: true,
			},
		...
		document.addEventListener(
			"focusin",
			(e) => this.trackFocus(FocusType.IN, e),
			{
				capture: true,
				passive: true,
			},
		...
		document.addEventListener(
			"focusout",
			(e) => this.trackFocus(FocusType.OUT, e),
			{
				capture: true,
				passive: true,
			},
	...
	private convertMutationsToChanges(mutations: MutationRecord[]): void {
		...
		for (const mutation of mutations) {
			...
			if (type === MutationRecordType.childList) {
				...
				for (
					let i = 0, length = mutation.addedNodes.length;
					i < length;
					i += 1
				) {
			...
			if (type === MutationRecordType.attributes) {
				...
				serial.attributes ??= {};
				serial.attributes[mutation.attributeName] = (
					target as Element
				...
				if (mutation.attributeNamespace && mutation.attributeNamespace !== "") {
					...
					serial.attributeNamespaces[mutation.attributeName] =
	...
	private serializeNodeToRoot(
		...
		if (!serial.parentNodeId && node.parentNode) {
			const parentSerial = this.serializeNodeToRoot(
				node.parentNode,
				changeTime,
				addedNodeMap,
			);
		...
		if (!serial.previousSiblingId && node.previousSibling) {
			const previous = this.serializeNodeToRoot(
		...
		switch (data.nodeType) {
			...
			case Node.ELEMENT_NODE: {
				...
				if (
					...
					element.namespaceURI !== defaultNamespaceUri
				) {
				...
				if (element.attributes.length) {
					data.attributes = {};
					for (
						let i = 0, length = element.attributes.length;
						i < length;
						i += 1
					) {
						...
						if (
							attr.namespaceURI &&
							attr.namespaceURI !== defaultNamespaceUri
						) {
				...
				if (
					!this.isListeningForInteractionEvents &&
					!hiddenTags.has(data.tagName)
				) {
	...
	private pushChange(
		...
		if (action === DomActionType.added) {
			...
			if (this.doNotTrackElementsById.has(serial.previousSiblingId)) {
				...
				let previousNode: ChildNode = this.doNotTrackElementsById.get(
					serial.previousSiblingId,
				);
