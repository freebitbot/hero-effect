...
export interface IFrontendMouseEvent
	extends Omit<
		IMouseEventRecord,
		"frameId" | "tabId" | "commandId" | "timestamp" | "event"
	> {
	frameIdPath: string;
}
...
function delegateInteractToSubframe(
	event: { frameIdPath: string },
	action: string,
) {
...
function highlightNodes(nodes: IHighlightedNodes) {
	...
	if (
		nodes &&
		nodes.frameIdPath &&
		nodes.frameIdPath !== window.selfFrameIdPath
	) {
		clearHighlights();
...
const offsetsAtPageY = new Map<
	number,
	{ pageOffset: number; elementOffset: number }
>();
...
function updateMouse(mouseEvent: IFrontendMouseEvent) {
	...
	if (mouseEvent.pageX !== undefined) {
		const targetNode = window.getNodeById(
			mouseEvent.targetNodeId,
		) as HTMLElement;
		...
		if (mouseEvent.targetNodeId && targetNode && targetNode.isConnected) {
			...
			const pageOffsetsAtHeight =
				offsetsAtPageY.get(pageOffsetsYKey) ??
				offsetsAtPageY.get(pageOffsetsYKey - offsetBlock);
			// if there's a page translation we've found that's closer than this one, use it
			if (
				pageOffsetsAtHeight &&
				Math.abs(pageOffsetsAtHeight.elementOffset) <
					Math.abs(mouseEvent.offsetY)
			) {
				...
				if (
					...
					Math.abs(offsetAtYHeightEntry.elementOffset) >
						Math.abs(mouseEvent.offsetY)
				) {
	...
	if (mouseEvent.buttons !== undefined) {
		for (let i = 0; i < 5; i += 1) {
			mouse.classList.toggle(
				`button-${i}`,
				(mouseEvent.buttons & (1 << i)) !== 0,
			);
		}
	}
}
...
function repositionInteractElements() {
	...
	if (lastHighlightNodes)
		highlightNodes({
			frameIdPath: window.selfFrameIdPath,
			nodeIds: lastHighlightNodes,
		});
