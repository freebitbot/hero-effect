...
class DomActions {
	...
	public static replayDomEvent(
		event: IFrontendDomChangeEvent,
		isReverse = false,
	): void {
		let { action } = event;
		if (action === DomActionType.newDocument)
			return DomActions.onNewDocument(event);
		if (action === DomActionType.location)
			return DomActions.onLocation(event, isReverse);
		...
		if (DomActions.isPreservedElement(event, isReverse)) return;

		let node: Node;
		let parentNode: Node;
		try {
			parentNode = DomActions.getNode(
				isReverse ? event.previousParentNodeId : event.parentNodeId,
			);
			node = DomActions.deserializeNode(
				event,
			...
			if (action === DomActionType.added)
				DomActions.onNodeAdded(node, parentNode, event, isReverse);
			if (action === DomActionType.removed)
				DomActions.onNodeRemoved(node, parentNode, event, isReverse);
			if (action === DomActionType.attribute)
				DomActions.setNodeAttributes(node as Element, event, isReverse);
			if (action === DomActionType.property)
				DomActions.setNodeProperties(node as Element, event, isReverse);
			if (action === DomActionType.text) node.textContent = event.textContent;
			if (
				node instanceof HTMLIFrameElement ||
				node instanceof HTMLFrameElement
			) {
				DomActions.onFrameModified(node, event);
			}
		} catch (error) {
	...
	public static isNavigation(action: DomActionType): boolean {
		return (
			action === DomActionType.location || action === DomActionType.newDocument
		);
	}

	private static onFrameModified(node, event) {
		for (const cb of DomActions.onFrameModifiedCallbacks) {
			cb(node, event);
		}
	}
	...
	private static isPreservedElement(
		...
		if (!tagName) {
			const existing = DomActions.getNode(nodeId);
		...
		if (event.attributes) {
			DomActions.setNodeAttributes(elem, event, isReverse);
		}
		if (event.properties) {
			DomActions.setNodeProperties(elem, event, isReverse);
		}
		return true;
	}
	...
	private static onNodeAdded(
		...
		if (!isReverse) DomActions.trackPreviousState(node, event);
		...
		const previousSiblingId = isReverse
			? event.previousPreviousSiblingId
			: event.previousSiblingId;

		if (!previousSiblingId) {
			(parentNode as Element).prepend(node);
		} else {
			const previous = DomActions.getNode(previousSiblingId);
			if (previous) {
	...
	private static trackPreviousState(
		...
		if (node.isConnected) {
			event.previousParentNodeId = node.parentNode
				? NodeTracker.getNodeId(node.parentNode)
				: null;
		...
		for (const [name, value] of Object.entries(properties)) {
			if (name === "sheet.cssRules") {
				...
				for (i = 0; i < sheet.cssRules.length; i += 1) {
					...
					if (getPrevious)
						(data.previousProperties[name] as string[]).push(currentRule);
	...
	private static deserializeNode(
		...
		let node = DomActions.getNode(data.nodeId);
		if (node) {
			if (isReverse) return node;
			DomActions.setNodeProperties(node as Element, data);
			DomActions.setNodeAttributes(node as Element, data);
