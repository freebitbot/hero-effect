...
import type { IFocusRecord } from "@ulixee/hero-core/models/FocusEventsTable";
import {
	type IMouseEventRecord,
	MouseEventType,
} from "@ulixee/hero-core/models/MouseEventsTable";
	...
	private sortTicks(): void {
		for (const tab of this.tabs) {
			...
			for (const tick of ticks) {
				...
				if (
					tick.eventType === "init" ||
					lastEvents.paintEventIndex === undefined
				) {
			...
			tab.ticks = ticks.filter((tick) => {
				tick.timelineOffsetPercent ??=
					this.timeline.getTimelineOffsetForTimestamp(tick.timestamp);
				return (
					tick.timelineOffsetPercent <= 100 && tick.timelineOffsetPercent >= 0
				);
			});
		}
	}
