...
export default class TimetravelTab extends TypedEventEmitter<{
	...
	public get focusedPaintIndexes(): [start: number, end: number] {
		if (!this.focusedTickRange) {
			return [
				this.currentTick?.paintEventIndex,
				this.currentTick?.paintEventIndex,
			];
		}
	...
	public async loadTick(
		...
		if (
			newTickOrIdx === this.currentTickIndex ||
			newTickOrIdx === this.currentTick
		) {
			return;
		}
		...
		this.currentTimelineOffsetPct =
			specificTimelineOffset ?? newTick.timelineOffsetPercent;
		...
		if (nodesToHighlight || mouseEvent || scrollEvent) {
			await mirrorPage.showInteractions(
				nodesToHighlight,
				mouseEvent,
				scrollEvent,
			);
