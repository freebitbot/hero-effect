...
export default class TimetravelPlayer extends TypedEventEmitter<{
	...
	public async setTabState(state: ITabDetails[]): Promise<void> {
		...
		for (const tabDetails of state) {
			...
			this.events.on(
				tab,
				"new-offset",
				this.onNewOffset.bind(this, tabDetails.tabId),
			);
			this.events.on(
				tab,
				"new-paint-index",
				this.onNewPaintIndex.bind(this, tabDetails.tabId),
			);
	...
	private onNewOffset(
		tabId: number,
		event: TimetravelTab["EventTypes"]["new-offset"],
	): void {
		this.emit("new-offset", { ...event, tabId });
	}

	private onNewTickCommand(
		event: TimetravelTab["EventTypes"]["new-tick-command"],
	): void {
