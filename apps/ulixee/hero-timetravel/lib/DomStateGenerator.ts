...
import type { IFrameNavigationRecord } from "@ulixee/hero-core/models/FrameNavigationsTable";
import type { IStorageChangesEntry } from "@ulixee/hero-core/models/StorageChangesTable";
...
export default class DomStateGenerator {
	...
	constructor(
		readonly id: string,
		private core: Core,
		private emulateSessionId?: string,
	) {
	...
	public async import(
		savedState: IDomStateGeneratorAssertionBatch,
	): Promise<void> {
		...
		for (const session of savedState.sessions) {
			...
			const db = await this.sessionRegistry
				.retain(sessionId)
				.catch(() => null as SessionDb);
	...
	private async doEvaluate(): Promise<void> {
		for (const session of this.sessionsById.values()) {
			...
			const domChangeRecords = db.domChanges.getChangesSinceNavigation(
				lastNavigation.id,
			);
			...
			session.domRecording.paintEvents =
				session.domRecording.paintEvents.filter((x) => x.timestamp <= end);
			...
			this.processStorageChanges(
				storage,
				sessionId,
				session.mainFrameIds.values().next().value,
			);
		}
		...
		const validSessionIds = [...this.sessionsById.values()]
			.filter((x) => x.db)
			.map((x) => x.sessionId);
	...
	private async checkResultsInPage(): Promise<void> {
		for (const session of this.sessionsById.values()) {
			...
			for (const [
				,
				assertions,
			] of this.sessionAssertions.iterateSessionAssertionsByFrameId(
				session.sessionId,
			)) {
				...
				for (let i = 0; i < xpathAsserts.length; i += 1) {
					...
					if (domResult !== xpathAssert.result) {
						...
						if (
							typeof domResult === "number" &&
							domResult !== xpathAssert.result
						) {
	...
	private async createMirrorPageIfNeeded(
		...
		const networkInterceptor = MirrorNetwork.createFromSessionDb(
			await session.db,
			session.tabId,
			{
				hasResponse: true,
		...
		session.mirrorPage = new MirrorPage(
			networkInterceptor,
			session.domRecording,
			false,
		);

		const fromSessionId = this.emulateSessionId ?? session.sessionId;
		this.browserContext ??= MirrorContext.createFromSessionDb(
			fromSessionId,
			this.core,
			false,
		)
	...
	private processDomChanges(
		...
		for (const change of changes) {
			...
			if (
				change.action === DomActionType.location ||
				change.action === DomActionType.newDocument
			) {
			...
			const isElement =
				nodeType === NodeType.Element || domNode?.nodeType === NodeType.Element;
			const isTextnode =
				nodeType === NodeType.Text || domNode?.nodeType === NodeType.Text;
			...
			if (isElement && isAttributeChange) {
				const tagPath = xpathGenerator.getTagPath(domNode, false);
				const attributeSelector = xpathGenerator.attributeSelector(
					tagPath,
					domNode,
				);
				this.recordXpath(
					sessionId,
					frameId,
					xpathGenerator.count(attributeSelector),
					1,
				);
			}
			...
			if (
				...
				if (containingElement && containingElement.children.length === 1) {
					this.recordXpath(
						sessionId,
						frameId,
						xpathGenerator.textContent(
							xpathGenerator.getTagPath(containingElement),
						),
						change.textContent,
					);
					...
					this.recordXpath(
						sessionId,
						frameId,
						xpathGenerator.countTagsWithText(
							containingElement.tagName,
							change.textContent,
						),
						1,
					);
	...
	private async findLastNavigation(
		session: IDomStateSession,
	): Promise<IFrameNavigationRecord> {
		...
		for (const nav of frameNavigations.getMostRecentTabNavigations(
			tabId,
			session.mainFrameIds,
		)) {
...
export interface IDomStateGeneratorAssertionBatch
	extends IDomStateAssertionBatch {
	sessions: {
		sessionId: string;
