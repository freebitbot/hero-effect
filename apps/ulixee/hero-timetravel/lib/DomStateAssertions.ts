...
export default class DomStateAssertions {
	...
	public sessionAssertionsCount(sessionId: string): IAssertionCounts {
		return DomStateAssertions.countAssertions(
			this.assertsBySessionId[sessionId],
		);
	}
	...
	public recordAssertion(
		...
		assertion.key ??= DomStateAssertions.generateKey(
			assertion.type,
			assertion.args,
		);
	...
	public getCommonSessionAssertions(
		...
		for (const sessionId of sessionIds) {
			...
			const sessionAssertionsByFrameId =
				this.assertsBySessionId[sessionId] ?? {};
			...
			for (const [frameId, sessionAssertions] of Object.entries(
				sessionAssertionsByFrameId,
			)) {
				for (const [key, sessionAssert] of Object.entries(sessionAssertions)) {
					...
					if (
						...
						sharedAssertion.result = Math.min(
							sharedAssertion.result,
							sessionAssert.result,
						);
	...
	public static removeAssertsSharedBetweenStates(
		states: IFrameAssertions[],
	): void {
			...
			for (const [frameId, asserts] of Object.entries(state)) {
				for (const path of Object.keys(asserts)) {
					for (const otherState of states) {
						...
						if (
							otherAsserts[path] &&
							asserts[path].result === otherAsserts[path].result
						) {
