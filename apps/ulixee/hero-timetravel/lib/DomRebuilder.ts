...
export default class DomRebuilder {
	...
	private clearStats(change: IDomChangeRecord): void {
		...
		const hasClassChange =
			change.action === DomActionType.attribute && change.attributes?.class;
		const hasIdChange =
			change.action === DomActionType.attribute && change.attributes?.id;
		...
		if (
			node.classes &&
			(change.action === DomActionType.removed || hasClassChange)
		) {
	...
	private updateStats(change: IDomChangeRecord): void {
		...
		if (
			node.classes &&
			(hasClassChange || change.action === DomActionType.added)
		) {
			for (const classname of node.classes) {
