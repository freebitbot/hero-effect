...
import type Session from "@ulixee/hero-core/lib/Session";
import type ICommandMeta from "@ulixee/hero-interfaces/ICommandMeta";
import type ICommandTimelineOffset from "@ulixee/hero-interfaces/ICommandTimelineOffset";
...
export default class CommandTimeline<T extends ICommandMeta = ICommandMeta> {
	...
	constructor(commands: T[], allNavigations: INavigation[]) {
			...
			if (
				...
				(!this.firstCompletedNavigation ||
					navigation.statusChanges.get(LoadStatus.DomContentLoaded) <
						this.firstCompletedNavigation.statusChanges.get(
		...
		for (let i = 0; i < commands.length; i += 1) {
			...
			if (
				command.clientStartDate &&
				command.clientStartDate < command.runStartDate
			) {
	...
	public getTimestampForOffset(percentOffset: number): number {
		const millis =
			Math.round(100 * this.runtimeMs * (percentOffset / 100)) / 100;
		...
		return {
			...
			commands: this.commands.map((x) => {
				return {
					...
					callsite: x.callsite.map((site) =>
						SourceMapSupport.getOriginalSourcePosition(site),
					),
				};
			}),
		};
