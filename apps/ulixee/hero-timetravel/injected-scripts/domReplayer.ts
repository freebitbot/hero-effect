...
class DomReplayer {
	...
	private pendingDelegatedEventsByChildNodeId: {
		[nodeId: number]: {
			changes: IFrontendDomChangeEvent[];
			isReverse: boolean;
		};
	} = {};

	private pendingDomChanges: IFrontendDomChangeEvent[] = [];
	private frameContentWindows = new WeakMap<
		Window,
		{ isReady: boolean; frameNodeId: number }
	>();
	...
	private sendPendingEvents(frameNodeId: number): void {
		...
		if (
			window.waitForFramesReady &&
			!this.frameContentWindows.get(frame.contentWindow)?.isReady
		) {
			return;
		}
		...
		frame.contentWindow.postMessage(
			{
				recipientFrameIdPath: `${window.selfFrameIdPath}_${frameNodeId}`,
				events,
				action: "dom",
			},
			"*",
	...
	static load() {
		...
		window.addEventListener("message", (ev) => {
			...
			if (ev.data.action === "dom") {
				replayer.applyDomChanges(
					ev.data.events.changes,
					ev.data.events.isReverse,
				);
			}
		});
