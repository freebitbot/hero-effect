...
import type HttpResponseCache from "./HttpResponseCache";
...
export default class MitmRequestContext {
	...
	public static createFromResourceRequest(
		...
		return {
			id: MitmRequestContext.nextId(),
			...resourceLoadDetails,
			requestOriginalHeaders: Object.freeze({
				...resourceLoadDetails.requestHeaders,
			}),
			didInterceptResource: !!resourceLoadDetails.browserBlockedReason,
			cacheHandler: null,
			clientToProxyRequest: null,
			stateChanges: new Map<ResourceState, Date>([
				[ResourceState.End, new Date()],
			]),
			setState() {},
		};
	}
	...
	public static create(
		...
		if (
			...
			url = new URL(
				`${expectedProtocol}//${providedHost}${clientToProxyRequest.url}`,
			);
		}
		...
		const ctx: IMitmRequestContext = {
			...
			requestHeaders,
			requestOriginalHeaders: Object.freeze(
				parseRawHeaders(clientToProxyRequest.rawHeaders),
			),
			clientToProxyRequest,
			proxyToClientResponse,
			requestTime: Date.now(),
			protocol:
				(clientToProxyRequest.socket as TLSSocket)?.alpnProtocol || "http/1.1",
			documentUrl: clientToProxyRequest.headers.origin as string,
			originType: MitmRequestContext.getOriginType(url, requestHeaders),
			didInterceptResource: false,
			cacheHandler: null,
			stateChanges: state,
			setState(stateStep: ResourceState) {
				state.set(stateStep, new Date());
				requestSession.emit("resource-state", {
					context: ctx,
					state: stateStep,
				});
			},
	...
	public static createFromHttp2Push(
		...
		ctx.cacheHandler = new CacheHandler(
			parentContext.cacheHandler.responseCache,
			ctx,
		);
		return ctx;
	}

	public static toEmittedResource(
	...
	public static assignMitmSocket(
		ctx: IMitmRequestContext,
	...
	public static readHttp1Response(
		ctx: IMitmRequestContext,
		...
		ctx.responseHeaders = HeadersHandler.cleanResponseHeaders(
			ctx,
			ctx.responseOriginalHeaders,
		);
	...
	private static nextId(): number {
		MitmRequestContext.contextIdCounter += 1;
		if (!Number.isSafeInteger(MitmRequestContext.contextIdCounter))
			MitmRequestContext.contextIdCounter = 1;
		return MitmRequestContext.contextIdCounter;
	}
}
