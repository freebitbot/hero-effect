...
export default class SocketPool {
	...
	public getSocket(
		...
		return this.queue.run(async () => {
			...
			if (
				this.pooled >= this.maxConnections &&
				(this.pending.length || this.free.size === 0)
			) {
			...
			this.events.on(
				mitmSocket,
				"close",
				this.onSocketClosed.bind(this, mitmSocket),
			);
	...
	public close(): void {
		...
		for (const pending of this.pending) {
			pending.reject(
				new CanceledPromiseError("Shutting down socket pool"),
				true,
			);
		}
