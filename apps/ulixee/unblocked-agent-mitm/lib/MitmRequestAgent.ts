...
export default class MitmRequestAgent {
	...
	private async http2Request(
		...
		ctx.setState(ResourceState.CreateProxyToServerRequest);
		const weight = (ctx.clientToProxyRequest as Http2ServerRequest).stream
			?.state?.weight;

		return client.request(ctx.requestHeaders, {
	...
	private async createHttp2Session(
		ctx: IMitmRequestContext,
		...
		const clientToProxyH2Session = (
			ctx.clientToProxyRequest as Http2ServerRequest
		).stream?.session;
		...
		this.events.on(
			...
			async (stream, headers, flags, rawHeaders) => {
				try {
					const pushPromise = new Http2PushPromiseHandler(
						ctx,
						stream,
						headers,
						flags,
