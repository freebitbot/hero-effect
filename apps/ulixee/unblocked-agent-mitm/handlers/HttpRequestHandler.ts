...
export default class HttpRequestHandler extends BaseHttpHandler {
		...
		try {
			...
			const responsePromise = new Promise<HttpServerResponse>((resolve) =>
				this.context.events.once(
					...
					"response",
					(r, flags, headers) => resolve([r, flags, headers]),
				),
			);

			clientToProxyRequest.resume();

			const socketClosedPromise =
				this.context.proxyToServerMitmSocket.closedPromise.promise;

			// now write request - make sure socket doesn't exit before writing
			const didWriteRequest = await Promise.race([
				this.writeRequest(),
				socketClosedPromise,
			]);
	...
	protected onError(kind: string, error: Error): void {
		...
		try {
			if (!proxyToClientResponse.headersSent) {
				...
				const errorText = this.context.requestSession.respondWithHttpErrorStacks
					? error.stack
					: "";
	...
	private bindErrorListeners(): void {
		const { clientToProxyRequest, proxyToClientResponse, events } =
			this.context;
		events.on(
			clientToProxyRequest,
			"error",
	...
	private async writeResponse(): Promise<void> {
		...
		if (serverToProxyResponse instanceof http.IncomingMessage) {
			context.responseTrailers = parseRawHeaders(
		...
		context.requestSession.emit(
			"response",
			MitmRequestContext.toEmittedResource(context),
		);
	}
