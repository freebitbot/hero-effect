...
export default class HeadersHandler {
	public static async determineResourceType(
		...
		const fetchDest = HeadersHandler.getRequestHeader<string>(
			ctx,
			SecFetchDest,
		);
		const fetchSite = HeadersHandler.getRequestHeader<string>(
			ctx,
			SecFetchSite,
		);
		const fetchMode = HeadersHandler.getRequestHeader<string>(
			ctx,
			SecFetchMode,
		);
		const hasUserActivity = HeadersHandler.getRequestHeader<string>(
			ctx,
			SecFetchUser,
		);
		const origin = HeadersHandler.getRequestHeader<string>(ctx, "origin");
		...
		if (fetchDest)
			ctx.resourceType = resourceTypesBySecFetchDest.get(fetchDest as string);
		...
		if (ctx.resourceType === "Websocket") {
			ctx.browserRequestId =
				await session.getWebsocketUpgradeRequestId(requestHeaders);
		}

		if (
			!ctx.resourceType ||
			ctx.resourceType === "Fetch" ||
			HeadersHandler.isWorkerDest(ctx, "shared")
		) {
	...
	public static getRequestHeader<T = string | string[]>(
		...
		const exactMatch =
			ctx.requestHeaders[name] ?? ctx.requestHeaders[lowerName];
		...
		for (const [headerName, value] of Object.entries(originalRawHeaders)) {
			...
			if (Array.isArray(value)) {
				if (singleValueHttp2Headers.has(lowerHeaderName)) {
					...
					headers[canonizedKey] = [...value].filter(
						(x) => !checkInvalidHeaderChar(x),
					);
				}
			} else {
	...
	public static checkForRedirectResponseLocation(
		context: IMitmRequestContext,
	): URL {
		if (redirectCodes.has(context.status)) {
			const redirectLocation =
				context.responseHeaders.location || context.responseHeaders.Location;
			if (redirectLocation) {
	...
	public static prepareRequestHeadersForHttp2(ctx: IMitmRequestContext): void {
		...
		let headers: IHttpHeaders = {
			...
			[HTTP2_HEADER_AUTHORITY]:
				oldHeaders[HTTP2_HEADER_AUTHORITY] ??
				HeadersHandler.getRequestHeader<string>(ctx, "host"),
		...
		for (const header of Object.keys(oldHeaders)) {
			...
			if (
				stripHttp1HeadersForH2.has(lowerKey) ||
