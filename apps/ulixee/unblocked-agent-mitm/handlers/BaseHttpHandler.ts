...
export default abstract class BaseHttpHandler {
	...
	protected constructor(
		...
		this.context = MitmRequestContext.create(
			{ ...request, isUpgrade },
			responseCache,
		);
	}
	...
	> {
		...
		try {
			...
			if (session.isClosing)
				return context.setState(ResourceState.SessionClosed);
	...
	protected bindHttp2ErrorListeners(
		...
		events.on(stream, "streamClosed", (code) => {
			if (!code) return;
			this.onError(
				`${source}.Http2StreamError`,
				new Error(`Stream Closed ${code}`),
			);
		});
