...
export default class Http2PushPromiseHandler {
	...
	constructor(
		...
		this.session.emit(
			"request",
			MitmRequestContext.toEmittedResource(this.context),
		);
	}
	...
	public async onRequest(): Promise<void> {
		...
		const serverPushStream = this.context
			.serverToProxyResponse as http2.ClientHttp2Stream;
		...
		this.onResponseHeadersPromise = new Promise<void>((resolve) => {
			...
			events.once(
				serverPushStream,
				"push",
				(responseHeaders, responseFlags, rawHeaders) => {
		...
		const clientToProxyRequest =
			parentContext.clientToProxyRequest as http2.Http2ServerRequest;
	...
	private async onClientPushPromiseCreated(
		...
		const serverToProxyPushStream = this.context
			.serverToProxyResponse as ClientHttp2Stream;
		...
		if (
			proxyToClientPushStream.destroyed ||
			serverToProxyPushStream.destroyed
		) {
			return;
		}
		...
		try {
			if (cache.shouldServeCachedData) {
				if (!proxyToClientPushStream.destroyed) {
					proxyToClientPushStream.write(cache.cacheData, (err) => {
						if (err)
							this.onHttp2PushError(
								"Http2PushProxyToClient.CacheWriteError",
								err,
							);
					});
				}
				...
				proxyToClientPushStream.respond(this.context.responseHeaders, {
					waitForTrailers: true,
				});
				events.on(proxyToClientPushStream, "wantTrailers", (): void => {
					this.context.responseTrailers = trailers;
					if (trailers)
						proxyToClientPushStream.sendTrailers(
							this.context.responseTrailers ?? {},
						);
			...
			this.onHttp2PushError(
				"Http2PushProxyToClient.UnhandledError",
				writeError,
			);
