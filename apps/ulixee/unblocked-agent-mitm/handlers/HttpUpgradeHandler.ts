...
export default class HttpUpgradeHandler extends BaseHttpHandler {
	constructor(
		request: Pick<
			IMitmRequestContext,
			"requestSession" | "isSSL" | "clientToProxyRequest"
		>,
	...
	protected onError(errorType: string, error: Error): void {
		...
		session.emit("http-error", {
			request: MitmRequestContext.toEmittedResource(context),
			error,
		});
	...
	private async onResponse(
		...
		const formattedResponse = MitmRequestContext.toEmittedResource(
			this.context,
		);
	...
	public static async onUpgrade(
		...
		const handler = new HttpUpgradeHandler(
			request,
			request.socket,
			request.head,
		);
