...
export default function findUaClientHintsPlatformVersion(
	osId: string,
): string[] {
	if (!data) {
		data = {
			windowsUniversalApiMap: JSON.parse(
				readFileSync(
					Path.join(dataDir, "manual/windowsUniversalApiMap.json"),
					"utf8",
				),
			),
	...
	if (osId.startsWith("windows")) {
		...
		for (const [release, platform] of Object.entries(
			data.windowsUniversalApiMap,
		)) {
		...
		for (const release of Object.keys(data.osReleaseDates)) {
			...
			uaClientHintsPlatformVersions.push(
