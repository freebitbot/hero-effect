...
import type IOperatingSystem from "../interfaces/IOperatingSystem";
...
export function createOsId(
	os: Pick<IOperatingSystem, "name" | "version">,
): string {
	const name = os.name.toLowerCase().replace(/\s/g, "-").replace("os-x", "os");
	const minorVersion =
		os.name.startsWith("Win") && os.version.minor === "0"
			? null
			: os.version.minor;
...
export function createOsVersion(
	...
	if (majorVersion.match(/^([a-z\s]+)/i)) {
		...
		if (osName.startsWith("Mac") || osName.startsWith("OS X")) {
			...
			winOsNameToVersionMap[majorVersion]
		) {
			[majorVersion, minorVersion] =
				winOsNameToVersionMap[majorVersion].split(".");
		}
	} else {
...
const macOsVersionToNameMap = Object.entries(macOsNameToVersionMap).reduce(
	(obj, [a, b]) => {
...
const winOsVersionToNameMap = Object.entries(winOsNameToVersionMap).reduce(
	(obj, [a, b]) => {
		return Object.assign(obj, { [b]: a });
	},
	{},
