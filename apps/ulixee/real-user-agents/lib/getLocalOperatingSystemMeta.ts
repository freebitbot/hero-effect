...
function extractWindowsVersion(release: string): string {
	...
	const minors = Object.keys(windowsVersionTree[majorMatch]).map((x) =>
		Number(x),
	);
...
function extractMacOsVersion(release: string): string {
	...
	const minors = Object.keys(darwinVersionTree[majorMatch]).map((x) =>
		Number(x),
	);
	const minorMatch = getClosestNumberMatch(Number(minor), minors);
	versionMatch += `.${minorMatch}`;
...
const darwinVersionTree = convertVersionsToTree(
	Object.keys(darwinToMacOsVersionMap),
);
const windowsVersionTree = convertVersionsToTree(
	Object.keys(windowsToWindowsVersionMap),
);
