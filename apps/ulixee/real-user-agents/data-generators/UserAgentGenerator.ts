...
export default class UserAgentGenerator {
	public static async run(data: IRealUserAgentsData): Promise<void> {
		...
		for (const value of data.userAgents) {
			...
			const [osName, osVersionMajor, osVersionMinor] = osId
				.replace("mac-os", "mac")
				.split("-");
			...
			if (name === "Chrome") {
				const chromeVersion = data.stableChromeVersions.find(
					(x) => x.id === browserId,
				);
				if (chromeVersion) {
					browserVersion = {
				...
				allPatchVersions = UserAgentGenerator.getAllPatches(
					data,
					version.major,
				);
				stablePatchVersions = UserAgentGenerator.getStablePatches(
					data,
					browserId,
					osName,
				);
				...
				if (Number(version.major) > 89) {
					uaClientHintsPlatformVersions =
						findUaClientHintsPlatformVersion(osId);
					if (
			...
			byId[userAgentId] = {
				...
				browserBaseVersion: [
					...
					browserVersion.build !== undefined
						? Number(browserVersion.build)
						: undefined,
					browserVersion.patch !== undefined
						? Number(browserVersion.patch)
						: undefined,
				],
				uaClientHintsPlatformVersions,
		...
		for (const browser of Object.values(Browsers.all())) {
			const userAgents = Object.values(byId).filter(
				(x) => x.browserId === browser.id,
			);
			...
			for (const userAgent of userAgents) {
				...
				userAgent.marketshare =
					Math.floor(browser.marketshare * percentOfOsTraffic * 100) / 100;
			}
		}
		...
		const values = Object.entries(byId).sort(([, a], [, b]) => {
			...
			for (let i = 0; i < aSort.length; i += 1) {
				...
				if (typeof aSort[i] === "number")
					return (bSort[i] as number) - (aSort[i] as number);
	...
	private static getAllPatches(
		data: IRealUserAgentsData,
		majorVersion: string,
	): number[] {
		let buildVersions = data.chromiumBuildVersions.filter((x) =>
			x.startsWith(`${majorVersion}.0`),
		);
