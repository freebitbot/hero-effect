import type { IStatcounterMarketshare } from "../data";
...
export default class OsMarketshareGenerator {
	...
	constructor(statcounter: {
		...
		const macPct = averagePercent(
			osVersions.results["OS X"].map((s) => Number(s)),
		);
		for (const [rawOsString, rawValues] of Object.entries(
			macVersions.results,
		)) {
		...
		const winPct = averagePercent(
			osVersions.results.Windows.map((s) => Number(s)),
		);
		for (const [rawOsString, rawValues] of Object.entries(
			winVersions.results,
		)) {
...
function averagePercent(counts: number[]): number {
	const avg = Math.round(
		(10 * counts.reduce((tot, vl) => tot + vl, 0)) / counts.length,
	);
	return avg / 10;
}
