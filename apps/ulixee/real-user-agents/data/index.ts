...
export default async function loadData(): Promise<IRealUserAgentsData> {
	const datafiles = Object.entries({
		...
		darwinToMacOsVersionMap: readJsonFile(
			`os-mappings/darwinToMacOsVersionMap.json`,
		),
		macOsNameToVersionMap: readJsonFile(
			`os-mappings/macOsNameToVersionMap.json`,
		),
		macOsVersionAliasMap: readJsonFile(`os-mappings/macOsVersionAliasMap.json`),
		winOsNameToVersionMap: readJsonFile(
			`os-mappings/winOsNameToVersionMap.json`,
		),
		windowsToWindowsVersionMap: readJsonFile(
			`os-mappings/windowsToWindowsVersionMap.json`,
		),
	});
