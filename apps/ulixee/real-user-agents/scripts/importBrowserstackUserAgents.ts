...
const userAgentsPath = getDataFilePath(
	"external-raw/browserstack/userAgents.json",
);
...
export default async function importBrowserstackUserAgents(): Promise<void> {
	...
	for (let i = 0; i < capabilities.length; i += 1) {
		...
		const osId = createOsId({
			name: osName,
			version: createOsVersion(osName, osVersion, null),
		});
		todoList.push({ osId, id, capability });
	}

	console.log(
		`TOTAL OF ${capabilities.length} browsers. ${todoList.length} need Processing.`,
	);
	console.log("---------------------");
...
async function getRunnerForAgent(
	...
	try {
		...
		const body = await driver.wait(
			until.elementLocated(By.css("#userAgent")),
			10e3,
		);
