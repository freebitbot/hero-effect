...
import type OperatingSystem from "./lib/OperatingSystem";
...
export default class RealUserAgents {
	...
	public static random(
		...
		const availableUserAgents = RealUserAgents.all();
		const userAgentCount = availableUserAgents.length;

		const selectedUserAgents: UserAgent[] = [];
		while (
			selectedUserAgents.length < countToGet &&
			selectedUserAgents.length < userAgentCount
		) {
			if (!availableUserAgents.length) break;
			const selectedIndex = Math.floor(
				Math.random() * availableUserAgents.length,
			);
	...
	public static popular(
		...
		const sortedUserAgents = RealUserAgents.all().sort(
	...
	public static extractMetaFromUserAgentId(
		...
		const matches = userAgentId.match(
			/^(([a-z-]+)(-([0-9-]+))?)--(([a-z-]+)-([0-9-]+))$/,
		);
