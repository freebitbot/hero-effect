...
import type { IApiSpec } from "../interfaces/IApiHandlers";
import type ICoreEventPayload from "../interfaces/ICoreEventPayload";
...
> extends TypedEventEmitter<IConnectionToCoreEvents<TEventSpec>> {
	...
	public async sendRequest<T extends keyof TCoreApiHandlers & string>(
		...
		const activeConnectHook =
			this.connectAction?.isCallingHook && this.connectAction;
		const activeDisconnectHook =
			this.disconnectAction?.isCallingHook && this.disconnectAction;
	...
	protected onMessage(
		payload:
			| ICoreResponsePayload<TCoreApiHandlers, any>
			| ICoreEventPayload<TEventSpec, any>,
	): void {
	...
	protected async onConnectionTerminated(): Promise<void> {
		...
		if (this.connectAction?.hookMessageId) {
			this.onResponse({
				...
				data: !this.connectAction.isAutomatic
					? new DisconnectedError(this.transport.host)
					: null,
			});
		}
		this.connectAction = null;
