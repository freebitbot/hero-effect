...
export default class TransportBridge {
	...
	constructor(
		...
		this.transportToClient.on("outbound", (msg) =>
			this.sendToTransport(msg, this.transportToCore),
		);
		this.transportToCore.on("outbound", (msg) =>
			this.sendToTransport(msg, this.transportToClient),
		);
	}
	...
	private async sendToTransport(
		...
		if (this.shouldSerialize) {
			message = TypeSerializer.parse(
				TypeSerializer.stringify(message),
				this.serializationMarker,
			);
		}
