...
export default class ChromeApp {
	...
	public get browsersDir(): string {
		return (
			process.env.BROWSERS_DIR ?? Path.join(this.cacheDir, "ulixee", "chrome")
		);
	}
	...
	constructor(
		...
		this.osPlatformName =
			options?.osPlatformName ?? ChromeApp.getOsPlatformName();

		if (!this.osPlatformName)
			throw new Error("This operating system is not supported");

		let overriddenFullVersion: string;
		if (
			options?.fullVersionOverridesByOs &&
			options.fullVersionOverridesByOs[this.osPlatformName]
		) {
		...
		const relativePath =
			ChromeApp.relativeChromeExecutablePathsByOs[this.osPlatformName];

		this.executablePathEnvVar =
			options?.executablePathEnvVar ??
			`CHROME_${fullVersion.split(".").shift()}_BIN`;
