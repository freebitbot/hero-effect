...
dbMethods.forEach((dbMethod) => {
	...
	const filepath = Path.resolve(
		...
		"mdn-specs",
		dbMethod.mdnFilename,
	);
	const documentExtractor = new DocumentationExtractor(filepath);
	if (!documentExtractor.foundFile) return;

	const existingSignatures = db
		.prepare(`SELECT * FROM method_signatures WHERE methodName=?`)
		.all([methodName]);

	const existingSignatureCount = new Set(
		existingSignatures.map((x) => x.signatureIndex),
	).size;
	const mdnMethodSignatures = documentExtractor.extractMethodSignatures(
		dbMethod.name.split(".").pop(),
	);
	...
	for (const mdnSignature of mdnMethodSignatures) {
		...
		for (
			let paramIndex = 0;
			paramIndex < mdnSignature.parameters.length;
			paramIndex += 1
		) {
...
console.log(
	`${accurateParametersCount} of ${methodParameterCount} WEBIDL Method Parameters ARE ACCURATE`,
);
console.log("");
...
function logDiffs(
	...
	if (!existing) {
		...
		if (
			mdnParameter.name
				.toLowerCase()
				.includes(existing.paramName.toLowerCase()) ||
			existing.paramName
				.toLowerCase()
				.includes(mdnParameter.name.toLowerCase()) ||
