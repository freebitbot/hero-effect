...
const mdnInterfacesPath = Path.resolve(
	__dirname,
	config.filesImportedDir,
	"mdn-interfaces.json",
);
if (!Fs.existsSync(mdnInterfacesPath))
	throw new Error(`mdn-interfaces.json was not found: ${mdnInterfacesPath}`);
...
for (const mdnInterface of mdnInterfaces) {
	...
	if (
		[
			...
			"DocumentFragment",
			"XPathEvaluator",
		].includes(mdnInterface.name)
	) {
		tags.push("Document");
	} else if (
		["Node", "Element", "Comment", "Text"].includes(mdnInterface.name)
	) {
		tags.push("Node");
	} else if (
		[
...
for (const inter of Object.values(interfacesByName)) {
	...
	if (existing) {
		...
		db.prepare(
			`INSERT INTO interfaces (${fields}) VALUES(${placeholders})`,
		).run(values);
	}
}
...
for (const { parent, child, type } of interfaceRelationships) {
	const existing = db
		.prepare(`SELECT * FROM interface_relationships WHERE parent=? AND child=?`)
		.get([parent, child]);
	if (existing) {
		db.prepare(
			"UPDATE interface_relationships SET type=? WHERE parent=? AND child=?",
		).run([type, parent, child]);
	} else {
