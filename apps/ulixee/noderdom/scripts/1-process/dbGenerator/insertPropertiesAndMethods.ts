...
import type { Param } from "../../../src/Types";
...
const allInterfaces = Object.values(components.allInterfacesMap).sort(
	compareName,
);
...
allInterfaces.forEach((i) => {
	...
	console.log(
		`${name} has ${fromIDL.properties.length} properties + ${fromIDL.methods.length} methods`,
	);
...
function insertProperties(
	...
	properties.forEach((property) => {
		...
		if (!existing) {
			...
			db.prepare(`UPDATE properties SET ${placeholders} WHERE name=?`).run(
				values.concat([name]),
			);
		}
	});
...
function insertMethods(interfaceName: string, methods: IIDLMethodItem[]) {
	methods.forEach((method) => {
		...
		const existing = db
			.prepare(`SELECT * FROM methods WHERE name=?`)
			.get([name]);
		if (!existing) {
			const fields = Object.keys(data).join(", ");
			const values = Object.values(data);
			const placeholders = [...values].fill("?").join(", ");
			db.prepare(`INSERT INTO methods (${fields}) VALUES(${placeholders})`).run(
				values,
			);
		} else {
			const fields = Object.keys(data);
			const values = Object.values(data);
			const placeholders = fields.map((x) => `${x}=?`).join(", ");
			db.prepare(`UPDATE methods SET ${placeholders} WHERE name=?`).run(
				values.concat([name]),
			);
		}
...
function insertSignatures(methodName: string, signatures: IIDLSignature[]) {
	signatures.forEach((signature, signatureIndex) => {
		signature.params.forEach(({ extractedType, param }, paramIndex) => {
			...
			const existing = db
				.prepare(
					`SELECT * FROM method_signatures WHERE methodName=? and signatureIndex=? and paramIndex=?`,
				)
			...
			if (!existing) {
				...
				const placeholders = [...values].fill("?").join(", ");
				db.prepare(
					`INSERT INTO method_signatures (${fields}) VALUES(${placeholders})`,
				).run(values);
			} else {
				const fields = Object.keys(data);
				const values = Object.values(data);
...
function fetchFromIDL(name: string, i: Types.Interface) {
	...
	const properties: IIDLPropertyItem[] = bodyPrinter.properties.map((p) =>
		createPropertyItem(p.name, !!p.readOnly, p),
	);
...
function injectMissingIDLData(className: string, fromIDL: IFromIDL) {
	...
	if (className === "HTMLCollection") {
		...
		const methods: IIDLMethodItem[] = bodyPrinter.methods.map((m) =>
			createMethodItem(m.name, !!m.static, m),
		);
