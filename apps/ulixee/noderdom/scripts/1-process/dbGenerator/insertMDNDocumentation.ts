...
import DocumentationExtractor, {
	type IMDNMethodItem,
	type IMDNPropertyItem,
} from "../../../src/DocumentationExtractor";
import Printer from "../../../src/Printer";
import TsBodyPrinter from "../../../src/TsBodyPrinter";
import { compareName } from "../../../src/utils";

const componentsPath = Path.join(
	config.filesProcessedDir,
	"components-standard.json",
);
...
if (process.argv[2]) {
	...
	const record = db
		.prepare(`SELECT * FROM interfaces WHERE name=?`)
		.get([name]);
...
interfaces.forEach((i) => {
	...
	const dbProperties = db
		.prepare("SELECT * FROM properties WHERE interfaceName=?")
		.all([name]);
	const dbMethods = db
		.prepare("SELECT * FROM methods WHERE interfaceName=?")
		.all([name]);

	interfaceCount += 1;
	console.log("MDN -> %s", name);
	injectMissingMDNData(name, fromMDN, dbProperties, dbMethods);
	const mdnChromePropertyNames = extractValidPropertyNames(
		fromMDN,
		dbProperties,
	);
	...
	const propertyDiffs = extractPropertyDiffs(
		name,
		mdnChromePropertyNames,
		dbPropertyNames,
	);
	const methodDiffs = extractMethodDiffs(
		name,
		mdnChromeMethodNames,
		dbMethodNames,
	);
	...
	if (propertyDiffs.length) {
		const allPropertyNames = Array.from(
			new Set([...mdnChromePropertyNames, ...dbPropertyNames]),
		).sort();
		console.log(
			`${name} Properties`.padEnd(50),
			"MDN".padEnd(25),
			"IDL".padEnd(25),
			"Chrome",
		);
		...
		allPropertyNames.forEach((n) => {
			console.log(
				n.padEnd(50),
				(mdnChromePropertyNames.includes(n)
					? "X"
					: supportedByChrome(fromMDN, n)
						? "PARTIAL"
						: ""
				).padEnd(25),
	...
	if (methodDiffs.length) {
		const allMethodNames = Array.from(
			new Set([...mdnChromeMethodNames, ...dbMethodNames]),
		).sort();
		console.log("");
		console.log(
			`${name} Methods`.padEnd(50),
			"MDN".padEnd(25),
			"IDL".padEnd(25),
			"Chrome",
		);
		...
		allMethodNames.forEach((n) => {
			console.log(
				n.padEnd(50),
				(mdnChromeMethodNames.includes(n)
					? "X"
					: supportedByChrome(fromMDN, n)
						? "PARTIAL"
						: ""
...
function insertIntoDb(interfaceName: string, fromMDN: any) {
	...
	if (existing) {
		...
		db.prepare(
			`UPDATE interfaces SET ${Object.keys(data).map((k) => `${k}=?`)} WHERE name=?`,
		).run(values);
	}
	fromMDN.properties.items.forEach((item: IMDNPropertyItem) =>
		insertPropertyIntoDb(interfaceName, item),
	);
	fromMDN.methods.items.forEach((item: IMDNPropertyItem) =>
		insertMethodIntoDb(interfaceName, item),
	);
}

function insertPropertyIntoDb(
	interfaceName: string,
	property: IMDNPropertyItem,
) {
	const name = `${interfaceName}.${property.name}`;
	const existing = db
		.prepare(`SELECT * FROM properties WHERE name=?`)
		.get([name]);
	if (existing) {
		const data = {
		...
		db.prepare(
			`UPDATE properties SET ${Object.keys(data).map((k) => `${k}=?`)} WHERE name=?`,
		).run(values);
	}
}
...
function insertMethodIntoDb(interfaceName: string, method: IMDNMethodItem) {
	...
	if (existing) {
		...
		const values = [...Object.values(data), name];
		db.prepare(
			`UPDATE methods SET ${Object.keys(data).map((k) => `${k}=?`)} WHERE name=?`,
		).run(values);
		// } else {
...
function extractPropertyDiffs(
	...
	if (className === "HTMLOptionsCollection")
		removeFromArray(diffs, "selectedIndex");
...
function extractValidPropertyNames(
	fromMDN: any,
	dbProperties: any[],
): string[] {
...
function extractValidMethodNames(fromMDN: any, dbMethods: any[]): string[] {
	const mdnChromeMethods = fromMDN.methods.items
		.filter((item: IMDNMethodItem) => {
			...
			if (!fromMDN.browserSupport[n] && item.isExperimental) return false;
			return (
				arrayHasLocalName(dbMethods, n) ||
				(supportedByChrome(fromMDN, n) && !isDeprecated(fromMDN, n))
			);
		})
		.sort(compareName)
...
function injectMissingMDNData(
	...
	if (aliases.has(className)) {
		...
		if (mdnReadonly) {
			...
			mdnReadonly.properties.items.forEach(
				(item) => (propertiesByName[item.name] = item),
			);
			dbProperties.forEach((p: any) =>
				mdnPropertyItems.push(propertiesByName[p.localName]),
			);

			const methodsByName: { [name: string]: IMDNMethodItem } = {};
			mdnReadonly.methods.items.forEach(
				(item) => (methodsByName[item.name] = item),
			);
			dbMethods.forEach((p: any) =>
				mdnMethodItems.push(methodsByName[p.localName]),
			);
		}
	}
	...
	if (className === "Document") {
		// need to remove properties defined in DocumentAndElementEventHandlers
		{
			...
			bodyPrinter.properties.forEach((p) => {
				const index = mdnPropertyItems.findIndex(
					(item: IMDNPropertyItem) => item.name === p.name,
				);
		...
		{
			const index = mdnPropertyItems.findIndex(
				(item: IMDNPropertyItem) => item.name === "onselectionchange",
			);
		...
		// MDN incorrectly shows onpointerlockchange and onpointerlockerror as properties of GlobalEventHandlers
		{
			...
			if (globalEventHandlers) {
				["onpointerlockchange", "onpointerlockerror"].forEach((name) => {
					const index = globalEventHandlers.properties.items.findIndex(
						(item) => item.name === name,
					);
					if (index < 0) return;
		...
		do {
			...
			const index = mdnPropertyItems.findIndex(
				(item: IMDNPropertyItem) => item.name === name,
			);
			if (index >= 0) break;
			mdnPropertyItems.push({
				name,
		...
		inherited.forEach((name) => {
			...
			bodyPrinter.methods.map((method) => {
				const index = mdnMethodItems.findIndex(
					(m: IMDNMethodItem) => m.name === method.name,
				);
		...
		["queryCommandIndeterm", "queryCommandValue"].forEach((name) => {
			const index = mdnMethodItems.findIndex(
				(item: IMDNMethodItem) => item.name === name,
			);
			if (index >= 0) mdnMethodItems.splice(index, 1);
		});
		...
		if (documentOrShadowRoot) {
			const index = documentOrShadowRoot.methods.items.findIndex(
				(item) => item.name === "getSelection",
			);
			if (index) {
				fromMDN.browserSupport.getSelection =
					documentOrShadowRoot.browserSupport.getSelection;
	...
	if (className === "DocumentOrShadowRoot") {
		...
		const index = mdnPropertyItems.findIndex(
			(item: IMDNPropertyItem) => item.name === "styleSheets",
		);
		if (index >= 0) mdnPropertyItems.splice(index, 1);
	}
	...
	if (className === "File") {
		...
		{
			const index = mdnPropertyItems.findIndex(
				(item: IMDNPropertyItem) => item.name === "webkitRelativePath",
			);
			if (index >= 0) mdnPropertyItems.splice(index, 1);
		}
		...
		{
			const index = mdnPropertyItems.findIndex(
				(item: IMDNPropertyItem) => item.name === "type",
			);
			if (index >= 0) mdnPropertyItems.splice(index, 1);
		}
	}
	...
	if (className === "FileList") {
		...
		const index = fromMDN.methods.items.findIndex(
			(m: any) => m.name === "item",
		);
		if (index < 0) {
	...
	if (className === "HTMLAnchorElement") {
		...
		{
			const index = mdnPropertyItems.findIndex(
				(item: IMDNPropertyItem) => item.name === "media",
			);
			if (index >= 0) mdnPropertyItems.splice(index, 1);
		}
	}
	...
	if (className === "HTMLButtonElement") {
		...
		{
			const index = mdnPropertyItems.findIndex(
				(item: IMDNPropertyItem) => item.name === "accessKey",
			);
			if (index >= 0) mdnPropertyItems.splice(index, 1);
		}
		...
		{
			const index = mdnPropertyItems.findIndex(
				(item: IMDNPropertyItem) => item.name === "tabIndex",
			);
			if (index >= 0) mdnPropertyItems.splice(index, 1);
		}
	}
	...
	if (className === "HTMLElement") {
		...
		mixins.forEach((name) => {
			...
			bodyPrinter.properties.map((prop) => {
				const index = mdnPropertyItems.findIndex(
					(p: IMDNPropertyItem) => p.name === prop.name,
				);
				if (index >= 0) mdnPropertyItems.splice(index, 1);
			});
			bodyPrinter.methods.map((meth) => {
				const index = mdnMethodItems.findIndex(
					(m: IMDNMethodItem) => m.name === meth.name,
				);
				if (index >= 0) mdnMethodItems.splice(index, 1);
			});
		});
		...
		const index = mdnPropertyItems.findIndex(
			(item: IMDNPropertyItem) => item.name === "noModule",
		);
		if (index >= 0) mdnPropertyItems.splice(index, 1);
	}
	...
	if (className === "HTMLFormElement") {
		const index = mdnPropertyItems.findIndex(
			(item: IMDNPropertyItem) => item.name === "encoding",
		);
		const mdnDocumentationPath = "/en-US/docs/Web/API/HTMLFormElement/enctype";
		const enctype = Object.assign({}, mdnPropertyItems[index], {
			name: "enctype",
			mdnDocumentationPath,
		});
	...
	if (className === "HTMLStyleElement") {
		...
		const index = mdnPropertyItems.findIndex(
			(item: IMDNPropertyItem) => item.name === "disabled",
		);
		if (index >= 0) mdnPropertyItems.splice(index, 1);
	}
	...
	if (className === "HTMLTextAreaElement") {
		...
		const index = mdnPropertyItems.findIndex(
			(item: IMDNPropertyItem) => item.name === "autocapitalize",
		);
		if (index >= 0) mdnPropertyItems.splice(index, 1);
	}
	...
	if (className === "Text") {
		...
		mdnPropertyItems.splice(
			...
			...mdnPropertyItems.filter(
				(item: IMDNPropertyItem) => item.name !== "assignedSlot",
			),
		);
	}
}
