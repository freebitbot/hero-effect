...
const webIDLExtensionsDir = Path.join(
	config.filesImportedDir,
	"webidl-extensions",
);
const webIDLReplacementsDir = Path.join(
	config.filesImportedDir,
	"webidl-replacements",
);
const componentFiltersPath = Path.join(
	config.filesImportedDir,
	"component-filters-awaited.json",
);

function run() {
	const components = createComponents();
	const superInterfaces = Object.values(
		SuperGenerator.injectIntoComponents(components).awaitedSupers,
	);
...
function resolveConflicts(
	superInterfaces: Types.Interface[],
	...
	superInterfaces.forEach((superInterface) => {
		...
		for (const name of superInterface.implements!) {
			const i =
				components.awaitedIsolates[name] ||
			...
			const tsBodyPrinter = new TsBodyPrinter(
				i,
				printer,
				components,
				bodyPrinterOptions,
			...
			const records = Object.values(
				i[type === "property" ? "properties" : "methods"] || {},
			);
			...
			for (const record of records) {
				...
				definitionsByName[record.name].push({
					klass: name,
					name: record.name,
					value: value,
				});
				if (Object.keys(definitionsByName[record.name]).length > 1)
		...
		console.log(
			`${type.toUpperCase()} CONFLICTS WITHIN ${superInterface.name}:`,
		);
		for (const definitionName of Object.keys(definitionsByName)) {
			...
			definitions.forEach((definition, index) => {
				const isolateInterface =
					components.awaitedIsolates[definition.klass] ||
					components.mixins[definition.klass];
				if (type === "property") {
					const properties = isolateInterface.properties!;
					// already processed
					if (properties[definitionName].compromiseType) return;

					const cleanType = TypeUtils.convertDomTypeToTsType(
						properties[definitionName],
					);
					properties[definitionName] = Object.assign(
						{},
						properties[definitionName],
						overrides,
					);
					...
					if (property.compromiseType) {
						property.compromiseType = property.compromiseType.map((entry) => {
							const cleanCompromiseType =
								TypeUtils.convertDomTypeToTsType(entry);
							const isOfficial =
								cleanCompromiseType === cleanType ? 1 : undefined;
							return {
								...entry,
								isOfficial,
							};
					...
					isolateInterface.properties = properties as Record<
						string,
						Types.Property
					>;
				} else {
					const methods = isolateInterface.methods!;
					const method = (methods[definitionName] = Object.assign(
						{},
						methods[definitionName],
					));
					method.signatures = method.signatures.map((s, idx) =>
						Object.assign({}, s, overrides[idx]),
					);
					isolateInterface.methods = methods as Record<string, Types.Method>;
				}
				console.log(
					`${index ? "" : definitionName}`.padEnd(25),
					definition!.klass.padEnd(30),
					definition!.value,
				);
			});
			...
			const relatedIsolate =
				components.awaitedIsolates[klass] || components.mixins[klass];
			const bodyPrinterOptions = {
				domType: DomType.standard,
				skipImplementation: true,
			};
...
function createComponents() {
	...
	loadWebidls(
		componentizer,
		webIDLExtensionsDir,
		Fs.readdirSync(webIDLExtensionsDir),
	);
	loadWebidls(
		componentizer,
		webIDLReplacementsDir,
		Fs.readdirSync(webIDLReplacementsDir),
	);
...
function createPropertyOverrides(
	components: Components,
	propDefinitions: IDefinition[],
): any {
	const overrides: {
		readOnly?: 1;
		nullable?: 1;
...
function createMethodOverrides(
	components: Components,
	methDefinitions: IDefinition[],
): any {
	...
	typesByCleanType.forEach((typeByCleanType, index) => {
		const types = Object.values(typeByCleanType);
		if (types.length > 1)
			overrides[index].type = types.map((t) => ({ type: t }));
	});
