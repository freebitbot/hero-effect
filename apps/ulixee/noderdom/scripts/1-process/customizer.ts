...
server.listen({ port: process.env.PORT ?? 3000 }, () => {
	console.log(
		"Noderdom Selector started at localhost:" +
	...
	server.on("request", async (req, res) => {
		...
		if (req.url.startsWith("/js") || req.url.startsWith("/css")) {
			res.setHeader(
				"Content-Type",
				req.url.startsWith("/js") ? "application/javascript" : "text/css",
			);
			return Fs.createReadStream(Path.join(__dirname, "site", req.url)).pipe(
				res,
			);
		}
		res.writeHead(404);
