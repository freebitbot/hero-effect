import db from "../../../db";
import type IChoiceMeta from "./IChoiceMeta";
import { ChoiceItemType, type IChoiceItemType } from "./IChoiceMeta";

export default class Customizer {
	public readonly klasses: any[];
	public readonly choicesMetaMap: {
		[domType: string]: { [name: string]: IChoiceMeta };
	} = {
		detached: {},
		awaited: {},
	};
	private allInterfaces = db
		.prepare("SELECT * FROM interfaces WHERE hasDefinedIDL=1")
		.all();
	private allComponentFilters = db
		.prepare("SELECT * FROM component_filters")
		.all();
	...
	constructor() {
		this.allComponentFilters.forEach((componentFilter: any) => {
			...
			const choicesByName =
				this.choicesMetaMap[componentFilter.domType as string];
			const { isCore, isEnabled, isHidden, isWritable, isAbstract } =
				componentFilter;
			const options = {
				isCore: isCore === 1,
			...
			choicesByName[name] = this.createChoiceMeta(
				componentFilter.name,
				componentFilter.itemType,
				options,
			);
		});
	...
	private getSuper(name: string) {
		...
		this.supersByName[name] =
			this.supersByName[name] || db.prepare(sql).get([name]);
	...
	private isSuperDescendent(itemType: IChoiceItemType, name: string) {
		if (itemType !== ChoiceItemType.interface) return false;
		return !!db
			.prepare("SELECT * FROM super_isolates WHERE interfaceName=?")
			.get([name]);
	}
	...
	private createChoiceMeta(
		...
		else if (itemType === ChoiceItemType.property)
			meta.isWritable = options.isWritable || false;
		...
		if (
			itemType === ChoiceItemType.property ||
			itemType === ChoiceItemType.method
		) {
	...
	private ensureChoiceMeta(name: string, itemType: IChoiceItemType) {
		this.choicesMetaMap.detached[name] =
			this.choicesMetaMap.detached[name] ||
			this.createChoiceMeta(name, itemType);
		this.choicesMetaMap.awaited[name] =
			this.choicesMetaMap.awaited[name] ||
			this.createChoiceMeta(name, itemType);
	}
	...
	private createClasses() {
		return this.allInterfaces.map((inter) => {
			...
			properties.forEach((property) => {
				...
				Object.assign(this.choicesMetaMap.awaited[property.name], {
					itemType: "property",
				});
				Object.assign(this.choicesMetaMap.detached[property.name], {
					itemType: "property",
				});
				property.nativeReturnTypes = (property.nativeReturnTypes || "")
					.split(",")
					.filter((t: string) => t);
				property.customReturnTypes = (property.customReturnTypes || "")
					.split(",")
					.filter((t: string) => t);
				property.returnTypes = property.nativeReturnTypes.concat(
					property.customReturnTypes,
				);
			...
			methods.forEach((method) => {
				...
				Object.assign(this.choicesMetaMap.awaited[method.name], {
					itemType: "method",
				});
				Object.assign(this.choicesMetaMap.detached[method.name], {
					itemType: "method",
				});
				method.nativeArgTypes = (method.nativeArgTypes || "")
					.split(",")
					.filter((t: string) => t);
				method.customArgTypes = (method.customArgTypes || "")
					.split(",")
					.filter((t: string) => t);
				method.argTypes = method.nativeArgTypes.concat(method.customArgTypes);

				method.nativeReturnTypes = (method.nativeReturnTypes || "")
					.split(",")
					.filter((t: string) => t);
				method.customReturnTypes = (method.customReturnTypes || "")
					.split(",")
					.filter((t: string) => t);
				method.returnTypes = method.nativeReturnTypes.concat(
					method.customReturnTypes,
				);

				method.nativeTypes = method.nativeArgTypes.concat(
					method.nativeReturnTypes,
				);
				method.customTypes = method.customArgTypes.concat(
					method.customReturnTypes,
				);
	...
	private fetchDependencies(name: string) {
		const records = db
			.prepare(`SELECT parent FROM interface_relationships WHERE child=?`)
			.all([name]);
	...
	private fetchDependents(name: string) {
		const records = db
			.prepare(`SELECT child FROM interface_relationships WHERE parent=?`)
			.all([name]);
