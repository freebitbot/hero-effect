...
async function run() {
	{
		...
		await extract(urlPath, document, "DOMCore", "#Obsolete_DOM_interfaces", {
			isObsolete: true,
		});
	...
	}
	{
		...
		await extract(
			...
			document,
			"BrowserWindow",
			"#Web_app_and_browser_integration_interfaces",
		);
		await extract(
			urlPath,
			document,
			"CanvasAndImage",
			"#Canvas_and_image_interfaces",
		);
		...
		await extract(
			...
			document,
			"Miscellaneous",
			"#Miscellaneous_and_supporting_interfaces",
		);
		...
		await extract(
			...
			document,
			"ExternalWebStorage",
			"#Web_storage_interfaces",
		);
		await extract(
			urlPath,
			document,
			"ExternalWebWorkers",
			"#Web_Workers_interfaces",
		);
		await extract(
			urlPath,
			document,
		...
		await extract(
			urlPath,
			document,
			"ExternalServerSentEvents",
			"#Server-sent_events_interfaces",
		);
	}
	{
...
async function extract(
	...
	for (const anchor of elem.querySelectorAll("li a")) {
		...
		const isDeprecated = extractBoolean(
			flags.isDeprecated,
			parentElement.querySelector("i.icon-thumbs-down-alt"),
		);
		const isExperimental = extractBoolean(
			flags.isExperimental,
			parentElement.querySelector("i.icon-beaker"),
		);
		const isObsolete = extractBoolean(
			flags.isObsolete,
			parentElement.querySelector("i.icon-trash"),
		);
		const isDocumented = extractBoolean(
			urlPath ? await loadAndSaveHTML(urlPath) : false,
		);
		const filename = extractFilename(urlPath);
...
async function extractCherryPickedInterfaces(interfaceNames: string[]) {
	...
	for (const name of interfaceNames) {
		...
		const urlPath = (anchor as IElement).getAttribute("href") || ""; // n.href;
		const isDeprecated = extractBoolean(
			elem.querySelector("i.icon-thumbs-down-alt"),
		);
