...
import type { IElement } from "noderdom-detached/base/interfaces";
...
async function run() {
	...
	for (const filename of mdnDocs) {
		...
		const mdnInterface = mdnInterfaces.find(
		...
		for (const anchor of document.querySelectorAll(".sidebar a")) {
			...
			if (
				!urlPath.includes(mdnInterface.urlPath) ||
				mdnFilesToCheck.has(extractedFilename)
			) {
				continue;
			}
