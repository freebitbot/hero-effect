/* ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 * This script creates inputs/elementsMeta.json, which provides meta data on element classes. This includes details
 * such as their related tag names (li, div, etc), their namespace (SVG, HTML, etc), as well as other properties.
 */
...
import type { IElementMeta } from "../../src/ElementsMeta";

const outputFilepath = Path.join(config.filesImportedDir, "elementsMeta.json");
const data: Record<string, any> = {};
const sources = {
	preprocessedTypes:
		"https://raw.githubusercontent.com/microsoft/TSJS-lib-generator/master/inputfiles/browser.webidl.preprocessed.json",
	addedTypes:
		"https://raw.githubusercontent.com/microsoft/TSJS-lib-generator/master/inputfiles/addedTypes.json",
	overridingTypes:
...
async function run() {
	...
	for (const typeName of typeNames.values()) {
		...
		const preprocessedElements = extractElements(
			data.preprocessedTypes.interfaces.interface[typeName],
		);
		const addedElements = extractElements(
			data.addedTypes.interfaces.interface[typeName],
		);
		const overriddenElements = extractElements(
			data.overridingTypes.interfaces.interface[typeName],
		);
		if (
			!preprocessedElements.length &&
		...
		const tagWithNamespace = tags.find(
			(t: { namespace: string }) => t.namespace,
		);
		const namespace = tagWithNamespace
			? tagWithNamespace.namespace
			: extractNamespaceFromTypeName(typeName);

		tags.forEach((tag: any) => {
			if (tag["html-self-closing"]) tag.selfClosing = tag["html-self-closing"];
			if (tag.namespace && tag.namespace !== namespace) {
				throw new Error(
					`namespaces do not match: ${namespace} vs ${tag.namespace}`,
				);
			}
			delete tag.specs;
