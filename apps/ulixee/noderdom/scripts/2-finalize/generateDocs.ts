...
import type IComponentFilters from "../../src/interfaces/IComponentFilters";
import type IDomType from "../../src/interfaces/IDomType";
import { DomType } from "../../src/interfaces/IDomType";

const componentFilteringPath = Path.join(
...
if (!Fs.existsSync(componentsPath))
	throw new Error(`components-standard.json was not found: ${componentsPath}`);

const componentFilters: IComponentFilters = JSON.parse(
	Fs.readFileSync(componentFilteringPath, "utf-8"),
);
...
const allInterfaces = db
	.prepare("SELECT * FROM interfaces WHERE hasDefinedIDL=1 ORDER BY name")
	.all();
const allSupers = db.prepare("SELECT * FROM supers ORDER BY name").all();
const docsPath = Path.join(
	config.filesFinalizedDir,
	`${domType}-dom`,
	"docs.json",
);
...
filteredInterfaces.map((inter) => {
	...
	const inheritedKlassNames = inheritsFrom.klassNames.concat(
		inheritsFrom.mixinNames,
	);
	...
	for (const klassName of inheritedKlassNames) {
		...
		inheritedProps.forEach((p) =>
			properties.push(convertToProperty(klassName, p)),
		);
		...
		inheritedMethods.forEach((m) =>
			methods.push(convertToMethod(klassName, m)),
		);
	}
	...
	isolates.forEach((d) => {
		...
		if (!docsByName[d.interfaceName]) {
			console.log(
				"MISSING DEPENDENCY: ",
				d.interfaceName,
				allInheritedKlassNames.has(d.interfaceName),
			);
		}
		dependencies.push({
...
function extractVariableName(name: string, tags: string[]) {
	if (
		name === "Element" ||
		tags.includes("HTMLElement") ||
		tags.includes("SVGElement")
	) {
		return "elem";
	}
...
function convertToProperty(componentName: string, property: any) {
	...
	if (!componentFilter)
		throw new Error(`MISSING COMPONENT FILTER: ${componentName}`);

	const isAbstract =
		!!componentFilter.propertiesByName[property.localName]?.isAbstract;
	const returnType = convertReturnType(
		property.customReturnTypes || property.nativeReturnTypes,
		isAbstract,
	);
	...
	return {
		...
		returnType,
		isImplemented:
			!!componentFilter.propertiesByName[property.localName]?.isEnabled,
	};
}

function convertReturnType(returnType: string, isAbstract: boolean) {
	if (returnType.startsWith("Promise,"))
		returnType = `Promise<${returnType.split(",").pop()}>`;
...
function convertToMethod(componentName: string, method: any) {
	...
	const isAbstract =
		!!componentFilter.methodsByName[method.localName]?.isAbstract;

	return {
		name: method.localName,
		overview: method.docOverview,
		componentName: componentName,
		returnType: convertReturnType(
			method.customReturnTypes || method.nativeReturnTypes,
			isAbstract,
		),
