...
async function run() {
	...
	typescriptExtractor
		.extractBasicInterfaces()
		.forEach((m) => (basicTypes[m.name] = m));

	const interfaceClasses: { [name: string]: ICodeModule } = {};
	typescriptExtractor
		.extractOfficialInterfaces()
		.forEach((m) => (interfaceClasses[m.name] = m));

	const visited: ICodeModulesByName = {};
	const unvisited: ICodeModulesByName = Object.assign(
		{},
		basicTypes,
		interfaceClasses,
	);
...
function visit(
	...
	visited: ICodeModulesByName,
	unvisited: ICodeModulesByName,
) {
	for (const defined of codeModule.definedObjects) {
