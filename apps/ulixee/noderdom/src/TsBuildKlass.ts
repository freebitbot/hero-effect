...
import type { IObjectMetaByName } from "./TsBuilder";
...
export default class TsBuildKlass {
	...
	constructor(i: Types.Interface, components: Components, options: IOptions) {
		...
		const inheritsFrom = [i.extends || "Object"]
		...
		this.bodyPrinter = new TsBodyPrinter(
			i,
			this.printer,
			components,
			bodyOptions,
		...
		if (this.bodyPrinter.iteratorExtractor.hasIterable())
			this.i.isAwaitedNodePointer = true;
	}
	...
	private printClassGeneratorFunction() {
		...
		this.printer.printLine(
			`export function ${i.name}Generator(${args.join(", ")}) {`,
		);
		this.printer.increaseIndent();
	}
	...
	private printBaseClassDeclaration() {
		...
		const typeParameterName =
			i.typeParameters && i.typeParameters[0] && i.typeParameters[0].name;
		const iClassName = typeParameterName
			? `${toIType(i.name)}<${typeParameterName}>`
			: toIType(i.name);
		...
		this.printer.printLine(
			`return class ${className} ${extendsStr}implements ${implementsList.join(", ")} {`,
		);
	}
	...
	private printImplClassDeclaration() {
		...
		const iClassName = typeParameterName
			...
			: toIType(i.name);
		this.printer.printLine(
			`export default class ${className} extends ${i.name}BaseClass implements ${iClassName} {`,
		);
	}
	...
	private printStateMachineInterfaces() {
		...
		stateMachinePrinter.printInterfaces(
			this.inheritsFrom,
			this.bodyPrinter.constants,
			this.bodyPrinter.properties,
		);
	}
	...
	private prependInitializerCode() {
		...
		if (this.buildType === BuildType.base) {
			...
			printable.push(
				`export const ${handlerName} = new ${handlerClassName}<I${name}>('${name}', getState, setState);`,
			);

			if (this.i.isAwaitedNodePointer) {
				printable.push(
					`export const nodeFactory = new NodeFactory<I${name}>(getState, setState, ${handlerName});`,
				);
			}
			if (this.bodyPrinter.iteratorExtractor.hasIterable()) {
				printable.push(
					this.bodyPrinter.iteratorExtractor.getIteratorInitializer(
						handlerName,
					),
				);
			}
			...
			printable.push(
				`const ${i.name}BaseClass = ${i.name}Generator(${this.inheritsFrom.join(", ")});`,
			);
		}
	...
	private importsForBase() {
		const i: Types.Interface = this.i;
		const baseDir =
			Path.relative(this.currentDir, this.pathsByBuildType.base.root) || ".";
		const printable: string[] = [];

		const isAwaited = this.domType === DomType.awaited;
		const handlerClassName = `${isAwaited ? "Awaited" : "Detached"}Handler`;
		printable.push(
			`import ${handlerClassName} from '${baseDir}/${handlerClassName}';`,
		);
		printable.push(
			`import inspectInstanceProperties from '${baseDir}/inspectInstanceProperties';`,
		);
		...
		if (this.bodyPrinter.iteratorExtractor.hasIterable()) {
			printable.push(
				`import AwaitedIterator from '${baseDir}/AwaitedIterator';`,
			);
		}
		if (this.i.isAwaitedNodePointer) {
		...
		const references: string[] = [
			i.name,
			...this.inheritsFrom,
			...this.bodyPrinter.referencedObjects,
		];
		printable.push(
			tsImporter.extractAll(references, BuildType.base, ObjectStruct.interface),
		);
		...
		this.inheritsFrom.forEach((name) => {
			...
			const importCode = tsImporter.extractSingle(
				name,
				null,
				objectsToImport,
				BuildType.base,
	...
	private importsForImpl() {
		...
		const baseDir =
			Path.relative(this.currentDir, this.pathsByBuildType.base.root) || ".";
		const implDir =
			Path.relative(this.currentDir, this.pathsByBuildType.impl.root) || ".";
		const printable: string[] = [];
		...
		const referencedImports = tsImporter.extractAll(
			references,
			BuildType.base,
		...
		if (this.bodyPrinter.referencedCreateMethods.size) {
			...
			printable.push(
				`import { ${names.map((n) => `create${n}`).join(", ")} } from '${implDir}/create';`,
			);
		}
