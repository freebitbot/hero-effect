...
import type ICodeModule from "../src/interfaces/ICodeModule";
...
export default class DependencyCollector {
	...
	constructor(components: Components, domType: IDomType) {
		...
		const tsBuilder = new TsBuilder(components, {
			domType,
			buildType: BuildType.base,
		});
		const codeModulesByName: ICodeModulesByName = {};
		tsBuilder
			.extractOfficialInterfaces()
			.forEach((m) => (codeModulesByName[m.name] = m));
	...
	public get(
		name: string,
		collectUpstream: boolean = true,
		collectDownstream: boolean = true,
	) {
	...
	private collectUpstreamClasses(
		name: string,
		classCollection: IClassCollection,
		visits: IVisits,
	) {
	...
	private collectDownstreamClasses(
		name: string,
		classCollection: IClassCollection,
		visits: IVisits,
	) {
	...
	private organizeDependencies(
		...
		if (inter.extends && inter.extends !== "Object")
			dependencies.push(inter.extends);
