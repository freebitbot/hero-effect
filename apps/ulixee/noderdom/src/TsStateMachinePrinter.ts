...
import type TsIteratorExtractor from "./TsIteratorExtractor";
...
export default class TsStateMachinePrinter {
	...
	public printInitializer() {
		...
		this.printer.print(
			`export const { ${vars.join(", ")} } = StateMachine<I${i.name}, I${i.name}Properties>();`,
		);
	}

	public printInterfaces(
		classNames: string[],
	...
	private printPropertyKeys(
		...
		this.printer.printLine("");
		this.printer.printLine(
			`export const ${i.name}PropertyKeys = [${propertyKeys.join(", ")}];`,
		);
	}
	...
	private printConstantKeys(classNames: string[], constants: Types.Constant[]) {
		...
		this.printer.printLine("");
		this.printer.printLine(
			`export const ${i.name}ConstantKeys = [${constantKeys.join(", ")}];`,
		);
	}

	// privates

	private printPropertiesInterface(
		classNames: string[],
		properties: Types.Property[],
	) {
		const i: Types.Interface = this.i;
		const extendsStr = classNames.length
			? ` extends ${classNames.map((x) => `I${x}Properties`).join(", ")}`
			: "";
