		...
		if (Object.getOwnPropertyDescriptor(src, k)) {
			if (Object.getOwnPropertyDescriptor(target, k)) {
				...
				if (Array.isArray(targetProp) && Array.isArray(srcProp)) {
					...
					if (
						shallow &&
						typeof (target[k] as any).name === "string" &&
						typeof (src[k] as any).name === "string"
					) {
						target[k] = src[k];
					} else {
...
function mergeNamedArrays<T extends { name: string; newType: string }>(
	targetProp: T[],
	srcProp: T[],
) {
	const mapped: any = {};
...
export function followTypeReferences(
	webidl: Types.WebIdl,
	filteredInterfaces: Record<string, Types.Interface>,
) {
...
export function compareName(c1: { name: string }, c2: { name: string }) {
	...
	if (!c1.name.match(eventListenerRegexp) && c2.name.match(eventListenerRegexp))
		return -1;
	return c1.name < c2.name ? -1 : c1.name > c2.name ? 1 : 0;
}

function typeParameterWithDefault(
	type: Types.TypeParameter,
	convertToIType: boolean = false,
) {
	const extendsStr = type.extends
		? ` extends ${convertToIType ? toIType(type.extends) : type.extends}`
		: "";
	const defaultStr = type.default
		? ` = ${convertToIType ? toIType(type.default) : type.default}`
		: "";
...
export function toIType(type: string) {
	...
	return type[0] === "I" && type[1] === type[1].toUpperCase()
		? type
		: `I${type}`;
}
...
export function nameWithForwardedTypes(
	i: Types.Interface,
	convertToIType: boolean = false,
) {
