...
export default class DocumentationExtractor {
	...
	public extractInheritance() {
		...
		const sidebarDetails = document.querySelectorAll(
			".sidebar .quick-links ol details",
		);
	...
	public extractProperties(className: string) {
		const properties = extractItemsFrom(
			this.document,
			'h2[id^="Properties"] + *',
			className,
		);

		const attributes = extractItemsFrom(
			this.document,
			'h2[id^="Attributes"] + *',
			className,
		);
		...
		if (className === "Document") {
			const extensions = extractItemsFrom(
				this.document,
				'h3[id="Extensions_for_HTMLDocument"] + *',
				className,
			);
			properties.items.push(...extensions.items);
		}

		const eventHandlers1 = extractItemsFrom(
			this.document,
			'h2[id="Event_handlers"] + *',
			className,
		);
		properties.items.push(...eventHandlers1.items);

		const eventHandlers2 = extractItemsFrom(
			this.document,
			'h3[id="Event_handlers"] + *',
			className,
		);
		properties.items.push(...eventHandlers2.items);

		const eventHandlers3 = extractItemsFrom(
			this.document,
			'h3[id="Handlers"] + *',
			className,
		);
	...
	public extractMethods(className: string) {
		...
		const methods = extractItemsFrom(
			this.document,
			`h2[id^="${id}"] + *`,
			className,
			true,
		...
		if (className === "Document") {
			const extensions = extractItemsFrom(
				this.document,
				'h3[id^="Extension_for_HTML_documents"] + *',
				className,
				true,
			);
		...
		const staticMethods = extractItemsFrom(
			this.document,
			'h2[id^="Static_methods"] + *',
			className,
			true,
		);
	...
	public extractMethodSignatures(methodName: string) {
		...
		while (elem && !["h2", "h3"].includes(elem.localName)) {
			...
			if (elem.localName === "dl") {
				...
				(elem.children[0].localName === "code" ||
					elem.children[0].localName === "var")
			) {
				if (!isAdded) signatures.push({ parameters });
				isAdded = true;
	...
	public extractBrowserSupport() {
		...
		const browserSupport: {
			[name: string]: { chrome: boolean; isDeprecated: boolean };
		} = {};

		const tbodyElem: IElement | null = document.querySelector(
			"table.bc-table-web tbody",
		);
		...
		for (const trElem of tbodyElem.children) {
			...
			const chromeSupports =
				chromeElem && chromeElem.classList.contains("bc-supports-yes");
			if (name)
				browserSupport[name] = { chrome: !!chromeSupports, isDeprecated };
		}
...
function extractParameter(
	...
	const paramName = elem.children.length
	...
	if (!isParameterInSyntax(syntax, methodName, paramName, elem.textContent))
		return null;
...
function extractItemsFrom(
	...
	while (elem && !["h2", "h3"].includes(elem.localName)) {
		if (elem.localName === "p") {
			...
			elem.localName === "dl" ||
			(elem.children[0] && elem.children[0].localName === "dl")
		) {
...
function extractItemsFromDL(
	...
	for (const dt of dts) {
		...
		if (aElem) {
			...
			const textNodes = Array.from(dtElem.childNodes).filter(
				(n) => n.nodeType === 3,
			);
...
function extractItemsFromTable(
	...
	if (!thElems.length)
		thElems = tableElem.querySelectorAll("thead > tr > td.header");
	if (!thElems.length)
		thElems = tableElem.querySelectorAll("tbody > tr > td.header");
	...
	if (
		!headers.length &&
		tableElem.querySelector("tbody > tr")!.children.length === 2
	) {
