...
export default class TsElementExtractor {
	...
	public extract() {
		...
		const response = [
			this.printElementTagNameMap(
				"IHTMLElementTagNameMap",
				tagNameToElementName.htmlResult.active,
			),
			this.printElementTagNameMap(
				"IHTMLElementDeprecatedTagNameMap",
				tagNameToElementName.htmlResult.deprecated,
			),
			this.printElementTagNameMap(
				"ISVGElementTagNameMap",
				tagNameToElementName.svgResult.active,
			),
			this.printElementTagNameMap(
				"ISVGElementDeprecatedTagNameMap",
				tagNameToElementName.svgResult.deprecated,
			),
		];
		return response;
	}
	...
	public extractImports() {
		...
		this.printer.reset();
		this.printer.print(
			`import { ${Array.from(this.domTypes).join(", ")} } from './${ObjectType.official}';`,
		);
		const results = this.printer.getResult();
		return results;
	}

	private printElementTagNameMap(
		name: string,
