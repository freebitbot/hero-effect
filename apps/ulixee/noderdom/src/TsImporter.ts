...
import type { IObjectMeta, IObjectMetaByName } from "./TsBuilder";
...
export default class TsImporter {
	...
	public extractSingle(
		...
		const { path } = this.extractPathAndFilename(
			buildType,
			objectStruct,
			objectMeta,
			filename,
	...
	public extractAll(
		...
		references.forEach((name) => {
			...
			if (!objectMeta.pathTypeByObjectStruct.has(objectStruct)) {
				throw new Error(
					`${name} does not have requested ObjectStruct: ${objectStruct}`,
				);
			}

			const { path, filename } = this.extractPathAndFilename(
				buildType,
				objectStruct,
				objectMeta,
				name,
			...
			const objectName =
				objectStruct === ObjectStruct.interface ? `I${name}` : name;
	...
	private extractPathAndFilename(
		...
		if (
			...
			objectStruct === ObjectStruct.interface
		) {
			path = `${this.pathsByBuildType.base.interfaces}`;
			filename = objectMeta.objectType;
		} else if (
			objectStruct !== ObjectStruct.interface &&
			objectMeta.pathTypeByObjectStruct.has(objectStruct)
		) {
			const pathType = objectMeta.pathTypeByObjectStruct.get(
				objectStruct,
			) as Omit<IPathType, PathType.interfaces>;
			path = (paths as any)[pathType as string];
		} else {
			throw new Error(
				`Could not extract path for ${name}: ${JSON.stringify({ buildType, objectType, objectStruct })}`,
			);
		}
