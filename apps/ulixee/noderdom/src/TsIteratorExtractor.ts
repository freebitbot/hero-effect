...
import { arrayToMap, compareName, integerTypes, toIType } from "./utils";
...
export default class TsIteratorExtractor {
	...
	constructor(i: Types.Interface, components: Components, options: IOptions) {
		...
		const sequenceTypedefs = (this.components.typedefs || [])
			...
			.map((typedef) => ({
				...typedef,
				type: (typedef.type as Types.Typed[]).filter(
					(t) => t.type === "sequence",
				),
			}))
		...
		this.sequenceMethods = Object.values(i.methods ? i.methods : {})
			...
			.map((m) => ({
				...m,
				signatures: this.replaceTypedefsInSignatures(
					m.signatures.filter((s) => this.hasSequenceArgument(s)),
				),
			}))
	...
	public hasIterable() {
		if (
			...
			(this.methodNames.length ||
				this.hasIterableIterator() ||
				this.extendsHtmlCollection())
		) {
			return true;
		}
		return false;
	...
	public getIteratorInitializer(handlerName: string) {
		...
		const name = i.typeParameters
			? `${iType}<${i.typeParameters!.map((p) => p.name).join(", ")}>`
			: iType;
	...
	private extendsHtmlCollection() {
		return (
			...
			(this.i.implements &&
				this.i.implements.includes("HTMLCollectionBaseIsolate"))
		);
	}
	...
	private printIteratorForEach() {
		const i: Types.Interface = this.i;
		const subtype = i.iterator!.type.map((o) =>
			TypeUtils.convertDomTypeToTsType(o, true),
		);
		...
		if (this.skipImplementation) {
			this.printer.printLine(
				`forEach(callbackfn: (${args}) => void, thisArg?: any): Promise<void>;`,
			);
		} else {
			this.printer.printSeparatorLine();
			this.printer.printLine(
				`public async forEach(callbackfn: (${args}) => void, thisArg?: any): Promise<void> {`,
			);
			if (this.buildType === BuildType.base) {
	...
	private findIterableGetter() {
		...
		const anonymousGetter = anonymousMethods.find((m) =>
			this.isIterableGetter(m),
		);

		if (anonymousGetter) return anonymousGetter;
		if (i.methods)
			return Object.values(i.methods).find((m) => this.isIterableGetter(m));
		return undefined;
	}
	...
	private getIteratorSubtypes() {
		...
		if (i.iterator) {
			...
			return i.iterator.type.map((t) =>
				TypeUtils.convertDomTypeToTsType(t, true),
			);
		}
		if (i.name !== "Window") {
			const iterableGetter = this.findIterableGetter();
			const lengthProperty =
				this.findLengthProperty(i) ||
				this.findLengthProperty(this.components.allInterfacesMap[i.extends]);
			if (iterableGetter && lengthProperty) {
				return [
					TypeUtils.convertDomTypeToTsType(
						{ type: iterableGetter.signatures[0].type },
						true,
					),
				];
			}
	...
	private printDeclarationMethod(m: IDeclarationMethod) {
		if (this.skipImplementation) {
			...
			if (this.buildType === BuildType.base) {
				...
				this.printer.printLine(
					`  return awaitedIterator.load(this).then(x => ${loadConvertor}.${m.name}());`,
				);
			} else {
				this.printer.printLine(`  // implementation required`);
			}
	...
	private hasSequenceArgument(s: Types.Signature) {
		return (
			!!s.params && s.params.some((p) => this.typeIncludesSequence(p.type))
		);
	}

	private replaceTypedefsInSignatures(
		...
		return signatures.map((s) => {
			const params = s.params!.map((p) => {
				const typedef =
					typeof p.type === "string"
						? this.sequenceTypedefMap[p.type]
						: undefined;
				if (!typedef) {
					return p;
				}
