...
export default class TsInterfaceExtractor {
	...
	private printInterfaceEventMap() {
		...
		const { iNameToEhList, iNameToAttributelessEhList, iNameToEhParents } =
			this.components;
		...
		if (
			...
			if (ehParentCount) {
				const extend = iNameToEhParents[i.name].map(
					(p) => `I${p.name}EventMap`,
				);
	...
	private emitInterfaceEventMapEntry(eHandler: Types.EventHandler) {
		const i: Types.Interface = this.i;
		const eventType = toIType(
			this.components.getEventTypeInInterface(eHandler.eventName, i),
		);
