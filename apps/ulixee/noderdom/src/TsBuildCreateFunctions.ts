...
export default class TsBuildCreateFunctions {
	...
	public run() {
		...
		Object.keys(this.objectMetaByName).forEach((name) => {
			...
			if (
				pathType !== PathType.officialKlasses &&
				pathType !== PathType.superKlasses
			)
				return;
			names.push(name);
	...
	private functionCode(klassName: string, pathType: IPathType) {
		...
		const klassDir = Path.relative(
			paths.root,
			(paths as any)[pathType as string],
		);
		const klassPath = `./${klassDir}/${klassName}`;
		const args = "awaitedPath: AwaitedPath, awaitedOptions: IAwaitedOptions";
		printable.push(
			`export function create${klassName}<IAwaitedOptions = {}>(${args}): I${klassName} {`,
		);
		printable.push(
			`  const ${klassName} = require('${klassPath}').default; // tslint:disable-line:variable-name`,
		);
	...
	private imports(names: string[]) {
		...
		printable.push(
			tsImporter.extractAll(names, BuildType.base, ObjectStruct.interface),
		);
		printable.push("");
