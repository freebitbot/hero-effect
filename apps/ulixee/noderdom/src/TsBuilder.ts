...
import type IObjectStruct from "./interfaces/IObjectStruct";
import { ObjectStruct } from "./interfaces/IObjectStruct";
import type IObjectType from "./interfaces/IObjectType";
import { ObjectType } from "./interfaces/IObjectType";
import {
	type IPathsByBuildType,
	type IPathType,
	PathType,
} from "./interfaces/IPaths";
...
export default class TsBuilder {
	...
	constructor(
		...
		this.klasses = allInterfaces
			.sort(compareName)
			.filter((i) => !i.legacyNamespace && !i.noInterfaceObject);
		this.mixins = allInterfaces
			.sort(compareName)
			.filter((i) => !i.legacyNamespace && i.noInterfaceObject);
	}
	...
	public extractOfficialInterfaces() {
		...
		const codeModules = this.interfaces.map((i) =>
			new TsInterfaceExtractor(this.components, i, options).run(),
		);
	...
	public extractOfficialInterfaceImports(
		...
		const importsCodeModule = this.extractImportCodeModule(
			BuildType.base,
			ObjectStruct.interface,
			{
				currentDir: this.pathsByBuildType.base.interfaces,
				currentFile: entrypoint,
				references: [...codeModules, ...basicTypes]
					.map((x) => x.referencedObjects)
					.reduce((a, b) => a.concat(b), []),
				localReferences: [...codeModules, ...basicTypes]
					.map((x) => x.definedObjects)
					.reduce((a, b) => a.concat(b), []),
			},
		);
	...
	public extractIsolateInterfaces() {
		...
		for (const i of Object.values(this.components.awaitedIsolates)) {
			codeModules.push(
				new TsBuildInterfaceIsolate(this.components, i, options).run(),
			);
		}
	...
	public extractSuperInterfaces() {
		const codeModules = Object.values(this.components.awaitedSupers).map(
			(i) => {
				return new TsInterfaceExtractor(this.components, i, {
	...
	public extractSuperKlasses() {
		if (this.domType !== DomType.awaited)
			throw new Error("Supers can only be created for AwaitedDOM");
		const { domType, buildType, pathsByBuildType, objectMetaByName } = this;
		const isBaseBuild = buildType === BuildType.base;
		const currentDir = isBaseBuild
			? pathsByBuildType.base.superKlasses
			: pathsByBuildType.impl.superKlasses;
		const options = {
			pathsByBuildType,
			currentDir,
			objectMetaByName,
		...
		const codeModules = Object.values(this.components.awaitedSupers).map(
			(i) => {
				if (i.implements) {
					i.isAwaitedNodePointer =
						i.implements.includes("NodeIsolate") ||
						i.implements.includes("HTMLCollectionBaseIsolate");
				}
	...
	public extractIsolateMixins() {
		if (this.domType !== DomType.awaited)
			throw new Error("Isolates can only be created for AwaitedDOM");
		const { domType, buildType, pathsByBuildType, objectMetaByName } = this;
		const isBaseBuild = buildType === BuildType.base;
		const currentDir = isBaseBuild
			? pathsByBuildType.base.isolateMixins
			: pathsByBuildType.impl.isolateMixins;
		const codeModules = [];
		for (const i of Object.values(this.components.awaitedIsolates)) {
	...
	private extractImportCodeModule(
		...
		const importCode = tsImporter.extractAll(
			references,
			importFromBuildType,
			importFromObjectStruct,
			currentFile,
		);
		return {
			name: "Import",
	...
	private addToObjectMeta(
		...
		names.forEach((name) => {
			const objectMeta = this.objectMetaByName[name] || {
				objectType,
				pathTypeByObjectStruct: new Map(),
			};
			if (this.objectMetaByName[name] && objectMeta.objectType !== objectType) {
				throw new Error(
					`${name} has mismatched ObjectType: ${objectMeta.objectType} != ${objectType}`,
				);
			}
