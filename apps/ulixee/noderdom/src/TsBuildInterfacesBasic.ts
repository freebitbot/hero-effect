...
export default class TsBuildInterfacesBasic {
	...
	constructor(components: Components) {
		...
		this.dictionaries = Object.values(this.components.dictionaries)
			.sort(compareName)
			.filter((i) => !i.legacyNamespace);
		this.callbackFunctions = Object.values(
			this.components.callbackFunctions,
		).sort(compareName);
		this.callbackInterfaces = Object.values(
	...
	private extractEnum(e: Types.Enum): ICodeModule {
		...
		this.printer.printLine(
			`export type I${name} = ${values.map((v) => `'${v}'`).join(" | ")};`,
		);
		return {
			name,
			definedObjects,
			referencedObjects,
			category: "Enum",
			code: this.printer.getResult().trim(),
		};
	}
	...
	private extractDictionaries() {
		return this.dictionaries.map((dictionary) =>
			this.extractDictionary(dictionary),
		);
	}
	...
	private extractDictionary(dictionary: Types.Dictionary): ICodeModule {
		...
		if (!dictionary.extends || dictionary.extends === "Object") {
			...
			this.printer.printLine(
				`export interface I${name} extends I${dictionary.extends} {`,
			);
		}
	...
	private extractCallbackFunction(cb: Types.CallbackFunction): ICodeModule {
		...
		if (cb.signatures.length === 1) {
			...
			const paramsString = signature.params
				? ParamUtils.paramsToString(signature.params, true)
				: "";
			const returnType = signature.type
				? TypeUtils.convertDomTypeToTsType(signature, true)
				: "void";
			this.printer.print(
				`export type I${cb.name} = (${paramsString}) => ${returnType};`,
			);
		} else {
		...
		return {
			...
			definedObjects,
			referencedObjects,
			category: "CallbackFunction",
			code: this.printer.getResult(),
		};
	}
