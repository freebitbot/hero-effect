...
import { exposesTo, filter, followTypeReferences, mapToArray } from "./utils";
...
export default class ComponentCleaner {
	...
	public run() {
		...
		filtered.interfaces = filter(original.interfaces, (i) =>
			exposesTo(i, target),
		);
		const unexposedInterfaces = mapToArray(original.interfaces).filter(
		...
		const referencedTypes = original.typedefs.filter((t) =>
			this.referencedIDLTypes.has(t.newType),
		);
		const { exposed, removed } = this.filterTypedefs(
			referencedTypes,
			this.unexposedTypes,
		);
		...
		filtered.callbackFunctions! = this.filterProperties(
			original.callbackFunctions,
			"callbackFunction",
		);
		filtered.callbackInterfaces! = this.filterProperties(
			original.callbackInterfaces,
			"callbackInterface",
		);
		filtered.dictionaries = this.filterProperties(
		...
		const final = this.deepClone(filtered, (c) => {
			if (Array.isArray(c.type)) {
				const type = this.filterUnexposedTypeFromUnion(
					c.type,
					this.unexposedTypes,
				);
				return { ...c, type };
	...
	private runComponentFiltering() {
		...
		const componentFilters: IComponentFilters = JSON.parse(
			Fs.readFileSync(this.componentFilteringPath, "utf-8"),
		);

		const callbackInterfaceNames = Object.keys(
			this.components.callbackInterfaces,
		);
		const interfacesNames = Object.keys(this.components.interfaces);
		const mixinNames = Object.keys(this.components.mixins);
		const names = [
			...callbackInterfaceNames,
			...interfacesNames,
			...mixinNames,
		];
		names.forEach((name) => {
			...
			if (
				i.extends &&
				componentFilters[i.extends] &&
				!componentFilters[i.extends].isEnabled
			) {
				i.extends = "Object";
			}
	...
	private filterProperties<T, U extends T>(
		obj: Record<string, U>,
		objName: string,
	): Record<string, U> {
	...
	private filterUnexposedTypeFromUnion(
		union: Types.Typed[],
		...
		for (const type of union) {
			if (Array.isArray(type.type)) {
				const filteredUnion = this.filterUnexposedTypeFromUnion(
					type.type,
					unexposedTypes,
				);
	...
	private filterDuplicateSignatures(signatures: Types.Signature[]) {
		...
		let maxParamsSignature: Types.Signature | undefined = signatures.find(
			(x) => x.params && x.params.length,
		);
		...
		for (const param of maxParamsSignature.params!) {
			...
			for (const signature of signatures) {
				if (!signature.params) {
					...
					const matchingParam = signature.params.find(
						(x) => x.name === param.name,
					);
	...
	private filterUnknownTypeFromSignature(
	...
	private filterTypedefs(
		...
		typedefs.forEach((typedef) => {
			if (Array.isArray(typedef.type)) {
				const filteredType = this.filterUnexposedTypeFromUnion(
					typedef.type,
					unexposedTypes,
				);
