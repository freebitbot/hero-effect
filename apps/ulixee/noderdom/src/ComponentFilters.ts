...
import type {
	IComponentFilter,
	IComponentFilterItem,
} from "./interfaces/IComponentFilters";
...
export default class ComponentFilters {
	...
	constructor(domType: IDomType, records?: any[]) {
		this.records =
			records ||
			db
				.prepare("SELECT * FROM component_filters WHERE domType=?")
				.all(domType);
	}
	...
	public saveToDb() {
		this.records.forEach((record) => {
			...
			const existing = db
				.prepare("SELECT * FROM component_filters WHERE name=? AND domType=?")
				.get(primaryKey);
			if (existing) {
				...
				db.prepare(
					`INSERT INTO component_filters (${fields}) VALUES(${placeholders})`,
				).run(values);
			}
		});
	}
	...
	public toMap() {
		...
		allInterfaces.forEach((inter) => {
			...
			componentFiltersMap[inter.name] = Object.assign({}, choice, {
				propertiesByName: {},
				methodsByName: {},
			});
			...
			properties.forEach((p) => {
				...
				componentFiltersMap[inter.name].propertiesByName[p.localName] =
					choicesByName[p.name];
			});
			...
			methods.forEach((m) => {
				...
				componentFiltersMap[inter.name].methodsByName[m.localName] =
					choicesByName[m.name];
			});
		});
	...
	public static fromMap(
		domType: IDomType,
		map: { [name: string]: IComponentFilter },
	) {
		const records: any[] = [];
		Object.entries(map).forEach(([interName, interObj]) => {
			...
			Object.entries(interObj.propertiesByName).forEach(
				([propName, propObj]) => {
					records.push({
			...
			Object.entries(interObj.methodsByName).forEach(
