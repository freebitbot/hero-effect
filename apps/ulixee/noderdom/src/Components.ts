...
import type IComponents from "./interfaces/IComponents";
...
export default class Components implements IComponents {
	...
	public toJSON(): IComponents {
		return {
			...
			awaitedSupers: Object.values(this.awaitedSupers).length
				? this.awaitedSupers
				: undefined,
			awaitedIsolates: Object.values(this.awaitedIsolates).length
				? this.awaitedIsolates
				: undefined,
		};
	}
	...
	public getParentEventHandler(i: Types.Interface): Types.Interface[] {
		const hasEventListener =
			this.iNameToEhList[i.name] && this.iNameToEhList[i.name].length;
		if (hasEventListener) {
			return [i];
		}
	...
	public getParentsWithEventHandler(inter: Types.Interface) {
		...
		const parentWithEventHandler =
			(this.allInterfacesMap[iExtends] &&
				this.getParentEventHandler(this.allInterfacesMap[iExtends])) ||
			[];
	...
	public getParentConstant(i: Types.Interface): Types.Interface[] {
		const hasConst =
			this.iNameToConstList[i.name] && this.iNameToConstList[i.name].length;
		return (hasConst ? [i] : []).concat(this.getParentsWithConstant(i));
	}

	public getParentsWithConstant(inter: Types.Interface) {
		const mixinsWithConstant = flatMap(inter.implements || [], (v) =>
			this.getParentConstant(this.allInterfacesMap[v]),
		);
	...
	public dependsOn(i1Name: string, i2Name: string) {
		return this.iNameToIDependList[i1Name]
			? this.iNameToIDependList[i1Name].includes(i2Name)
			: i2Name === "Object";
	}
	...
	private initialize() {
		// Global print target
		this.allNonCallbackInterfaces = Object.values(this.interfaces).concat(
		...
		this.allLegacyWindowAliases = flatMap(
			this.allInterfaces,
		...
		this.distinctETypeList = distinct(
			flatMap(this.allNonCallbackInterfaces, (i) =>
				i.events ? i.events.map((e) => e.type) : [],
			).concat(
				this.allNonCallbackInterfaces
					.filter(
						(i) =>
		...
		this.iNameToEhList = arrayToMap(
			...
			(i) =>
				!i.properties
					? []
					: mapDefined<Types.Property, Types.EventHandler>(
							Object.values(i.properties),
							(p) => {
								...
								const eventType =
									eType === "Event" || this.dependsOn(eType, "Event")
										? eType
										: defaultEventType;
		...
		this.iNameToEhParents = arrayToMap(
			this.allInterfaces,
			(i) => i.name,
			this.getParentsWithEventHandler.bind(this),
		);

		this.iNameToConstParents = arrayToMap(
			this.allInterfaces,
			(i) => i.name,
			this.getParentsWithConstant.bind(this),
		);
	}
}
