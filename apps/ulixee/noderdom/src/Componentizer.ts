...
export default class Componentizer {
	...
	public addWebIDL(webIDL: string, filename: string) {
		...
		for (const rootType of rootTypes) {
			if (rootType.type === "interface") {
				...
				this.callbackFunctions[rootType.name] =
	...
	public run() {
		...
		const allNonCallbackInterfaces = Object.values(this.interfaces).concat(
...
function convertInterfaceCommon(
	i: webidl2.InterfaceType | webidl2.InterfaceMixinType,
) {
	const result: Types.Interface = {
		...
		noInterfaceObject: hasExtAttr(i.extAttrs, "NoInterfaceObject")
			? 1
			: undefined,
	...
	for (const member of i.members) {
		if (member.type === "const") {
			...
			if (member.name in properties!) {
				throw new Error(
					"need to handle adding to namesake... see TSJS-lib-generator",
				);
			} else {
			...
			} else if (Object.hasOwn(methods, member.name)) {
