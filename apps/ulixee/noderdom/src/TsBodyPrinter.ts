...
export default class TsBodyPrinter {
	...
	constructor(
		...
		for (const method of allExtendedMethods) {
			...
			const exists = this.indexers.find(
				(x) =>
					TypeUtils.convertDomTypeToTsType(x.signatures[0].params[0]) ===
					indexType,
			);
			if (exists) continue;
			...
			for (const returnType of returnTypes)
				this.referencedObjects.add(returnType);
		...
		this.iteratorExtractor = new TsIteratorExtractor(
			this.i,
			this.components,
			iteratorOptions,
		);
	...
	public printAll(): void {
		...
		if (
			...
			(iterators || this.indexers.length)
		) {
	...
	public printConstructor() {
		...
		if (
			...
			!constructor.signatures.length
		)
			constructor = undefined;
		else if (
			!constructor!.signatures[0].params ||
			!constructor!.signatures[0].params.length
		)
			constructor = undefined;
		...
		if (this.skipImplementation && signature) {
			const args = signature.params
				? ParamUtils.paramsToString(signature.params, true)
				: "";
			this.printer.printSeparatorLine();
			this.printer.printLine(`// constructor(${args})`);
		} else if (signature) {
			this.printer.printSeparatorLine(
				this.constructor ? "// constructor required for this class" : "",
			);
		}

		if (!this.skipImplementation && !this.skipConstructor) {
			const args = signature
				? ParamUtils.paramsToString(signature.params!, true, true)
				: "";
			const isChildClass =
				(i.extends && i.extends !== "Object") ||
				(i.implements && i.implements.length);
			...
			if (isChildClass) {
				this.printer.printLine(
					`super(${signature ? ParamUtils.paramNames(signature.params!, true).join(", ") : ""});`,
				);
			}
			if (this.buildType === BuildType.base) {
				if (
					this.i.isAwaitedNodePointer ||
					this.iteratorExtractor.hasIterable()
				) {
					...
					if (
						this.iteratorExtractor.hasIterable() &&
						!this.iteratorExtractor.hasMaplikeSequence
					) {
						const interableInterface =
							this.iteratorExtractor.getIteratableInterface();
						if (interableInterface.startsWith("I")) {
							this.printer.printLine(
								`createIterableName: 'create${interableInterface.substr(1)}',`,
							);
						} else {
							this.printer.printLine(
								`createIterableName: '${interableInterface}',`,
							);
						}
					}
				...
				if (this.indexers?.length) {
					const indexers = this.indexers.map((x) => ({
						m: x,
						indexType: TypeUtils.convertDomTypeToTsType(
							x.signatures[0].params![0],
						),
					}));
					...
					if (integerIndexer) {
						...
						this.printer.printLine(
							`return target.${integerIndexer.m.name}(param);`,
						);
						this.printer.decreaseIndent();
						this.printer.printLine(`}`);
					}
					if (stringIndexer) {
						...
						this.printer.increaseIndent();
						this.printer.printLine(
							`return target.${stringIndexer.m.name}(prop as string);`,
						);
						this.printer.decreaseIndent();
						this.printer.printLine(`}`);
					}
	...
	public printProperties(
		...
		this.printer.printSeparatorLine(
			this.skipImplementation ? "" : "\n// properties",
		);
		(properties || this.properties).forEach((p) =>
			this.printProperty(p, forceOptional),
		);
	}
	...
	public printProperty(
		...
		if (isAbstract && property.compromiseType && abstractClass.includes("|")) {
			const officialType = property.compromiseType!.find(
				(x) => x.isOfficial === 1,
			)!;
		...
		if (this.skipImplementation) {
			const isRequired = forceOptional
				? false
				: property.required === undefined || property.required === 1;
			const requiredModifier = isRequired ? "" : "?";
			const readOnlyModifier = isReadonly ? "readonly " : "";
			this.printer.printLine(
				`${readOnlyModifier}${property.name}${requiredModifier}: ${returnType};`,
			);
			return;
		}
		...
		if (isAbstract && this.buildType === BuildType.base) {
			this.printer.printLine(
				`throw new Error('${i.name}.${property.name} getter not implemented');`,
			);
		} else if (isAbstract) {
			this.printer.printLine(
		...
		if (!isReadonly) {
			this.printer.printSeparatorLine();
			this.printer.printLine(
				`public set ${property.name}(value: ${genericsType}) {`,
			);
			this.printer.increaseIndent();
			if (isAbstract) {
				this.printer.printLine(
					`throw new Error('${i.name}.${property.name} setter not implemented');`,
				);
			} else {
				this.printer.printLine(
					`${this.handler}.setProperty<${genericsTypeCleaned}>(this, '${property.name}', value);`,
				);
			}
			this.printer.decreaseIndent();
			this.printer.printLine("}");
	...
	public printMethod(method: Types.Method) {
		...
		method.signatures.forEach((signature) => {
			...
			if (this.skipImplementation) {
				...
				this.printer.printSeparatorLine();
				const argNames = signature.params
					? ParamUtils.paramNames(signature.params).join(", ")
					: "";
				...
				if (isAbstract && this.buildType === BuildType.base) {
					this.printer.printLine(
						`throw new Error('${i.name}.${method.name} not implemented');`,
					);
				} else if (isAbstract) {
			...
			TypeUtils.extractCustomTypes(signature).forEach(
				this.referencedObjects.add,
				this.referencedObjects,
			);
		});
	...
	private extractMethodValueType(
		method: Types.Method,
		signature: Types.Signature,
	) {
		const isAbstract = this.isAbstract(method);
	...
	private extractPropertyReturnType(p: Types.Property): string {
		...
		if (p.compromiseType) {
			...
			for (const cType of p.compromiseType) {
				...
				if (this.domType === DomType.awaited) {
					if (cType.isAbstract === 1) {
						...
						if (!typePart.startsWith("Promise<"))
							typePart = `Promise<${typePart}>`;
					}
				} else if (!isRequired) {
					typePart += " | undefined";
				}
			...
			TypeUtils.extractCustomTypes(p).forEach(
				this.referencedObjects.add,
				this.referencedObjects,
			);
		...
		if (isEventHandler(p)) {
			...
			const eType = toIType(
				p.eventHandler
					? this.components.getEventTypeInInterface(p.eventHandler!, i)
					: "Event",
			);
			this.referencedObjects
				.add(nameWithForwardedTypes(i, true).slice(1))
				.add(eType.slice(1));
	...
	private printEventHandlerMethods() {
		...
		for (const { addOrRemove, optionsType, iParent } of eventHandlersToPrint) {
			...
			if (this.skipImplementation) {
				...
				this.printer.printLine(
					`  ${this.handler}.runMethod<void>(this, '${methodName}', [type, listener, options]);`,
				);
				this.printer.printLine(`}`);
			}
	...
	private eventHandlersToPrint() {
		...
		const hasEventListener =
			this.components.iNameToEhList[i.name] &&
			this.components.iNameToEhList[i.name].length;
		const ehParentCount =
			this.components.iNameToEhParents[i.name] &&
			this.components.iNameToEhParents[i.name].length;
		const eventHandlers = [];
		for (const addOrRemove of ["add", "remove"]) {
			...
			if (hasEventListener || ehParentCount > 1) {
				...
				eventHandlers.push({
					addOrRemove,
					optionsType,
					iParent: this.components.iNameToEhParents[i.name][0],
				});
			}
		}
	...
	private printTypedEventHandler(
		addOrRemove: string,
		...
		this.printer.printLine(
			`${scope}${methodName}<K ${extender}>(${args}): void;`,
		);
	...
	private getIndexerReturnType(method: Types.Method): string {
		...
		const extendedType =
			firstTypeParam && firstTypeParam.extends ? firstTypeParam.name : null;
		const typeObj2 = {
	...
	private hasIndexerSignature(meth: Types.Method) {
		if (
			meth.getter &&
			meth.signatures &&
			meth.signatures[0].params &&
			meth.signatures[0].params!.length === 1
		) {
	...
	private isTypescriptCompatibleIndexer(meth: Types.Method) {
		...
		const tsType = TypeUtils.convertDomTypeToTsType(
			meth.signatures[0].params![0],
		);
		if (tsType === "number") {
			return true;
		}
		...
		const mTypes = distinct(
			(i.methods &&
				map(
					i.methods,
					(m) =>
						(m.signatures && m.signatures.length && m.signatures[0].type) ||
						"void",
				).filter((t) => t !== "void")) ||
				[],
		);
		const amTypes = distinct(
			(i.anonymousMethods &&
				i.anonymousMethods
					.map((m) => m.signatures[0].type)
					.filter((t) => t !== "void")) ||
				[],
		); // |>  Array.distinct
		const pTypes = distinct(
			(i.properties &&
				map(i.properties, (m) => m.type).filter((t) => t !== "void")) ||
				[],
		); // |>  Array.distinct
	...
	private printStaticMethods() {
		...
		this.printer.printSeparatorLine(
			this.skipImplementation ? "" : "// static methods",
		);
