...
export default class SuperGenerator {
	...
	public static injectIntoComponents(components: Components) {
		...
		SuperGenerator.baseNames.forEach((baseName) => {
			...
			const superInterface = Object.assign(
				{},
				components.allInterfacesMap[baseName],
				{ name: superName },
			);
			...
			inheritsFrom.klassNames.forEach((klassName) => {
				...
				const isolateInterface = Object.assign(
					{},
					components.allInterfacesMap[klassName],
					{ name: isolateName },
				);

				isolateInterface.properties = Object.assign(
					{},
					isolateInterface.properties,
				);
