...
const reservedTsKeywords: Set<string> = new Set([
	"default",
	"delete",
	"continue",
	"arguments",
...
export default class ParamUtils {
	...
	public static paramsToString(
		params: Types.Param[],
		convertToIType: boolean = false,
		isUnused: boolean = false,
	) {
		return params
			.map((p) => ParamUtils.paramToString(p, convertToIType, isUnused))
			.join(", ");
	}

	public static paramNames(ps: Types.Param[], isUnused: boolean = false) {
		return ps.map((p) => ParamUtils.adjustParamName(p.name, isUnused));
	}
	...
	private static paramToString(
		...
		const pType = TypeUtils.convertDomTypeToTsType(
			param,
			convertToIType,
			false,
		);
		...
		return (
			(param.variadic ? "..." : "") +
			ParamUtils.adjustParamName(param.name, isUnused) +
			(isOptional ? "?: " : ": ") +
			(variadicParams ? "(" : "") +
			pType +
			(variadicParams ? ")" : "") +
	...
	private static adjustParamName(name: string, isUnused: boolean = false) {
		if (reservedTsKeywords.has(name))
			throw new Error(`${name} is a reserved typescript keyword`);
