...
import type { IConnectAction } from "@ulixee/net/lib/ConnectionToCore";
...
export default class ConnectionToHeroCore extends ConnectionToCore<any, {}> {
	...
	public async createSession(
		...
		try {
			...
			if (error instanceof DisconnectedError && this.disconnectAction)
				return null;
			throw error;
		}
	}
	...
	protected async beforeDisconnect(
		...
		this.coreSessions.stop(
			!this.transport.isConnected && !connectAction
				? new Error(
						`No host connection was established (${this.transport.host})`,
					)
		...
		if (connectAction && !connectAction?.resolvable.isResolved) {
			if (hasSessions && !connectAction.isAutomatic) {
				connectAction.resolvable.reject(
					new DisconnectedFromCoreError(this.transport.host),
				);
			} else {
