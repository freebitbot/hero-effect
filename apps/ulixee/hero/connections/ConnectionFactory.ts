...
export default class ConnectionFactory {
	...
	public static createConnection(
		...
		if (options.host) {
			const host = Promise.resolve(options.host).then(
				ConnectionToHeroCore.resolveHost,
			);
			...
			if (host) {
				const transport = new WsTransportToCore(
					ConnectionToHeroCore.resolveHost(host),
				);
				connection = new ConnectionToHeroCore(
					transport,
					{ ...options, version },
					callsiteLocator,
				);
