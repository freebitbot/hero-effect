...
import type IClientPlugin from "@ulixee/hero-interfaces/IClientPlugin";
import type { IClientPluginClass } from "@ulixee/hero-interfaces/IClientPlugin";
import type IDomState from "@ulixee/hero-interfaces/IDomState";
import type { IDomStateAllFn } from "@ulixee/hero-interfaces/IDomState";
import type IFlowCommandOptions from "@ulixee/hero-interfaces/IFlowCommandOptions";
...
import {
	type IElementInteractVerification,
...
import type IHeroDefaults from "../interfaces/IHeroDefaults";
import type IInteractions from "../interfaces/IInteractions";
import {
	Command,
	type IInteraction,
	type ITypeInteraction,
} from "../interfaces/IInteractions";
import type IWaitForResourceFilter from "../interfaces/IWaitForResourceFilter";
import type IWaitForResourcesFilter from "../interfaces/IWaitForResourcesFilter";
import AwaitedEventTarget from "./AwaitedEventTarget";
import CallsiteLocator from "./CallsiteLocator";
import type CoreFrameEnvironment from "./CoreFrameEnvironment";
import CoreSession from "./CoreSession";
import type DetachedElement from "./DetachedElement";
import DetachedElements from "./DetachedElements";
import type DetachedResource from "./DetachedResource";
import DetachedResources from "./DetachedResources";
import { type IDomExtensionClass, isDomExtensionClass } from "./DomExtender";
import type DomState from "./DomState";
import type FileChooser from "./FileChooser";
import type FrameEnvironment from "./FrameEnvironment";
import { getCoreFrameEnvironmentForPosition } from "./FrameEnvironment";
import Interactor from "./Interactor";
import { InternalPropertiesSymbol } from "./internal";
import Resource from "./Resource";
import ScriptInvocationMeta from "./ScriptInvocationMeta";
import type Tab from "./Tab";
import { createTab, getCoreTab } from "./Tab";
import WebsocketResource from "./WebsocketResource";

const { version } = require("../package.json");

export type ISessionOptions = ISessionCreateOptions &
	Pick<IHeroCreateOptions, "connectionToCore">;
...
export default class Hero extends AwaitedEventTarget<IHeroEvents> {
	...
	get [InternalPropertiesSymbol](): ISharedInternalProperties {
		const coreSessionPromise = (): Promise<CoreSession> =>
			this.#getCoreSessionOrReject();
	...
	constructor(createOptions: IHeroCreateOptions = {}) {
		...
		const { name, connectionToCore, callsiteLocator, ...options } =
			createOptions;
	...
	public async addToDetached(
		...
		if (
			elementOrResource instanceof Resource ||
			elementOrResource instanceof WebsocketResource
		) {
	...
	public async click(
		...
		coreFrame ??=
			await this.activeTab.mainFrameEnvironment[InternalPropertiesSymbol]
				.coreFramePromise;
	...
	public use(
		...
		if (typeof PluginObject === "string") {
			...
			const ClientPlugins = filterPlugins<IClientPluginClass>(
				Plugins,
			...
			ClientPlugins.forEach(
				(ClientPlugin) => (ClientPluginsById[ClientPlugin.id] = ClientPlugin),
			);
		} else {
		...
		for (const ClientPlugin of Object.values(ClientPluginsById)) {
			...
			this.#options.dependencyMap[ClientPlugin.id] =
				ClientPlugin.coreDependencyIds || [];
		}
	...
	public async registerFlowHandler(
		...
		return await this.activeTab.registerFlowHandler(
			name,
			state,
			handlerCallbackFn,
		);
			...
			this.#coreSessionPromise = this.#connectionToCore
				...
				.then((session) => {
					if (session instanceof CoreSession)
						this.#initializeClientPlugins(this.#clientPlugins);
				...
				.then((x) => {
					...
					if (!x)
						throw new CanceledPromiseError(
							"No connection to Hero Core established.",
						);
					return x.firstTab;
				})
				.catch((err) => err);
	...
	#initializeClientPlugins(plugins: IClientPlugin[]): void {
		...
		for (const clientPlugin of plugins) {
			if (clientPlugin.onHero)
				clientPlugin.onHero(this, this.#sendToActiveTab.bind(this));
		}
	}
