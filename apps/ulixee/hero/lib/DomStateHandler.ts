import type { CanceledPromiseError } from "@ulixee/commons/interfaces/IPendingWaitEvent";
...
import type { IStateAndAssertion } from "@ulixee/hero-interfaces/IDomState";
import type IDomStateListenArgs from "@ulixee/hero-interfaces/IDomStateListenArgs";
import type { IRawCommand } from "@ulixee/hero-interfaces/IDomStateListenArgs";
import type IDomStateResult from "@ulixee/hero-interfaces/IDomStateResult";
...
import type IFlowCommand from "../interfaces/IFlowCommand";
import type CoreTab from "./CoreTab";

let counter = 0;

const NullPropertyAccessRegex =
...
export default class DomStateHandler {
		...
		try {
			...
			if (typeof assertion === "function") {
				...
				if (isPromise(didPass)) {
					throw new Error(
						`DomState Assertions can't return promises\n\n${assertion?.toString()}`,
					);
				}
				return didPass === true;
			}
			...
			if (
				NullPropertyAccessRegex.test(err) ||
				NullPropertyAccessRegexNode16.test(err)
			) {
				return false;
			}
			throw err;
	...
	private async createAssertionSets(): Promise<IStateAndAssertion<any>[]> {
		...
		this.domState.all((statePromise, assertion) => {
			assertionSets.push([
				Promise.resolve(statePromise).catch((err) => err),
				assertion,
			]);
		});
