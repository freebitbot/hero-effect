...
import {
	...
	type IMousePosition as ICoreMousePosition,
...
import type { IKeyboardKeyCode } from "@ulixee/unblocked-specification/agent/interact/IKeyboardLayoutUS";
import type { IKeyboardShortcut } from "@ulixee/unblocked-specification/agent/interact/IKeyboardShortcuts";
import type IInteractions from "../interfaces/IInteractions";
import {
	Command,
	type ICommand,
	type IInteraction,
	type ISuperElementWithVerification,
...
function convertToInteractionGroups(
	interactions: IInteractions,
): IInteractionGroups {
	...
	interactions.forEach((interaction) => {
		if (typeof interaction === "string") {
			const interactionStep: IInteractionStep =
				convertCommandToInteractionStep(interaction);
			interactionStep.mousePosition = lastPosition;
			interactionGroups.push([interactionStep]);
		} else {
			const interactionGroup =
				convertInteractionToInteractionGroup(interaction);
			lastPosition =
				interactionGroup[interactionGroup.length - 1].mousePosition;
...
function convertToCoreMousePosition(
	...
	mousePosition: ICoreMousePosition;
...
function convertInteractionToInteractionGroup(
	...
	Object.entries(interaction).forEach(([key, value]) => {
		switch (key) {
			...
			case Command.keyPress: {
				...
				return iGroup.push({
					command,
					keyboardCommands: [{ keyCode: value as IKeyboardKeyCode }],
				});
			}
			case Command.keyDown: {
				...
				return iGroup.push({
					command,
					keyboardCommands: [{ down: value as IKeyboardKeyCode }],
				});
			}
			...
			case Command.keyUp: {
				...
				return iGroup.push({
					command,
					keyboardCommands: [{ up: value as IKeyboardKeyCode }],
				});
			}
			case Command.type: {
				const command = CoreCommand.type;
...
function convertCommandToInteractionStep(
	interaction: ICommand,
): IInteractionStep {
	switch (interaction) {
		case Command.move: {
		...
		case Command.doubleclickLeft: {
			return {
				command: CoreCommand.doubleclick,
				mouseButton: MouseButton.left,
			};
			break;
		}
