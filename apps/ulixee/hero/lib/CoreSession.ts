...
import type ICoreSession from "../interfaces/ICoreSession";
import type { IOutputChangeToRecord } from "../interfaces/ICoreSession";
	...
	implements IJsPathEventTarget, ICoreSession
{
	...
	public async getTabs(): Promise<CoreTab[]> {
		const tabSessionMetas =
			await this.commandQueue.run<ISessionMeta[]>("Session.getTabs");
	...
	public addTab(tabMeta: ISessionMeta): CoreTab {
		if (!this.tabsById.has(tabMeta.tabId)) {
			this.tabsById.set(
				tabMeta.tabId,
				new CoreTab(
					{ ...tabMeta, sessionName: this.sessionName },
	...
	public async getCollectedAssetNames(
		...
		return await this.commandQueue.run(
			"Session.getCollectedAssetNames",
			sessionId,
		);
	}
		...
		try {
			...
			if (result?.didKeepAlive === true) {
				...
				const didClose = new Promise((resolve) =>
					this.addEventListener(null, "close", resolve),
				);
				await this.watchRelaunchLogs();
				this.keepAlivePrompt = new CoreKeepAlivePrompt(
					result.message,
	...
	private async watchRelaunchLogs(): Promise<void> {
		await this.addEventListener(null, "rerun-stdout", (msg) =>
			process.stdout.write(msg),
		);
		await this.addEventListener(null, "rerun-stderr", (msg) =>
