import AwaitedHandler, {
	NotImplementedError,
} from "@ulixee/awaited-dom/base/AwaitedHandler";
import type AwaitedPath from "@ulixee/awaited-dom/base/AwaitedPath";
import type Constructable from "@ulixee/awaited-dom/base/Constructable";
...
async function getProperty<T, TClass>(
	...
	const { awaitedPath, coreFrame, awaitedOptions } = await getAwaitedState(
		stateHandler,
		instance,
	);
	const finalPath = awaitedPath.addProperty(instance as any, name);

	const result = await execJsPath<T>(
		coreFrame,
		awaitedOptions,
		finalPath.toJSON(),
	);
...
async function createNodePointer<TClass>(
	...
	const finalPath = awaitedPath.addMethod(
		instance as any,
		getNodePointerFnName,
	);

	const result = await execJsPath<null>(
		coreFrame,
		awaitedOptions,
		finalPath.toJSON(),
	);
...
function runStatic<T, TClass>(
	...
	throw new NotImplementedError(
		`${stateHandler.className}.${name} static method not implemented`,
	);
}

function construct<TClass>(self: AwaitedHandler<TClass>): TClass {
	throw new NotImplementedError(
		`${self.className} constructor not implemented`,
	);
}
...
const { getState: getAwaitedPathState } = StateMachine<
	any,
	{ awaitedPath?: AwaitedPath }
>();
