...
import type IConfigureSessionOptions from "@ulixee/hero-interfaces/IConfigureSessionOptions";
...
import type IFlowHandler from "../interfaces/IFlowHandler";
...
export default class CoreTab implements IJsPathEventTarget {
	...
	constructor(
		...
		this.commandQueue.registerCommandRetryHandlerFn(
			this.shouldRetryFlowHandlers.bind(this),
		);
		...
		this.frameEnvironmentsById.set(
			frameId,
			new CoreFrameEnvironment(this, meta, null),
		);
	...
	public async waitForState(
		...
		const callsitePath =
			sourceCode?.callsitePath ?? this.coreSession.callsiteLocator.getCurrent();
	...
	public async registerFlowHandler(
		...
		await this.commandQueue.runOutOfBand(
			"Tab.registerFlowHandler",
			name,
			id,
			callsitePath,
		);
	...
	public async runFlowCommand<T>(
		...
		const flowCommand = await this.flowCommands.create(
			commandFn,
			exitState,
			callsitePath,
			options,
	...
	public async triggerFlowHandlers(): Promise<{
		...
		try {
			...
			this.commandQueue.setCommandMetadata({
				activeFlowHandlerId: flowHandler.id,
			});
			await flowHandler.handlerFn();
			return {
				triggeredFlowHandler: flowHandler,
				matchedFlowHandlers: matchingStates,
			};
		} finally {
	...
	public async getCoreFrameEnvironments(): Promise<CoreFrameEnvironment[]> {
		const frameMetas = await this.commandQueue.run<IFrameMeta[]>(
			"Tab.getFrameEnvironments",
		);
	...
	public async getResourceProperty<T = any>(
		id: number,
		propertyPath: string,
	): Promise<T> {
		return await this.commandQueue.runOutOfBand(
			"Tab.getResourceProperty",
			id,
			propertyPath,
		);
	}
	...
	public async waitForNewTab(opts: IWaitForOptions): Promise<CoreTab> {
		const sessionMeta = await this.commandQueue.run<ISessionMeta>(
			"Tab.waitForNewTab",
			opts,
		);
	...
	public async addEventListener(
		...
		await this.eventHeap.addListener(
			jsPath,
			eventType,
			listenerFn,
			options,
	...
	public async removeEventListener(
		...
		await this.eventHeap.removeListener(
			jsPath,
			eventType,
			listenerFn,
			options,
