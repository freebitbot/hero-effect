...
export default class CallsiteLocator {
	public static readonly ignoreModulePaths = [
		"node:internal",
		AwaitedDomPath,
		HeroLibPath,
	];
	public static readonly ignoreModulePathFragments = [
	...
	public getCurrent(): ISourceCodeLocation[] {
		...
		for (const callsite of stack) {
			...
			if (
				CallsiteLocator.ignoreModulePaths.some((x) => filename.startsWith(x))
			) {
				continue;
			}
			if (
				CallsiteLocator.ignoreModulePathFragments.some((x) =>
					filename.includes(x),
				)
			) {
				continue;
