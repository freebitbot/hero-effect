...
import type WebsocketResource from "./WebsocketResource";
...
export default class Tab extends AwaitedEventTarget<IEventType> {
	...
	constructor(
		...
		async function sendToTab(pluginId: string, ...args: any[]): Promise<any> {
			return (await coreTabPromise).commandQueue.run(
				"Tab.runPluginCommand",
				pluginId,
				args,
			);
		}
	...
	public async getFrameEnvironment(
		...
		const frameMeta = await elementCoreFrame.getChildFrameEnvironment(
			awaitedPath.toJSON(),
		);
		if (!frameMeta) return null;

		const coreTab = await this.#coreTabPromise;
		return await this.#getOrCreateFrameEnvironment(
			coreTab.getCoreFrameForMeta(frameMeta),
		);
	}
	...
	public xpathSelectorAll(
		...
		return this.mainFrameEnvironment.xpathSelectorAll(
			xpath,
			orderedNodeResults,
		);
	}
	...
	public async waitForState(
		...
		const coreTab = await this.#coreTabPromise;
		return await coreTab.waitForState(state, options, {
			callstack,
			callsitePath,
		});
	}
	...
	public async triggerFlowHandlers(): Promise<{
		...
		return {
			...
			matchedFlowHandlers: coreResult.matchedFlowHandlers.map(
	...
	public async flowCommand<T = void>(
		...
		return await coreTab.runFlowCommand(
			commandFn,
			exitState,
			callsitePath,
			options,
	...
	async #getOrCreateFrameEnvironment(
		...
		const frameEnvironment = new FrameEnvironment(
			this.#hero,
			this,
			Promise.resolve(coreFrame),
		);
	...
	async #getRefreshedFrameEnvironments(): Promise<FrameEnvironment[]> {
		...
		await Promise.all(
			coreFrames.map((x) => this.#getOrCreateFrameEnvironment(x)),
		);
