...
import type { IElementInteractVerification } from "@ulixee/unblocked-specification/agent/interact/IInteractions";
...
import type { ITypeInteraction } from "../interfaces/IInteractions";
...
const NodeExtensionFns: INodeExtensionFns = {
	async $click(
		verification: IElementInteractVerification = "elementAtPath",
	): Promise<void> {
		const coreFrame = await getCoreFrame(this);
		await Interactor.run(coreFrame, [
			{ click: { element: this, verification } },
		]);
	},
	...
	async $clearInputText(): Promise<void> {
		...
		const coreFrame = await awaitedOptions.coreFrame;
		const callsitePath =
			coreFrame.coreTab.coreSession.callsiteLocator.getCurrent();
	...
	async $detach(): Promise<globalThis.Element> {
		...
		const coreFrame = await awaitedOptions.coreFrame;
		const detachedElementsRaw = await coreFrame.detachElement(
			undefined,
			awaitedPath.toJSON(),
			true,
			false,
		...
		return createSuperDocument<IAwaitedOptions>(frameAwaitedPath, {
			coreFrame: awaitedOptions.coreFrame
				.then((x) => {
					return Promise.all([
						x.coreTab,
						x.getChildFrameEnvironment(frameJsPath),
					]);
				})
...
export function extendNodes<IFunctions, IGetters>(
...
export type IDomExtensionClass =
	| ISuperElement
	| ISuperNode
	| ISuperHTMLElement
	| IElement
...
extendNodes<INodeExtensionFns, INodeExtensionGetters>(
	NodeExtensionFns,
	NodeExtensionGetters,
);
