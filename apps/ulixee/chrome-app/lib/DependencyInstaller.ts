...
`;
			...
			resolutionMessage = `You need to install the following libraries: ${[
				"",
				...missingDeps,
			].join("\n    ")}`;
		}
	...
	private async appendAptInstallNeeded(): Promise<boolean> {
		...
		const hasInstaller = await existsAsync(
			`${installPath}/install-dependencies.deb`,
		);
