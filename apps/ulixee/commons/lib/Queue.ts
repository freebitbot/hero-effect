...
import type IResolvablePromise from "../interfaces/IResolvablePromise";
...
export default class Queue<TResult = any> extends TypedEventEmitter<{
	...
	public get isActive(): boolean {
		return (
			(this.activeCount > 0 || this.queue.length > 0) && !this.stopDequeuing
		);
	}
	...
	public stop(error?: CanceledPromiseError): void {
		const canceledError =
			error ?? new CanceledPromiseError("Canceling Queue Item");
