...
export class SourceMapSupport {
	...
	static clearStackPath(stackPath: string): void {
		SourceMapSupport.stackPathsToClear.add(stackPath);
	}
	...
	static clearCache(filepath: string): void {
		SourceMapSupport.sourceMapCache[filepath] = null;
	}
	...
	static install(): void {
		...
		if (!Error[Symbol.for("source-map-support")]) {
			...
			Error.prepareStackTrace =
				SourceMapSupport.prepareStackTrace.bind(SourceMapSupport);
		}
	}

	static getSourceFile(filename: string): { path: string; content?: string } {
		const sourceMap = SourceMapSupport.retrieveSourceMap(filename);
		if (!sourceMap.map)
			return {
				path: filename,
			};
		...
		const source =
			sourceIndex >= 0 ? sourceMap.map.resolvedSources[sourceIndex] : filename;
	...
	static getOriginalSourcePosition(
		...
		if (position.source) {
			if (includeContent && "content" in position)
				(position as any).content = SourceLoader.getFileContents(
					position.source,
				);
			return position;
		}
		const sourceMap = SourceMapSupport.retrieveSourceMap(position.filename);
	...
	static retrieveSourceMap(
		...
		const cacheKey = SourceMapSupport.getCacheKey(source);
		if (SourceMapSupport.sourceMapCache[cacheKey])
			return SourceMapSupport.sourceMapCache[cacheKey];
		...
		if (sourceMapData) {
			...
			SourceMapSupport.sourceMapCache[cacheKey] = sourceMap;
			...
			if (sourceMap.map.sourcesContent) {
				...
				const content = sourceMap.map.resolvedSources.map((x) =>
					SourceLoader.getFileContents(x),
				);
			...
			// eslint-disable-next-line no-multi-assign
			SourceMapSupport.sourceMapCache[cacheKey] = {
				url: null,
				map: null,
				rawMap: null,
			};
	...
	public static getCacheKey(pathOrFileUrl): string {
		if (SourceMapSupport.cacheKeys[pathOrFileUrl])
			return SourceMapSupport.cacheKeys[pathOrFileUrl];
		...
		SourceMapSupport.cacheKeys[pathOrFileUrl] = result;
		return result;
	}
	...
	private static resolvePath(base: string, relative: string): string {
		...
		if (!SourceMapSupport.resolvedPathCache[key]) {
			...
			SourceMapSupport.resolvedPathCache[key] =
				protocol + path.resolve(basePath, relative);
		}
		return SourceMapSupport.resolvedPathCache[key];
	}
	...
	private static prepareStackTrace(
		...
		for (let i = stack.length - 1; i >= 0; i--) {
			...
			if (frame.isNative()) {
				...
				const filename =
					frame.getFileName() || (frame as any).getScriptNameOrSourceURL();
				if (filename && !filename.endsWith(".ts")) {
					const position = SourceMapSupport.getOriginalSourcePosition({
						filename,
					...
					if (position.source) {
						...
						for (const toReplace of SourceMapSupport.stackPathsToClear) {
...
function matchStyleOfPathOrUrl(
	matchStyleOf: string,
	pathOrUrl: string,
): string {
	try {
