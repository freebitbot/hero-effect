...
export default abstract class SqliteTable<T> {
	...
	protected constructor(
		...
		if (!db.readonly) {
			...
			this.insertByKeyStatement = this.db.prepare(
				this.buildInsertByKeyStatement(),
			);
		}
	}
	...
	public subscribe(callbackFn: (records: T[]) => void): void {
		...
		const pendingRecords = this.pendingInserts.map((x) =>
			this.insertToObject(x),
		);
	...
	public all(): T[] {
		const sort = this.defaultSortOrder
			? ` ORDER BY ${this.defaultSortOrder}`
			: "";
		return this.db
			.prepare(`select * from ${this.tableName}${sort}`)
			.all() as T[];
	}
	...
	private createTableStatement(): string {
		const primaryKey = this.columns.filter((x) =>
			x[2]?.includes("PRIMARY KEY"),
		);
		let primaryKeyAddon = "";
