import type { IHTMLTableElementIsolate } from "../../base/interfaces/isolate";
import type {
...
import HTMLTableElementIsolateBase, {
	type IHTMLTableElementIsolateProperties,
} from "../../base/isolate-mixins/HTMLTableElementIsolate";
...
export default class HTMLTableElementIsolate
	extends HTMLTableElementIsolateBase
	implements IHTMLTableElementIsolate
{
	public get caption(): IHTMLTableCaptionElement {
		const { awaitedPath, awaitedOptions } = getState(this);
		return createHTMLTableCaptionElement(
			awaitedPath.addProperty(this, "caption"),
			awaitedOptions,
		);
	}
	...
	public get tBodies(): ISuperHTMLCollection {
		const { awaitedPath, awaitedOptions } = getState(this);
		return createSuperHTMLCollection(
			awaitedPath.addProperty(this, "tBodies"),
			awaitedOptions,
		);
	}
	...
	public get tFoot(): IHTMLTableSectionElement {
		const { awaitedPath, awaitedOptions } = getState(this);
		return createHTMLTableSectionElement(
			awaitedPath.addProperty(this, "tFoot"),
			awaitedOptions,
		);
	}
	...
	public get tHead(): IHTMLTableSectionElement {
		const { awaitedPath, awaitedOptions } = getState(this);
		return createHTMLTableSectionElement(
			awaitedPath.addProperty(this, "tHead"),
			awaitedOptions,
		);
	}
}
