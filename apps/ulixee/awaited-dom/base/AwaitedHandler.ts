...
export default class AwaitedHandler<TClass> {
	...
	public async getProperty<T>(
		instance: TClass,
		name: string,
		hasNullDefault: boolean = false,
	): Promise<T> {
		return await AwaitedHandler.delegate?.getProperty(
			this,
			instance,
			name,
			hasNullDefault,
...
export interface IAwaitedHandlerDelegate {
	...
	setProperty<T, TClass>(
		self: AwaitedHandler<TClass>,
		instance: TClass,
		name: string,
		value: T,
	): void;
	createNodePointer<TClass>(
		self: AwaitedHandler<TClass>,
		instance: TClass,
