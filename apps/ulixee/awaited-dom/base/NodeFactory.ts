...
export default class NodeFactory<TClass> {
	...
	private readonly setState: (
		instance: TClass,
		state: Partial<IRemoteNodeProperties>,
	) => void;
	...
	public async createInstanceWithNodePointer(
		instance: TClass,
	): Promise<TClass> {
		const { awaitedOptions, awaitedPath, createInstanceName } =
			this.getState(instance);
		const state = await this.awaitedHandler.createNodePointer(instance);
		if (!state?.id) {
			return null as any;
		}
		let createNewInstance =
			NodeFactory.instanceCreatorsByName[createInstanceName];
		if (state.type) {
			const typeCreator =
				NodeFactory.instanceCreatorsByName[`create${state.type}`];
			if (typeCreator) createNewInstance = typeCreator;
		}

		const newAwaitedPath = state?.id
			? awaitedPath.withNodeId(awaitedPath.parent as any, state?.id)
			: awaitedPath;
