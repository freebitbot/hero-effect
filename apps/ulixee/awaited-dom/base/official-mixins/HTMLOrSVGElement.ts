...
import type { IHTMLOrSVGElement } from "../interfaces/official";
...
>();
export const awaitedHandler = new AwaitedHandler<IHTMLOrSVGElement>(
	"HTMLOrSVGElement",
	getState,
	setState,
);
...
export default class HTMLOrSVGElement implements IHTMLOrSVGElement {
	public get dataset(): Promise<Record<string, string>> {
		return awaitedHandler.getProperty<Record<string, string>>(
			this,
			"dataset",
			false,
		);
