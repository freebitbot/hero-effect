...
export default function ClassMixer(
	klass: Constructable,
	otherKlasses: Function[],
): any {
	...
	otherKlasses.forEach((otherKlass) => {
		if (!otherKlass) {
			throw new Error(
				`2nd argument has has an empty item: ${otherKlasses.map((m) => !!m)}`,
			);
		}
		if (!isClassable(otherKlass)) {
			throw new Error(
				`${otherKlass.name} is not a Classible mixin: ${otherKlass}`,
			);
		}
	});
