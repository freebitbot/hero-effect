...
import type { IAttr, INode } from "../interfaces/official";
...
export const awaitedHandler = new AwaitedHandler<IAttr>(
	"Attr",
	getState,
	setState,
);
export const nodeFactory = new NodeFactory<IAttr>(
	getState,
	setState,
	awaitedHandler,
);
...
export function AttrGenerator(Node: Constructable<INode>) {
	return class Attr extends Node implements IAttr, PromiseLike<IAttr> {
		...
		public then<TResult1 = IAttr, TResult2 = never>(
			onfulfilled?:
				| ((value: IAttr) => PromiseLike<TResult1> | TResult1)
				| undefined
				| null,
			onrejected?:
