...
import type { IDOMTokenList } from "../interfaces/official";
...
>();
export const awaitedHandler = new AwaitedHandler<IDOMTokenList>(
	"DOMTokenList",
	getState,
	setState,
);
export const nodeFactory = new NodeFactory<IDOMTokenList>(
	getState,
	setState,
	awaitedHandler,
);
export const awaitedIterator = new AwaitedIterator<IDOMTokenList, string>(
	getState,
	setState,
	awaitedHandler,
);
...
export function DOMTokenListGenerator() {
	return class DOMTokenList
		implements IDOMTokenList, PromiseLike<IDOMTokenList>
	{
		constructor() {
			setState(this, {
		...
		public replace(token: string, newToken: string): Promise<boolean> {
			return awaitedHandler.runMethod<boolean>(this, "replace", [
				token,
				newToken,
			]);
		}
		...
		public then<TResult1 = IDOMTokenList, TResult2 = never>(
			onfulfilled?:
				| ((value: IDOMTokenList) => PromiseLike<TResult1> | TResult1)
				| undefined
				| null,
			onrejected?:
		...
		public [Symbol.for("nodejs.util.inspect.custom")]() {
			return inspectInstanceProperties(
				this,
				DOMTokenListPropertyKeys,
				DOMTokenListConstantKeys,
			);
		}
	};
}
