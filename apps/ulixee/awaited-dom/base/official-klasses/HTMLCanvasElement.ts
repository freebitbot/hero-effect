...
>();
export const awaitedHandler = new AwaitedHandler<IHTMLCanvasElement>(
	"HTMLCanvasElement",
	getState,
	setState,
);
export const nodeFactory = new NodeFactory<IHTMLCanvasElement>(
	getState,
	setState,
	awaitedHandler,
);

export function HTMLCanvasElementGenerator(
	HTMLElement: Constructable<IHTMLElement>,
) {
	return class HTMLCanvasElement
		extends HTMLElement
		implements IHTMLCanvasElement, PromiseLike<IHTMLCanvasElement>
	{
		constructor() {
			super();
			setState(this, {
		...
		public toDataURL(type?: string, quality?: any): Promise<string> {
			return awaitedHandler.runMethod<string>(this, "toDataURL", [
				type,
				quality,
			]);
		}
		...
		public transferControlToOffscreen(): IOffscreenCanvas {
			throw new Error(
				"HTMLCanvasElement.transferControlToOffscreen not implemented",
			);
		}

		public then<TResult1 = IHTMLCanvasElement, TResult2 = never>(
			onfulfilled?:
				| ((value: IHTMLCanvasElement) => PromiseLike<TResult1> | TResult1)
				| undefined
				| null,
			onrejected?:
		...
		public [Symbol.for("nodejs.util.inspect.custom")]() {
			return inspectInstanceProperties(
				this,
				HTMLCanvasElementPropertyKeys,
				HTMLCanvasElementConstantKeys,
			);
		}
	};
}
...
export const HTMLCanvasElementPropertyKeys = [
	...HTMLElementPropertyKeys,
	"height",
	"width",
];
