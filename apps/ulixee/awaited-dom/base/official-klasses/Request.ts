...
export const { getState, setState } = StateMachine<
	...
	IRequestProperties
>();
export const awaitedHandler = new AwaitedHandler<IRequest>(
	"Request",
	getState,
	setState,
);
...
export function RequestGenerator(Body: Constructable<IBody>) {
	return class Request extends Body implements IRequest {
		...
		public get credentials(): Promise<IRequestCredentials> {
			return awaitedHandler.getProperty<IRequestCredentials>(
				this,
				"credentials",
				false,
			);
		...
		public get destination(): Promise<IRequestDestination> {
			return awaitedHandler.getProperty<IRequestDestination>(
				this,
				"destination",
				false,
			);
		...
		public get redirect(): Promise<IRequestRedirect> {
			return awaitedHandler.getProperty<IRequestRedirect>(
				this,
				"redirect",
				false,
			);
		...
		public get referrerPolicy(): Promise<IReferrerPolicy> {
			return awaitedHandler.getProperty<IReferrerPolicy>(
				this,
				"referrerPolicy",
				false,
			);
...
export const RequestPropertyKeys = [
	...BodyPropertyKeys,
	"cache",
	"credentials",
	"destination",
