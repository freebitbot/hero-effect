...
export const awaitedHandler = new AwaitedHandler<IRange>(
	"Range",
	getState,
	setState,
);
...
export function RangeGenerator(AbstractRange: Constructable<IAbstractRange>) {
	return class Range extends AbstractRange implements IRange {
		...
		public compareBoundaryPoints(
			how: number,
			sourceRange: IRange,
		): Promise<number> {
			return awaitedHandler.runMethod<number>(this, "compareBoundaryPoints", [
				how,
				sourceRange,
			]);
		}
		...
		public comparePoint(node: INodeIsolate, offset: number): Promise<number> {
			return awaitedHandler.runMethod<number>(this, "comparePoint", [
				node,
				offset,
			]);
		}
		...
		public isPointInRange(
			node: INodeIsolate,
			offset: number,
		): Promise<boolean> {
			return awaitedHandler.runMethod<boolean>(this, "isPointInRange", [
				node,
				offset,
			]);
		}
		...
		public surroundContents(newParent: INodeIsolate): Promise<void> {
			return awaitedHandler.runMethod<void>(this, "surroundContents", [
				newParent,
			]);
		}
...
export const RangePropertyKeys = [
	...AbstractRangePropertyKeys,
	"commonAncestorContainer",
];

export const RangeConstantKeys = [
	...AbstractRangeConstantKeys,
	"END_TO_END",
	"END_TO_START",
	"START_TO_END",
