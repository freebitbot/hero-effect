...
import type AwaitedHandler from "./AwaitedHandler";
import NodeFactory, { type IRemoteNodeProperties } from "./NodeFactory";
...
export default class AwaitedIterator<TClass, T> {
	...
	private readonly setState: (
		instance: TClass,
		state: Partial<IAwaitedIteratorProperties>,
	) => void;
	...
	constructor(
		...
		this.nodeFactory = new NodeFactory<TClass>(
			getState,
			setState,
			awaitedHandler,
		);
	...
	public async load(instance: TClass): Promise<T[]> {
		const nodePointerInstance =
			await this.nodeFactory.createInstanceWithNodePointer(instance);
	...
	public iterateNodePointers(instance: TClass): Iterator<T> {
		...
		if (!nodePointer.iterableItems) {
			throw new Error(
				`Please add an await to ${awaitedPath.hasNodeId ? "re-" : ""}run this iterator`,
			);
		}

		let index = 0;
		const defaultCreateChild =
			AwaitedIterator.creators[state.createIterableName as string];
		return {
