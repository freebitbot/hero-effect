...
export default class SeleniumRunners {
	...
	constructor() {
		ShutdownHandler.register(() =>
			Promise.allSettled([...this.runners].map((x) => x.stop())),
		);
	}
	...
	public async singleAssignment(
		...
		try {
			const assignment =
				await client.createSingleUserAgentIdAssignment(userAgentId);
			console.log("Running agent [%s]", userAgentId, agent);
			const driver = await BrowserStack.buildWebDriver(agent);
			if (!driver) return false;

			const runnerOptions = SeleniumRunner.getRunnerOptions(
				agent.browser,
