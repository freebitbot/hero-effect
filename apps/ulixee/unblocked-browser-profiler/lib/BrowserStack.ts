...
import type IBrowser from "@ulixee/real-user-agents/interfaces/IBrowser";
import type IOperatingSystem from "@ulixee/real-user-agents/interfaces/IOperatingSystem";
...
export default class BrowserStack {
	...
	public static async createAgent(
		...
		if (await BrowserStack.isBrowserSupported(agent)) {
			return agent;
		}
	...
	static async getCapabilities(): Promise<IBrowserstackAgent[]> {
		if (!BrowserStack.supportedCapabilities.length) {
			BrowserStack.supportedCapabilities = await Axios.get(
	...
	private static async isBrowserSupported(
		...
		return capabilities.find((x) => {
			return (
				...
				(x.browser_version === browser_version ||
					x.browser_version === `${browser_version}.0`)
			);
		});
	}
}
...
function shouldBypassSecurity({ os, os_version: osVersion }): boolean {
	...
	if (
		os === "OS X" &&
		(osVersion === "El Capitan" || osVersion === "Yosemite")
	) {
		return true;
	}
	return false;
