...
export default class BrowserProfiler {
	...
	public static get profilesDir(): string {
		return Path.join(BrowserProfiler.dataDir, "profiles");
	}

	public static get profiledDoms(): string {
		return Path.join(BrowserProfiler.dataDir, "profiled-doms");
	}

	public static get userAgentIds(): string[] {
		return Fs.readdirSync(BrowserProfiler.profilesDir).filter(
			(x) => !x.startsWith(".") && isDirectory(BrowserProfiler.userAgentDir(x)),
		);
	}

	public static userAgentDir(userAgentId: string): string {
		return Path.join(BrowserProfiler.profilesDir, userAgentId);
	}

	public static init(): void {
		const baseProfilesDir = BrowserProfiler.profilesDir;
	...
	public static loadDataFile<T>(relativePath: string): T {
		const absolutePath = Path.resolve(BrowserProfiler.dataDir, relativePath);
		return JSON.parse(Fs.readFileSync(absolutePath, "utf8"));
	}

	public static extractMetaFromUserAgentId(
		userAgentId: string,
	): IUserAgentMeta {
		return RealUserAgents.extractMetaFromUserAgentId(userAgentId);
	}

	public static getProfile<TProfile = any>(
		pluginId: string,
		userAgentId: string,
	): TProfile {
		const profileDir = `${BrowserProfiler.profilesDir}/${userAgentId}`;
		const defaultProfilePath = `${profileDir}/${pluginId}.json`;

		if (
			Fs.existsSync(defaultProfilePath) ||
			Fs.existsSync(`${defaultProfilePath}.gz`)
		) {
	...
	public static getProfiles<TProfile = any>(pluginId: string): TProfile[] {
		...
		BrowserProfiler.userAgentIds.forEach((userAgentId) => {
			const profile = BrowserProfiler.getProfile(pluginId, userAgentId);
	...
	public static cleanPluginProfiles(pluginIds: string[]): void {
		for (const userAgentId of BrowserProfiler.userAgentIds) {
			const profileDir = BrowserProfiler.userAgentDir(userAgentId);
	...
	public static findMissingPlugins(
		userAgentId: string,
		plugins: Plugin[],
	): string[] {
		const userAgentDir = BrowserProfiler.userAgentDir(userAgentId);
	...
	public static isMissingPlugins(
		...
		if (rerunPluginIds.length) {
			for (const pluginId of rerunPluginIds) {
				const doesPluginAlreadyExist = profileFiles.some((x) =>
					x.startsWith(pluginId),
				);
		...
		const expectedFileCount = plugins.reduce(
			(total, x) => x.outputFiles + total,
			0,
		);
