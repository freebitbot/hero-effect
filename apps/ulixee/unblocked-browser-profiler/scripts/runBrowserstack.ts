...
export default async function runBrowserstack(): Promise<void> {
	...
	for (const agent of RealUserAgents.all()) {
		const userAgentId = createUserAgentIdFromIds(
			agent.operatingSystemId,
			agent.browserId,
		);
		const operatingSystem = RealUserAgents.getOperatingSystem(
			agent.operatingSystemId,
		);
		const browser = RealUserAgents.getBrowser(agent.browserId);
		if (!browser) {
			console.log(
				"RealUserAgents is missing browser: ",
				agent.operatingSystemId,
				agent.browserId,
			);
			continue;
		...
		if (process.env.BROWSER_ID && !browser.id.includes(process.env.BROWSER_ID))
			continue;

		const rerunPluginIds = BrowserProfiler.findMissingPlugins(
			userAgentId,
			plugins,
		);
		...
		const browserStackAgent = await BrowserStack.createAgent(
			operatingSystem,
			browser,
		);
		...
		void queue.add(
			runners.singleAssignment.bind(runners, browserStackAgent, userAgentId, {
				rerunPluginIds,
			}),
		);
		totalCount += 1;
...
export function cleanProfiles(): void {
	const removePluginIds =
		process.argv[1]?.split(",")?.map((x) => x.trim()) ?? [];
