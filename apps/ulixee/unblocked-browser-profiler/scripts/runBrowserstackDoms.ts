...
const browserDomPlugin = getAllPlugins().find(
	(x) => x.id === "browser-dom-environment",
);
...
export default async function runBrowserstack(): Promise<void> {
	...
	for (const userAgentId of BrowserProfiler.userAgentIds) {
		...
		const operatingSystem =
			RealUserAgents.getOperatingSystem(operatingSystemId);
		...
		void queue.add(
			createSecondDomProfile.bind(this, browserStackAgent, userAgentId, domDir),
		);
		totalCount += 1;
	}

	console.log(
		`${totalCount} queued, wait for complete`,
		queue.size,
		queue.pending,
	);
	await queue.onIdle();
	...
	for (const profileFileName of await Fs.readdir(profilesDir)) {
		if (profileFileName.startsWith(browserDomPlugin.id)) {
			...
			const destPath = Path.join(
				domDir,
				profileFileName.replace(".json.gz", "--1.json.gz"),
			);
