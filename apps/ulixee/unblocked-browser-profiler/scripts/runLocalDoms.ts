...
import AssignmentsClient, {
	type IAssignment,
} from "@double-agent/runner/lib/AssignmentsClient";
...
const defaultShouldRunDockers = Boolean(
	JSON.parse(process.env.RUN_DOCKERS ?? "false"),
);
const defaultShouldRunLocal = Boolean(
	JSON.parse(process.env.RUN_LOCAL ?? "true"),
);
...
export default async function runLocalDoms(
	...
	if (shouldCommit) {
		execSync(
			`git commit -am "chore(local-doms): generate profiles" && git push`,
			{
...
async function createAssignment(
	...
	return await new AssignmentsClient(tag).createSingleUserAgentIdAssignment(
		userAgentId,
	);
}
...
async function waitUntilDomAssignmentFinishes(
