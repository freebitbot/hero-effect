	...
	implements IFrameNavigations
{
	...
	public didGotoUrl(url: string): boolean {
		return this.history.some(
			(x) => x.requestedUrl === url && x.navigationReason === "goto",
		);
	}
	...
	public hasLoadStatus(status: ILoadStatus): boolean {
		...
		if (status === LoadStatus.JavascriptReady) {
			return (
				statuses.has(LoadStatus.AllContentLoaded) ||
				this.top.finalUrl === "about:blank"
			);
		}

		if (status === LoadStatus.DomContentLoaded) {
			return (
				statuses.has(LoadStatus.DomContentLoaded) ||
				statuses.has(LoadStatus.AllContentLoaded)
			);
		}
	...
	public onNavigationRequested(
		...
		if (reason === "inPage") {
			...
			if (lastHttpResponse) {
				...
				nextTop.resourceIdResolvable.resolve(
					lastHttpResponse.resourceIdResolvable.promise,
				);
			} else {
				nextTop.statusChanges.set(
					LoadStatus.AllContentLoaded,
					nextTop.initiatedTime,
				);
			...
			if (
				nextTop.requestedUrl?.includes("#") &&
				this.lastHttpNavigationRequest
			) {
				const baseUrl = (
					this.lastHttpNavigationRequest.finalUrl ??
					this.lastHttpNavigationRequest.requestedUrl
				).split("#")[0];
	...
	public onHttpRequested(
		...
		if (redirectedFromUrl) {
			const redirectedNavigation = this.recordRedirect(
				redirectedFromUrl,
				url,
				loaderId,
			);
	...
	public onDomPaintEvent(
		...
		if (event === "LargestContentfulPaint") {
			...
			const contentPaintHistory = this.findMostRecentHistory(
				(x) => x.finalUrl === url,
			);
			if (contentPaintHistory?.statusChanges?.has(LoadStatus.JavascriptReady)) {
				...
				setTimeout(
					() => this.onDomPaintEvent(event, url, timestamp, true),
					100,
				);
			}
		}
	...
	public adjustInPageLocationChangeTime(
	...
	public pendingResourceId(
		...
		const match = this.findMostRecentHistory((x) => {
			...
			if (
				browserRequestId &&
				x.browserRequestId &&
				browserRequestId !== x.browserRequestId
			)
				return false;
			return true;
		});
		...
		if (
			(match.finalUrl && finalUrl === match.finalUrl) ||
	...
	public getLastLoadedNavigation(): INavigation {
		...
		for (let i = this.history.length - 1; i >= 0; i -= 1) {
			...
			if (
				!navigation.finalUrl ||
				!navigation.statusChanges.has(LoadStatus.HttpResponded)
			)
				continue;
	...
	public findMostRecentHistory(
		callback: (history: INavigation) => boolean,
	): INavigation {
		for (let i = this.history.length - 1; i >= 0; i -= 1) {
	...
	private checkStoredNavigationReason(
		...
		if (
			...
			(!navigation.navigationReason ||
				navigation.navigationReason === "newFrame")
		) {
	...
	private changeNavigationStatus(
		...
		if (navigation.statusChanges.has(newStatus)) {
			if (
				statusChangeDate &&
				statusChangeDate < navigation.statusChanges.get(newStatus)
			) {
	...
	private recordStatusChange(
		...
		this.emit("status-change", {
			...
			// @ts-expect-error - Typescript refuses to recognize this function
