...
export default class ConsoleMessage {
	...
	static create(
		...
		const location = `//#${context ?? "nocontext"}${ConsoleMessage.printStackTrace(stackTrace)}`;
	...
	static exceptionToError(exceptionDetails: ExceptionDetails): Error {
		...
		if (exceptionDetails.exception) {
			...
			error.stack = ConsoleMessage.printStackTrace(exceptionDetails.stackTrace);
		}
		return error;
	}
...
function stringifyRemoteObject(
