...
export default class BrowserProcess extends TypedEventEmitter<{ close: void }> {
	...
	constructor(
		...
		this.transport.connectedPromise
			...
			.catch((err) =>
				setTimeout(() => this.isProcessFunctionalPromise.reject(err), 1.1e3),
			);
	...
	private launch(): ChildProcess {
		...
		for (const arg of launchArguments) {
			if (arg.startsWith(disablePrefix)) {
				...
				features.forEach(
					(feature) => feature.trim() && disabledFeatures.add(feature.trim()),
				);
			} else if (arg.startsWith(enablePrefix)) {
				const features = arg.substring(enablePrefix.length).split(",");
				features.forEach(
					(feature) => feature.trim() && enabledFeatures.add(feature.trim()),
				);
			} else {
				otherArgs.push(arg);
			}
		...
		child.on("error", (e) => {
			if (!this.isProcessFunctionalPromise) {
				this.isProcessFunctionalPromise.reject(
					new Error(`Failed to launch browser: ${e}`),
				);
			}
		});
		return child;
	...
	private gracefulCloseBrowser(): void {
		try {
			...
			if (this.transport && !this.transport.isClosed) {
				this.transport.send(
					JSON.stringify({ method: "Browser.close", id: -1 }),
				);
		...
		try {
			if (!launchedProcess.killed && !this.processKilled) {
				const closed = new Promise<void>((resolve) =>
					launchedProcess.once("exit", resolve),
				);
				if (process.platform === "win32") {
					childProcess.execSync(
						`taskkill /pid ${launchedProcess.pid} /T /F 2> nul`,
					);
				} else {
