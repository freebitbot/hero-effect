...
export default class DevtoolsSessionLogger extends TypedEventEmitter<IDevtoolsLogEvents> {
	...
	public static defaultTruncateParams: DevtoolsSessionLogger["truncateParams"] =
		new Map([
			["Fetch.fulfillRequest", [{ maxLength: 50, path: "body" }]],
			["Page.screencastFrame", [{ maxLength: 0, path: "data" }]],
			[
				"Page.addScriptToEvaluateOnNewDocument",
				[{ maxLength: 50, path: "source" }],
			],
	...
	public readonly truncateParams: Map<
		string,
	...
	private onEventSend(
		...
		if (details.sessionType === "browser") {
			...
			if (
				initiator &&
				initiator instanceof BrowserContext &&
				initiator !== this.browserContext
			) {
				return;
			}
	...
	private onEventReceive(
		...
		if (details.sessionType === "browser") {
			...
			if (
				"id" in event &&
				!this.browserContextInitiatedMessageIds.has(event.id)
			)
				return;

			if ("params" in event) {
				const isOtherContext = this.isOtherBrowserContextTarget(
	...
	private filterAndSendMessage(
		event: IDevtoolsLogEvents["devtools-message"],
	): void {
		...
		if (
			...
			params.message.text.startsWith("hero:")
		) {
			return;
		}
			...
			if (tuncaters) {
				...
				for (const truncPath of tuncaters) {
					...
					if (value && typeof value === "string" && value.length > maxLength) {
						eventParent[propName] =
							`${value.substr(0, maxLength)}... [truncated ${
