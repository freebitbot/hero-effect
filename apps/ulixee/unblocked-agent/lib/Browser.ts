...
import type { IBrowserContextHooks } from "@ulixee/unblocked-specification/agent/hooks/IBrowserHooks";
import type {
	IBrowserHooks,
	IHooksProvider,
...
import BrowserContext, {
	type IBrowserContextCreateOptions,
} from "./BrowserContext";
...
export default class Browser
	extends TypedEventEmitter<IBrowserEvents>
	implements IBrowser
{
	...
	public async launch(): Promise<Browser> {
		...
		try {
			...
			const processError = await this.process.isProcessFunctionalPromise.catch(
				(error) => error,
			);
	...
	public async newContext(
		...
		if (!this.launchPromise)
			throw new CanceledPromiseError("This Browser has been shut down");
		if (!this.isLaunchStarted)
	...
	public async connectToPage(
		...
		await this.devtoolsSession.send("Target.attachToTarget", {
			targetId,
			flatten: true,
		});
	...
	public async close(): Promise<void | Error> {
		...
		try {
			...
			this.isShuttingDown = new Promise<Error | void>(async (resolve) => {
				try {
					if (this.launchPromise) {
						const err = await this.launchPromise.catch(
	...
	protected bindDevtoolsEvents(): void {
		...
		this.devtoolsSession.on(
			...
			this.onTargetCreated.bind(this),
		);
		this.devtoolsSession.on(
			"Target.targetInfoChanged",
			this.onTargetInfoChanged.bind(this),
		);
		this.devtoolsSession.on(
	...
	private applyDefaultLaunchArgs(options: IBrowserUserConfig): void {
		...
		if (
			...
			!launchArgs.some((x) => x.startsWith("--proxy-server"))
		) {
			launchArgs.push(
	...
	private onAttachedToTarget(
		...
		if (targetInfo.type === "shared_worker") {
			...
			const context = this.getBrowserContext(targetInfo.browserContextId);
			context
				?.onSharedWorkerAttached(devtoolsSession, targetInfo)
				.catch(() => null);
		}
		...
		if (isDevtoolsPanel) {
			...
			this.preferencesInterceptor
				?.installOnConnect(devtoolsSession)
				.catch(() => null);
			void this.hooks
