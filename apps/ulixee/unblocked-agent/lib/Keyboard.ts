...
import type { IKeyboard } from "@ulixee/unblocked-specification/agent/interact/IInput";
import type { IKeyboardKey } from "@ulixee/unblocked-specification/agent/interact/IKeyboardLayoutUS";
import type DevtoolsSession from "./DevtoolsSession";
import { type IKeyDefinition, keyDefinitions } from "./KeyboardLayoutUS";

type KeyDescription = Required<
	Pick<IKeyDefinition, "key" | "text" | "code" | "location">
> & {
	keyCode: number;
};
...
export class Keyboard implements IKeyboard {
	...
	private macCommandsForCode(code: string): string[] {
		const parts = ["Shift", "Control", "Alt", "Meta"].filter((x) =>
			this.isModifierActive(x),
		);
		parts.push(code);
		...
		const commandShortcuts =
			process.platform === "darwin"
				? macEditingCommands
				: otherOsEditingCommands;
		let commands =
			sharedEditingCommands[shortcut] ?? commandShortcuts[shortcut] ?? [];
...
const sharedEditingCommands: { [key: string]: string | string[] } = {
...
const otherOsEditingCommands: { [key: string]: string | string[] } = {
