...
import DomStorageTracker, { type IDomStorageEvents } from "./DomStorageTracker";
...
export default class Page
	extends TypedEventEmitter<IPageLevelEvents>
	implements IPage
{
	...
	constructor(
		...
		this.installJsPathIntoIsolatedContext =
			pageOptions?.installJsPathIntoDefaultContext !== true;
		...
		this.events.on(
			this.networkManager,
			"websocket-frame",
			this.onWebsocketFrame.bind(this),
		);
	...
	async setNetworkRequestInterceptor(
		...
		return await this.networkManager.setNetworkInterceptor(
			networkRequestsFn,
			true,
		);
	}
	...
	type(text: string): Promise<void> {
		return this.mainFrame.interact([
			{ command: "type", keyboardCommands: [{ string: text }] },
		]);
	}
	...
	): Promise<{ loaderId: string }> {
		const navigationResponse = await this.devtoolsSession.send(
			"Page.navigate",
			{
				url,
	...
	async goto(
		...
		if (options?.timeoutMs) {
			this.browserContext.resources.setNavigationConnectTimeoutMs(
				formattedUrl,
				options.timeoutMs,
			);
		}
	...
	waitForLoad(
		...
		return this.mainFrame.waitForLoad({
			...(options ?? {}),
			loadStatus: status,
		});
	}
	...
	async reload(options?: { timeoutMs?: number }): Promise<IResourceMeta> {
		...
		await timer.waitForPromise(
			this.devtoolsSession.send("Page.reload"),
			timeoutMessage,
		);
	...
	async screenshot(options: IScreenshotOptions): Promise<Buffer> {
		...
		if (options.fullPage) {
			const layoutMetrics = await this.devtoolsSession.send(
	...
	onWorkerAttached(
		...
		this.browserContext.beforeWorkerAttached(
			devtoolsSession,
			targetId,
			this.targetId,
		);
	...
	onTargetKilled(errorCode: number): void {
		this.emit("crashed", {
			error: new Error(
				`Page crashed - killed by Chrome with code ${errorCode}`,
			),
			fatal: true,
		});
		this.didClose();
	...
	didClose(closeError?: Error): void {
		...
		try {
			...
			Timer.expireAll(
				this.waitTimeouts,
				closeError ?? new CanceledPromiseError(cancelMessage),
			);
	...
	private async navigateToHistory(delta: number): Promise<string> {
		const history = await this.devtoolsSession.send(
			"Page.getNavigationHistory",
		);
		const entry = history.entries[history.currentIndex + delta];
		if (!entry) return null;
		await Promise.all([
			this.devtoolsSession.send("Page.navigateToHistoryEntry", {
				entryId: entry.id,
			}),
	...
	private async initialize(): Promise<void> {
		...
		this.events.on(
			this.mainFrame,
			"frame-navigated",
			this.onMainFrameNavigated.bind(this),
		);
		...
		if (this.opener && this.opener.popupInitializeFn) {
			...
			if (this.mainFrame.isDefaultUrl) {
				...
				await this.mainFrame
					.waitOn("frame-loader-created", null, 2e3)
					.catch(() => null);
	...
	private onAttachedToTarget(
		...
		const devtoolsSession =
			this.devtoolsSession.connection.getSession(sessionId);
		if (targetInfo.type === "iframe") {
			this.browserContext.onIframeAttached(
				devtoolsSession,
				targetInfo,
				this.targetId,
			);
			return this.framesManager.onFrameTargetAttached(
				devtoolsSession,
				targetInfo,
			);
		}
	...
	private onMainFrameNavigated(
		event: Frame["EventTypes"]["frame-navigated"],
	): void {
	...
	private onWebsocketFrame(event: IPageEvents["websocket-frame"]): void {
		const resourceId =
			this.browserContext.resources.getBrowserRequestLatestResource(
