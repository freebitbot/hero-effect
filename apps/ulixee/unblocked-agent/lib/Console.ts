...
export class Console extends TypedEventEmitter<IConsoleEvents> {
	...
	private async handleConsoleMessage(
		msgAdded: Protocol.Console.MessageAddedEvent,
	): Promise<void> {
		if (
			msgAdded.message.source !== "console-api" ||
			msgAdded.message.level !== "debug"
		)
			return;
...
function injectedScript(): void {
	...
	if (scheme === "data") {
		...
		console.info(
			"Using fetch with data:// scheme, http:// not supported inside chrome:// urls, ignore fetch errors including data:// scheme",
		);
	}
