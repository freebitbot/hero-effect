...
export default class DomStorageTracker extends TypedEventEmitter<IDomStorageEvents> {
	...
	constructor(
		...
		if (
			session
				.listeners("DOMStorage.domStorageItemAdded")
				.includes(this.onDomStorageAdded)
		) {
			return;
		}
		...
		this.events.on(
			...
			this.onIndexedDBListUpdated.bind(this),
		);
		this.events.on(
			session,
	...
	public track(securityOrigin: string): void {
		...
		if (
			!securityOrigin ||
			securityOrigin === "null" ||
			this.trackedOrigins.has(securityOrigin)
		)
			return;
	...
	> {
		...
		for (const origin of this.trackedOrigins) {
			...
			record.databaseNames = record.storageForOrigin.indexedDB.map(
				(x) => x.name,
			);
			result.push(record);

			try {
				record.frame = this.page.frames.find(
					(x) => x.securityOrigin === origin && x.isAttached,
				);
	...
	private onDomStorageRemoved(
		...
		const list = isLocalStorage
			? originStorage.localStorage
			: originStorage.sessionStorage;
	...
	private async onIndexedDBListUpdated(
		...
		try {
			const dbs = await this.devtoolsSession.send(
				"IndexedDB.requestDatabaseNames",
				{
					securityOrigin,
				},
	...
	private async onIndexedDBContentUpdated(
		...
		try {
			...
			const objectStore = db.objectStores.find(
				(x) => x.name === objectStoreName,
			);

			const metadata = await this.devtoolsSession.send(
			...
			const originStorage = this.storageForOrigin(securityOrigin);
			const dbIndex = originStorage.indexedDB.findIndex(
				(x) => x.name === databaseName,
			);
			if (dbIndex === -1) {
				originStorage.indexedDB.push(db);
			} else {
