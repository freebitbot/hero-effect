...
import type ICertificateGenerator from "@ulixee/unblocked-agent-mitm/interfaces/ICertificateGenerator";
import type { ICertificateStore } from "@ulixee/unblocked-agent-mitm/interfaces/ICertificateGenerator";
...
export default class Pool extends TypedEventEmitter<{
	...
	public async getBrowser(
		...
		return await this.browserCreationQueue.run(async () => {
			...
			for (const existingBrowser of this.browsersById.values()) {
				...
				if (
					agents < this.maxConcurrentAgentsPerBrowser &&
					existingBrowser.isEqualEngine(engine)
				) {
			...
			this.events.once(browser, "close", () =>
				this.onBrowserClosed(browser.id, contextEvent),
			);
	...
	public async close(): Promise<void> {
		...
		try {
			...
			this.browserCreationQueue.stop(
				new CanceledPromiseError("Browser pool shutting down"),
			);
	...
	private watchForContextPagesClosed(
		event: Browser["EventTypes"]["new-context"],
	): void {
		...
		this.events.once(browserContext, "close", () =>
			this.events.off(registeredEvent),
		);
	}
