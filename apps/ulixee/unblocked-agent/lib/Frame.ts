...
import type IJsPath from "@ulixee/js-path/interfaces/IJsPath";
import type {
	IFrame,
	IFrameEvents,
	ILifecycleEvents,
...
import type { NavigationReason } from "@ulixee/unblocked-specification/agent/browser/NavigationReason";
import type { IInteractHooks } from "@ulixee/unblocked-specification/agent/hooks/IHooks";
import type {
...
export default class Frame
	extends TypedEventEmitter<IFrameEvents>
	implements IFrame
{
	...
	public readonly pendingNewDocumentScripts: {
		script: string;
		isolated: boolean;
	}[] = [];
	...
	public async evaluate<T>(
		...
		if (this.page.activeDialog) {
			throw new Error(
				"Cannot run frame.evaluate while an active dialog is present!!",
			);
		}
		if (this.closedWithError) throw this.closedWithError;
		if (!this.parentId) {
		...
		const isolateFromWebPageEnvironment =
			options?.isolateFromWebPageEnvironment ?? false;
		...
		try {
			...
			if (isolateFromWebPageEnvironment || this !== this.page?.mainFrame) {
				...
				if (!contextId) {
					const notFound: any = new Error(
						"Could not find a valid context for this request",
					);
			...
			const result: Protocol.Runtime.EvaluateResponse =
			...
			if (isNotFoundError) {
				...
				const activeContextId = await this.waitForContextId(
				...
				throw new CanceledPromiseError(
					"The page context to evaluate javascript was not found",
				);
			}
			throw err;
		}
	...
	async waitForLoad(
		...
		if (this.isOopif() && !this.url) {
			await new Promise((resolve) =>
				this.events.once(this, "frame-navigated", resolve),
			);
		}
	...
	public click(
		...
		return this.interact([
			{ command: "click", mousePosition: jsPath, verification },
		]);
	}
	...
	public async interact(
		...
		try {
			...
			frameCancelEvent = this.events.once(
				this,
				"frame-navigated",
				cancelOnNavigate,
			);
	...
	public async waitForScrollStop(
		timeoutMs?: number,
	...
	public async getNodePointerId(devtoolsObjectId: string): Promise<number> {
		return await this.evaluateOnNode<number>(
			devtoolsObjectId,
  ...
  return retVal;
	...
	public async evaluateOnNode<T>(
		devtoolsObjectId: string,
		expression: string,
	): Promise<T> {
		if (this.closedWithError) throw this.closedWithError;
		try {
	...
	public async getFrameElementDevtoolsNodeId(): Promise<string> {
		try {
			...
			this.frameElementDevtoolsNodeId = this.parentFrame.devtoolsSession
				...
				.then((owner) =>
					this.parentFrame.resolveDevtoolsNodeId(owner.backendNodeId, true),
				);
	...
	public async resolveDevtoolsNodeId(
		...
		const result = await this.devtoolsSession.send(
			...
			{
				backendNodeId,
				executionContextId: await this.waitForContextId(
					resolveInIsolatedContext,
				),
			},
			this,
		);
	...
	public async trackBackendNodeAsNodePointer(
		backendNodeId: number,
	): Promise<number> {
	...
	public onAttached(internalFrame: Protocol.Page.Frame): void {
		...
		if (this.url || internalFrame.unreachableUrl) {
			...
			this.navigationLoadersById[internalFrame.loaderId].setNavigationResult(
				internalFrame.url,
			);
		}
	}
	...
	public onNavigated(
		...
		const loader =
			this.navigationLoadersById[frame.loaderId] ?? this.activeLoader;
		...
		if (this.isDefaultUrl && this.parentId && !this.navigations.top) {
			const top = this.navigations.onNavigationRequested(
				"newFrame",
				this.url,
				0,
				loader.id,
	...
	public onNavigatedWithinDocument(url: string): void {
		...
		const isDomLoaded =
			isForActiveDomain && this.activeLoader?.lifecycle?.DOMContentLoaded;
	...
	public async waitForDefaultLoader(): Promise<void> {
		const hasLoaderError =
			await this.navigationLoadersById[this.defaultLoaderId]
				?.navigationResolver;
		if (hasLoaderError instanceof Error) throw hasLoaderError;
		await this.page.isReady;
	}

	public async waitForNavigationLoader(
		loaderId?: string,
		timeoutMs?: number,
	): Promise<void> {
		if (!loaderId) {
			...
			if (loaderId === this.defaultLoaderId) {
				...
				const frameLoader = await this.waitOn(
		...
		const hasLoaderError =
			await this.navigationLoadersById[loaderId]?.navigationResolver;
		if (hasLoaderError instanceof Error) throw hasLoaderError;
	}

	public onLifecycleEvent(
		name: string,
		timestamp?: number,
		pageLoaderId?: string,
	): void {
		...
		if (name === "init" && pageLoaderId) {
			...
			if (
				...
				this.activeLoader.setNavigationResult(
					new CanceledPromiseError("net::ERR_ABORTED"),
				);
			}
		...
		if (loaderId !== this.activeLoaderId) {
			...
			for (const [historicalLoaderId, loader] of Object.entries(
				...
				if (
					checkLoaderForInPage &&
					historicalLoaderId.startsWith(InPageNavigationLoaderPrefix)
				) {
		...
		if (loaderId !== this.defaultLoaderId) {
			this.triggerLifecycleEvent(
				name,
				this.navigationLoadersById[loaderId],
				timestamp,
			);
		}
	}
	...
	private async waitForContextId(
	...
	private updateUrl(): void {
		if (this.internalFrame.url) {
			this.url =
				this.internalFrame.url + (this.internalFrame.urlFragment ?? "");
