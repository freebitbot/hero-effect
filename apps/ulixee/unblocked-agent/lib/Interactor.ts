...
import type IRect from "@ulixee/unblocked-specification/agent/browser/IRect";
...
import type {
	IKeyboard,
	IMouse,
} from "@ulixee/unblocked-specification/agent/interact/IInput";
import {
...
import type { JsPath } from "./JsPath";
...
export default class Interactor implements IInteractionsHelper {
	...
	public play(
		...
		resolvablePromise: IResolvablePromise<any>,
	): void {
		...
		switch (interactionStep.command) {
			...
			case InteractionCommand.scroll: {
				...
				if (isMousePositionXY(interactionStep.mousePosition) === false) {
					...
					const isRectVisible = this.isRectInViewport(
						interactRect,
						this.viewportSize,
						50,
					);
					...
					[scrollToX, scrollToY] =
						interactionStep.mousePosition as IMousePositionXY;
				}

				const maxX =
					scrollOffset.width - this.viewportSize.width - scrollOffset.x;
				const maxY =
					scrollOffset.height - this.viewportSize.height - scrollOffset.y;
			...
			case InteractionCommand.doubleclick: {
				const { delayMillis, mouseButton, command, mouseResultVerifier } =
					interactionStep;
				...
				if (
					...
					interactionStep.simulateOptionClickOnNodeId
				) {
					await this.jsPath.simulateOptionClick([
						interactionStep.simulateOptionClickOnNodeId,
					]);
					break;
				}

				const [x, y] = await this.getMousePositionXY(
					interactionStep,
					true,
					interactRect,
				);
	...
	private async getWindowOffset(): Promise<IWindowOffset> {
		...
		this.viewportSize = {
			width: windowOffset.innerWidth,
			height: windowOffset.innerHeight,
		};
		return windowOffset;
	}
	...
	private async injectScrollToPositions(
		...
		for (const group of interactions) {
			...
			for (const step of group) {
				if (
					commandsNeedingScroll.has(InteractionCommand[step.command]) &&
					step.mousePosition
				) {
