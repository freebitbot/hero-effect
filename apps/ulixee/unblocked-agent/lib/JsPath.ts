...
import type IJsPathFunctions from "@ulixee/unblocked-specification/agent/browser/IJsPathFunctions";
import {
	getClientRectFnName,
...
export class JsPath implements IJsPathFunctions {
	...
	public async exec<T>(
		...
		await this.frame.navigationsObserver.waitForLoad(
	...
	public async reloadJsPath<T>(
		...
		if (
			typeof jsPath[0] === "number" &&
			!isMousePositionXY(jsPath.slice(0, 2))
		) {
	...
	private async runJsPath<T>(
		...
		if (result.pathError) {
			throw new InjectedScriptError(
				result.pathError.error,
				result.pathError.pathState,
			);
	...
	private async runInjectedScriptFn<T>(
		fnName: string,
		...args: ISerializable[]
	): Promise<T> {
