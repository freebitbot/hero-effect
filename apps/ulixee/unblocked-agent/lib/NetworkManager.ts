...
export default class NetworkManager extends TypedEventEmitter<IBrowserNetworkEvents> {
	...
	private readonly requestPublishingById = new Map<
		string,
		IResourcePublishing
	>();
	...
	private mockNetworkRequests?: (
		...
		Protocol.Fetch.FulfillRequestRequest | Protocol.Fetch.ContinueRequestRequest
	>;
	...
	constructor(
		...
		this.events.on(
			...
			this.onWebsocketHandshake,
		);
		this.events.on(
			session,
			"Network.webSocketCreated",
			this.onWebSocketCreated.bind(this),
		);
		this.events.on(
			session,
		...
		this.events.on(
			...
			this.onWebsocketFrame.bind(this, false),
		);

		this.events.on(
			session,
			"Network.requestWillBeSent",
			this.onNetworkRequestWillBeSent,
		);
		this.events.on(
			session,
		...
		this.events.on(
			...
			this.onNetworkRequestServedFromCache,
		);
	}

	public override emit<
		K extends
			| (keyof IBrowserNetworkEvents & string)
			| (keyof IBrowserNetworkEvents & symbol),
	...
	private async onRequestPaused(
		...
		try {
			...
			if (
				networkRequest.request.url.includes(
					`hero.localhost/?secretKey=${this.secretKey}`,
				)
			) {
		...
		if (
			networkRequest.networkId &&
			!this.requestsById.has(networkRequest.networkId)
		) {
	...
	private onNetworkRequestWillBeSent(
		...
		if (!this.monotonicOffsetTime)
			this.monotonicOffsetTime =
				networkRequest.wallTime - networkRequest.timestamp;
		const redirectedFromUrl = networkRequest.redirectResponse?.url;

		const isNavigation =
			networkRequest.requestId === networkRequest.loaderId &&
			networkRequest.type === "Document";
		if (isNavigation) {
			this.navigationRequestIdsToLoaderId.set(
				networkRequest.requestId,
				networkRequest.loaderId,
			);
		}
		let resource: IBrowserResourceRequest;
		try {
		...
		const publishing = this.getPublishingForRequestId(
		...
		const isNewRedirect =
			redirectedFromUrl && existing && existing.url !== resource.url;

		// NOTE: same requestId will be used in devtools for redirected resources
		if (existing) {
			if (isNewRedirect) {
				const existingRedirects =
					this.redirectsById.get(resource.browserRequestId) ?? [];
				...
				this.redirectsById.set(resource.browserRequestId, [
					...existingRedirects,
					existing,
				]);
	...
	private onNetworkRequestWillBeSentExtraInfo(
		...
		const hasNetworkRequest =
			this.requestPublishingById.get(requestId)?.hasRequestWillBeSentEvent ===
			true;
	...
	private doEmitResourceRequested(browserRequestId: string): boolean {
		...
		const event = <IBrowserNetworkEvents["resource-will-be-requested"]>{
			...
			isDocumentNavigation:
				this.navigationRequestIdsToLoaderId.has(browserRequestId),
	...
	private onNetworkResponseReceived(event: ResponseReceivedEvent): void {
		...
		if (resource) {
			...
			if (
				!this.requestPublishingById.get(requestId)?.isPublished &&
				resource.url?.href
			) {
	...
	private onNetworkRequestServedFromCache(
		...
		if (resource) {
			...
			setTimeout(
				() => this.emitLoaded(requestId, resource.requestTime),
				500,
			).unref();
		}
	}
	...
	private onWebsocketFrame(
		...
		const message =
			opcode === 1 ? payloadData : Buffer.from(payloadData, "base64");
