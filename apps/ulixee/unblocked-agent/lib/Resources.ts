...
import type RequestSession from "@ulixee/unblocked-agent-mitm/handlers/RequestSession";
import type {
...
import type IBrowserRequestMatcher from "@ulixee/unblocked-agent-mitm/interfaces/IBrowserRequestMatcher";
import type IMitmRequestContext from "@ulixee/unblocked-agent-mitm/interfaces/IMitmRequestContext";
...
{
	public readonly browserRequestIdToResources = new Map<
		string,
		IResourceMeta[]
	>();
	...
	public onWebsocketHandshake(
		tabId: number,
		event: IPageEvents["websocket-handshake"],
	): void {
	...
	public async beforeHttpResponse(
		resource: IHttpResourceLoadDetails,
	): Promise<any> {
		if (HeadersHandler.isWorkerDest(resource, "shared")) {
			const worker = this.browserContext.workersById.get(
	...
	public getBrowserRequestLatestResource(
	...
	public trackBrowserRequestToResourceId(
		browserRequestId: string,
		...
		if (!resources) {
			resources = this.browserRequestIdToResources
				.set(browserRequestId, [])
	...
	public onBrowserWillRequest(
		...
		if (
			...
			const requestTimeDiff = Math.abs(
				resource.requestTime - pendingRequest.requestTime,
			);
	...
	public onBrowserDidRequest(
		...
		if (
			mitmResourceNeedsResolve &&
			!mitmResourceNeedsResolve.browserRequestedPromise.isResolved
		) {
	...
	public onBrowserResourceLoaded(
		...
		if (!this.isCollecting) return;
		const knownResource = this.getBrowserRequestLatestResource(
			resource.browserRequestId,
		);
		if (knownResource) {
	...
	public onBrowserRequestFailed(
		...
		resourceId ??= this.getBrowserRequestLatestResource(
			resource.browserRequestId,
		)?.id;
		...
		return this.emitBrowserRequestFailed(
			tabId,
			MitmRequestContext.toEmittedResource({
				id: resourceId,
				...resource,
			} as any),
			loadError,
	...
	public resolveBrowserRequest(mitmResource: IHttpResourceLoadDetails): void {
		const pendingBrowserRequest = this.findMatchingRequest(
			mitmResource,
			"noMitmResourceId",
		);
		if (
			pendingBrowserRequest &&
			!pendingBrowserRequest.browserRequestedPromise.isResolved
		) {
	...
	public determineResourceType(mitmResource: IHttpResourceLoadDetails): void {
		...
		if (
			...
			(HeadersHandler.getRequestHeader(mitmResource, "sec-fetch-mode") ===
				"navigate" &&
				HeadersHandler.getRequestHeader(mitmResource, "sec-fetch-dest") ===
					"empty")
		) {
			pendingBrowserRequest.browserRequestedPromise.resolve(null);
		}

		mitmResource.browserHasRequested =
			pendingBrowserRequest.browserRequestedPromise.promise
				.then(() => {
					// eslint-disable-next-line promise/always-return
					if (!pendingBrowserRequest?.browserRequestId) return;
					mitmResource.resourceType = pendingBrowserRequest.resourceType;
					mitmResource.browserRequestId =
	...
	protected onMitmResponse(event: IRequestSessionResponseEvent): void {
		...
		const page = this.browserContext.pagesByTabId.get(
			resource.tabId ?? defaultTabId,
		);
		page?.framesManager.checkForResolvedNavigation(
	...
	private emitBrowserRequestFailed(
		...
		try {
			const convertedMeta = this.resourceEventToMeta(
				tabId,
				resourceFailedEvent,
			);
	...
	private createPendingResource(
		request: Pick<
			...
			"url" | "method" | "requestTime" | "requestHeaders"
		>,
	): IMitmRequestPendingBrowserRequest {
	...
	private findMatchingRequest(
		...
		const secDest = HeadersHandler.getRequestHeader(
			resourceToMatch,
			"sec-fetch-dest",
		);
		if (secDest) {
		...
		// if we have sec-fetch-dest headers, make sure they match
		const secSite = HeadersHandler.getRequestHeader(
			resourceToMatch,
			"sec-fetch-site",
		);
		if (secSite) {
	...
	private matchesMitmError(
		...
		for (const entry of errorsMatchingUrl) {
			...
			const isMatch =
				...
				Math.abs(request.timestamp - requestTime) < 500;
			if (!isMatch) continue;
	...
	private resourceEventToMeta(
		...
		if (
			response?.statusCode ||
			response?.browserServedFromCache ||
			response?.browserLoadFailure
		) {
	...
	private recordCookies(
		tabId: number,
		responseEvent: IRequestSessionResponseEvent,
	): void {
		...
		for (const cookieHeader of setCookie) {
			...
			if (
				cookie.domain &&
				cookieHeader.toLowerCase().includes(`domain=.${domain}`)
			) {
				domain = `.${domain}`;
			}
	...
	public static translateResourceError(
		resource: IBrowserResourceRequest,
	): Error {
...
function getHeaderDetails(
	httpResourceLoad: Pick<IHttpResourceLoadDetails, "requestHeaders">,
): {
	origin: string;
	referer: string;
	secFetchDest: string;
	...
	const origin = HeadersHandler.getRequestHeader<string>(
		httpResourceLoad,
		"origin",
	);
	const referer = HeadersHandler.getRequestHeader<string>(
		httpResourceLoad,
		"referer",
	);
	const secFetchDest = HeadersHandler.getRequestHeader<string>(
		httpResourceLoad,
		"sec-fetch-dest",
	);
	const secFetchSite = HeadersHandler.getRequestHeader<string>(
		httpResourceLoad,
		"sec-fetch-site",
	);
