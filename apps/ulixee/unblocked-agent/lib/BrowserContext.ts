...
import type { IBrowserContextEvents } from "@ulixee/unblocked-specification/agent/browser/IBrowserContext";
...
import type FrameOutOfProcess from "./FrameOutOfProcess";
import Page, { type IPageCreateOptions } from "./Page";
	...
	implements IBrowserContext
{
	...
	async newPage(options?: IPageCreateOptions): Promise<Page> {
		...
		const { targetId } = await this.sendWithBrowserDevtoolsSession(
			"Target.createTarget",
			{
				url: "about:blank",
		...
		const idx = this.creatingTargetPromises.indexOf(
			createTargetPromise.promise,
		);
	...
	async onPageAttached(
		...
		let opener = targetInfo.openerId
			? this.pagesById.get(targetInfo.openerId) || null
			: null;
	...
	async onSharedWorkerAttached(
		...
		const page: Page =
			...
			this.pagesById.values().next().value;
		...
		try {
			...
			if (this.browser.devtoolsSession.isConnected()) {
				...
				if (this.id) {
					...
					await Promise.race([
						this.sendWithBrowserDevtoolsSession(
	...
	async getCookies(url?: URL): Promise<ICookie[]> {
		const { cookies } = await this.sendWithBrowserDevtoolsSession(
			"Storage.getCookies",
			{
				browserContextId: this.id,
			},
