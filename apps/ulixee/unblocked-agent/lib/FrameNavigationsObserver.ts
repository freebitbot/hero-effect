...
export default class FrameNavigationsObserver {
	...
	public waitForLocation(
		...
		const commandMarker =
			this.navigations.frame.page.browserContext.commandMarker;
		...
		return this.createStatusTriggeredPromise(
			status,
			options.timeoutMs,
			sinceCommandId,
		);
	...
	public async waitForNavigationResourceId(
	...
	private onLoadStatusChange(): void {
		for (const trigger of [...this.statusTriggers]) {
			...
			if (
				trigger.status === LocationTrigger.change ||
				trigger.status === LocationTrigger.reload
			) {
				const resolver = this.hasLocationTrigger(
					trigger.status,
					trigger.startCommandId,
				);
				if (resolver) {
	...
	private refreshPendingLoadTrigger(trigger: IStatusTrigger): void {
		...
		const { isStable, timeUntilReadyMs } =
			this.navigations.getPaintStableStatus();
		...
		for (const history of this.navigations.history) {
			...
			if (isMatch) {
				...
				if (trigger === LocationTrigger.reload) {
					isTriggered = FrameNavigationsObserver.isNavigationReload(
					...
					if (
						...
						isTriggered =
							previousLoadedNavigation.loaderId !== history.loaderId;
					}
				}
				...
				if (
					...
					const isInPageUrlAdjust =
						...
						history.finalUrl.replace(previousLoadedNavigation.finalUrl, "")
							.length <= 1;
