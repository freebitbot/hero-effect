...
import type IInteractionsHelper from "@ulixee/unblocked-specification/agent/interact/IInteractionsHelper";
import type IDnsSettings from "@ulixee/unblocked-specification/agent/net/IDnsSettings";
import type IHttp2ConnectSettings from "@ulixee/unblocked-specification/agent/net/IHttp2ConnectSettings";
...
export default class Plugins implements IUnblockedPlugins {
	...
	private hooksByName: {
		[K in keyof IUnblockedPlugin]-?: IUnblockedPlugin[K][];
	} = {
		configure: [],
		onClose: [],
		addDomOverride: [],
	...
	constructor(
		...
		for (const Plugin of pluginClasses) {
			...
			if (config === true) {
				...
				config === false ||
				Plugin.shouldActivate?.(this.profile, config) === false
			) {
				continue;
			} else {
	...
	public async playInteractions(
		...
		if (this.hooksByName.playInteractions.length) {
			const playFn =
				this.hooksByName.playInteractions[
					this.hooksByName.playInteractions.length - 1
				];
	...
	public async onNewBrowser(
		browser: IBrowser,
		launchArgs: IBrowserUserConfig,
	): Promise<void> {
		this.isStarted = true;
		await Promise.all(
			this.hooksByName.onNewBrowser.map((fn) => fn(browser, launchArgs)),
		);
	}
	...
	public async onNewFrameProcess(frame: IFrame): Promise<any> {
		await Promise.all(
			this.hooksByName.onNewFrameProcess.map((fn) => fn(frame)),
		);
	}
	...
	public async onNewBrowserContext(context: IBrowserContext): Promise<void> {
		this.isStarted = true;
		await Promise.all(
			this.hooksByName.onNewBrowserContext.map((fn) => fn(context)),
		);
	}
	...
	public async onDevtoolsPanelAttached(
		...
		await Promise.all(
			this.hooksByName.onDevtoolsPanelAttached.map((fn) => fn(devtoolsSession)),
		);
	}

	public async onDevtoolsPanelDetached(
		devtoolsSession: IDevtoolsSession,
	): Promise<any> {
		await Promise.all(
			this.hooksByName.onDevtoolsPanelDetached.map((fn) => fn(devtoolsSession)),
		);
	}
	...
	public async onHttpAgentInitialized(agent: IHttpSocketAgent): Promise<void> {
		await Promise.all(
			this.hooksByName.onHttpAgentInitialized.map((fn) => fn(agent)),
		);
	}
	...
	public async onHttp2SessionConnect(
		...
		await Promise.all(
			this.hooksByName.onHttp2SessionConnect.map((fn) =>
				fn(resource, settings),
			),
		);
	}

	public async shouldInterceptRequest(
		url: URL,
	...
	public async handleInterceptedRequest(
		url: URL,
		type: IResourceType,
	...
	public async beforeHttpRequest(
		resource: IHttpResourceLoadDetails,
	): Promise<void> {
		await Promise.all(
			this.hooksByName.beforeHttpRequest.map((fn) => fn(resource)),
		);
	}

	public async beforeHttpRequestBody(
		resource: IHttpResourceLoadDetails,
	): Promise<void> {
		await Promise.all(
			this.hooksByName.beforeHttpRequestBody.map((fn) => fn(resource)),
		);
	}
	...
	public async beforeHttpResponse(
		...
		await Promise.all(
			this.hooksByName.beforeHttpResponse.map((fn) => fn(resource)),
		);
	}

	public async beforeHttpResponseBody(
		resource: IHttpResourceLoadDetails,
	): Promise<void> {
		await Promise.all(
			this.hooksByName.beforeHttpResponseBody.map((fn) => fn(resource)),
		);
	}

	public async afterHttpResponse(
		resource: IHttpResourceLoadDetails,
	): Promise<any> {
		await Promise.all(
			this.hooksByName.afterHttpResponse.map((fn) => fn(resource)),
		);
	}
