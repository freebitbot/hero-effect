...
export default class FramesManager extends TypedEventEmitter<IFrameManagerEvents> {
	...
	constructor(page: Page, devtoolsSession: DevtoolsSession) {
		...
		this.console = new Console(
			devtoolsSession,
			this.page.browserContext.secretKey,
		);

		this.events.on(
			page,
			"resource-will-be-requested",
			this.onResourceWillBeRequested,
		);
		...
		this.events.on(
			this.networkManager,
			"internal-request",
			(event: IBrowserNetworkEvents["internal-request"]) => {
	...
	public initialize(devtoolsSession: DevtoolsSession): Promise<void> {
		...
		this.events.once(devtoolsSession, "disconnected", () =>
			this.events.endGroup(id),
		);
		...
		this.isReady = new Promise<void>(async (resolve, reject) => {
			try {
				const [framesResponse, , readyStateResult] = await Promise.all([
					...
					devtoolsSession.send("Page.setLifecycleEventsEnabled", {
						enabled: true,
					}),
				...
				if (
					this.main.securityOrigin &&
					!this.main.activeLoader?.lifecycle?.load
				) {
					...
					if (loadName)
						setImmediate(() =>
							this.main.onLifecycleEvent(loadName, null, loaderId),
						);
				}
			} catch (error) {
	...
	public async addNewDocumentScript(
		...
		if (callbacks) {
			...
			for (const [name, onCallbackFn] of Object.entries(callbacks)) {
				if (onCallbackFn) {
					if (
						this.pageCallbacks.has(name) &&
						this.pageCallbacks.get(name) !== onCallbackFn
					)
		...
		const installedScript = await devtoolsSession.send(
		...
		this.pendingNewDocumentScripts.push({
			script,
			isolated: installInIsolatedScope,
		});
		// sometimes we get a new anchor link that already has an initiated frame. If that's the case, newDocumentScripts won't trigger.
		// NOTE: we DON'T want this to trigger for internal pages (':', 'about:blank')
	...
	public frameWithPendingNavigation(
		...
		for (const frame of this.framesById.values()) {
			const isMatch = frame.navigations.pendingResourceId(
				browserRequestId,
				requestedUrl,
				finalUrl,
			);
	...
	public async getDefaultContextIdForFrameId(opts: {
		...
		const subcriber = this.events.on(
			devtoolsSession,
	...
	private async onFrameNavigated(
		...
		this.emit("frame-navigated", {
			frame,
			loaderId: navigatedEvent.frame.loaderId,
		});
		if (
			!frame.isDefaultUrl &&
			!frame.parentId &&
	...
	private async onFrameAttached(
		...
		this.recordFrame(devtoolsSession, {
			id: frameId,
	...
	private async onLifecycleEvent(
		...
		const frame = this.recordFrame(devtoolsSession, {
			id: frameId,
			loaderId,
		} as any);
		...
		if (resourceEvents) {
			for (const { event: resourceEvent, type } of resourceEvents) {
				...
				if (type === "resource-was-requested")
					...
					this.onResourceLoaded(resourceEvent as any);
			}
		}
	...
	private onResourceWillBeRequested(
		event: IPageEvents["resource-will-be-requested"],
	): void {
		const lastCommandId = this.page.browserContext.commandMarker.lastId;
		const { resource, isDocumentNavigation, frameId, redirectedFromUrl } =
			event;
		const url = resource.url.href;

		const frame = frameId
			? this.getFrameForEventOrQueueForReady(
					"resource-will-be-requested",
					event,
				)
			: this.main;
		...
		this.resources.onBrowserWillRequest(
			this.page.tabId,
			frame.frameId,
			resource,
		);
	...
	private onResourceWasRequested(
		event: IPageEvents["resource-was-requested"],
	): void {
		...
		this.resources.onBrowserDidRequest(
			this.page.tabId,
			frame.frameId,
			event.resource,
		);
	...
	private onResourceLoaded(event: IPageEvents["resource-loaded"]): void {
		...
		const isKnownResource = this.resources.onBrowserResourceLoaded(
			this.page.tabId,
			resource,
		);
		...
		if (
			...
			this.resources
				...
				.then(
					(meta) =>
						meta &&
						this.checkForResolvedNavigation(resource.browserRequestId, meta),
				)
				.catch(() => null);
		}
	}
	...
	private onResourceFailed(event: IPageEvents["resource-failed"]): void {
		...
		if (resourceMeta) {
			...
			this.checkForResolvedNavigation(
				browserRequestId,
				resourceMeta,
				loadError,
			);
	...
	private onNavigationResourceResponse(
		event: IPageEvents["navigation-response"],
	): void {
		const frame = event.frameId
