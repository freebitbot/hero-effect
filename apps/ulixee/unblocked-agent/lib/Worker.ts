	...
	implements IWorker
{
	...
	initialize(pageNetworkManager: NetworkManager): Promise<void> {
		...
		const result = Promise.all([
			this.networkManager
				.initializeFromParent(pageNetworkManager)
				.catch((err) => {
			...
			this.type === "shared_worker"
				? this.devtoolsSession.send("Network.setCacheDisabled", {
						cacheDisabled: true,
					})
				: null,
	...
	close(): void {
		this.devtoolsSession
			.send("Target.closeTarget", { targetId: this.id })
			.catch(() => undefined);
	...
	private async initializeEmulation(
