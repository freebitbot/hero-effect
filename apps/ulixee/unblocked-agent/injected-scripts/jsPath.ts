...
class JsPath {
	public static simulateOptionClick(
		jsPath: IJsPath,
	): IExecJsPathResult<boolean> {
		...
		try {
			...
			if (!currentObject || !(currentObject instanceof HTMLOptionElement)) {
				return objectAtPath.toReturnError(
					new Error("Option element not found"),
				);
			}
	...
	public static async exec(
		...
		try {
			...
			if (
				...
				(result.nodePointer?.iterableIsNodePointers ||
					result.value instanceof Node)
			) {
...
class ObjectAtPath {
	...
	public get obstructedByElement() {
		...
		this._obstructedByElement = ObjectAtPath.elementFromPoint(
			centerX,
			centerY,
			element,
		);
	...
	public get isObstructedByAnotherElement() {
		...
		const isContained =
			this.closestElement.contains(overlapping) ||
			this.closestElement === overlapping;
	...
	constructor(
		...
		readonly containerOffset: IPoint = lastContainerOffset,
	) {
		if (!jsPath?.length) return;
		this.containerOffset = containerOffset ?? { x: 0, y: 0 };
		lastContainerOffset = this.containerOffset;

		// @ts-expect-error - start listening for events since we've just looked up something on this frame
	...
	public getComputedVisibility(): INodeVisibility {
		...
		visibility.isUnobstructedByOtherElements =
			!this.isObstructedByAnotherElement;
		if (visibility.isUnobstructedByOtherElements === false) {
			visibility.obstructedByElementId = this.obstructedByElementId;
			visibility.obstructedByElementRect = this.getElementRect(
				this.obstructedByElement,
			);
		}
		...
		visibility.isOnscreenVertical =
			...
			rect.y < window.innerHeight + this.containerOffset.y;
		visibility.isOnscreenHorizontal =
			rect.x + rect.width > 0 &&
			rect.x < window.innerWidth + this.containerOffset.x;
		...
		if (isIterableOrArray(objectAtPath)) {
			...
			if (state.iterableItems.length && isCustomType(state.iterableItems[0])) {
				...
				for (const item of items) {
					const nodePointer = ObjectAtPath.createNodePointer(item, true);
	...
	public static elementFromPoint(
		x: number,
		y: number,
		target: Element,
	): Element {
