...
import type { Agent, Page } from "@ulixee/unblocked-agent";
	...
	override async runPage(
		...
		if (!this.hasNavigated || sessionPage.url !== this.page.mainFrame.url) {
			...
			if (statusCode >= 400) {
				console.log(
					`${statusCode} ERROR: `,
					resource.response.buffer?.toString(),
				);
		...
		if (sessionPage.waitForElementSelector) {
			console.log(
				"%s waitForElementSelector -- %s",
				step,
				sessionPage.waitForElementSelector,
			);

			await wait(
				async () => {
					const visibility = await this.page.mainFrame.jsPath.getNodeVisibility(
						["document", ["querySelector", sessionPage.waitForElementSelector]],
					);
		...
		if (sessionPage.clickElementSelector) {
			...
			await wait(
				async () => {
					const visibility = await this.page.mainFrame.jsPath.getNodeVisibility(
						["document", ["querySelector", sessionPage.clickElementSelector]],
					);
					if (visibility.isVisible) return true;
				},
