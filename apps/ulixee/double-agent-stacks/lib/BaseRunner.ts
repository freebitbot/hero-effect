...
export default abstract class BaseRunner implements IRunner {
	...
	public async run(
		...
		try {
			...
			for (const [pluginId, pages] of Object.entries(
				assignment.pagesByPlugin,
			)) {
				if (
					filters?.onlyRunPluginIds &&
					!filters?.onlyRunPluginIds.includes(pluginId)
				) {
