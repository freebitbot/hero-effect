...
export default class SeleniumRunner extends BaseRunner {
	...
	constructor(
		...
		options: ISeleniumRunnerOptions,
	) {
		super();
	...
	override async runPage(
		...
		if (page.clickElementSelector) {
			console.log(
				"%s clickElementSelector -- %s on %s",
				step,
				page.clickElementSelector,
				page.url,
			);
	...
	private async isSafariErrorPage(url: string): Promise<boolean> {
		try {
			const errorText = await this.driver
				.findElement(By.css(".error-title"))
				.getText();
	...
	public static getRunnerOptions(
		...
		const [browserMajor, browserMinor] = browserVersion
			.split(/[.-]/)
			.map(Number);
