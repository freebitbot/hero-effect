...
import type { IRunnerFactory } from "@double-agent/runner/interfaces/IRunnerFactory";
...
function parseRunnerID(value: string, previous: RunnerId): RunnerId {
	switch (value.toLowerCase().trim()) {
		...
		default: {
			console.warn(
				`parseRunnerID: ignore unrecognized runner value: '${value}'`,
			);
			return previous;
		}
	}
...
const userAgentsToTestPath = getExternalDataPath(
	`/2-user-agents-to-test/userAgentsToTest.json`,
);
