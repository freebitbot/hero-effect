...
export const sendMessageFromBrowserToCoreFnName =
	"sendMessageFromBrowserToUlixeeCore";
...
export type IRestOfMessageObject = Omit<
	IMessageObject,
	"destLocation" | "responseCode"
>;
...
export function messageExpectsResponse(
	...
	if (typeof message === "string") {
		if (isPackedMessage(message)) {
			const responseCode = message.substr(
				startOfResponseCodeField,
				lengthOfResponseCodeField,
			);
...
export function extractStringifiedComponentsFromMessage(
	...
	destLocation: string,
	responseCode: keyof typeof ResponseCode,
	stringifiedMessage: string,
] {
	if (typeof message === "string" && isPackedMessage(message)) {
		const destLocation = message.substr(
			startOfDestLocationField,
			lengthOfDestLocationField,
		);
		const responseCode = message.substr(
			startOfResponseCodeField,
			lengthOfResponseCodeField,
...
export function extractResponseIdFromMessage(
	message: IMessageObject | string,
): string {
	if (typeof message === "string" && isPackedMessage(message)) {
		const stringifiedMessage = message.substr(startOfStringifiedMessageField);
