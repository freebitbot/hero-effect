...
import type {
	IDatabrokerAccount,
	IWallet,
} from "@ulixee/datastore/interfaces/IPaymentService";
...
import type IChromeAliveSessionApi from "./IChromeAliveSessionApi";
import type IDatastoreApi from "./IDatastoreApi";
import type { ICloudConnected } from "./IDesktopApis";
import type IDevtoolsBackdoorApi from "./IDevtoolsBackdoorApi";
import type IHeroSessionsApi from "./IHeroSessionsApi";
...
export type IDatastoreResultItem =
	IDatastoreApiTypes["Datastores.list"]["result"]["datastores"][0];

export type TCredit = IArgonFile["credit"];

export type IArgonFileMeta = {
	file: IArgonFile;
	name: string;
	rawJson: string;
};
...
export type IDesktopAppPrivateApis = {
	...
	"Argon.transferFromMainchain": (arg: {
		address: string;
		microgons: bigint;
	}) => Promise<void>;
	"Argon.transferToMainchain": (arg: {
		address: string;
		microgons: bigint;
	}) => Promise<void>;
...
export type IChromeAliveSessionApiResponse<
	T extends keyof IChromeAliveSessionApis,
