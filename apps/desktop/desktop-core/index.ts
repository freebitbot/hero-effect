...
import type DatastoreCore from "@ulixee/datastore-core";
import type {
	IChromeAliveSessionApis,
	IDesktopAppApis,
} from "@ulixee/desktop-interfaces/apis";
import type IAppApi from "@ulixee/desktop-interfaces/apis/IAppApi";
import type IChromeAliveSessionEvents from "@ulixee/desktop-interfaces/events/IChromeAliveSessionEvents";
import type IDesktopAppEvents from "@ulixee/desktop-interfaces/events/IDesktopAppEvents";
import type { INewHeroSessionEvent } from "@ulixee/desktop-interfaces/events/IDesktopAppEvents";
...
import type {
	ICloudApis,
	ICloudApiTypes,
} from "@ulixee/platform-specification/cloud";
...
type IConnectionToDesktopClient = IConnectionToClient<
	IDesktopAppApis,
	IDesktopAppEvents
>;
...
export default class DesktopCore {
	...
	private get connectionToDatastoreCore(): ConnectionToDatastoreCore {
		if (!this._connectionToDatastoreCore) {
			...
			this._connectionToDatastoreCore = new ConnectionToDatastoreCore(
	...
	public bindConnection(
		connectionToCloudCore: ConnectionToCore<ICloudApis, {}>,
	): void {
	...
	public registerWsRoutes(
		addWsRoute: (
			route: string | RegExp,
			callbackFn: IWsHandleFn,
			useTransport?: boolean,
		) => any,
	): void {
		addWsRoute(
			/\/desktop-devtools\?id=.+/,
			this.addAppDevtoolsWebsocket.bind(this),
			false,
		);
		addWsRoute(/\/desktop(\?.+)?/, this.addDesktopConnection.bind(this));
		addWsRoute(
			/\/chromealive\/.+\?id=.+/,
			this.addChromeAliveConnection.bind(this),
		);
	}

	public addAppDevtoolsWebsocket(
		ws: WebSocket,
		...
		if (!id)
			throw new Error(
				"A ChromeAlive devtools connection was made without an id parameter.",
			);
		const connection = new AppDevtoolsConnection(ws);
		this.appDevtoolsConnectionsById.set(id, connection);
		connection.onCloseFns.push(() =>
			this.appDevtoolsConnectionsById.delete(id),
		);
	}
	...
	> {
		const chromeAliveMatch = request.url.match(
			/\/chromealive\/([0-9a-zA-Z-_]{6,})\?id=(.+)/,
		);
		if (chromeAliveMatch) {
			...
			if (controller)
				return controller.addConnection(transport, request) as any;
			return await this.loadSessionController(
				heroSessionId,
				transport,
				request,
				connectionId,
	...
	public addDesktopConnection(
		...
		>(transport, {
			...
			"Datastores.list": this.delegateToDatastoreCore.bind(
				this,
				"Datastores.list",
			),
			"Datastore.meta": this.getDatastoreMetaWithExamples.bind(this),
			"Datastore.stats": this.delegateToDatastoreCore.bind(
				this,
				"Datastore.stats",
			),
			"Datastore.versions": this.delegateToDatastoreCore.bind(
				this,
				"Datastore.versions",
			),
			"Datastore.creditsIssued": this.delegateToDatastoreCore.bind(
				this,
	...
	public activatePlugin(): void {
		...
		this.events.on(
			HeroSession.events,
			"new",
			this.onHeroSessionCreated.bind(this),
		);
	...
	private async onHeroSessionCreated(event: {
		session: HeroSession;
	}): Promise<void> {
		...
		if (
			...
			heroSession.options.replaySessionId
		) {
		...
		try {
			...
			const { sessionController, appConnectionId } =
			...
			log.error("ERROR launching ChromeAlive for Session", {
				error,
				sessionId,
			});
		}
	...
	private createSessionController(
		...
		if (!this.appDevtoolsConnectionsById.has(appConnectionId)) {
			console.warn(
				"showChromeAlive requested for Hero, but no Desktops available",
			);
		...
		const devtoolsConnection =
			this.appDevtoolsConnectionsById.get(appConnectionId);
	...
	private delegateToDatastoreCore<
		TCommand extends keyof IDatastoreApiTypes & string,
	>(
		command: TCommand,
	...
	private async getDatastoreMetaWithExamples(
		...
		IDatastoreApiTypes["Datastore.meta"]["result"] & {
			examplesByEntityName: {
				[name: string]: { formatted: string; args: Record<string, any> };
			};
		}
	> {
		const meta = (await this.delegateToDatastoreCore(
			"Datastore.meta",
			args,
		)) as Awaited<ReturnType<DesktopCore["getDatastoreMetaWithExamples"]>>;
	...
	private async loadSessionController(
		...
		const db = await this.heroCore.sessionRegistry.retain(
			heroSessionId,
			customDbPath,
		);
		...
		const { sessionController } = this.createSessionController(
			db,
			options,
			appConnectionId,
		);
