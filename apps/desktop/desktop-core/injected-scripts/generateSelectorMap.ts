...
export default function generateSelectorMap(
	...
	iterateBitmaskBySubsets(maxSelectorCombinationBitmask, (mask) => {
		...
		if (counter > nextBreakpoint) {
			...
			console.log(
				"Checking for matches after %s permutations",
				counter,
				context.selectors.length,
			);
...
function getSelectorCounts(
	...
	for (
		let targetBitmask = 1n;
		targetBitmask <= maxSelectorBitmask;
		targetBitmask += 1n
	) {
		...
		try {
			...
			console.error(
				'Error running selector "%s" to create counts',
				finalSelector,
				err,
			);
			throw err;
...
function generateAncestors(element: Element, boundary: ParentNode): IAncestors {
	...
	while (element) {
		...
		const selectorOptions = extractSelectorOptions(
			element,
			boundary,
			ancestors.length,
		);
...
function extractSelectorOptions(
	...
	const id =
		element.id && !element.id.match(/^[0-9]/) ? `#${element.id}` : null;
	const classes = Array.from(element.classList).map((x) => `.${x}`);
	const attrNames = element
		.getAttributeNames()
	...
	if (tagName !== "body") {
		...
		const indexParts = [
			tagName,
			options.find((x) => x.type === "class")?.value,
			indexOption.value,
		];
...
function iterateBitmaskBySubsets(
	bitmask: Bitmask,
	iterateFn: (mask: bigint) => boolean,
): void {
	/**
...
export function decodeBitmaskIndexes<T>(
	bitmask: Bitmask,
	options: T[],
): number[] {
