...
export default class ChromeAliveWindowController
	implements IBrowserContextHooks
{
	...
	private readonly pendingPagePromisesByTabId = new Map<
		number,
		Resolvable<Page>
	>();
	private readonly idsByTargetId = new Map<
		string,
		{ chromeTabId: number; heroTabId: number }
	>();
	...
	public onDevtoolsPanelAttached(
		...
		return this.devtoolsBackdoorModule.onDevtoolsPanelAttached(
			devtoolsSession,
			targetInfo,
		);
	}
	...
	public async addTarget(target: {
		...
		const page = await this.appDevtoolsConnection.attachToPage(
			targetId,
			browserContextId,
			this,
		);
