...
import type { IChromeAliveSessionApis } from "@ulixee/desktop-interfaces/apis";
import type IChromeAliveSessionApi from "@ulixee/desktop-interfaces/apis/IChromeAliveSessionApi";
import type { ISessionResumeArgs } from "@ulixee/desktop-interfaces/apis/IChromeAliveSessionApi";
import type IChromeAliveSessionEvents from "@ulixee/desktop-interfaces/events/IChromeAliveSessionEvents";
import type IDatastoreCollectedAssetEvent from "@ulixee/desktop-interfaces/events/IDatastoreCollectedAssetEvent";
import type IDatastoreOutputEvent from "@ulixee/desktop-interfaces/events/IDatastoreOutputEvent";
import type IHeroSessionUpdatedEvent from "@ulixee/desktop-interfaces/events/IHeroSessionUpdatedEvent";
import type ISessionAppModeEvent from "@ulixee/desktop-interfaces/events/ISessionAppModeEvent";
import type IDatastoreCollectedAssets from "@ulixee/desktop-interfaces/IDatastoreCollectedAssets";
import type { ISelectorMap } from "@ulixee/desktop-interfaces/ISelectorMap";
import type { Session as HeroSession, Tab } from "@ulixee/hero-core";
...
import type IScriptInvocationMeta from "@ulixee/hero-interfaces/IScriptInvocationMeta";
...
import TimetravelTicks, {
	type ITabDetails,
} from "@ulixee/hero-timetravel/player/TimetravelTicks";
...
import type AppDevtoolsConnection from "./AppDevtoolsConnection";
import type DevtoolsBackdoorModule from "./app-extension-modules/DevtoolsBackdoorModule";
import type ElementsModule from "./app-extension-modules/ElementsModule";
...
export default class SessionController extends TypedEventEmitter<{
	...
	constructor(
		...
		this.events.on(
			this.timetravelPlayer,
			"new-tick-command",
			this.sendCommandFocusedEvent,
		);
		this.events.on(
			this.timetravelPlayer,
			"new-paint-index",
			this.sendPaintIndexEvent,
		);
		this.events.on(
			this.timetravelPlayer,
			"new-offset",
		...
		this.selectorRecommendations = new SelectorRecommendations(
			this.scriptInvocationMeta,
		);
		...
		this.events.on(
			sourceCode,
			"command",
			this.sendApiEvent.bind(this, "Command.updated"),
		);
		this.events.on(
			sourceCode,
			"source",
			this.sendApiEvent.bind(this, "SourceCode.updated"),
		);

		if (Fs.existsSync(this.sourceCodeTimeline.entrypoint)) {
			this.scriptLastModifiedTime = Fs.statSync(
				this.scriptInvocationMeta.entrypoint,
			)?.mtimeMs;
	...
	public bindLiveSession(heroSession: HeroSession): void {
		...
		this.events.on(this.timelineWatch, "updated", () =>
	...
	public loadTimelineTicks(): ITabDetails[] {
		...
		if (this.liveSession) {
			domRecordings = Object.entries(this.mirrorPagesByTabId).map(
				([tabId, mirrorPage]) => {
					return {
						tabId: Number(tabId),
						domRecording: mirrorPage.domRecording,
					};
				},
			);
		} else {
	...
	public async loadFromDb(): Promise<void> {
		...
		for (const resource of this.db.resources.all()) {
			void this.resourcesWatch
				.onTabResource(
					resource.tabId,
					await this.db.resources.getMeta(resource.id, false),
				)
				.catch(() => null);
		}
	...
	public addConnection(
		...
		const connection: TConnectionToChromeAliveSessionClient =
			new ConnectionToClient(transport, this.apiHandlers);
		...
		this.events.on(connection, "response", (msg) => {
			this.logger.info(
				`${msg.request.command} response (${msg.request?.messageId})`,
				{
		...
		setImmediate(() => {
			connection.sendEvent({
				...
				data: this.toHeroSessionEvent(),
			});
			connection.sendEvent({
				eventType: "Datastore.output",
				data: this.getDatastoreOutput(),
			});
		});
		return connection;
	}
	...
	public bindExtractor(extractorSession: HeroSession): void {
		const evt = this.events.on(
			extractorSession,
			"output",
			this.onOutputUpdated,
		);
	...
	public relaunchSession(
		...
		try {
			this.logger.info("Relaunch Session", {
				execPath,
				args: [...execArgv, script, ...args],
			});
	...
	public toHeroSessionEvent(): IHeroSessionUpdatedEvent {
		...
		for (const nav of commandTimeline.loadedNavigations) {
			...
			const urlOffset =
				urls.length === 0
					? 0
					: commandTimeline.getTimelineOffsetForTimestamp(nav.initiatedTime);
			...
			for (const [loadStatus, name] of loadStatusLookups) {
				...
				const offsetPercent =
					commandTimeline.getTimelineOffsetForTimestamp(timestamp);
	...
	public getSourceCodeAtCommandId(
		commandId: number,
	): (ISourceCodeLocation & { code: string })[] {
		const command = this.db.commands
			.loadHistory()
			.find((x) => x.id === commandId);
		if (!command) return [];
		return command.callsite
			.map((x) => SourceLoader.getSource(x))
			.filter(Boolean);
	}

	public addSourceCodeLocation(
		record: { commandId: number } & ISourceCodeReference,
	): void {
		record.sourcecode = this.getSourceCodeAtCommandId(record.commandId);
	}

	public onCollectedAsset(
		event: HeroSession["EventTypes"]["collected-asset"],
	): void {
	...
	public onTabCreated(event: HeroSession["EventTypes"]["tab-created"]): void {
		...
		this.events.on(
			tab,
			"page-events",
			this.sendDomRecordingUpdates.bind(this, tab),
		);
		this.events.on(
			tab,
			"page-events",
		...
		this.events.on(
			event.tab,
			"resource",
			this.onTabResource.bind(this, event.tab.id),
		);
	...
	public sendDomRecordingUpdates(
		tab: Tab,
		events: Tab["EventTypes"]["page-events"],
	): void {
	...
	public async refreshLiveMirrorPage(
		tabId: number,
		force = false,
	): Promise<void> {
		const now = Date.now();
		...
		if (
			!force &&
			this.mirrorRefreshLastUpdated &&
			now - this.mirrorRefreshLastUpdated < 100
		) {
	...
	public async getDomRecording(
		tabId?: number,
	): ReturnType<IChromeAliveSessionApi["getDom"]> {
		tabId ??=
			this.timetravelPlayer.activeTabId ??
			Number(Object.keys(this.mirrorPagesByTabId)[0] ?? 1);
	...
	public async getScriptState(): ReturnType<
		IChromeAliveSessionApi["getScriptState"]
	> {
	...
	public async getTimetravelState(): ReturnType<
		IChromeAliveSessionApi["getTimetravelState"]
	> {
		await this.onFirstTab;
		const activeTab = await this.timetravelPlayer.loadTab();
		...
		return Promise.resolve({
			...
			percentOffset:
				this.mode === "Live" ? 100 : (activeTab?.currentTimelineOffsetPct ?? 0),
		});
	}

	public async timetravel(
		args: Parameters<IChromeAliveSessionApi["timetravel"]>[0],
	): Promise<{
	...
	public async openMode(
		args: Parameters<IChromeAliveSessionApi["openMode"]>[0],
	): Promise<void> {
		try {
	...
	public async searchDom(
		...
		const lastUrl = metadata.urls[metadata.urls.length - 1];
		const commandTimeline = CommandTimeline.fromDb(this.db);
		const lastNavigation = commandTimeline.navigationsById.get(
			lastUrl.navigationId,
		);
	...
	public async getResourceDetails(
		...
		const contentType =
			resource.responseHeaders?.["content-type"] ??
			resource.responseHeaders?.["Content-Type"];
		const responseBody =
	...
	public getResources(): ReturnType<
		IChromeAliveSessionApis["Session.getResources"]
	> {
	...
	public async generateQuerySelector(id: {
		...
		try {
			...
			const activePageUrl = (await this.timetravelPlayer.loadTab())?.mirrorPage
				.page.mainFrame.url;
	...
	private sendAppModeEvent(
		...
		this.sendApiEvent("Session.appMode", {
			mode: this.mode,
			position,
			trigger,
		});
