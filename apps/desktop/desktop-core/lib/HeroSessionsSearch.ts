...
import type HeroCore from "@ulixee/hero-core";
import type { Session as HeroSession } from "@ulixee/hero-core";
import CommandFormatter from "@ulixee/hero-core/lib/CommandFormatter";
import type FuseJs from "fuse.js";
...
export default class HeroSessionsSearch extends TypedEventEmitter<{
	...
	private commandsBySessionId: {
		[sessionId: string]: { id: number; label: string }[];
	} = {};
	...
	async close(): Promise<void> {
		...
		await Promise.allSettled(
			this.sessions.map((x) =>
				this.heroCore.sessionRegistry.close(x.heroSessionId, false),
			),
		);
		this.heroCore = null;
	}
	...
	onNewSession(heroSession: HeroSession): void {
		...
		this.events.group(
			...
			this.events.on(
				heroSession,
				"kept-alive",
				this.onHeroSessionKeptAlive.bind(this, entry),
			),
			this.events.on(
				heroSession,
				"resumed",
				this.onHeroSessionResumed.bind(this, entry),
			),
			this.events.on(
				heroSession,
				"closed",
				this.onHeroSessionClosed.bind(this, entry),
			),
		);
	}
	...
	private async processSession(
		...
		const sessionDb = await this.heroCore.sessionRegistry.get(
			sessionId,
			customPath,
		);
		...
		try {
			...
			let state: IHeroSessionsListResult["state"] = closeDate
				? "complete"
				: "running";
			...
			return {
				...
				error: errorOnLastCommand
					? `${errorOnLastCommand.error?.message ?? "Error"}`
					: null,
			...
			console.warn("ERROR loading search index for Hero Replay", error, {
				sessionId,
			});
		}
	}
	...
	private async onHeroSessionClosed(
		entry: IHeroSessionsListResult,
	): Promise<void> {
	...
	private async onHeroSessionKeptAlive(
		entry: IHeroSessionsListResult,
	): Promise<void> {
		const update = await this.processSession(entry.heroSessionId, entry.dbPath);
		if (!update) return;
