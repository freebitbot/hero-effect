import type EventSubscriber from "@ulixee/commons/lib/EventSubscriber";
import { TypedEventEmitter } from "@ulixee/commons/lib/eventUtils";
import type IResourceOverview from "@ulixee/desktop-interfaces/IResourceOverview";
import type IResourceSearchResult from "@ulixee/desktop-interfaces/IResourceSearchResult";
import type { ISearchContext } from "@ulixee/desktop-interfaces/ISessionDomSearchResult";
...
export default class SessionResourcesWatch extends TypedEventEmitter<{
	...
	public search(
		...
		const searchResults = this.searchIndexByTabId[tabId].search(finalQuery, {
			limit: 10,
		});
		for (const result of searchResults) {
			...
			const timestamp =
				resource.browserLoadedTimestamp ?? resource.responseTimestamp;
	...
	public async onTabResource(
		...
		this.resourcesById[resource.id] = {
			...
			didMitmModifyHeaders:
				resourcesRecord.requestHeaders !==
				resourcesRecord.requestOriginalHeaders,
