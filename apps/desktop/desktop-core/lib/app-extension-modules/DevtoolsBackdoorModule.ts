...
import type IElementSummary from "@ulixee/desktop-interfaces/IElementSummary";
...
export default class DevtoolsBackdoorModule {
	...
	private devtoolsSessionToIds = new Map<
		IDevtoolsSession,
		{ tabId: number; targetId: string }
	>();
	...
	constructor(
		...
		this.logger = log.createChild(module, {
			sessionId: chromeAliveWindowController.sessionId,
		});
	}
	...
	public async onDevtoolsPanelAttached(
		...
		this.devtoolsSessionToIds.set(devtoolsSession, {
			tabId,
			targetId: targetInfo.targetId,
		});
		const byTabId =
			this.devtoolsSessionByTabId.get(tabId) ?? new Set<IDevtoolsSession>();
		...
		await devtoolsSession.send("Runtime.addBinding", {
			name: ___emitFromDevtoolsToCore,
		});
	}
	...
	public async showElementsPanel(page: IPage): Promise<void> {
		const tabId = this.chromeAliveWindowController.getChromeTabIdByPageId(
			page.id,
		);
	...
	private handleIncomingMessageFromBrowser(tabId: number, message: any): void {
		...
		if (event === EventType.ElementWasSelected) {
			this.emitElementWasSelected(tabId, payload.backendNodeId).catch(
				console.error,
			);
	...
	private async emitElementWasSelected(
		...
		const page =
			await this.chromeAliveWindowController.getPageByChromeTabId(tabId);
		if (!page) {
			this.logger.warn("Element emitted for non-ChromeAlive tab", {
				tabId,
				backendNodeId,
			});
			return;
		...
		this.chromeAliveWindowController.sendApiEvent(
