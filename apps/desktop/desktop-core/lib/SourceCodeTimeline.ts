...
import type ICommandUpdatedEvent from "@ulixee/desktop-interfaces/events/ICommandUpdatedEvent";
import type ISourceCodeUpdatedEvent from "@ulixee/desktop-interfaces/events/ISourceCodeUpdatedEvent";
import CallsiteLocator from "@ulixee/hero/lib/CallsiteLocator";
import type { Session } from "@ulixee/hero-core";
...
export default class SourceCodeTimeline extends TypedEventEmitter<{
	...
	constructor(
		...
		if (this.entrypoint.includes(datastoresDir)) {
			SourceMapSupport.retrieveSourceMap(
				this.entrypoint,
	...
	private checkForSourceUpdates(
		...
		for (const sourcePosition of sourceLocations) {
			...
			this.sourceFileLines[source] ??=
				SourceLoader.getSourceLines(sourcePosition);
	...
	private filterDesktopPaths(
		sourceLocation: ISourceCodeLocation & { source?: string },
	): boolean {
