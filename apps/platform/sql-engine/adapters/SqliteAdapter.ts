...
import type { IDbJsTypes, IDbTypeNames } from "../interfaces/IDbTypes";
import type ISqlAdapter from "../interfaces/ISqlAdapter";
...
export default class SqliteAdapter implements ISqlAdapter {
	...
	toEngineValue(
		type: IDbTypeNames,
		value: IDbJsTypes,
	): [serialized: any, temporaryType?: IDbTypeNames] {
		...
		if (["record", "object", "array"].includes(type)) {
			return [
				value ? TypeSerializer.stringify(value, { sortKeys: true }) : null,
			];
		}
