...
import type { ConnectionToDatastoreCore } from "@ulixee/datastore";
...
import type ResultIterable from "@ulixee/datastore/lib/ResultIterable";
import type { IOutputSchema } from "../interfaces/IInputOutput";
...
export default class ClientForDatastore<TDatastore extends Datastore> {
	...
	public fetch(name, inputFilter): any {
		const instance =
			this.datastore.extractors[name] || this.datastore.tables[name];
		if (!instance)
			throw new Error(
				`${name} is not a valid Datastore Extractor or Table name.`,
			);
	...
	public async crawl<T extends keyof TDatastore["crawlers"]>(
		...
		const result = await this.datastore.crawlers[name].runInternal({
			input: inputFilter,
		});
		return result[0];
	}
