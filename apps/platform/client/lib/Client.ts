...
import type { IInputFilter, IOutputSchema } from "../interfaces/IInputOutput";
import type ILocationStringOrObject from "../interfaces/ILocationStringOrObject";
...
export interface IClientConfig
	extends Partial<
		Pick<
			IQueryOptions,
			"authentication" | "affiliateId" | "onQueryResult" | "queryId"
		>
	> {
...
export default class ClientForRemote {
		...
		if (isDnsOrDomain && typeof uriOrObject === "string") {
			try {
				...
				if (!config?.argonMainchainUrl)
					throw new Error(
						"No mainchain url provided to lookup this datastore host",
					);
	...
	>(
		...
		if (!this.database) {
			throw new Error(
				"You Client connection must specific a datastore to fetch",
			);
		}
	...
	public async crawl<TInputFilter extends IInputFilter = IInputFilter>(
		...
		if (!this.database) {
			throw new Error(
				"You Client connection must specific a datastore to crawl",
			);
		}
		await this.loadPayments();
		const apiClient = await this.getApiClient();
	...
	public async query<TSchema extends IOutputSchema = IOutputSchema>(
		...
		if (!this.database) {
			throw new Error(
				"You Client connection must specific a datastore to query",
			);
		}

		const apiClient = await this.getApiClient();
		const { outputs } = await apiClient.query(
			this.datastoreId,
			this.version,
			sql,
			{
	...
	public static forTable<T extends Table>(
		table: T,
		options?: IClientOptions,
	): ClientForTable<T> {
