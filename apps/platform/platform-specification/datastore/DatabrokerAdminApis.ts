...
export const DatabrokerAdminApisSchema = {
	"System.overview": {
		args: z.object({}),
		result: z.object({
			localchainBalance: microgonsValidation.describe(
				"The balance in microgons",
			),
			...
			channelHolds: z
				...
				.int()
				.nonnegative()
				.describe("The number of channelHolds created"),
			openChannelHolds: z
				.number()
				.int()
				.nonnegative()
				.describe("The number of open channelHolds"),
	...
	"Organization.create": {
		args: z.object({
			...
			balance: microgonsValidation.describe(
				"The initial balance to allocate to this organization",
			),
		}),
		result: z.object({
	...
	"Organization.grant": {
		args: z.object({
			organizationId: organizationIdValidation,
			amount: microgonsValidation.describe(
				"The amount to grant to the organization in microgons",
			),
		}),
		result: OkSchema,
	},
...
type IDatabrokerAdminApiTypes = IZodSchemaToApiTypes<
	typeof DatabrokerAdminApisSchema
>;

export type { IDatabrokerAdminApiTypes };
