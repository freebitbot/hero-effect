...
import {
	DatastorePaymentRecipientSchema,
	minDate,
} from "../types/IDatastoreManifest";
...
const EntityMetaSchema = z.object({
	...
	netBasePrice: microgonsValidation.describe(
		"The aggregate base price per query.",
	),
...
const DatastoreQueryResultSchema = z.object({
	...
	runError: z
		.instanceof(Error)
		.optional()
		.describe("An error that occurred during the query."),
	outputs: z
		.any()
		.optional()
		.array()
	...
	authentication: z
		.object({
			...
			nonce: z
				.string()
				.length(10)
				.describe("A random nonce adding signature noise."),
		})
		.optional(),
	affiliateId: z
		.string()
...
export const DatastoreApiSchemas = {
	"Datastore.upload": {
		args: z.object({
			compressedDbx: z
				.instanceof(Buffer)
				.describe("Bytes of a compressed .dbx directory."),
	...
	"Datastore.download": {
		args: z.object({
			...
			requestDate: z
				.date()
				.describe("Date of this request. Must be in last 10 seconds."),
		...
		result: z.object({
			adminIdentity: identityValidation.describe(
				"The admin identity who uploaded this Datastore.",
			),
			adminSignature: identitySignatureValidation.describe(
			...
			compressedDbx: z
				.instanceof(Buffer)
				.describe("Bytes of the compressed .dbx directory."),
		}),
	},
	"Datastore.start": {
		args: z.object({
			...
			dbxPath: z
				.string()
				.describe("Path to a local file system Database path."),
	...
	"Datastore.creditsBalance": {
		args: z.object({
			id: datastoreIdValidation.describe("The datastore id"),
			version: semverValidation.describe(
				"The Datastore version to look at credits for.",
			),
			creditId: z.string().describe("CreditId issued by this datastore."),
		}),
		result: z.object({
			issuedCredits: microgonsValidation.describe(
				"Issued credits balance in microgons.",
			),
			balance: microgonsValidation.describe(
				"Remaining credits balance in microgons.",
			),
		}),
	},
		...
		args: z.object({
			...
			adminFunction: z.object({
				...
				ownerName: z
					.string()
					.describe(
						"The name of the owning extractor, table or crawler (if applicable).",
					)
					.optional(),
			...
			functionArgs: z
				.any()
				.array()
				.describe("The args to provide to the function."),
		}),
					...
					z.object({
						parameters: z
							.record(
								z
									.string()
									.describe("Parameter name matching the entity's schema."),
								z
									.any()
									.describe(
										"Parameter value as a Javascript type (will be converted by engine).",
									),
							)
							.optional()
							.describe(
								"Parameters to simulate for this virtual function or table",
							),
						records: z
							.any()
							.array()
							.describe(
								"Records to simulate (NOTE: must match the Datastore schema).",
							),
					}),
				)
				.optional()
			...
			datastores: z
				.object({
					...
					isStarted: z
						.boolean()
						.describe(
							"Only relevant in development mode - is this Datastore started.",
						),
...
export type IDatastoreQueryMetadata = z.infer<
	typeof DatastoreQueryMetadataSchema
>;
