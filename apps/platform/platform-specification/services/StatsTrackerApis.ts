...
export const StatsTrackerApiSchemas = {
	"StatsTracker.getByVersion": {
		...
		result: z.object({
			...
			statsByEntityName: z.record(
				z.string().describe("Entity name"),
				EntityStatsSchema,
			),
		}),
	},
	...
	"StatsTracker.recordQuery": {
		args: z.object({
			...
			affiliateId: z
				.string()
				.optional()
				.describe("An affiliate id used for this query"),
			heroSessionIds: z
				.string()
				.array()
				.optional()
...
export type IStatsTrackerApiTypes = IZodSchemaToApiTypes<
