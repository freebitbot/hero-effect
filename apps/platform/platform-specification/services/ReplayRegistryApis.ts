...
export const ReplayRegistryApiSchemas = {
	"ReplayRegistry.store": {
		args: z.object({
			sessionId: z
				.string()
				.describe("The session id of this Hero Replay session."),
			timestamp: z.number().describe("Unix millis since epoch."),
			db: z
				.instanceof(Buffer)
				.describe("The compressed raw bytes of the database."),
		}),
		result: z.object({
			success: z.boolean(),
		}),
	...
	"ReplayRegistry.get": {
		...
		result: z.object({
			db: z
				.instanceof(Buffer)
				.describe("Compressed raw bytes of the database."),
		}),
	},
	"ReplayRegistry.ids": {
		args: z.object({}),
...
export type IReplayRegistryApiTypes = IZodSchemaToApiTypes<
