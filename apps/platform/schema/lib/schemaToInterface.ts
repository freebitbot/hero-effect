...
import type { ISchemaAny } from "../index";
...
export default function schemaToInterface(
	schema:
		| ISchemaAny
		| Record<string, ISchemaAny>
		| Record<string, Record<string, ISchemaAny>>,
): ts.TypeNode {
	if (
		schema !== null &&
	...
	switch (schema.typeName) {
		...
		case "object": {
			const properties = Object.entries(schema.fields) as [
				string,
				ISchemaAny,
			][];
...
export function printNode(
	...
	return (
		printer
			...
			// change to 2 spaces
			.replace(/^(\s{4})+/gm, (match) =>
				new Array(match.length / 4 + 1).join("  "),
			)
	);
}
