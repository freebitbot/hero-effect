...
import BaseSchema, { type IBaseConfig, isDefined } from "./BaseSchema";
...
> extends BaseSchema<number, TOptional, INumberSchemaConfig<TOptional>> {
	...
	constructor(config: INumberSchemaConfig<TOptional> = {}) {
		...
		if (isDefined(config.min))
			assert(typeof config.min === "number", "Min value must be a number");
		if (isDefined(config.max))
			assert(typeof config.max === "number", "Max value must be a number");
		if (isDefined(config.decimals)) {
			assert(
				typeof config.decimals === "number",
				"number of required decimals must be a number",
			);
		}
	...
	protected validationLogic(value: any, path, tracker): void {
		...
		if (config.min !== undefined && config.min !== null && value < config.min) {
			return this.failedConstraint(
				value,
				" This value is smaller than the min.",
				path,
				tracker,
			);
		}
		...
		if (config.max !== undefined && config.max !== null && value > config.max) {
			return this.failedConstraint(
				value,
				" This value is larger than the max.",
				path,
				tracker,
			);
		}
