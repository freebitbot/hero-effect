...
export default class BigintSchema<
	TOptional extends boolean = false,
> extends BaseSchema<bigint, TOptional, IBigintSchemaConfig<TOptional>> {
	...
	constructor(config: IBigintSchemaConfig<TOptional> = {}) {
		...
		if (isDefined(config.min))
			assert(typeof config.min === "bigint", "Min value must be a bigint");
		if (isDefined(config.max))
			assert(typeof config.max === "bigint", "Max value must be a bigint");
	}
