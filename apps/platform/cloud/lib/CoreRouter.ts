...
export default class CoreRouter {
	...
	private wsConnectionByType = {
		...
		datastore: (transport) =>
			this.cloudNode.datastoreCore.addConnection(transport) as any,
		services: (transport) => this.addHostedServicesConnection(transport),
		cloud: (transport) =>
			this.cloudApiRegistry.createConnection(transport, this.getApiContext()),
	} as const;
	...
	public async register(): Promise<void> {
		...
		this.cloudNode.datastoreCore.registerHttpRoutes(
			this.addHttpRoute.bind(this),
		);

		if (this.cloudNode.desktopCore) {
			const bridge = new TransportBridge();
			this.cloudApiRegistry.createConnection(
				bridge.transportToClient,
				this.getApiContext(),
			);
	...
	private addHostedServicesConnection(
		...
		const connection =
			this.cloudNode.datastoreCore.addHostedServicesConnection(transport);
		this.hostedServiceEndpoints?.attachToConnection(
			connection as any,
			this.getApiContext(),
		);
	...
	private addWsRoute(
		...
		if (useTransport) {
			this.cloudNode.publicServer.addWsRoute(
				route,
				this.handleSocketRequest.bind(this, key),
			);
		} else {
			this.cloudNode.publicServer.addWsRoute(
				route,
				this.handleRawSocketRequest.bind(this, key),
			);
		}
	}
