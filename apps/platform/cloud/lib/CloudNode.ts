...
export default class CloudNode {
	...
	public cloudConfiguration: ICloudConfiguration = {
		...
		hostedServicesServerOptions:
			(Env.hostedServicesPort ?? Env.hostedServicesListenHostname)
				? {
						port: Env.hostedServicesPort
							? Number(Env.hostedServicesPort)
							: undefined,
	...
	private connectionsToServicesByHost: {
		[host: string]: ConnectionToCore<any, any>;
	} = {};
	...
	public async close(): Promise<void> {
		...
		try {
			...
			await Promise.allSettled([
				...Object.values(this.connectionsToServicesByHost).map((x) =>
					x.disconnect(),
				),
	...
	private async startCores(): Promise<void> {
		...
		if (
			...
			this.cloudConfiguration.nodeRegistryHost ??=
				servicesSetup.nodeRegistryHost;
		...
		this.nodeRegistry = new NodeRegistry({
			...
			serviceClient: this.createConnectionToServiceHost(
				this.cloudConfiguration.nodeRegistryHost,
			),
		...
		if (
			(this.nodeRegistry.serviceClient ||
				this.cloudConfiguration.nodeRegistryHost) &&
	...
	private createConnectionToServiceHost(
		...
		this.connectionsToServicesByHost[hostURL.host] ??= new ConnectionToCore<
			any,
			any
