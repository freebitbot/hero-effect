...
import type INodeRegistryApiTypes from "@ulixee/platform-specification/services/NodeRegistryApis";
import type { ICloudNodeMeta } from "@ulixee/platform-specification/services/NodeRegistryApis";

type THealth = INodeRegistryApiTypes["NodeRegistry.health"]["args"];

export default class NodeTracker extends TypedEventEmitter<{
	new: { node: ICloudNodeMeta };
}> {
	...
	private sortSeenNodes(): void {
		this.prune();
		this.nodesInSeenOrder.sort(
			(a, b) => b.lastSeenDate.getTime() - a.lastSeenDate.getTime(),
		);
	}
