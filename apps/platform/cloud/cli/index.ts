		...
		.addOption(
			program
				.createOption(
					...
					"Do not enable Ulixee Desktop apis.",
				)
				.argParser(parseEnvBool)
...
export async function startCloudViaCli(opts: any): Promise<CloudNode> {
	...
	if (argonLocalchainPath || argonLocalchainCreateIfMissing) {
		localchainConfig = {
			...
			keystorePassword: {
				password: argonLocalchainPassword
					? Buffer.from(argonLocalchainPassword)
					: undefined,
	...
	const cloudNode = new CloudNode(
		filterUndefined({
			...
			datastoreConfiguration: filterUndefined({
				...
				adminIdentities: parseIdentities(
					opts.adminIdentities,
					"Admin Identities",
				),
				localchainConfig,
			}),
