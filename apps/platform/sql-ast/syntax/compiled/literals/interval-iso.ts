...
const grammar: Grammar = {
	...
	ParserRules: [
		...
		},
		{
			...
			symbols: [lexerAny.has("Y") ? { type: "Y" } : Y],
		},
		{
			name: "long$subexpression$1",
			symbols: [lexerAny.has("M") ? { type: "M" } : M],
		},
		{
			name: "long$subexpression$1",
			symbols: [lexerAny.has("W") ? { type: "W" } : W],
		},
		{
			name: "long$subexpression$1",
			symbols: [lexerAny.has("D") ? { type: "D" } : D],
		},
		{
			name: "long",
			symbols: ["num", "long$subexpression$1"],
			postprocess: ([n, u]) => {
		...
		{
			name: "short$subexpression$1",
			symbols: [lexerAny.has("S") ? { type: "S" } : S],
		},
		{
			name: "short",
			symbols: ["short$ebnf$1", "num", "short$subexpression$1"],
			postprocess: ([_, n, u]) => {
				n = parseFloat(n[0].value);
				u = u[0].type;
				switch (u) {
