...
const grammar: Grammar = {
	...
	ParserRules: [
		...
		{
			name: "unit",
			symbols: [lexerAny.has("years") ? { type: "years" } : years],
		},
		{
			name: "unit",
			symbols: [lexerAny.has("months") ? { type: "months" } : months],
		},
		{ name: "unit", symbols: [lexerAny.has("days") ? { type: "days" } : days] },
		{
			name: "unit",
			symbols: [lexerAny.has("hours") ? { type: "hours" } : hours],
		},
		{
			name: "unit",
			symbols: [lexerAny.has("minutes") ? { type: "minutes" } : minutes],
		},
		{
			name: "unit",
			symbols: [lexerAny.has("seconds") ? { type: "seconds" } : seconds],
		},
		{
			name: "unit",
			symbols: [
				lexerAny.has("milliseconds") ? { type: "milliseconds" } : milliseconds,
			],
		},
		...
		},
		{
			...
			symbols: [lexerAny.has("neg") ? { type: "neg" } : neg],
		},
		{
			name: "int$ebnf$1",
		...
		{
			name: "int",
			symbols: ["int$ebnf$1", lexerAny.has("int") ? { type: "int" } : int],
		...
		{
			name: "float$ebnf$1",
			symbols: ["float$ebnf$1$subexpression$1"],
			postprocess: id,
		},
		...
		{ name: "float$ebnf$2", symbols: [], postprocess: () => null },
		{
			...
			symbols: [
				...
				lexerAny.has("int") ? { type: "int" } : int,
			],
			postprocess: ([neg, ...v]) =>
				parseFloat(v.map((v) => (v ? v.text : "0")).join("")) * (neg ? -1 : 1),
		},
		{
			name: "time$ebnf$1$subexpression$1",
			symbols: [lexerAny.has("colon") ? { type: "colon" } : colon, "uint"],
		},
		{
			name: "time$ebnf$1",
			symbols: ["time$ebnf$1$subexpression$1"],
			postprocess: id,
		},
		...
		{
			name: "time$ebnf$2",
			symbols: ["time$ebnf$2$subexpression$1"],
			postprocess: id,
		},
		{ name: "time$ebnf$2", symbols: [], postprocess: () => null },
		{
			...
			symbols: [
				"uint",
				lexerAny.has("colon") ? { type: "colon" } : colon,
				"uint",
				"time$ebnf$1",
				"time$ebnf$2",
			],
