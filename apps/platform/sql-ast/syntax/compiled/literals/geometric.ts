...
const grammar: Grammar = {
	...
	ParserRules: [
		...
		},
		{
			...
			symbols: ["point$macrocall$1$subexpression$1"],
			postprocess: unwrap,
		},
		{ name: "point", symbols: ["point$macrocall$1"], postprocess: unwrap },
		{
			...
			postprocess: (x) => ({ x: x[0], y: x[2] }),
		},
		{
			name: "line",
			symbols: [
				lexerAny.has("lcurl") ? { type: "lcurl" } : lcurl,
				"number",
				"comma",
				"number",
				"comma",
				"number",
				lexerAny.has("rcurl") ? { type: "rcurl" } : rcurl,
			],
			postprocess: (x) => ({
				a: x[1],
				b: x[3],
		...
		},
		{
			name: "path",
			symbols: ["open_path"],
			postprocess: ([path]) => ({ closed: false, path }),
		},
		{
			name: "path",
			symbols: ["closed_path"],
			postprocess: ([path]) => ({ closed: true, path }),
		},
		{
			name: "open_path$macrocall$2",
			symbols: [lexerAny.has("lbracket") ? { type: "lbracket" } : lbracket],
		},
		{
			name: "open_path$macrocall$3",
			symbols: [lexerAny.has("rbracket") ? { type: "rbracket" } : rbracket],
		},
		...
		},
		{
			...
			symbols: ["closed_path$subexpression$1$macrocall$1"],
			postprocess: last,
		},
		...
		{
			name: "closed_path$subexpression$1",
			symbols: ["closed_path$subexpression$1$macrocall$4"],
			postprocess: last,
		},
		{
			name: "closed_path",
			symbols: ["closed_path$subexpression$1"],
			postprocess: get(0),
		},
		{ name: "polygon", symbols: ["closed_path"], postprocess: get(0) },
		{
			name: "circle_body",
			symbols: ["point", "comma", "number"],
			postprocess: (x) => ({ c: x[0], r: x[2] }),
		},
		{
			name: "circle$subexpression$1$macrocall$2",
			symbols: [lexerAny.has("lcomp") ? { type: "lcomp" } : lcomp],
		},
		{
			name: "circle$subexpression$1$macrocall$3",
			symbols: [lexerAny.has("rcomp") ? { type: "rcomp" } : rcomp],
		},
		{
		...
		},
		{
			...
			symbols: ["circle$subexpression$1$macrocall$1"],
		},
		{
		...
		{
			name: "circle$subexpression$1",
			symbols: ["circle$subexpression$1$macrocall$4"],
		},
		{ name: "circle$subexpression$1", symbols: ["circle_body"] },
		{
			name: "circle",
			symbols: ["circle$subexpression$1"],
			postprocess: unwrap,
		},
	],
	ParserStart: "number",
