...
const grammar: Grammar = {
	...
	ParserRules: [
		...
		{ name: "main$ebnf$1", symbols: [], postprocess: () => null },
		{
			name: "main",
			symbols: [
				lexerAny.has("start_list") ? { type: "start_list" } : start_list,
				"main$ebnf$1",
				lexerAny.has("end_list") ? { type: "end_list" } : end_list,
			],
			postprocess: (x) => x[1] || [],
		},
		{ name: "elements$ebnf$1", symbols: [] },
		{
			...
			symbols: [lexerAny.has("comma") ? { type: "comma" } : comma, "elt"],
			postprocess: last,
		},
		{
		...
		{
			name: "elt",
			symbols: [lexerAny.has("value") ? { type: "value" } : value],
