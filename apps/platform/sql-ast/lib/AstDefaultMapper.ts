...
import type INil from "../interfaces/INil";
import type * as a from "../interfaces/ISqlNode";
import type { IAstMapper } from "./astMapper";
...
export default class AstDefaultMapper implements IAstMapper {
	...
	update(val: a.IUpdateStatement): a.IStatement | INil {
		...
		const returning = arrayNilMap(val.returning, (c) =>
			this.selectionColumn(c),
		);
	...
	orderBy(
		orderBy: a.IOrderByStatement[] | null | undefined,
	): a.IOrderByStatement[] {
	...
	values(from: a.IValuesStatement): a.ISelectStatement | INil {
		const values = arrayNilMap(from.values, (x) =>
			arrayNilMap(x, (y) => this.expr(y)),
		);
		if (!values?.length) {
			return null; // nothing to select from
		}
