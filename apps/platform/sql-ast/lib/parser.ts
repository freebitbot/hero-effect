...
import type IParseOptions from "../interfaces/IParseOptions";
import type {
	IBox,
	ICircle,
	IExpr,
...
): { ast: IStatement[]; comments: ISqlComment[] } {
...
export function parse(sql: string, optEntry?: string | IParseOptions): any {
	...
	const entry = typeof optEntry === "string" ? optEntry : optEntry?.entry;
	const opts = typeof optEntry === "string" ? null : optEntry;

	// parse sql
	const doParse = (): any => _parse(sql, sqlCompiled, entry);
	let parsed = opts?.locationTracking ? tracking(doParse) : doParse();
