import { compile, keywords, type Token } from "moo";
import type { INodeLocation, ISqlComment } from "../../interfaces/ISqlNode";
...
lexer.next = ((next) => () => {
	...
	while ((tok = next.call(lexer))) {
		...
		if (tok.type === "commentFullOpen") {
			if (commentFull === null) {
				// initial open - start collecting content
				commentFull = {
					nested: 0,
					offset: tok.offset,
					text: tok.text,
...
export function trackingComments<T>(act: () => T): {
	ast: T;
...
export function doubleQuoted(
	value: any,
): { doubleQuoted: boolean } | undefined {
