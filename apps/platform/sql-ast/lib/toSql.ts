...
export type IAstToSql = {
	readonly [key in keyof IAstPartialMapper]-?: ReplaceReturnType<
		IAstPartialMapper[key],
		string
	>;
};
...
const visitor = astVisitor<IAstFullVisitor>((m) => ({
	...
	fromTable: (s) => {
		join(m, s.join, () => {
			...
			if (s.name.columnNames) {
				if (!s.name.alias) {
					throw new Error(
						"Cannot specify aliased column names without an alias",
					);
				}
	...
	member: (e) => {
		...
		ret.push(e.op);
		ret.push(
			typeof e.member === "number" ? e.member.toString(10) : pgEscape(e.member),
		);
	},
