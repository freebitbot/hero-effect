...
import { astMapper, type IAstMapper } from "../lib/astMapper";
...
export function checkInsert(
	value: string | string[],
	expected: IInsertStatement,
) {
	checkTree(value, expected, (p, m) => m.statement(p));
}
export function checkInsertLoc(
	value: string | string[],
	expected: IInsertStatement,
) {
...
function deepEqual<T>(
	...
	if (Array.isArray(a)) {
		...
		for (let i = 0; i < a.length; i++) {
			if (!deepEqual(a[i], b[i], strict, depth - 1, numberDelta)) return false;
		}
		return true;
	}
	...
	if (strict && ak.length !== bk.length) return false;
	const set: Iterable<string> = strict
		? Object.keys(a)
		: new Set([...Object.keys(a), ...Object.keys(b)]);
	for (const k of set) {
		if (
			!deepEqual((a as any)[k], (b as any)[k], strict, depth - 1, numberDelta)
		)
			return false;
	}
	return true;
...
function checkTree<T>(
	value: string | string[],
	expected: T,
	mapper: (parsed: T, m: IAstMapper | IAstToSql) => any,
	start?: string,
			...
			function doParse(psql: string) {
				...
				if (ret.length !== 1) {
					...
					if (noLocs.slice(1).every((p) => deepEqual(p, noLocs[0]))) {
						fail(
							`${noLocs.length} ambiguous syntaxes, but they yielded the same ASTs : ${inspect(noLocs[0])}`,
						);
					} else {
						fail(
							`${noLocs.length} ambiguous syntaxes, AND THEY HAVE YIELDED DIFFERENT ASTs : \n${noLocs
								.map(inspect)
								.join("\n\n           ======================      \n\n")}`,
						);
					}
...
export function columns(...vals: (IExpr | string)[]): ISelectedColumn[] {
	return vals.map<ISelectedColumn>((expr) =>
		typeof expr === "string" ? { expr: { type: "ref", name: expr } } : { expr },
	);
}
